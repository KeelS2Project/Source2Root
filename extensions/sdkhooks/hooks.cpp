...
bool Consistent(const KeelEntityInfo& value) {
    return value.size == sizeof(value) && !value.reserved && value.index >= 0 && value.epoch &&
           value.source2_handle != UINT32_MAX;
}

bool Weapon(Kind kind) {
    return kind >= Kind::weapon_can_use;
}

bool Secondary(Kind kind) {
    return kind == Kind::touch || Weapon(kind);
}
...
void Numbers(const KeelDamageEdit& edit) {
    ...
    for (unsigned i = 0; i < 3; ++i)
        if (!std::isfinite(edit.force[i]) || !std::isfinite(edit.position[i]))
            throw Error("Damage vectors must be finite.");
}
}
...
const KeelDamageInfo& Frame::Damage() const {
    if (!damage_valid_)
        throw Error("This hook frame has no damage snapshot.");

    return damage_;
}

void Frame::Damage(const KeelDamageEdit& edit) {
    if (!damage_valid_ || phase_ != KH_PHASE_PRE)
        throw Error("Damage edits require a pre-damage callback.");

    Numbers(edit);
    edit_ = edit;
    edited_ = true;
    damage_.damage = edit.damage;
    damage_.damage_type = edit.damage_type;
    std::copy_n(edit.force, 3, damage_.force);
    std::copy_n(edit.position, 3, damage_.position);
}

bool Frame::Result() const {
    if (kind_ != Kind::weapon_can_use)
        throw Error("Only weapon-can-use hooks have a boolean result.");
...
Service::Service(KeelPluginHandle owner,
    ...
    if (entities.size != sizeof(entities) || entities.api_version != KEELS2_ENTITIES_API_VERSION ||
        !entities.find_by_source2_handle || !entities.describe || !entities.release || access.size != sizeof(access) ||
        access.api_version != KEELS2_ENTITY_ACCESS_API_VERSION || !access.visit || capture.size != sizeof(capture) ||
        capture.api_version != KEELS2_ENTITY_CAPTURE_API_VERSION || !capture.capture || data.size != sizeof(data) ||
        data.api_version != KEELS2_ENTITY_HOOK_DATA_API_VERSION || !data.read_damage || !data.write_damage ||
        !data.weapon_matches)
        throw Error("Incompatible SDKHooks host services.");

    if (construction && (construction->size != sizeof(*construction) ||
                         construction->api_version != KEELS2_ENTITY_CONSTRUCTION_API_VERSION ||
                         !construction->describe || !construction->observe || !construction->visit))
        throw Error("Incompatible pending entity observation service.");

    states_.reserve(256);
}

void Service::Thread() const {
    Check(runtime_.check_game_thread(owner_), "SDKHooks operation");
}
...
Kind Service::Validate(const dhooks::Definition& value) {
    ...
    Kind kind{};
    unsigned count = 0, result = KH_VALUE_VOID;

    if (policy.kind == "damage") {
        kind = Kind::damage;
        count = 3;
    } else if (policy.kind == "touch") {
        kind = Kind::touch;
        count = 2;
    } else if (policy.kind == "spawn") {
        kind = Kind::spawn;
        count = 2;
    } else if (policy.kind == "weapon_can_use") {
        kind = Kind::weapon_can_use;
        count = 2;
        result = KH_VALUE_BOOL;
    } else if (policy.kind == "weapon_select") {
        kind = Kind::weapon_select;
        count = 2;
    } else if (policy.kind == "weapon_drop") {
        kind = Kind::weapon_drop;
        count = 4;
    } else
        throw Error("Target has no supported SDKHooks kind.");

    if (!value.method || value.allow_calls || !value.buffers.empty() || !value.entities.empty() ||
        value.result != result || value.arguments != std::vector<KeelHookValueType>(count, KH_VALUE_POINTER))
        throw Error("SDKHooks target prototype or adapters do not match its kind.");
    ...
    if (Weapon(kind) && policy.class_name != "CCSPlayerPawn")
        throw Error("Weapon hooks require the current CCSPlayerPawn component owner.");
    ...
    if (policy.block != "preserve_result")
        throw Error("SDKHooks requires a reviewed preserve_result blocking contract.");
...
std::shared_ptr<Lease> Service::Acquire(std::uint32_t source) {
    ...
    if (source == UINT32_MAX || leases_ >= 512)
        throw Error("Invalid entity reference or SDKHooks entity limit (512).");
    ...
    if (result == KEEL_RESULT_NOT_FOUND && construction_.observe) {
        result = construction_.observe(owner_, source, &handle);
        lease->observer = result == KEEL_RESULT_OK;
    }
    ...
    if (!Consistent(lease->identity) || lease->identity.source2_handle != source)
        throw Error("Host returned an invalid entity identity.");
...
std::shared_ptr<Lease> Service::Capture(const void* pointer) {
    ...
    if (leases_ >= 512)
        throw Error("SDKHooks entity limit (512) reached.");
    ...
    if (!handle)
        throw Error("Host returned an empty captured entity.");
    ...
    if (!Consistent(lease->identity))
        throw Error("Host returned an invalid captured entity.");
...
bool Service::Valid(const Lease& lease) const {
    KeelEntityInfo value{};
    value.size = sizeof(value);
    return Describe(lease, value) == KEEL_RESULT_OK && Consistent(value) &&
           value.source2_handle == lease.identity.source2_handle && value.epoch == lease.identity.epoch &&
           value.index == lease.identity.index;
}

bool Service::Matches(const State& state, const void* pointer) const {
    if (!pointer || !Valid(*state.entity))
        return false;
    ...
    return Visit(*state.entity, state.definition.entity_hook.class_name.c_str(), visitor, &match) == KEEL_RESULT_OK &&
           match.matched;
}
...
std::unique_ptr<Hook> Service::Attach(const dhooks::Definition& definition, std::uint32_t entity,
    ...
    if (!phases || (phases & ~KH_PHASE_BOTH) || !callback || !retire)
        throw Error("Invalid SDKHooks registration.");

    auto state = std::make_shared<State>();
    state->service = shared_from_this();
    state->kind = Validate(definition);
    state->definition = definition;
    state->entity = Acquire(entity);
    state->phases = phases;
    ...
    Check(Visit(
              *state->entity,
              definition.entity_hook.class_name.c_str(),
              [](void* raw, void* const* pointers, unsigned count) -> KeelResult {
                  if (count != 1 || !pointers || !pointers[0])
              ...
              &visited),
          "Validate SDKHooks entity class");

    if (!visited)
        throw Error("Host did not validate the SDKHooks entity class.");

    auto target = transport_->Open(definition);
    auto hook = std::make_unique<Hook>();
    hook->state_ = state;
    hook->native_ = transport_->Attach(
        *target,
        KH_PHASE_BOTH,
        priority,
        [state](dhooks::Frame& native) {
            return state->service->Dispatch(*state, native);
        },
        std::move(retire));
...
int Service::Dispatch(State& state, dhooks::Frame& native) {
    ...
    auto found = std::find_if(state.pending.begin(), state.pending.end(), [&](const auto& value) {
    ...
    if (pre) {
        ...
        if (!Matches(state, instance))
            return KH_ACTION_CONTINUE;

        if (state.pending.size() == 8 || (secondary && leases_ >= 512))
            return KH_ACTION_CONTINUE;

        try {
            if (Secondary(state.kind))
                context.other = Capture(secondary);
        } catch (...) {
            native.retire_ = true;

            if (state.kind == Kind::weapon_can_use)
                native.SetInteger(0, 0);
        ...
        if (context.instance != instance || context.secondary != secondary || !Matches(state, instance) ||
            (context.other && !Valid(*context.other)))
            return KH_ACTION_CONTINUE;
    }

    if (!(state.phases & native.Phase()))
        return KH_ACTION_CONTINUE;

    Frame frame;
    frame.kind_ = state.kind;
    frame.phase_ = native.Phase();
    frame.flags_ = native.Flags();
    frame.entity_ = state.entity->identity.source2_handle;

    if (context.other)
        frame.other_ = context.other->identity.source2_handle;
    ...
    try {
        if (state.kind == Kind::damage) {
            ...
            for (unsigned i = 0; i < 3; ++i)
                if (!std::isfinite(frame.damage_.force[i]) || !std::isfinite(frame.damage_.position[i]))
                    throw Error("Invalid damage vector.");

            frame.damage_valid_ = true;
        }

        if (state.kind == Kind::weapon_can_use)
            frame.result_ = native.Integer(0) != 0;
        // A metadata lookup can reenter the host; avoid delivering stale data.
        if (Matches(state, instance) && (!context.other || Valid(*context.other)))
            action = state.callback(frame);
    } catch (...) {
        action = -2;
    }
    ...
    if (state.kind == Kind::weapon_can_use) {
        native.SetInteger(0, frame.result_ ? 1 : 0);
...
bool Service::Empty() const {
    return transport_->Empty() && !leases_;
}
}
