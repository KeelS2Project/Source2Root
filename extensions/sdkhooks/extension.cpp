...
public:
    static constexpr PluginInfo Info{
        "Source2Root SDKHooks", "KeelS2 Project", "1.0.0", "Typed per-entity damage, touch, spawn and weapon hooks"};

    static constexpr PluginRequirement Requirements[]{{"Source2Root", "1.0.0", DependencyRequirement::exact}};
    SDKHooks() : Extension("source2root.sdkhooks") {
        frames_.reserve(8);
    }
    ...
    void OnGameFrame(bool, bool, bool) override {
        try {
            if (service_)
                service_->Collect(true);
        } catch (const std::exception& error) {
            LogError("SDKHooks cleanup: {}", error.what());
        }
    }
...
private:
    ...
    template<class T> const T& Require(const char* name, unsigned version) {
        ...
        if (HostContext().QueryService(name, version, &raw) != KEEL_RESULT_OK || !raw)
            throw sdk::Error("Required SDKHooks host service is unavailable.");

        const auto* api = static_cast<const T*>(raw);

        if (api->size != sizeof(T) || api->api_version != version)
            throw sdk::Error("Incompatible SDKHooks service.");
    ...
    const KeelEntityConstructionApi* OptionalConstruction() {
        ...
        if (result != KEEL_RESULT_OK || !raw)
            throw sdk::Error("Pending entity service query failed.");

        const auto* api = static_cast<const KeelEntityConstructionApi*>(raw);

        if (api->size != sizeof(*api) || api->api_version != 1)
            throw sdk::Error("Incompatible pending entity service.");
    ...
    sdk::Frame& Frame(NativeCall& call) {
        ...
        for (const auto& window : frames_)
            if (window.id == id && window.owner == call.Owner())
                return *window.frame;

        throw std::invalid_argument("SDKHooks frame is stale, inactive or belongs to another script.");
    }

    static sdk::Hook& Hook(NativeCall& call) {
        return call.Resource<sdk::Hook>(call.Int(1), HookType);
    }

    std::int32_t Add(NativeCall& call) {
        return Invoke(call, [&] {
            const auto definition = source2root::dhooks::ReadDefinition(
                std::filesystem::path(call.ConfigPath()) / "targets.json", call.String(2), call.ScriptId());

            const auto owner = call.Owner();
            const auto priority = call.Int(5), data = call.Int(6), phases = call.Int(3);

            if (phases < 1 || (phases & ~KH_PHASE_BOTH))
                throw sdk::Error("Invalid SDKHooks phases.");

            const auto token = call.Callback(4);

            try {
                if (RetainCallback(token) != KEEL_RESULT_OK)
                    throw sdk::Error("Persistent SDKHooks callbacks are unavailable.");

                auto hook = service_->Attach(
                    definition,
                    static_cast<std::uint32_t>(call.Int(1)),
                    static_cast<unsigned>(phases),
                    priority,
                    [this, owner, token, data](sdk::Frame& frame) {
    ...
    int Dispatch(std::uint64_t owner, SrCallback token, std::int32_t data, sdk::Frame& frame) {
        ...
        if (action < 0 || action > 2 || (frame.Phase() == KH_PHASE_POST && action == 2) ||
            ...
            LogError("Invalid SDKHooks action; callback retired.");
            return -2;
        }
    ...
    std::int32_t Enable(NativeCall& call) {
        return Invoke(call, [&] {
            ...
            if (value != 0 && value != 1)
                throw sdk::Error("Enable requires a boolean.");
    ...
    std::int32_t Kind(NativeCall& call) {
        return static_cast<std::int32_t>(Frame(call).Type());
    }
    ...
    std::int32_t Entity(NativeCall& call) {
        return std::bit_cast<std::int32_t>(Frame(call).Entity());
    }

    std::int32_t Other(NativeCall& call) {
        return std::bit_cast<std::int32_t>(Frame(call).Other());
    }

    std::int32_t Damage(NativeCall& call) {
        for (unsigned i = 2; i <= 7; ++i)
            call.OutputCell(i, i >= 5 ? -1 : 0);

        call.OutputArray(8, 3, {0, 0, 0});
        call.OutputArray(9, 3, {0, 0, 0});
        return Invoke(call,[&] {
            const auto& damage = Frame(call).Damage();
            call.OutputCell(2, std::bit_cast<std::int32_t>(damage.damage));
            call.OutputCell(3, std::bit_cast<std::int32_t>(damage.damage_type));
            call.OutputCell(4, damage.damage_custom);
            call.OutputCell(5, std::bit_cast<std::int32_t>(damage.inflictor));
            call.OutputCell(6, std::bit_cast<std::int32_t>(damage.attacker));
            call.OutputCell(7, std::bit_cast<std::int32_t>(damage.ability));
            std::vector<std::int32_t> force(3), position(3);

            for (unsigned i = 0; i < 3; ++i) {
                force[i] = std::bit_cast<std::int32_t>(damage.force[i]);
                position[i] = std::bit_cast<std::int32_t>(damage.position[i]);
            }

            call.OutputArray(8, 3, force);
            call.OutputArray(9, 3, position);
            return 1;
        });
    }
    ...
    std::int32_t SetDamage(NativeCall& call) {
        return Invoke(call, [&] {
            KeelDamageEdit edit{};
            edit.size = sizeof(edit);
            edit.damage = call.Float(2);
            edit.damage_type = static_cast<std::uint32_t>(call.Int(3));
            const auto force = call.Array(4,3), position = call.Array(5,3);

            for (unsigned i = 0; i < 3; ++i) {
                edit.force[i] = std::bit_cast<float>(force[i]);
                edit.position[i] = std::bit_cast<float>(position[i]);
            }

            Frame(call).Damage(edit);
            return 1;
        });
    }

    std::int32_t Result(NativeCall& call) {
        call.OutputCell(2, 0);
        return Invoke(call, [&] {
            call.OutputCell(2, Frame(call).Result());
            return 1;
        });
    }
    ...
    std::int32_t SetResult(NativeCall& call) {
        return Invoke(call, [&] {
            ...
            if (value != 0 && value != 1)
                throw sdk::Error("Hook result requires a boolean.");

            Frame(call).Result(value != 0);
            return 1;
        });
    }
};
