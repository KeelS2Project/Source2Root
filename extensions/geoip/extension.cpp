...
public:
    static constexpr PluginInfo Info{
        "Source2Root GeoIP", "KeelS2 Project", "1.0.0", "Local IPv4 and IPv6 geographic database lookups"};
...
private:
    ...
    template <typename Function> std::int32_t Invoke(NativeCall& call, Function function) {
        try {
            return function();
        } catch (const geo::Error& error) {
            return call.Fail(error.what());
        } catch (const std::filesystem::filesystem_error&) {
            return call.Fail("GeoIP database directory is unavailable.");
        }
    }

    static DatabaseRef Db(NativeCall& call) {
        return call.Resource<DatabaseRef>(call.Int(1), DatabaseType);
    }

    static geo::Reader& Reader(const DatabaseRef& db) {
        if (!db->reader)
            throw geo::Error("GeoIP database is not ready.");
    ...
    void BeginLoad(NativeCall& call, const DatabaseRef& db) {
        if (db->ticket)
            throw geo::Error("GeoIP database already has a pending load or callback.");

        if (!queue_)
            queue_ = std::make_unique<source2root::WorkQueue>(1, 8);
        ...
        try {
            db->ticket = queue_->Submit([source, snapshots, result](const auto& canceled) {
                ...
                if (!result->applied) {
                    try {
                        ...
                        result->error = "GeoIP database file is unavailable.";
                    } catch (const std::exception& error) {
                        result->error = std::string(error.what()).substr(0, 1024);
                    } catch (...) {
                        result->error = "GeoIP database load failed.";
                    }

                    db->error = result->error;

                    if (!failure)
                        db->reader = std::move(result->reader);
            ...
            if (!db->ticket)
                throw geo::Error("GeoIP worker queue is full.");
        } catch (...) {
    ...
    std::int32_t Open(NativeCall& call) {
        return Invoke(call, [&] {
            ...
            geo::ValidateName(name);
            std::erase_if(databases_, [](const auto& db) {
                return db.expired();
            });

            if (databases_.size() >= 8)
                throw geo::Error("GeoIP database handle limit (8) reached.");

            auto db = std::make_shared<Database>();
            db->extension = this;
            const auto root = std::filesystem::path(call.DataPath(true));
            db->source = root / name;
            db->snapshots = root / ".snapshots";
            ...
            try {
                ...
                call.Close(db->handle, DatabaseType);
                throw;
            }
    ...
    std::int32_t Close(NativeCall& call) {
        call.Close(call.Int(1), DatabaseType);
        return 1;
    }

    std::int32_t Reload(NativeCall& call) {
        return Invoke(call, [&] {
            BeginLoad(call, Db(call));
            return 1;
        });
    }

    std::int32_t Ready(NativeCall& call) {
        return Db(call)->reader ? 1 : 0;
    }

    std::int32_t Loading(NativeCall& call) {
        return Db(call)->ticket ? 1 : 0;
    }

    std::int32_t ErrorText(NativeCall& call) {
        call.Output(2, call.Int(3), Db(call)->error);
        return 1;
    }
    ...
    std::int32_t Type(NativeCall& call) {
        ...
        return Invoke(call, [&] {
            call.Output(2, call.Int(3), Reader(Db(call)).Type());
            return 1;
        });
    }
    ...
    std::int32_t Epoch(NativeCall& call) {
        call.Output(2, call.Int(3), "");
        return Invoke(call, [&] {
            call.Output(2, call.Int(3), std::to_string(Reader(Db(call)).Epoch()));
            return 1;
        });
    }
    ...
    std::int32_t CloseRecord(NativeCall& call) {
        call.Close(call.Int(1), RecordType);
        return 1;
    }
    ...
    std::int32_t Text(NativeCall& call) {
        ...
        return Invoke(call, [&] {
            ...
            if (field < 0 || field >= static_cast<int>(geo::Field::Count))
                throw geo::Error("Unknown GeoIP string field.");

            if (!record.text[field])
                return 0;

            call.Output(3, call.Int(4), *record.text[field]);
            return 1;
        });
    }
    ...
    std::int32_t Number(NativeCall& call) {
        ...
        return Invoke(call, [&] {
            ...
            if (field < 0 || field >= static_cast<int>(geo::Number::Count))
                throw geo::Error("Unknown GeoIP numeric field.");

            if (!record.numbers[field])
                return 0;

            call.OutputCell(3, std::bit_cast<std::int32_t>(*record.numbers[field]));
            return 1;
        });
    }
    ...
    std::int32_t ASN(NativeCall& call) {
        ...
        call.Output(2, call.Int(3), std::to_string(*record.asn));
        return 1;
    }
    ...
    std::int32_t Distance(NativeCall& call) {
        ...
        return Invoke(call, [&] {
            ...
            if (unit != 0 && unit != 1)
                throw geo::Error("GeoIP distance unit must be kilometers (0) or miles (1).");

            const auto value = geo::Distance(call.Float(1), call.Float(2), call.Float(3), call.Float(4), unit == 1);
            call.OutputCell(6, std::bit_cast<std::int32_t>(static_cast<float>(value)));
            return 1;
        });
    }
};
