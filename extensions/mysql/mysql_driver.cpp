...
struct Session::Impl {
    ...
    Impl(const db::Settings& settings, const std::atomic_bool& cancel)
        ...
        if (!settings.timeout || settings.timeout > 30) throw db::Error("Database timeout must be from 1 to 30 seconds.");
        Check();
    ...
    void Thread() const {
        if (owner != std::this_thread::get_id()) throw db::Error("Database sessions require their creating thread.");
    }
    void Check() const {
        Thread();
    ...
    void Abort() noexcept { connection.reset(); statement.reset(); }
    db::QueryResult Execute(const db::QueryInput& input) {
        Thread();
        try {
            Check();
            if (!connection) throw db::Error("Database session is no longer available.");
            input.Validate();
            if (++commands > 512) throw db::Error("Database session exceeds 512 commands.");
            input_bytes += input.sql.size();
            for (const auto& parameter : input.parameters)
                if (const auto* value = std::get_if<std::string>(&*parameter)) input_bytes += value->size();
            if (input_bytes > 1024 * 1024) throw db::Error("Database session input exceeds 1 MiB.");
            ...
            if (columns) {
                ...
                for (;;) {
                    ...
                    for (unsigned i = 0; i < columns; ++i) {
                        ...
                        if (output_bytes > 1024 * 1024) throw db::Error("Database session results exceed 1 MiB.");
...
Session::Session(const db::Settings& settings, const std::atomic_bool& canceled) : impl_(std::make_unique<Impl>(settings, canceled)) {}
Session::~Session() = default;
db::QueryResult Session::Execute(const db::QueryInput& input) { return impl_->Execute(input); }
void Session::Commit() { impl_->Commit(); }
