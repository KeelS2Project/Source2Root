...
public:
    Runtime() {
        ...
        if (!runtime_users && mysql_library_init(0, nullptr, nullptr))
            throw db::Error("Database client initialization failed.");
...
void Option(MYSQL* connection, mysql_option option, const void* value) {
    if (mysql_optionsv(connection, option, value))
        throw db::Error("Database connection option is unavailable.");
}
...
db::QueryValue Value(const char* buffer, unsigned long length, bool null) {
    ...
    if (length > 4095 || std::memchr(buffer, 0, length))
...
struct Session::Impl {
    ...
    Runtime runtime;
    // Connection closes first on destruction; abort also closes it before
    // freeing an unread statement, so cleanup never drains an unbounded stream.
    Statement statement{nullptr, mysql_stmt_close};
    Connection connection{nullptr, mysql_close};
    unsigned commands = 0;
    std::size_t input_bytes = 0, output_bytes = 0;

    Impl(const db::Settings& settings, const std::atomic_bool& cancel)
        : canceled(cancel), deadline(std::chrono::steady_clock::now() + std::chrono::seconds(settings.timeout * 3)) {
        if (!settings.timeout || settings.timeout > 30)
            throw db::Error("Database timeout must be from 1 to 30 seconds.");

        Check();
        connection.reset(mysql_init(nullptr));

        if (!connection)
            throw db::Error("Database connection allocation failed.");
        ...
        Option(connection.get(),
               MARIADB_OPT_RESTRICTED_AUTH,
               "mysql_native_password,caching_sha2_password,sha256_password,client_ed25519");

        if (settings.tls) {
            Option(connection.get(), MARIADB_OPT_TLS_VERSION, "TLSv1.2,TLSv1.3");

            if (!settings.ca.empty())
                Option(connection.get(), MYSQL_OPT_SSL_CA, settings.ca.c_str());
        }
        ...
        if (settings.tls && !mysql_get_ssl_cipher(connection.get()))
            throw db::Error("Database connection did not negotiate TLS.");

        Check();

        if (mysql_autocommit(connection.get(), 0))
            throw db::Error("Database transaction setup failed.");
    }

    void Thread() const {
        if (owner != std::this_thread::get_id())
            throw db::Error("Database sessions require their creating thread.");
    }
    ...
    db::QueryResult Execute(const db::QueryInput& input) {
        ...
        try {
            ...
            if (!connection)
                throw db::Error("Database session is no longer available.");

            input.Validate();

            if (++commands > 512)
                throw db::Error("Database session exceeds 512 commands.");

            input_bytes += input.sql.size();

            for (const auto& parameter : input.parameters)
                if (const auto* value = std::get_if<std::string>(&*parameter))
                    input_bytes += value->size();

            if (input_bytes > 1024 * 1024)
                throw db::Error("Database session input exceeds 1 MiB.");

            statement.reset(mysql_stmt_init(connection.get()));

            if (!statement)
                throw db::Error("Database statement allocation failed.");

            if (mysql_stmt_prepare(statement.get(), input.sql.c_str(), input.sql.size()))
                StatementError(statement.get());

            if (mysql_stmt_param_count(statement.get()) != input.parameters.size())
                throw db::Error("Query parameter count does not match its bindings.");
            ...
            for (std::size_t i = 0; i < input.parameters.size(); ++i) {
                ...
                if (const auto* value = std::get_if<std::int32_t>(&parameter)) {
                    ...
                    bind.buffer = const_cast<std::int32_t*>(value);
            ...
            if (!bindings.empty() && mysql_stmt_bind_param(statement.get(), bindings.data()))
                StatementError(statement.get());

            Check();

            if (mysql_stmt_execute(statement.get()))
                StatementError(statement.get());
            ...
            if (columns) {
                ...
                for (unsigned i = 0; i < columns; ++i) {
                    ...
                    output.buffer = buffers[i].data();
                    output.buffer_length = buffers[i].size();
                    output.length = &sizes[i];
                    output.is_null = &nulls[i];
                    output.error = &errors[i];
                }

                if (mysql_stmt_bind_result(statement.get(), outputs.data()))
                    StatementError(statement.get());
                ...
                for (;;) {
                    ...
                    for (unsigned i = 0; i < columns; ++i) {
                        ...
                        output_bytes += value.text.size();

                        if (output_bytes > 1024 * 1024)
                            throw db::Error("Database session results exceed 1 MiB.");
            ...
            if (affected != static_cast<my_ulonglong>(-1)) {
                if (affected > static_cast<my_ulonglong>(std::numeric_limits<int>::max()))
            ...
            if (mysql_stmt_next_result(statement.get()) != -1)
    ...
    void Commit() {
        ...
        try {
            ...
            if (mysql_commit(connection.get()))
                throw db::Error("Database commit failed; outcome may be unknown.");
        } catch (...) {
            Abort();
            throw;
        }
...
Session::Session(const db::Settings& settings, const std::atomic_bool& canceled)
    : impl_(std::make_unique<Impl>(settings, canceled)) {
}

Session::~Session() = default;
db::QueryResult Session::Execute(const db::QueryInput& input) {
