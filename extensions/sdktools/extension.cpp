...
public:
    static constexpr PluginInfo Info{
        "Source2Root SDKTools", "KeelS2 Project", "1.0.0", "Owned entities, schema access and game operations"};
...
private:
    ...
    template <typename T> const T& Require(const char* name, unsigned version) {
        ...
        if (api->size != sizeof(T) || api->api_version != version)
            throw sdk::Error("Incompatible host service table.");
    ...
    const KeelEntityWritesApi* OptionalWrites() {
        ...
        if (result != KEEL_RESULT_OK || !raw)
            throw sdk::Error("Entity write service query failed.");

        const auto* api = static_cast<const KeelEntityWritesApi*>(raw);

        if (api->size != sizeof(*api) || api->api_version != KEELS2_ENTITY_WRITES_API_VERSION)
            throw sdk::Error("Incompatible entity write service table.");
    ...
    const KeelEntityToolsApi* OptionalTools() {
        ...
        if (result != KEEL_RESULT_OK || !raw)
            throw sdk::Error("Entity tools service query failed.");

        const auto* api = static_cast<const KeelEntityToolsApi*>(raw);

        if (api->size != sizeof(*api) || api->api_version != KEELS2_ENTITY_TOOLS_API_VERSION)
            throw sdk::Error("Incompatible entity tools service table.");
    ...
    const KeelEntityConstructionApi* OptionalConstruction() {
        ...
        if (result != KEEL_RESULT_OK || !raw)
            throw sdk::Error("Entity construction service query failed.");

        const auto* api = static_cast<const KeelEntityConstructionApi*>(raw);

        if (api->size != sizeof(*api) || api->api_version != 1)
            throw sdk::Error("Incompatible entity construction service.");
    ...
    const KeelEntityInputApi* OptionalInput() {
        ...
        if (result != KEEL_RESULT_OK || !raw)
            throw sdk::Error("Entity input service query failed.");

        const auto* api = static_cast<const KeelEntityInputApi*>(raw);

        if (api->size != sizeof(*api) || api->api_version != 1)
            throw sdk::Error("Incompatible entity input service.");

        return api;
    }

    bool OnExtensionStart() override {
        service_ = std::make_shared<sdk::Service>(
    ...
    static sdk::Entity& Entity(NativeCall& call, unsigned index = 1) {
        return call.Resource<sdk::Entity>(call.Int(index), EntityType);
    }

    static sdk::Field& Field(NativeCall& call, unsigned index = 2) {
        return call.Resource<sdk::Field>(call.Int(index), FieldType);
    }
    ...
    std::int32_t InputCapabilities(NativeCall& call) {
        ...
        return Invoke(call, [&] {
            ...
            const auto types = service->InputCapabilities();
            call.OutputCell(1, static_cast<std::int32_t>(types[0]));
            call.OutputCell(2, static_cast<std::int32_t>(types[1]));
            return 1;
        });
    }
    ...
    template<unsigned Type> std::int32_t Input(NativeCall& call) {
        ...
        bool invoked{};
        return Invoke(call, [&] {
            const auto name = call.String(2);
            std::string text;
            KeelEntityInputValue value{};
            value.size = sizeof(value);
            value.type = Type;

            if constexpr (Type == KEELS2_INPUT_STRING) {
                text = call.String(4);
                value.string_value = text.c_str();
            } else if constexpr (Type == KEELS2_INPUT_BOOL || Type == KEELS2_INPUT_INT32)
                value.int_value = call.Int(4);
            else if constexpr (Type == KEELS2_INPUT_FLOAT)
                value.float_value = call.Float(4);
            else if constexpr (Type == KEELS2_INPUT_COLOR) {
                const auto cells = call.Array(4,4);

                for (unsigned i = 0; i < 4; ++i) {
                    if (cells[i] < 0 || cells[i] > 255)
                        throw sdk::Error("Input color components require 0..255.");
                ...
                for (unsigned i = 0; i < 3; ++i)
                    value.vector_value[i] = std::bit_cast<float>(cells[i]);
            }

            constexpr unsigned offset = Type == KEELS2_INPUT_VOID ? 0 : 1;
            const auto queue = call.Int(6 + offset);
            const auto delay = call.Float(7 + offset);

            if (queue != 0 && queue != 1)
                throw sdk::Error("Input queue selection must be false or true.");

            auto* activator = call.Int(4+offset) ? &Entity(call,4+offset) : nullptr;
            auto* caller = call.Int(5+offset) ? &Entity(call,5+offset) : nullptr;
            sdk::Entity* payload{};

            if constexpr (Type == KEELS2_INPUT_ENTITY)
                payload = &Entity(call, 4);

            try {
                Entity(call).Input(name.c_str(), value, invoked, activator, caller, payload, queue != 0, delay);
            } catch (...) {
                call.OutputCell(3, invoked ? 1 : 0);
                throw;
            }
    ...
    std::int32_t Create(NativeCall& call) {
        return Invoke(call, [&] {
            const auto service = service_;
            return call.Own(EntityType, service->Create(call.String(1)));
        });
    }

    std::int32_t Pending(NativeCall& call) {
        return Invoke(call, [&] {
            return Entity(call).Pending() ? 1 : 0;
        });
    }
    ...
    template<unsigned Type> std::int32_t SetKey(NativeCall& call) {
        return Invoke(call, [&] {
            ...
            if constexpr (Type == KEELS2_ENTITY_KEY_STRING) {
                text = call.String(3);
                value.string_value = text.c_str();
            } else if constexpr (Type == KEELS2_ENTITY_KEY_BOOL || Type == KEELS2_ENTITY_KEY_INT32)
                value.int_value = call.Int(3);
            else if constexpr (Type == KEELS2_ENTITY_KEY_FLOAT)
                value.float_value = call.Float(3);
            else if constexpr (Type == KEELS2_ENTITY_KEY_COLOR) {
                const auto cells = call.Array(3,4);

                for (unsigned i = 0; i < 4; ++i) {
                    if (cells[i] < 0 || cells[i] > 255)
                        throw sdk::Error("Entity key color components require 0..255.");
    ...
    std::int32_t Spawn(NativeCall& call) {
        ...
        return Invoke(call, [&] {
            try {
                Entity(call).Spawn(invoked);
            } catch (...) {
                call.OutputCell(2, invoked ? 1 : 0);
                throw;
            }
    ...
    std::int32_t Find(NativeCall& call) {
        return Invoke(call, [&] {
            return call.Own(EntityType, service_->Find(call.Int(1)));
        });
    }

    std::int32_t FromHandle(NativeCall& call) {
        return Invoke(call, [&] {
            return call.Own(EntityType, service_->FromSource(std::bit_cast<std::uint32_t>(call.Int(1))));
        });
    }
    ...
    std::int32_t FromPlayer(NativeCall& call) {
        return Invoke(call, [&] {
            ...
            if (pawn != 0 && pawn != 1)
                throw sdk::Error("Pawn selection must be false or true.");

            SrPlayerIdentity player{};

            if (!call.Player(call.Int(1), player))
                throw sdk::Error("Player connection is no longer available.");
    ...
    std::int32_t Close(NativeCall& call) {
        ...
        return Invoke(call, [&] {
            call.Close(call.Int(1), EntityType);
            return 1;
        });
    }

    std::int32_t Valid(NativeCall& call) {
        return Entity(call).Valid() ? 1 : 0;
    }

    std::int32_t Index(NativeCall& call) {
        return Invoke(
            call,
            [&] {
                return Entity(call).Describe().index;
            },
            -1);
    }
    ...
    std::int32_t SourceHandle(NativeCall& call) {
        call.OutputCell(2, -1);
        return Invoke(call, [&] {
            call.OutputCell(2, std::bit_cast<std::int32_t>(Entity(call).Describe().source2_handle));
            return 1;
        });
    }

    std::int32_t Same(NativeCall& call) {
        return Invoke(call, [&] {
            return Entity(call).Same(Entity(call, 2)) ? 1 : 0;
        });
    }

    std::int32_t Resolve(NativeCall& call) {
        return Invoke(call, [&] {
            return call.Own(FieldType, service_->Resolve(call.String(1), call.String(2), call.Int(3)));
        });
    }
    ...
    std::int32_t CloseField(NativeCall& call) {
        return Invoke(call, [&] {
            Field(call, 1).Close();
            call.Close(call.Int(1), FieldType);
            return 1;
        });
    }

    std::int32_t Type(NativeCall& call) {
        return Field(call, 1).Type();
    }

    std::int32_t Size(NativeCall& call) {
        return Field(call, 1).Size();
    }

    std::int32_t InfoText(NativeCall& call) {
        call.Output(2, call.Int(3), "");
        call.Output(4, call.Int(5), "");
        call.Output(6, call.Int(7), "");
        return Invoke(call, [&] {
            auto& field = Field(call, 1);

            if (field.ClassName().size() >= static_cast<unsigned>(call.Int(3)) ||
                field.Name().size() >= static_cast<unsigned>(call.Int(5)) ||
                field.Profile().size() >= static_cast<unsigned>(call.Int(7)))
                throw sdk::Error("Schema metadata output buffer is too small.");

            call.Output(2, call.Int(3), field.ClassName());
            call.Output(4, call.Int(5), field.Name());
            call.Output(6, call.Int(7), field.Profile());
            return 1;
        });
    }
    ...
    std::int32_t Integer(NativeCall& call) {
        call.OutputCell(3, 0);
        return Invoke(call, [&] {
            call.OutputCell(3, Entity(call).Integer(Field(call)));
            return 1;
        });
    }
    ...
    std::int32_t IntegerText(NativeCall& call) {
        ...
        return Invoke(call, [&] {
            ...
            if (value.size() >= static_cast<unsigned>(call.Int(4)))
                throw sdk::Error("Integer output buffer is too small.");
    ...
    std::int32_t Number(NativeCall& call) {
        call.OutputCell(3, 0);
        return Invoke(call, [&] {
            call.OutputCell(3, std::bit_cast<std::int32_t>(Entity(call).Number(Field(call))));
            return 1;
        });
    }
    ...
    std::int32_t ReadHandle(NativeCall& call) {
        call.OutputCell(3, -1);
        return Invoke(call, [&] {
            call.OutputCell(3, std::bit_cast<std::int32_t>(Entity(call).SourceHandle(Field(call))));
            return 1;
        });
    }
    ...
    std::int32_t WriteCapabilities(NativeCall& call) {
        call.OutputCell(1, 0);
        return Invoke(call, [&] {
            call.OutputCell(1, service_->WriteCapabilities());
            return 1;
        });
    }
    ...
    std::int32_t ToolCapabilities(NativeCall& call) {
        ...
        return Invoke(call, [&] {
            const auto service = service_;
            call.OutputCell(1, service->ToolCapabilities());
            return 1;
        });
    }
    ...
    std::int32_t Teleport(NativeCall& call) {
        return Invoke(call,[&] {
            ...
            if (!flags || (flags & ~7u))
                throw sdk::Error("Teleport requires position, angles or velocity flags.");
            ...
            for (unsigned i = 0; i < 3; ++i) if (flags & (1u<<i)) {
                ...
                for (unsigned j = 0; j < 3; ++j)
                    vectors[i][j] = std::bit_cast<float>(cells[j]);
            }

            Entity(call).Teleport(flags, vectors[0], vectors[1], vectors[2]);
            return 1;
        });
    }
    ...
    std::int32_t SetModel(NativeCall& call) {
        return Invoke(call, [&] {
            const auto model = call.String(2);
    ...
    std::int32_t SetInteger(NativeCall& call) {
        return Invoke(call, [&] {
            Entity(call).SetInteger(Field(call), call.Int(3));
            return 1;
        });
    }
    ...
    std::int32_t SetIntegerText(NativeCall& call) {
        return Invoke(call, [&] {
            Entity(call).SetIntegerText(Field(call), call.String(3));
            return 1;
        });
    }
    ...
    std::int32_t SetNumber(NativeCall& call) {
        return Invoke(call, [&] {
            Entity(call).SetNumber(Field(call), call.Float(3));
            return 1;
        });
    }
    ...
    std::int32_t SetVector(NativeCall& call) {
        return Invoke(call, [&] {
            const auto cells = call.Array(3, 3);
            Entity(call).SetVector(
                Field(call),
                {std::bit_cast<float>(cells[0]), std::bit_cast<float>(cells[1]), std::bit_cast<float>(cells[2])});
    ...
    std::int32_t ReadEntity(NativeCall& call) {
        return Invoke(call, [&] {
            ...
            if (handle == KEELS2_INVALID_SOURCE2_ENTITY_HANDLE)
                return 0;
