...
std::string Copy(const char* value) {
    if (!value)
        throw Error("Host returned missing schema metadata.");

    std::size_t length = 0;

    while (length <= 255 && value[length])
        ++length;

    if (length > 255 || !length)
        throw Error("Host returned invalid schema metadata.");
...
IntegerValue DecodeInteger(unsigned type, const void* data) {
    ...
    case KEELS2_SCHEMA_UINT8:
        return std::uint64_t{Load<std::uint8_t>(data)};

    case KEELS2_SCHEMA_INT8:
        return std::int64_t{Load<std::int8_t>(data)};

    case KEELS2_SCHEMA_UINT16:
        return std::uint64_t{Load<std::uint16_t>(data)};

    case KEELS2_SCHEMA_INT16:
        return std::int64_t{Load<std::int16_t>(data)};

    case KEELS2_SCHEMA_UINT32:
        return std::uint64_t{Load<std::uint32_t>(data)};

    case KEELS2_SCHEMA_INT32:
        return std::int64_t{Load<std::int32_t>(data)};

    case KEELS2_SCHEMA_UINT64:
        return Load<std::uint64_t>(data);

    case KEELS2_SCHEMA_INT64:
        return Load<std::int64_t>(data);

    case KEELS2_SCHEMA_BOOL: {
        const auto value = Load<std::uint8_t>(data);

        if (value > 1)
            throw Error("Host returned an invalid boolean value.");
...
template <typename T> std::array<std::byte, 8> EncodeInteger(const IntegerValue& value) {
    const auto number = std::visit(
        [](auto source) -> T {
            if (!std::in_range<T>(source))
                throw Error("Integer is outside the schema field's range.");
    ...
    switch (type) {
        ...
        case KEELS2_SCHEMA_BOOL:
            if (!std::visit(
                    ...
                    value))
                throw Error("Boolean field requires zero or one.");
...
IntegerValue ParseInteger(const std::string& text) {
    if (text.empty() || text.size() > 21)
        throw Error("Integer text must contain a decimal integer.");
    ...
    const auto parse = [&]<typename T>() -> IntegerValue {
        ...
        if (error != std::errc{} || end != text.data() + text.size())
            throw Error("Integer text is invalid or outside 64-bit range.");
...
Service::Service(KeelPluginHandle plugin,
                 const KeelEntitiesApi& entities,
                 const KeelSchemaApi& schema,
                 const KeelPlayersApi& players,
                 const KeelNativeRuntimeApi& runtime,
                 const KeelEntityWritesApi* writes,
                 const KeelEntityToolsApi* tools,
                 const KeelEntityConstructionApi* construction,
                 const KeelEntityInputApi* input)
    : plugin_(plugin), entities_(entities), schema_(schema), players_(players), runtime_(runtime),
      writes_(writes ? *writes : KeelEntityWritesApi{}), tools_(tools ? *tools : KeelEntityToolsApi{}),
      construction_(construction ? *construction : KeelEntityConstructionApi{}),
      input_(input ? *input : KeelEntityInputApi{}) {
    if (!plugin || entities.size != sizeof(entities) || entities.api_version != KEELS2_ENTITIES_API_VERSION ||
        !entities.find_by_index || !entities.find_by_source2_handle || !entities.release || !entities.describe ||
        !entities.equal || !entities.read_field || schema.size != sizeof(schema) ||
        schema.api_version != KEELS2_SCHEMA_API_VERSION || !schema.resolve_field || !schema.release_field ||
        !schema.describe_field || players.size != sizeof(players) ||
        players.api_version != KEELS2_PLAYERS_API_VERSION || !players.validate_connection ||
        runtime.size != sizeof(runtime) || runtime.api_version != KEELS2_NATIVE_RUNTIME_API_VERSION ||
    ...
    if (writes && (writes->size != sizeof(*writes) || writes->api_version != KEELS2_ENTITY_WRITES_API_VERSION ||
                   !writes->capabilities || !writes->write_field))
        throw Error("Incompatible entity write service.");

    if (tools && (tools->size != sizeof(*tools) || tools->api_version != KEELS2_ENTITY_TOOLS_API_VERSION ||
                  !tools->capabilities || !tools->teleport || !tools->set_model || !tools->remove))
        throw Error("Incompatible entity tools service.");

    if (construction &&
        (construction->size != sizeof(*construction) ||
         construction->api_version != KEELS2_ENTITY_CONSTRUCTION_API_VERSION || !construction->ready ||
         !construction->create || !construction->describe || !construction->set || !construction->teleport ||
         !construction->spawn || !construction->observe || !construction->visit))
    ...
    if (input && (input->size != sizeof(*input) || input->api_version != KEELS2_ENTITY_INPUT_API_VERSION ||
        ...
        throw Error("Incompatible entity input service.");
}

void Service::Thread() const {
    Check(runtime_.check_game_thread(plugin_), "Entity operation");
}
...
void Service::ConstructionReady() const {
    ...
    if (!keep->construction_.ready)
        throw Error("Entity construction service is unavailable.");
...
std::unique_ptr<Entity> Service::Create(const std::string& classname) {
    ...
    if (!keep->construction_.create)
        throw Error("Entity construction service is unavailable.");

    if (keep->entity_count_ >= 256)
        throw Error("Entity provider limit (256 handles) reached.");

    if (keep->active_tools_ >= 8)
        throw Error("Entity operation recursion limit (8) reached.");
    ...
    if (!Consistent(info))
        throw Error("Host returned invalid created entity identity.");
...
KeelEntityInfo
Service::Describe(KeelEntityHandle handle, const KeelEntityInfo& expected, bool constructed, bool* pending) const {
    if (pending)
        *pending = false;
    ...
    if (!handle)
        throw Error("Entity handle is closed.");

    KeelEntityInfo current{};
    current.size = sizeof(current);
    auto result = constructed ? construction_.describe(plugin_,handle,&current) : KEEL_RESULT_NOT_FOUND;

    if (constructed && result == KEEL_RESULT_OK) {
        if (pending)
            *pending = true;
    } else if (result == KEEL_RESULT_NOT_FOUND)
        result = entities_.describe(plugin_, handle, &current);

    Check(result,"Validate entity");

    if (!Identity(expected, current))
        throw Error("Entity identity changed.");
...
unsigned Service::WriteCapabilities() const {
    ...
    if (!writes_.capabilities)
        throw Error("Entity write service is unavailable.");
...
unsigned Service::ToolCapabilities() const {
    ...
    if (!keep->tools_.capabilities)
        throw Error("Entity tools service is unavailable.");
...
std::unique_ptr<Entity> Service::Adopt(KeelEntityHandle handle) {
    ...
    try {
        entity.reset(new Entity(shared_from_this(), handle));
    } catch (...) {
        entities_.release(plugin_, handle);
        throw;
    }
    ...
    if (!Consistent(info))
        throw Error("Host returned invalid entity identity.");
...
std::unique_ptr<Entity> Service::Find(int index) {
    ...
    if (index < 0)
        throw Error("Entity index must be nonnegative.");
    ...
    if (entity->identity_.index != index)
        throw Error("Host returned a different entity index.");
...
std::unique_ptr<Entity> Service::FromSource(std::uint32_t source) {
    ...
    if (source == KEELS2_INVALID_SOURCE2_ENTITY_HANDLE)
        throw Error("Source2 entity handle is invalid.");
    ...
    if (entity->identity_.source2_handle != source)
        throw Error("Host returned a different Source2 entity handle.");

    return entity;
}

KeelPlayerInfo Service::Player(const KeelPlayerConnection& player) const {
    if (player.slot < 0 || player.reserved || !player.generation)
        throw Error("Player connection is invalid.");
    ...
    if (info.size != sizeof(info) || info.reserved || info.slot != player.slot ||
        ...
        throw Error("Player connection changed or is not connected.");
...
std::unique_ptr<Field> Service::Resolve(const std::string& classname, const std::string& name, unsigned type) {
    ...
    if (field_count_ >= 128)
        throw Error("Schema provider limit (128 field handles) reached.");
    ...
    if (!handle)
        throw Error("Host returned an empty schema field handle.");

    std::unique_ptr<Field> field;

    try {
        field.reset(new Field(shared_from_this(), handle));
    } catch (...) {
        schema_.release_field(plugin_, handle);
        throw;
    }
    ...
    field->class_ = Copy(info.class_name);
    field->name_ = Copy(info.field_name);
    field->profile_ = Copy(info.compatibility_profile);

    if (field->class_ != classname || field->name_ != name)
        throw Error("Host returned a different schema field.");
...
Field::Field(std::shared_ptr<Service> service, KeelSchemaFieldHandle handle)
    : service_(std::move(service)), handle_(handle) {
    ++service_->field_count_;
}

Field::~Field() {
    if (handle_)
        service_->schema_.release_field(service_->plugin_, handle_);
...
void Field::Close() {
    ...
    if (result != KEEL_RESULT_NOT_FOUND && result != KEEL_RESULT_NOT_READY)
        Check(result, "Release schema field");

    handle_ = 0;
}

Entity::Entity(std::shared_ptr<Service> service, KeelEntityHandle handle)
    : service_(std::move(service)), handle_(handle) {
...
Entity::~Entity() {
    ...
    if (handle)
        keep->entities_.release(keep->plugin_, handle);
}
...
bool Entity::Pending() const {
    ...
    const bool constructed = constructed_;
    bool pending{};
    keep->Describe(handle, expected, constructed, &pending);
    return pending;
}

bool Entity::Same(const Entity& other) const {
    const auto keep = service_;
    const auto left_handle = handle_, right_handle = other.handle_;
    ...
    if (keep != other.service_)
        throw Error("Entities belong to different service owners.");
    ...
    if (equal > KEEL_TRUE || (equal == KEEL_TRUE) != Identity(left, right))
        throw Error("Host returned inconsistent entity equality.");
...
void Entity::SetKey(const KeelEntityKeyValue& input) const {
    ...
    const auto text = [](const char* input, unsigned maximum, bool empty) {
        if (!input)
            throw Error("Missing entity key text.");
        ...
        if (length > maximum || (!length && !empty))
            throw Error("Entity key name or text exceeds its bounds.");
    ...
    const auto string = value.type == KEELS2_ENTITY_KEY_STRING
                            ? text(value.string_value, KEELS2_ENTITY_KEY_MAX_STRING, true)
                            : std::string{};

    value.name = name.c_str();
    value.string_value = string.c_str();

    if (value.type == KEELS2_ENTITY_KEY_BOOL && value.int_value != 0 && value.int_value != 1)
        throw Error("Boolean entity key requires zero or one.");

    if (value.type == KEELS2_ENTITY_KEY_FLOAT && !std::isfinite(value.float_value))
        throw Error("Entity key float must be finite.");

    if (value.type == KEELS2_ENTITY_KEY_VECTOR || value.type == KEELS2_ENTITY_KEY_ANGLES)
        for (const float number : value.vector_value)
            if (!std::isfinite(number))
                throw Error("Entity key vector must be finite.");

    keep->Thread();

    if (!constructed)
        throw Error("Entity was not created by this owner.");
    ...
    ++hold.count;
    bool pending{};
    keep->Describe(handle, expected, true, &pending);

    if (!pending)
        throw Error("Entity construction is already consumed.");
...
void Entity::Spawn(bool& invoked) const {
    ...
    KeelBool called{};
    const auto result = keep->construction_.spawn(keep->plugin_, handle, &called);
    invoked = called != KEEL_FALSE;

    if (called > KEEL_TRUE)
        throw Error("Host returned an invalid spawn invocation marker.");
...
std::array<unsigned,2> Service::InputCapabilities() {
    ...
    if (!keep->input_.capabilities)
        throw Error("Entity input service is unavailable.");
    ...
    if ((types[0] | types[1]) & ~511u)
        throw Error("Host returned unknown entity input types.");
...
void Entity::Input(const char* name, const KeelEntityInputValue& value, bool& invoked,
    ...
    for (std::size_t i = 0; i < handles.size(); ++i) if (participants[i]) {
            if (participants[i]->service_ != keep)
                throw Error("Input entities belong to different service owners.");

            handles[i] = participants[i]->handle_;
            expected[i] = participants[i]->identity_;

            if (!handles[i])
                throw Error("Input entity handle is closed.");
    }

    if ((value.type == KEELS2_INPUT_ENTITY) != (value_entity != nullptr))
        throw Error("Input entity payload does not match its type.");

    keels2::detail::EntityInputCopy copy;
    Check(copy.Assign(name,value,queued ? KEEL_TRUE : KEEL_FALSE,delay),"Entity input value");
    keep->Thread();

    if (!keep->input_.dispatch)
        throw Error("Entity input service is unavailable.");
    ...
    for (std::size_t i = 0; i < handles.size(); ++i)
        if (handles[i])
            keep->Describe(handles[i], expected[i], false);

    KeelEntityInputRequest request{};
    request.size = sizeof(request);
    request.input = copy.name.data();
    request.value = copy.value;
    request.activator = handles[1];
    request.caller = handles[2];
    request.value_entity = handles[3];
    ...
    if (called > KEEL_TRUE)
        throw Error("Host returned an invalid input invocation marker.");
...
void Entity::Read(const Field& field, void* output, unsigned size) const {
    const auto keep = service_;
    const auto handle = handle_, property = field.handle_;
    const auto expected = identity_;

    if (!handle || !property)
        throw Error("Entity or schema field is closed.");

    if (field.service_ != keep || field.size_ != size)
        throw Error("Schema field belongs to another owner or has a different type.");
...
void Entity::Write(const Field& field, const void* value, unsigned size) const {
    ...
    if (field.service_ != service || field.size_ != size)
        throw Error("Schema field belongs to another owner or has a different type.");

    service->Thread();

    if (service->active_writes_ >= 8)
        throw Error("Entity write recursion limit (8) reached.");
    ...
    } hold(service->active_writes_);

    if (!(service->WriteCapabilities() & KEELS2_ENTITY_WRITE_NUMERIC_FIELDS))
        throw Error("Entity field writes are unsupported by this game build.");
    ...
    if (!Identity(expected, current))
        throw Error("Entity identity changed before the write.");
...
void Entity::Tool(unsigned kind, const KeelEntityTeleport* request, const char* model) const {
    ...
    } hold(service->active_tools_);

    if (constructed && kind == KEELS2_ENTITY_TOOL_TELEPORT) {
        bool pending{};
        service->Describe(entity, expected, true, &pending);

        if (pending) {
            Check(service->construction_.teleport(service->plugin_, entity, request), "Teleport pending entity");
            return;
        }
    }

    if (!(service->ToolCapabilities() & kind))
        throw Error("Entity operation is unsupported by this game build.");

    KeelEntityInfo current{sizeof(current), -1, KEELS2_INVALID_SOURCE2_ENTITY_HANDLE, 0, 0};
    Check(service->entities_.describe(service->plugin_,entity,&current), "Validate entity operation");

    if (!Identity(expected, current))
        throw Error("Entity identity changed before the operation.");
    ...
    if (kind == KEELS2_ENTITY_TOOL_TELEPORT)
        ...
        Check(service->tools_.remove(service->plugin_, entity), "Remove entity");
}
...
void Entity::Teleport(unsigned flags, const std::array<float,3>& position,
    ...
    if (!flags || (flags & ~7u))
        throw Error("Teleport requires position, angles or velocity flags.");
    ...
    for (unsigned i = 0; i < 3; ++i) if (flags & (1u<<i))
        for (unsigned j = 0; j < 3; ++j) {
            ...
            if (!std::isfinite(value))
                throw Error("Teleport requires finite selected vectors.");
...
void Entity::SetModel(const std::string& model) const {
    ...
    if (asset.empty() || asset.size() > KEELS2_ENTITY_MODEL_MAX_BYTES ||
        std::any_of(asset.begin(), asset.end(), [](unsigned char c) {
            return c < 32 || c == 127;
        }))
...
void Entity::Remove() const {
    Tool(KEELS2_ENTITY_TOOL_REMOVE, nullptr, nullptr);
}
...
void Entity::SetNumber(const Field& field, float value) const {
    if (!std::isfinite(value))
        throw Error("Field write requires a finite float.");

    if (field.type_ == KEELS2_SCHEMA_FLOAT32)
        Write(field, &value, sizeof(value));
    else if (field.type_ == KEELS2_SCHEMA_FLOAT64) {
        const double wide = value;
        Write(field, &wide, sizeof(wide));
    } else
        throw Error("Field is not floating point.");
}

void Entity::SetVector(const Field& field, const std::array<float, 3>& value) const {
    if (field.type_ != KEELS2_SCHEMA_VECTOR3)
        throw Error("Field is not a vector.");

    if (!std::all_of(value.begin(), value.end(), [](float number) {
            return std::isfinite(number);
        }))
        throw Error("Vector write requires finite coordinates.");

    Write(field, value.data(), sizeof(value));
}

std::int32_t Entity::Integer(const Field& field) const {
    const auto type = field.type_;
    IntegerType(type);
    ...
    return std::visit(
        [](auto value) -> std::int32_t {
            if (value > std::numeric_limits<std::int32_t>::max())
                throw Error("Integer exceeds SourcePawn cell range; use decimal text.");
...
float Entity::Number(const Field& field) const {
    ...
    if (field.type_ == KEELS2_SCHEMA_FLOAT32) {
        float single{};
        Read(field, &single, sizeof(single));
        value = single;
    } else if (field.type_ == KEELS2_SCHEMA_FLOAT64)
        Read(field, &value, sizeof(value));
    else
        throw Error("Field is not floating point.");

    if (!std::isfinite(value) || std::abs(value) > std::numeric_limits<float>::max())
        throw Error("Value exceeds finite SourcePawn float range.");
...
std::array<float, 3> Entity::Vector(const Field& field) const {
    ...
    Read(field, value.data(), sizeof(value));

    if (!std::all_of(value.begin(), value.end(), [](float number) {
            return std::isfinite(number);
        }))
        throw Error("Vector contains a nonfinite coordinate.");

    return value;
}

std::uint32_t Entity::SourceHandle(const Field& field) const {
    if (field.type_ != KEELS2_SCHEMA_ENTITY_HANDLE)
        throw Error("Field is not an entity handle.");
