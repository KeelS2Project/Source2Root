...
Database::Database(const std::filesystem::path& filename) {
    ...
    sqlite3_set_authorizer(database_, [](void* raw, int action, const char* first, const char*, const char*, const char*) {
        ...
        if (action == SQLITE_PRAGMA &&
            (!first || (std::strcmp(first, "table_info") && std::strcmp(first, "index_list") &&
                        std::strcmp(first, "foreign_key_list"))))
            return SQLITE_DENY;
...
void Database::Budget() {
    deadline_ = std::chrono::steady_clock::now() + std::chrono::milliseconds(25);
}

void Database::Check(int result) const {
    if (result != SQLITE_OK)
        throw Error(sqlite3_errmsg(database_));
}

bool Database::InTransaction() const {
    return sqlite3_get_autocommit(database_) == 0;
}

std::int64_t Database::InsertId() const {
...
void Database::Execute(const std::string& sql) {
    ...
    if (!statement)
        throw Error("Empty SQL statement.");
...
Statement::Statement(std::shared_ptr<Database> database, const std::string& sql) : database_(std::move(database)) {
    if (!database_ || sql.empty() || sql.size() > 4095)
        throw Error("SQL must contain one statement of at most 4095 bytes.");
    ...
    try {
        while (tail && *tail) {
            ...
            if (multiple)
                throw Error("Prepare accepts exactly one SQL statement.");
...
void Statement::Parameter(int index) const {
    if (started_ || index < 1 || index > sqlite3_bind_parameter_count(statement_))
        throw Error("Invalid parameter or statement needs resetting.");
}

void Statement::BindInt(int index, std::int32_t value) {
    Parameter(index);
    database_->Check(sqlite3_bind_int(statement_, index, value));
}
...
void Statement::BindFloat(int index, double value) {
    ...
    if (!std::isfinite(value))
        throw Error("Database values must be finite.");
...
void Statement::BindNull(int index) {
    Parameter(index);
    database_->Check(sqlite3_bind_null(statement_, index));
}

bool Statement::Step() {
    if (done_)
        throw Error("Statement is complete; reset it before reuse.");
    ...
    if (result != SQLITE_DONE)
        throw Error(sqlite3_errmsg(database_->database_));
...
int Statement::Parameters() const {
    return sqlite3_bind_parameter_count(statement_);
}

void Statement::Column(int index) const {
    if (!row_ || index < 0 || index >= Columns())
        throw Error("No current row or invalid column.");
}
...
double Statement::Float(int column) const {
    Column(column);
...
std::string Statement::String(int column) const {
    ...
    if (size > 4095 || std::memchr(text, 0, size))
        throw Error("Value cannot fit a SourcePawn string.");
