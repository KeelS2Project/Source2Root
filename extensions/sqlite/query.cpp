...
QueryResult Query(const std::filesystem::path& filename, const db::QueryInput& input, const std::atomic_bool& canceled) {
    ...
    const auto check = [&] {
        if (canceled.load(std::memory_order_relaxed)) throw Error("Database request canceled.");
        if (std::chrono::steady_clock::now() >= deadline) throw Error("Database request time limit exceeded.");
    };
    check();
    ...
    if (static_cast<std::size_t>(statement.Parameters()) != input.parameters.size()) throw Error("Query parameter count does not match its bindings.");
    for (std::size_t i = 0; i < input.parameters.size(); ++i) {
        const auto& parameter = *input.parameters[i];
        const auto index = static_cast<int>(i + 1);
        if (const auto* value = std::get_if<std::int32_t>(&parameter)) statement.BindInt(index, *value);
        else if (const auto* value = std::get_if<double>(&parameter)) statement.BindFloat(index, *value);
        else if (const auto* value = std::get_if<std::string>(&parameter)) statement.BindString(index, *value);
        else statement.BindNull(index);
    }
    QueryResult result;
    result.columns = statement.Columns();
    if (result.columns > 32) throw Error("Database result exceeds 32 columns.");
    std::size_t bytes = 0;
    check();
    while (statement.Step()) {
        check();
        if (result.rows.size() >= 256) throw Error("Database result exceeds 256 rows.");
        ...
        for (int column = 0; column < result.columns; ++column) {
            ...
            if (!value.null) {
                ...
                if (bytes > 256 * 1024) throw Error("Database result exceeds 256 KiB.");
                try { value.integer = statement.Int(column); } catch (const Error&) {}
