...
public:
    static constexpr PluginInfo Info{
        "Source2Root Regex", "KeelS2 Project", "1.0.0", "Bounded PCRE2 patterns, captures and replacement"};
...
private:
    ...
    template <typename T> static void Available(std::vector<std::weak_ptr<T>>& values) {
        ...
        if (values.size() >= 64)
            throw rx::Error("Regex provider handle limit (64 patterns or 64 results) reached.");
    }
    ...
    static rx::Pattern& Pattern(NativeCall& call) {
        return *call.Resource<PatternRef>(call.Int(1), PatternType);
    }

    static rx::Result& Result(NativeCall& call) {
        return *call.Resource<ResultRef>(call.Int(1), ResultType);
    }
    ...
    static bool Boolean(NativeCall& call, unsigned index) {
        ...
        if (value != 0 && value != 1)
            throw rx::Error("Regex global option must be false or true.");
    ...
    static void CaptureText(NativeCall& call, const std::string& value) {
        ...
        if (capacity <= 0 || value.size() >= static_cast<unsigned>(capacity))
            throw rx::Error("Regex capture output buffer is too small.");
    ...
    std::int32_t Compile(NativeCall& call) {
        ...
        try {
            ...
            ErrorText(call, 3, call.Int(4), error);
            call.OutputCell(5, error.offset);
            return call.Fail(error.what());
        }
    }

    std::int32_t Close(NativeCall& call) {
        call.Close(call.Int(1), PatternType);
        return 1;
    }

    std::int32_t CloseResult(NativeCall& call) {
        call.Close(call.Int(1), ResultType);
        return 1;
    }
    ...
    std::int32_t Match(NativeCall& call) {
        ...
        try {
            ...
            ErrorText(call, 5, call.Int(6), error);
            return call.Fail(error.what());
        }
    }

    std::int32_t Count(NativeCall& call) {
        return static_cast<std::int32_t>(Result(call).Count());
    }

    std::int32_t Groups(NativeCall& call) {
        return static_cast<std::int32_t>(Result(call).Groups());
    }
    ...
    std::int32_t Named(NativeCall& call) {
        ...
        call.Output(4, call.Int(5), "");
        return Invoke(call, [&] {
            auto& result = Result(call);
            const auto match = call.Int(2);
    ...
    std::int32_t Offsets(NativeCall& call) {
        ...
        return Invoke(call, [&] {
            ...
            call.OutputCell(4, span.start);
            call.OutputCell(5, span.end);
            return span.start >= 0 ? 1 : 0;
        });
    }
    ...
    std::int32_t Replace(NativeCall& call) {
        ...
        call.Output(4, call.Int(5), "");
        call.Output(7, call.Int(8), "");

        try {
            const auto result = Pattern(call).Replace(subject, replacement, Boolean(call, 6), call.Int(9), call.Int(5));
            call.Output(4, call.Int(5), result.text);
            return static_cast<std::int32_t>(result.count);
        } catch (const rx::Error& error) {
            ErrorText(call, 7, call.Int(8), error);
            return call.Fail(error.what(), -1);
        }
    }
};
}
