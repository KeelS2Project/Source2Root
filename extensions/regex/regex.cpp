...
struct Budget {
    ...
    static void* Allocate(PCRE2_SIZE size, void* data) noexcept {
        ...
        if (size > budget.limit - budget.used || sizeof(Header) > budget.limit - budget.used - size)
            return nullptr;
...
struct Operation {
    ...
    explicit Operation(const pcre2_code* code) {
        ...
        if (general) {
            context.reset(pcre2_match_context_create(general.get()));
            data.reset(pcre2_match_data_create_from_pattern(code, general.get()));
        }

        if (!context || !data)
            throw Error("Regex operation memory limit reached.");
    ...
    void Check(int rc) const {
        if (too_many)
            throw Error("Regex global replacement limit (256) reached; output discarded.");

        if (exhausted)
            throw Error("Regex operation work budget reached.");
...
Pattern::Pattern(std::string_view pattern, std::uint32_t flags) : impl_(std::make_unique<Impl>()) {
    ...
    if (flags & ~511u)
        throw Error("Unknown regex compile flags.");

    auto& self = *impl_;
    self.general.reset(pcre2_general_context_create(Budget::Allocate, Budget::Release, &self.budget));

    if (!self.general)
        throw Error("Regex compile memory limit reached.");

    Ptr<pcre2_compile_context, pcre2_compile_context_free> context(pcre2_compile_context_create(self.general.get()),
    ...
    for (unsigned i = 0; i < std::size(options); ++i)
        if (flags & (1u << i))
            native |= options[i];
    ...
    if (!self.code)
        throw Error(Message(error), offset <= pattern.size() ? static_cast<int>(offset) : -1);

    if (Clock::now() > deadline)
        throw Error("Regex compilation time budget reached.");

    pcre2_pattern_info(self.code.get(), PCRE2_INFO_CAPTURECOUNT, &self.groups);

    if (self.groups > CaptureLimit)
        throw Error("Regex capture group limit (64) reached.");

    std::uint32_t count = 0, width = 0;
    PCRE2_SPTR names = nullptr;
...
Result Pattern::Match(std::string_view subject, bool all, unsigned offset) const {
    ...
    if (offset > subject.size())
        throw Error("Regex byte offset is outside subject.");

    Result result;
    result.subject_ = subject;
    result.groups_ = impl_->groups;
    result.names_ = impl_->names;
    Operation op(impl_->code.get());
    PCRE2_SIZE start = offset;
    std::uint32_t options = 0;
    do {
        const auto rc = pcre2_match(
            impl_->code.get(), Bytes(subject), subject.size(), start, options, op.data.get(), op.context.get());
        ...
        if (rc == 0)
            throw Error("Regex capture storage is insufficient.");

        if (result.Count() == MatchLimit)
            throw Error("Regex global match limit (256) reached; results discarded.");
        ...
        for (unsigned i = 0; i < static_cast<unsigned>(rc); ++i) {
            if (spans[i * 2] == PCRE2_UNSET)
                continue;

            if (spans[i * 2] > spans[i * 2 + 1] || spans[i * 2 + 1] > subject.size())
                throw Error("Regex produced invalid capture offsets.");
...
Replacement Pattern::Replace(
    std::string_view subject, std::string_view replacement, bool all, unsigned offset, unsigned capacity) const {
    Text(subject);
    Text(replacement);
    ...
    if (!capacity || capacity > TextLimit + 1)
        throw Error("Regex output capacity must be 1..4096.");

    Operation op(impl_->code.get());
    std::array<PCRE2_UCHAR, TextLimit + 1> output{};
    PCRE2_SIZE length = capacity;
...
Span Result::Offsets(unsigned match, unsigned group) const {
    if (match >= matches_.size() || group > groups_)
        throw Error("Regex match or group index is out of range.");
...
unsigned Result::Named(unsigned match, std::string_view name) const {
    if (match >= matches_.size())
        throw Error("Regex match index is out of range.");
    ...
    for (const auto& [candidate, group] : names_) if (candidate == name) {
            ...
            if (Offsets(match, group).start >= 0)
                return group;
    }

    if (!first)
        throw Error("Unknown regex capture name.");
