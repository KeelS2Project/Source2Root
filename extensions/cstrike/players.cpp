...
Players::Players(KeelPluginHandle plugin,
                 const KeelEntitiesApi& entities,
                 const KeelPlayersApi& players,
                 const KeelNativeRuntimeApi& runtime,
                 const KeelPlayerManagementApi& management,
                 const KeelPlayerStatisticsApi* statistics)
    ...
    if (!plugin || entities.size != sizeof(entities) || entities.api_version != KEELS2_ENTITIES_API_VERSION ||
        ...
        players.size != sizeof(players) || players.api_version != KEELS2_PLAYERS_API_VERSION ||
        !players.validate_connection || runtime.size != sizeof(runtime) ||
        runtime.api_version != KEELS2_NATIVE_RUNTIME_API_VERSION || !runtime.check_game_thread ||
        ...
        throw Error("Incompatible player management services.");

    if (statistics &&
        (statistics->size != sizeof(*statistics) || statistics->api_version != KEELS2_PLAYER_STATISTICS_API_VERSION ||
         !statistics->capabilities || !statistics->read || !statistics->write))
        throw Error("Incompatible player statistics service.");
}

void Players::Thread() const {
    Check(runtime_.check_game_thread(plugin_), "Counter-Strike operation");
}
...
KeelPlayerInfo Players::Player(const KeelPlayerConnection& connection) const {
    if (connection.slot < 0 || connection.reserved || !connection.generation)
...
void Players::Apply(const KeelPlayerConnection& connection, unsigned kind, int team) {
    ...
    if ((kind != KEELS2_PLAYER_MANAGEMENT_RESPAWN && kind != KEELS2_PLAYER_MANAGEMENT_CHANGE_TEAM &&
        ...
        (kind == KEELS2_PLAYER_MANAGEMENT_RESPAWN
             ? team != 0
             : team < (kind == KEELS2_PLAYER_MANAGEMENT_SWITCH_TEAM ? 2 : 1) || team > 3))
        throw Error("Expected respawn, a team from 1..3 for ChangeTeam, or T/CT (2..3) for SwitchTeam.");

    if (!(Capabilities() & kind))
        throw Error("This Counter-Strike action is unavailable for the current game build.");
...
void Players::Controller(const KeelPlayerConnection& connection,
                         const std::function<void(KeelEntityHandle)>& operation,
                         bool validate_after) {
    if (active_ >= 8)
        throw Error("Counter-Strike player operation recursion limit (8) reached.");

    struct Active {
        unsigned& count;
        explicit Active(unsigned& value) : count(value) {
            ++count;
        }
    ...
    if (!entity)
        throw Error("Host returned an empty player controller handle.");
    ...
    struct EntityHold {
        ...
        KeelEntityHandle entity;
        ~EntityHold() {
            api.release(plugin, entity);
        }
    } hold{entities_, plugin_, entity};
    KeelEntityInfo first{};
    first.size = sizeof(first);
    Check(entities_.describe(plugin_, entity, &first), "Describe player controller");

    if (!Identity(first, before.controller_handle))
        throw Error("Host returned an inconsistent player controller.");

    const auto after = Player(connection);

    if (after.controller_handle != before.controller_handle)
        throw Error("Player controller changed during the action lookup.");
    ...
    if (validate_after) {
        ...
        if (player.controller_handle != before.controller_handle || !Identity(current, before.controller_handle) ||
            current.epoch != first.epoch || current.index != first.index)
            throw Error("Player changed while reading statistics.");
    }
...
std::pair<unsigned,unsigned> Players::StatisticsCapabilities() const {
    ...
    if (!statistics_.capabilities)
        throw Error("Player statistics service is unavailable.");
...
std::int32_t Players::GetStatistic(const KeelPlayerConnection& player, unsigned key) {
    ...
    if (!(StatisticsCapabilities().first & key))
        throw Error("This player statistic is not readable for the current game build.");

    std::int32_t value = 0;
    Controller(
        player,
        [&](KeelEntityHandle entity) {
            Check(statistics_.read(plugin_, entity, key, &value), "Read player statistic");
        },
        true);
...
void Players::SetStatistic(const KeelPlayerConnection& player, unsigned key, std::int32_t value) {
    ...
    if (value < 0)
        throw Error("Player statistic value must be nonnegative.");

    if (!(StatisticsCapabilities().second & key))
        throw Error("This player statistic is not writable for the current game build.");

    Controller(
        player,
        [&](KeelEntityHandle entity) {
            Check(statistics_.write(plugin_, entity, key, value), "Write player statistic");
        },
        false);
}
}
