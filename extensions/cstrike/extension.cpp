...
private:
    ...
    const KeelPlayerStatisticsApi* OptionalStatistics() {
        const void* raw = nullptr;
        const auto result = HostContext().QueryService(KEELS2_PLAYER_STATISTICS_SERVICE_NAME,KEELS2_PLAYER_STATISTICS_API_VERSION,&raw);
        if (result == KEEL_RESULT_NOT_FOUND || result == KEEL_RESULT_UNSUPPORTED) return nullptr;
        if (result != KEEL_RESULT_OK || !raw) throw cs::Error("Could not query player statistics service.");
    ...
    bool OnExtensionStart() override {
        players_ = std::make_unique<cs::Players>(HostContext().PluginHandle(),
            ...
            Require<KeelPlayerManagementApi>(KEELS2_PLAYER_MANAGEMENT_SERVICE_NAME, KEELS2_PLAYER_MANAGEMENT_API_VERSION), OptionalStatistics());
    ...
    std::int32_t StatisticsCapabilities(NativeCall& call) {
        ...
        return Invoke(call,[&] {
            ...
            call.OutputCell(1,readable); call.OutputCell(2,writable);
        });
    }
    std::int32_t ComponentStat(NativeCall& call, unsigned key, bool write) {
        ...
        return Invoke(call,[&] {
            ...
            if (!call.Player(call.Int(1),player)) throw cs::Error("Player connection is no longer available.");
            const KeelPlayerConnection connection{player.slot,0,player.connection};
            if (write) players_->SetStatistic(connection,key,call.Int(2));
            else call.OutputCell(2,players_->GetStatistic(connection,key));
        });
    }
    std::int32_t GetMoney(NativeCall& call) { return ComponentStat(call,KEELS2_PLAYER_STAT_MONEY,false); }
    std::int32_t SetMoney(NativeCall& call) { return ComponentStat(call,KEELS2_PLAYER_STAT_MONEY,true); }
    std::int32_t GetKills(NativeCall& call) { return ComponentStat(call,KEELS2_PLAYER_STAT_MATCH_KILLS,false); }
    std::int32_t SetKills(NativeCall& call) { return ComponentStat(call,KEELS2_PLAYER_STAT_MATCH_KILLS,true); }
    std::int32_t GetDeaths(NativeCall& call) { return ComponentStat(call,KEELS2_PLAYER_STAT_MATCH_DEATHS,false); }
    std::int32_t SetDeaths(NativeCall& call) { return ComponentStat(call,KEELS2_PLAYER_STAT_MATCH_DEATHS,true); }
    std::int32_t GetAssists(NativeCall& call) { return ComponentStat(call,KEELS2_PLAYER_STAT_MATCH_ASSISTS,false); }
    std::int32_t SetAssists(NativeCall& call) { return ComponentStat(call,KEELS2_PLAYER_STAT_MATCH_ASSISTS,true); }
};
}
