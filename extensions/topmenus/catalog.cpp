...
void Label(const std::string& value) {
    if (value.empty() || value.size() > 96 || std::any_of(value.begin(), value.end(), [](unsigned char c) {
            return c < 32 || c == 127;
        }))
...
void Object::Close() {
    ...
    if (auto cleanup = std::exchange(release, {}))
        cleanup();
}
...
void Object::SetLabel(std::string value) {
    ...
    if (!live)
        throw Error("Top menu object is closed.");
...
std::shared_ptr<Object> Menu::Add(Kind kind, std::string key, std::string category, std::string label,
    ...
    if (!owner || plugin.empty() || (kind == Kind::Category && !category.empty()))
        throw Error("Invalid top menu registration.");

    std::erase_if(objects, [](const auto& weak) {
        const auto object = weak.lock();
        return !object || !object->live;
    });

    if (objects.size() >= 256 || next_sequence == std::numeric_limits<std::uint64_t>::max())
        throw Error("Top menu object limit (256) reached.");
    ...
    object->menu = shared_from_this();
    object->kind = kind;
    object->key = std::move(key);
    object->category = std::move(category);
    object->label = std::move(label);
    object->permission = std::move(permission);
    object->plugin = std::move(plugin);
    object->owner = owner;
    object->order = order;
    object->sequence = next_sequence++;
...
std::vector<std::shared_ptr<Object>> Menu::Active(const std::function<bool(std::uint64_t)>& running) {
    ...
    for (const auto& weak : objects) if (auto object = weak.lock(); object && object->live && running(object->owner)) {
        ...
        if (!current || current->sequence < object->sequence)
            current = std::move(object);
    }

    std::vector<std::shared_ptr<Object>> result;

    for (auto& [key, object] : latest)
...
Page Menu::Build(const std::string& category, std::size_t requested,
    ...
    for (const auto& object : active) if (object->kind == Kind::Category) {
        ...
        if (revision != page.revision)
            throw Error("Top menu changed during its access callback.");
    }
    ...
    auto visible = [&](const auto& object) {
        return visibility.at(object->key) != Access::Hidden;
    };
    ...
    if (!category.empty()) {
        ...
        if (found == active.end())
            throw Error("Top menu category is no longer available.");
    ...
    for (const auto& object : active) {
        ...
        if (object->kind == Kind::Item && object->category == category)
            shown.push_back(object);

        if (object->kind == Kind::Category && category.empty() &&
            std::any_of(active.begin(), active.end(), [&](const auto& child) {
    ...
    if (shown.empty())
        page.rows.push_back({"No available actions", false, {}, -1, false, true});

    if (page.page)
        page.rows.push_back({"Previous page", true, {}, static_cast<int>(page.page - 1)});

    if (first + per_page < shown.size())
        page.rows.push_back({"Next page", true, {}, static_cast<int>(page.page + 1)});

    if (!category.empty())
        page.rows.push_back({"Back to categories", true, {}, -1, true});
...
std::shared_ptr<Menu> Catalog::Open(const std::string& name, const std::string& title) {
    Key(name);
    Label(title);
    std::erase_if(menus_, [](const auto& weak) {
        return weak.expired();
    });

    for (const auto& weak : menus_)
        if (auto menu = weak.lock(); menu && menu->name == name)
            return menu;

    if (menus_.size() >= 32)
        throw Error("Shared top menu limit (32) reached.");
