...
namespace {
db::QueryResult Run(mysql::Session& session, const std::string& sql, std::initializer_list<db::Parameter> parameters = {}) {
    db::QueryInput query{sql, {}};
    for (const auto& value : parameters) query.Bind(static_cast<int>(query.parameters.size() + 1), value);
...
void Initialize(mysql::Session& session) {
    ...
    Run(session, "CREATE TABLE IF NOT EXISTS sr_prefs_cookies (name VARBINARY(30) PRIMARY KEY, description VARBINARY(255) NOT NULL, access INT NOT NULL) ENGINE=InnoDB");
    Run(session, "CREATE TABLE IF NOT EXISTS sr_prefs_values (account VARBINARY(20) NOT NULL, cookie VARBINARY(30) NOT NULL, value VARBINARY(255) NOT NULL, updated BIGINT NOT NULL, PRIMARY KEY(account,cookie), FOREIGN KEY(cookie) REFERENCES sr_prefs_cookies(name)) ENGINE=InnoDB");
    const auto engines = Run(session, "SELECT TABLE_NAME,ENGINE FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME IN ('sr_prefs_schema','sr_prefs_cookies','sr_prefs_values')");
    if (engines.rows.size() != 3) throw Error("Client preferences tables are unavailable.");
    for (const auto& row : engines.rows) if (row[1].text != "InnoDB") throw Error("Client preferences tables require InnoDB.");
...
Definition Row(const std::vector<db::QueryValue>& row) {
    if (row[0].null || row[1].null || !row[2].integer) throw Error("Invalid client preferences definition.");
...
std::vector<Definition> MysqlStore::Catalog() {
    mysql::Session session(settings_, canceled_); Initialize(session);
...
Definition MysqlStore::Register(const Definition& cookie) {
    ...
    if (!found.rows.empty()) {
        if (Row(found.rows[0]) != cookie) throw Error("Cookie already exists with different description or access mode.");
    } else {
        ...
        Run(session, "INSERT INTO sr_prefs_cookies(name,description,access) VALUES(?,?,?)",
...
Values MysqlStore::Load(std::uint64_t account) {
    ...
    mysql::Session session(settings_, canceled_); Initialize(session);
    Values values;
    const auto rows = Run(session, "SELECT v.cookie,v.value,v.updated FROM sr_prefs_values v JOIN sr_prefs_cookies c ON c.name=v.cookie WHERE account=? ORDER BY v.cookie", {std::to_string(account)});
    for (const auto& row : rows.rows) {
        Validate({row[0].text, "", Access::Public}); ValidateValue(row[1].text);
...
void MysqlStore::Save(std::uint64_t account, const Values& values) {
    ...
    for (const auto& [name, value] : values) {
        Validate({name, "", Access::Public}); ValidateValue(value.text);
        if (value.updated < 0) throw Error("Invalid client preferences timestamp.");
    }
    mysql::Session session(settings_, canceled_); Initialize(session);
    std::set<std::string> registered;
    for (const auto& row : Run(session, "SELECT name FROM sr_prefs_cookies").rows) registered.insert(row[0].text);
    for (const auto& [name, value] : values) {
        if (!registered.contains(name)) throw Error("Cannot save an unregistered cookie.");
        ...
        Run(session, "INSERT INTO sr_prefs_values(account,cookie,value,updated) VALUES(?,?,?,?) ON DUPLICATE KEY UPDATE value=?,updated=?",
            {std::to_string(account), name, value.text, std::to_string(value.updated), value.text, std::to_string(value.updated)});
    }
    session.Commit();
}
}
