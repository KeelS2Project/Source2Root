...
std::string ChoiceLabel(PrefabType type, bool enabled) {
    if (type == PrefabType::YesNo || type == PrefabType::YesNoInteger) return enabled ? "Yes" : "No";
    if (type == PrefabType::OnOff || type == PrefabType::OnOffInteger) return enabled ? "On" : "Off";
    throw Error("Invalid preferences prefab type.");
}
std::string ChoiceValue(PrefabType type, bool enabled) {
    if (type == PrefabType::YesNoInteger || type == PrefabType::OnOffInteger) return enabled ? "1" : "0";
    if (type == PrefabType::YesNo) return enabled ? "yes" : "no";
    if (type == PrefabType::OnOff) return enabled ? "on" : "off";
    throw Error("Invalid preferences prefab type.");
}
...
MenuPage BuildSettings(Service& service, const Identity& player,
    ...
    if (!service.Ready() || service.Status(player) != State::Ready) throw Error("Client preferences are not cached.");
    ...
    for (const auto& weak : prefabs) if (auto prefab = weak.lock()) {
        if (prefab->cookie->state == State::Ready) options[prefab->cookie->definition.name] = std::move(prefab);
    }
    ...
    for (std::size_t i = begin; i < std::min(cookies.size(), begin + SettingsPerPage); ++i) {
        ...
        if (definition.access == Access::Protected) row.text += " [read only]";
        if (editable) row.prefab = option->second;
        menu.rows.push_back(std::move(row));
    }
    if (cookies.empty()) menu.rows.push_back({"No visible preferences", false, {}, -1});
    if (menu.page) menu.rows.push_back({"Previous settings", true, {}, static_cast<int>(menu.page - 1)});
    if (begin + SettingsPerPage < cookies.size()) menu.rows.push_back({"More settings", true, {}, static_cast<int>(menu.page + 1)});
    return menu;
}
