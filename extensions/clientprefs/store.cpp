...
void Validate(const Definition& cookie) {
    ...
    if (cookie.access < Access::Public || cookie.access > Access::Private)
        throw Error("Invalid cookie access mode.");
}
...
Store::Store(const std::filesystem::path& filename) {
    if (filename.empty() || filename.parent_path().empty())
        throw Error("Missing client preferences data directory.");

    std::filesystem::create_directories(filename.parent_path());

    if (std::filesystem::is_symlink(filename.parent_path()))
        throw Error("Client preferences directory must not be a symbolic link.");
    ...
    try {
        ...
        if (schema.Step()) {
            if (schema.Int(0) != 1 || schema.Step())
                throw Error("Unsupported client preferences schema.");
        } else
            database_->Execute("INSERT INTO sr_prefs_schema VALUES (1)");

        database_->Execute(
            "CREATE TABLE IF NOT EXISTS sr_prefs_cookies (name TEXT PRIMARY KEY, description TEXT NOT NULL, access INTEGER NOT NULL CHECK(access BETWEEN 0 AND 2))");

        database_->Execute(
            "CREATE TABLE IF NOT EXISTS sr_prefs_values (account TEXT NOT NULL, cookie TEXT NOT NULL, value TEXT NOT NULL, updated INTEGER NOT NULL, PRIMARY KEY(account,cookie))");
...
std::vector<Definition> Store::Catalog() {
    ...
    while (query.Step()) {
        if (result.size() == MaxCookies)
            throw Error("Client preferences catalog exceeds 256 cookies.");
...
Definition Store::Register(const Definition& cookie) {
    ...
    try {
        ...
        if (find.Step()) {
            ...
            if (!count.Step() || count.Int(0) >= static_cast<int>(MaxCookies))
            ...
            insert.BindString(1, cookie.name);
            insert.BindString(2, cookie.description);
            insert.BindInt(3, static_cast<int>(cookie.access));
            insert.Step();
        }
...
Values Store::Load(std::uint64_t account) {
    ...
    while (query.Step()) {
        if (values.size() == MaxCookies)
            throw Error("Client preferences account exceeds 256 cookies.");
...
void Store::Save(std::uint64_t account, const Values& values) {
    ...
    try {
        ...
        sqlite::Statement save(
            database_,
            "INSERT INTO sr_prefs_values(account,cookie,value,updated) VALUES(?,?,?,?) ON CONFLICT(account,cookie) DO UPDATE SET value=excluded.value,updated=excluded.updated");
        ...
        for (const auto& [name, value] : values) {
            ...
            exists.Reset();
            save.BindString(1, std::to_string(account));
            save.BindString(2, name);
            save.BindString(3, value.text);
            save.BindString(4, std::to_string(value.updated));
            save.Step();
            save.Reset();
        }
