...
std::string Message(std::exception_ptr error) {
    ...
    catch (const std::exception& failure) { return std::string(failure.what()).substr(0, 4095); }
    catch (...) { return "Client preferences worker failed."; }
    return {};
}
}
        ...
        if (!failure) for (const auto& definition : *catalog) {
            if (const auto found = cookies_.find(definition.name); found != cookies_.end()) {
                if (found->second->definition != definition) {
                    ...
                    found->second->error = "Cookie already exists with different description or access mode.";
                    found->second = std::make_shared<Cookie>(Cookie{definition, State::Ready, {}});
                }
                continue;
            }
            if (cookies_.size() == MaxCookies) {
                catalog_state_ = State::Failed; catalog_error_ = "Client preferences catalog limit (256) reached."; break;
            }
            cookies_.emplace(definition.name, std::make_shared<Cookie>(Cookie{definition, State::Ready, {}}));
        }
    })) { catalog_state_ = State::Failed; catalog_error_ = "Client preferences worker queue is full."; }
}
void Service::RetryCatalog() {
    Thread();
    if (catalog_state_ != State::Failed) return;
    catalog_state_ = State::Loading; catalog_error_.clear();
    LoadCatalog();
}
Service::~Service() = default;
void Service::Thread() const {
    if (std::this_thread::get_id() != owner_) throw std::logic_error("Client preferences require the owning thread.");
}
...
bool Service::Submit(std::function<void()> work, std::function<void(std::exception_ptr)> completion) {
    ...
    try {
        task->ticket = queue_.Submit([work = std::move(work)](const auto&) { work(); },
            [task, completion = std::move(completion)](auto error) {
                task->done = true; completion(error); return true;
            });
    } catch (...) { tasks_.pop_back(); throw; }
    if (task->ticket) return true;
    tasks_.pop_back();
    return false;
}
...
std::shared_ptr<Cookie> Service::Register(const Definition& definition) {
    ...
    if (const auto old = Find(definition.name)) {
        if (old->definition != definition) throw Error("Cookie already exists with different description or access mode.");
        if (old->state != State::Failed) return old;
    ...
    cookie->state = State::Loading; cookie->error.clear();
...
std::vector<Definition> Service::UserCookies() const {
    ...
    for (const auto& [name, cookie] : cookies_)
        if (cookie->state == State::Ready && cookie->definition.access != Access::Private) result.push_back(cookie->definition);
    return result;
}

void Service::Sync(const std::vector<Identity>& players) {
    Thread();
    if (players.size() > MaxClients) throw Error("Client preferences connection limit (128) reached.");
    std::map<int, Identity> next;
    std::set<std::uint64_t> missing;
    for (const auto& player : players) {
        ValidateAccount(player.account);
        if (player.slot < 0 || player.slot >= static_cast<int>(MaxClients) || !player.connection || !next.emplace(player.slot, player).second)
            throw Error("Invalid or duplicate client preferences connection.");
        if (!accounts_.contains(player.account)) missing.insert(player.account);
    }
    // Do not discard existing sessions when an invalid snapshot is supplied.
    if (accounts_.size() + missing.size() > MaxAccounts) throw Error("Client preferences account cache limit (256) reached.");
...
std::shared_ptr<Service::Account> Service::Current(const Identity& player) const {
    ...
    if (found == players_.end() || found->second != player) throw Error("Player connection is no longer available.");
    return accounts_.at(player.account);
}

void Service::CheckCookie(const std::shared_ptr<Cookie>& cookie) const {
    Thread();
    if (!cookie || Find(cookie->definition.name) != cookie) throw Error("Foreign or invalid cookie.");
    if (cookie->state != State::Ready) throw Error(cookie->error.empty() ? "Cookie registration is not complete." : cookie->error);
}
State Service::Status(const Identity& player) const { return Current(player)->state; }
std::string Service::ErrorText(const Identity& player) const { return Current(player)->error; }
...
void Service::Set(const Identity& player, const std::shared_ptr<Cookie>& cookie, const std::string& text, std::int64_t now) {
...
void Service::UserSet(const Identity& player, const std::string& name, const std::string& text, std::int64_t now) {
    ...
    CheckCookie(cookie);
    if (cookie->definition.access != Access::Public) throw Error("This setting cannot be changed by players.");
...
void Service::Load(std::uint64_t id, const std::shared_ptr<Account>& account) {
    ...
    if (!Submit([path, id, values] { *values = Store(path).Load(id); }, [account, values](auto error) {
        ...
        if (!error) {
            ...
            for (const auto& [name, value] : account->dirty) account->values[name] = value;
        }
    })) { account->state = State::Failed; account->error = "Client preferences worker queue is full."; return; }
    account->loading = true; account->state = State::Loading;
}
...
void Service::RetryWrites() {
    Thread();
    for (const auto& [id, account] : accounts_) { account->write_failed = false; Save(id, account); }
}
void Service::Evict() {
    std::set<std::uint64_t> active;
    for (const auto& [slot, player] : players_) active.insert(player.account);
...
void Service::Pump() {
    ...
    for (const auto& [id, account] : accounts_) Save(id, account);
    Evict();
}
bool Service::CanStop() {
    Pump();
    if (Pending()) return false;
    for (const auto& [id, account] : accounts_) if (!account->dirty.empty()) return false;
    return true;
}
