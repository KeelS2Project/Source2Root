...
public:
    static constexpr PluginInfo Info{
        "Source2Root Database", "KeelS2 Project", "1.0.0", "Database access for script plugins"};
...
private:
    ...
    static std::int32_t Sql(NativeCall& call, Function function, std::int32_t failure = 0) {
        try {
            return function();
        } catch (const source2root::sqlite::Error& error) {
            return call.Fail(error.what(), failure);
        } catch (const std::filesystem::filesystem_error&) {
            return call.Fail("Database directory is unavailable.", failure);
        }
    }

    static Connection& Db(NativeCall& call) {
        return call.Resource<Connection>(call.Int(1), ConnectionType);
    }

    static Statement& Stmt(NativeCall& call) {
        return call.Resource<Statement>(call.Int(1), StatementType);
    }
    ...
    static std::filesystem::path Filename(NativeCall& call) {
        ...
        if (std::filesystem::is_symlink(directory))
            throw source2root::sqlite::Error("Database directory must not be a symbolic link.");
    ...
    std::int32_t QueryAsync(NativeCall& call) {
        return Sql(call, [&] {
            ...
            return Submit(call, [filename, sql](const auto& canceled) {
                return source2root::sqlite::Query(filename, sql, canceled);
            });
        });
    }
    ...
    std::int32_t Submit(NativeCall& call, std::function<source2root::db::QueryResult(const std::atomic_bool&)> work) {
        ...
        if (!queue_)
            queue_ = std::make_unique<source2root::WorkQueue>(1, 32);
            ...
            if (!result->ready) {
                if (error) {
                    try {
                        ...
                        result->error = std::string(failure.what()).substr(0, 4095);
                    } catch (...) {
                        result->error = "Database worker failed.";
                    }
                }
        ...
        if (!request->ticket)
            return call.Fail("Database queue is full (32 requests).");
    ...
    std::int32_t CloseQuery(NativeCall& call) {
        call.Close(call.Int(1), QueryType);
        return 1;
    }

    std::int32_t QueryBindInt(NativeCall& call) {
        return Sql(call, [&] {
            Input(call).Bind(call.Int(2), call.Int(3));
            return 1;
        });
    }

    std::int32_t QueryBindFloat(NativeCall& call) {
        return Sql(call, [&] {
            Input(call).Bind(call.Int(2), static_cast<double>(call.Float(3)));
            return 1;
        });
    }

    std::int32_t QueryBindString(NativeCall& call) {
        return Sql(call, [&] {
            Input(call).Bind(call.Int(2), call.String(3));
            return 1;
        });
    }

    std::int32_t QueryBindNull(NativeCall& call) {
        return Sql(call, [&] {
            Input(call).Bind(call.Int(2), {});
            return 1;
        });
    }
    ...
    std::int32_t ExecuteSQLiteAsync(NativeCall& call) {
        return Sql(call, [&] {
            ...
            input.Validate();
            return Submit(call, [filename, input](const auto& canceled) {
                return source2root::sqlite::Query(filename, input, canceled);
            });
        });
    }
        ...
        return Sql(call, [&] {
            ...
            return Submit(call, [config, shared, profile, plugin, input](const auto& canceled) {
                ...
                if (settings.driver == "sqlite") {
                    ...
                    if (std::filesystem::is_symlink(shared))
                        throw source2root::db::Error("Database directory must not be a symbolic link.");
    ...
    static source2root::sqlite::QueryResult& Completed(NativeCall& call) {
        ...
        if (!result.ready)
            throw source2root::sqlite::Error("Database request is not complete.");

        if (!result.error.empty())
            throw source2root::sqlite::Error(result.error);
    ...
    std::int32_t RequestReady(NativeCall& call) {
        return RequestResult(call).ready ? 1 : 0;
    }

    std::int32_t ResultRows(NativeCall& call) {
        return Sql(
            call,
            [&] {
                return static_cast<int>(Completed(call).rows.size());
            },
            -1);
    }

    std::int32_t ResultColumns(NativeCall& call) {
        return Sql(
            call,
            [&] {
                return Completed(call).columns;
            },
            -1);
    }

    std::int32_t ResultIsNull(NativeCall& call) {
        return Sql(
            call,
            [&] {
                return Value(call).null ? 1 : 0;
            },
            -1);
    }
    ...
    std::int32_t ResultInt(NativeCall& call) {
        ...
        return Sql(call, [&] {
            ...
            if (!value.integer)
                throw source2root::sqlite::Error("Value is null or exceeds SourcePawn cell range.");

            call.OutputCell(4, *value.integer);
            return 1;
        });
    }
    ...
    std::int32_t ResultFloat(NativeCall& call) {
        ...
        return Sql(call, [&] {
            ...
            if (!value.number)
                throw source2root::sqlite::Error("Value is null or exceeds finite SourcePawn float range.");

            call.OutputCell(4, std::bit_cast<std::int32_t>(*value.number));
            return 1;
        });
    }
    ...
    std::int32_t ResultString(NativeCall& call) {
        ...
        return Sql(call, [&] {
            call.Output(4, call.Int(5), Value(call).text);
            return 1;
        });
    }

    std::int32_t ResultChanges(NativeCall& call) {
        return Sql(
            call,
            [&] {
                return Completed(call).changes;
            },
            -1);
    }
    ...
    std::int32_t ResultInsertId(NativeCall& call) {
        ...
        return Sql(call, [&] {
            call.Output(2, call.Int(3), Completed(call).inserted);
            return 1;
        });
    }

    std::int32_t Close(NativeCall& call) {
        call.Close(call.Int(1), ConnectionType);
        return 1;
    }

    std::int32_t Prepare(NativeCall& call) {
        return Sql(call, [&] {
            return call.Own(StatementType, std::make_unique<Statement>(Db(call), call.String(2)));
        });
    }

    std::int32_t BindInt(NativeCall& call) {
        return Sql(call, [&] {
            Stmt(call).BindInt(call.Int(2), call.Int(3));
            return 1;
        });
    }

    std::int32_t BindFloat(NativeCall& call) {
        return Sql(call, [&] {
            Stmt(call).BindFloat(call.Int(2), call.Float(3));
            return 1;
        });
    }

    std::int32_t BindString(NativeCall& call) {
        return Sql(call, [&] {
            Stmt(call).BindString(call.Int(2), call.String(3));
            return 1;
        });
    }

    std::int32_t BindNull(NativeCall& call) {
        return Sql(call, [&] {
            Stmt(call).BindNull(call.Int(2));
            return 1;
        });
    }

    std::int32_t Step(NativeCall& call) {
        return Sql(
            call,
            [&] {
                return Stmt(call).Step() ? 1 : 0;
            },
            -1);
    }

    std::int32_t Reset(NativeCall& call) {
        return Sql(call, [&] {
            Stmt(call).Reset();
            return 1;
        });
    ...
    std::int32_t Finalize(NativeCall& call) {
        call.Close(call.Int(1), StatementType);
        return 1;
    }

    std::int32_t ColumnCount(NativeCall& call) {
        return Stmt(call).Columns();
    }
    ...
    std::int32_t IsNull(NativeCall& call) {
        return Sql(
            call,
            [&] {
                return Stmt(call).IsNull(call.Int(2)) ? 1 : 0;
            },
            -1);
    }
    ...
    std::int32_t GetInt(NativeCall& call) {
        call.OutputCell(3, 0);
        return Sql(call, [&] {
            call.OutputCell(3, Stmt(call).Int(call.Int(2)));
            return 1;
        });
    }
    ...
    std::int32_t GetString(NativeCall& call) {
        ...
        return Sql(call, [&] {
            call.Output(3, call.Int(4), Stmt(call).String(call.Int(2)));
            return 1;
        });
    }

    std::int32_t Begin(NativeCall& call) {
        return Sql(call, [&] {
            Db(call)->Execute("BEGIN IMMEDIATE");
            return 1;
        });
    }

    std::int32_t Commit(NativeCall& call) {
        return Sql(call, [&] {
            Db(call)->Execute("COMMIT");
            return 1;
        });
    }

    std::int32_t Rollback(NativeCall& call) {
        return Sql(call, [&] {
            Db(call)->Execute("ROLLBACK");
            return 1;
        });
    }

    std::int32_t AffectedRows(NativeCall& call) {
        return Db(call)->Changes();
    }

    std::int32_t InsertId(NativeCall& call) {
        call.Output(2, call.Int(3), std::to_string(Db(call)->InsertId()));
        return 1;
    }
};
}
