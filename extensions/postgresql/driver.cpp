...
std::string FirstKeyword(std::string_view sql) {
    std::size_t i = 0;
    for (;;) {
        while (i < sql.size() && Space(sql[i])) ++i;
        if (sql.substr(i, 2) == "--") {
            i += 2;
            ...
            if (depth) throw db::Error("Unterminated SQL comment.");
        } else break;
    }
    std::string keyword;
    ...
    if (keyword.empty()) throw db::Error("PostgreSQL SQL must begin with a command keyword.");
    return keyword;
}
...
std::string ConnectionInfo(const db::Settings& settings) {
    ...
    if (std::getenv("PGSERVICE")) throw db::Error("PGSERVICE must be unset for configured PostgreSQL connections.");
    std::string info;
...
db::QueryValue Value(const PGresult* result, int column) {
    ...
    if (length < 0 || length > 4095 || std::memchr(bytes, 0, length)) throw db::Error("Value cannot fit a SourcePawn string.");
...
class Session {
    ...
    void Check(Clock::time_point limit = Clock::time_point::max()) const {
        if (canceled_.load(std::memory_order_relaxed)) throw db::Error("Database request canceled.");
        if (Clock::now() >= std::min(deadline_, limit)) throw db::Error("Database request time limit exceeded.");
    }
    short Wait(short events, Clock::time_point limit = Clock::time_point::max()) {
        for (;;) {
            Check(limit);
            const auto socket = PQsocket(connection_.get());
            if (socket < 0) throw db::Error("PostgreSQL connection is unavailable.");
#if defined(_WIN32)
...
#endif
            if (result < 0) throw db::Error("PostgreSQL socket polling failed.");
            if (result) {
                Check(limit);
                if (descriptor.revents & (POLLNVAL | POLLERR)) throw db::Error("PostgreSQL connection failed.");
    ...
    void Consume() {
        if (!PQconsumeInput(connection_.get())) throw db::Error("PostgreSQL connection read failed.");
    }
    db::QueryResult Execute(const db::QueryInput& input) {
        ...
        for (std::size_t i = 0; i < input.parameters.size(); ++i) {
            ...
            if (const auto* integer = std::get_if<std::int32_t>(&parameter)) {
                ...
                if (converted.ec != std::errc{}) throw db::Error("Parameter conversion failed.");
                ...
                if (converted.ec != std::errc{}) throw db::Error("Parameter conversion failed.");
                text[i].assign(buffer.data(), converted.ptr);
            } else if (const auto* string = std::get_if<std::string>(&parameter)) text[i] = *string;
            else continue;
        ...
        for (;;) {
            ...
            if (pending < 0) throw db::Error("PostgreSQL connection write failed.");
            if (!pending) break;
            if (Wait(POLLIN | POLLOUT) & (POLLIN | POLLHUP)) Consume();
        }
        ...
        for (;;) {
            Check();
            while (PQisBusy(connection_.get())) { Wait(POLLIN); Consume(); Check(); }
            Result result(PQgetResult(connection_.get()), PQclear);
            if (!result) break;
            const auto status = PQresultStatus(result.get());
            if (status != PGRES_SINGLE_TUPLE && status != PGRES_TUPLES_OK && status != PGRES_COMMAND_OK) StatementError(result.get());
            if (complete) throw db::Error("Multiple result sets are not supported.");
            const auto columns = PQnfields(result.get());
            if (columns > 32) throw db::Error("Database result exceeds 32 columns.");
            output.columns = columns;
            if (status == PGRES_SINGLE_TUPLE) {
                if (PQntuples(result.get()) != 1) throw db::Error("Invalid PostgreSQL single-row result.");
                if (output.rows.size() >= 256) throw db::Error("Database result exceeds 256 rows.");
                ...
                if (PQntuples(result.get())) throw db::Error("PostgreSQL result was not streamed.");
                complete = true;
        ...
        if (!complete || PQstatus(connection_.get()) != CONNECTION_OK) throw db::Error("PostgreSQL query did not complete.");
        return output;
    }
public:
    Session(const db::Settings& settings, const std::atomic_bool& canceled)
        ...
        if (!connection_) throw db::Error("PostgreSQL connection allocation failed.");
        PQsetNoticeProcessor(connection_.get(), [](void*, const char*) {}, nullptr);
        auto connect = PGRES_POLLING_WRITING;
        while (connect != PGRES_POLLING_OK) {
            if (connect == PGRES_POLLING_FAILED) throw db::Error("PostgreSQL connection failed.");
            // libpq's asynchronous connection API ignores connect_timeout;
            // enforce it here as well as the total query deadline.
            if (connect != PGRES_POLLING_ACTIVE) Wait(connect == PGRES_POLLING_READING ? POLLIN : POLLOUT, connect_deadline);
            Check(connect_deadline);
            connect = PQconnectPoll(connection_.get());
        }
        Check(connect_deadline);
        if (PQsetnonblocking(connection_.get(), 1)) throw db::Error("PostgreSQL nonblocking mode is unavailable.");
        if (settings.tls && !PQsslInUse(connection_.get())) throw db::Error("PostgreSQL connection did not negotiate TLS.");
    }
    db::QueryResult Query(const db::QueryInput& input) {
        ...
        Check();
        try { Execute({"COMMIT", {}}); }
        catch (...) { throw db::Error("PostgreSQL commit failed; outcome may be unknown."); }
        return result;
    }
};
}
...
void ValidateQuery(const db::QueryInput& input) {
    ...
    for (const auto* prohibited : {"BEGIN", "START", "COMMIT", "END", "ROLLBACK", "ABORT", "SAVEPOINT", "RELEASE",
        ...
        if (keyword == prohibited) throw db::Error("PostgreSQL transaction, session-control and COPY commands are unavailable.");
}
