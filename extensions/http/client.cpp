...
void Curl(CURLcode code) {
    if (code != CURLE_OK)
        throw Error("HTTP transfer failed: " + std::string(curl_easy_strerror(code)));
}

void MultiCheck(CURLMcode code) {
    if (code != CURLM_OK)
        throw Error("HTTP transfer manager failed.");
}
...
bool Controls(std::string_view value, bool tab = false) {
    return std::any_of(value.begin(), value.end(), [tab](unsigned char ch) {
        return (ch < 32 && !(tab && ch == 9)) || ch == 127;
    });
}
...
Address Parse(std::string_view text) {
    if (text.empty() || text.size() > 4095 || Controls(text) || text.find(' ') != text.npos)
        throw Error("HTTP URL is invalid or too long.");
    ...
    if (normalized.empty() || normalized.size() > 4095)
        throw Error("Normalized HTTP URL exceeds its limit.");

    return {std::move(normalized),
            scheme + "://" + host + ":" + Get(url.get(), CURLUPART_PORT, CURLU_DEFAULT_PORT),
            scheme};
}
...
std::string Read(const std::filesystem::path& file, unsigned maximum) {
    try {
        ...
        if (!std::filesystem::is_regular_file(status) || std::filesystem::is_symlink(status))
            throw Error("HTTP input file must be a regular file, not a symlink.");

        if (std::filesystem::file_size(file) > maximum)
            throw Error("HTTP input file exceeds the transfer limit.");

        std::ifstream stream(file, std::ios::binary);

        if (!stream)
            throw Error("HTTP input file is unavailable.");
        ...
        while (stream) {
            ...
            const auto size = static_cast<unsigned>(stream.gcount());

            if (size > maximum - value.size())
                throw Error("HTTP input file grew beyond the transfer limit.");

            value.append(buffer, size);
        }

        if (!stream.eof())
            throw Error("HTTP input file could not be read.");
...
struct Operation {
    ...
    void Check() const {
        if (canceled && canceled())
            throw Error("HTTP request canceled.");

        if (Clock::now() >= deadline)
            throw Error("HTTP request deadline exceeded.");
    }

    template <typename Function>
    static std::size_t
    Receive(char* bytes, std::size_t size, std::size_t count, void* raw, Function function) noexcept {
        auto& self = *static_cast<Operation*>(raw);

        try {
            self.Check();

            if (count > HeaderLimit || size > 1)
                throw Error("HTTP callback block exceeds limits.");

            const auto length = size * count;
            function(self, std::string_view(bytes, length));
            return length;
        } catch (...) {
    ...
    static std::size_t Body(char* bytes, std::size_t size, std::size_t count, void* raw) noexcept {
        return Receive(bytes, size, count, raw, [](Operation& self, std::string_view value) {
            if (value.size() > self.body_limit - self.body_bytes)
                throw Error("HTTP response body exceeds its limit.");

            self.body_bytes += value.size();
            self.response.body.append(value);
        });
    }
    ...
    static std::size_t Header(char* bytes, std::size_t size, std::size_t count, void* raw) noexcept {
        return Receive(bytes, size, count, raw, [](Operation& self, std::string_view line) {
            if (line.size() > HeaderLimit - self.header_bytes)
                throw Error("HTTP response headers exceed 64KiB.");
            ...
            if (colon == line.npos || colon > 128 || !Token(line.substr(0, colon)))
                throw Error("HTTP response header is malformed.");
            ...
            if (value.size() > 4095 || Controls(value, true) || self.response.headers.size() >= 128)
                throw Error("HTTP response header exceeds limits.");
...
void ValidateHeader(std::string_view name, std::string_view value) {
    if (name.size() > 128 || !Token(name) || value.size() > 4095 || Controls(value, true))
        throw Error("HTTP request header is invalid.");

    const auto lower = Lower(name);

    for (const auto* blocked :
         {"host", "content-length", "transfer-encoding", "connection", "expect", "trailer", "upgrade", "te"})
        if (lower == blocked)
            throw Error("HTTP framing headers are managed by the transfer service.");

    if (lower.starts_with("proxy-"))
        throw Error("HTTP proxy headers are not supported.");
}
...
void Request::Validate() const {
    ...
    if (method < Method::Get || method > Method::Delete || timeout_ms < 100 || timeout_ms > 60000 || !response_limit ||
        response_limit > BodyLimit || redirects > 5)
        throw Error("HTTP method or request limits are invalid.");

    if (headers.size() > 32 || parts.size() > 32 || body.size() > BodyLimit)
        throw Error("HTTP request exceeds its size or item limits.");

    if ((!body.empty() || !parts.empty()) && (method == Method::Get || method == Method::Head))
        throw Error("HTTP GET/HEAD requests cannot carry a body.");

    if (!parts.empty() && (!body.empty() || method != Method::Post))
        throw Error("HTTP multipart forms require POST without a raw body.");
    ...
    if (header_bytes > 16384)
        throw Error("HTTP request headers exceed 16KiB.");
    ...
    for (const auto& part : parts) {
        ...
        if (!part.source.empty() && (!part.data.empty() || part.filename.empty()))
            throw Error("HTTP file part requires a filename and no inline data.");

        if (part.data.size() > BodyLimit - input_bytes)
            throw Error("HTTP upload data exceeds 1MiB.");
...
Response Client::Perform(Request request, const std::function<bool()>& canceled) const {
    ...
    for (auto& part : request.parts) {
        ...
        if (!part.source.empty()) {
            part.data = Read(part.source, BodyLimit - upload_bytes);
            part.source.clear();
        }

        if (part.data.size() > BodyLimit - upload_bytes)
            throw Error("HTTP upload data exceeds 1MiB.");
    ...
    if (!request.ca_file.empty() && ca.empty())
        throw Error("HTTP trust file is empty.");
    ...
    for (unsigned hop = 0;; ++hop) {
        ...
        if (!easy || !multi)
            throw Error("HTTP transfer allocation failed.");

        auto set = [&](CURLoption option, auto value) {
            Curl(curl_easy_setopt(easy.get(), option, value));
        };
        set(CURLOPT_ERRORBUFFER, detail.data());
        set(CURLOPT_URL, address.normalized.c_str());
        ...
        if (!ca.empty())
            set(CURLOPT_CAINFO_BLOB, &trust);

        set(CURLOPT_USERAGENT, "Source2Root/1.0.0");
        set(CURLOPT_ACCEPT_ENCODING, "");
        const auto remaining = std::max<long>(
            1, std::chrono::duration_cast<std::chrono::milliseconds>(operation.deadline - Clock::now()).count());

        set(CURLOPT_TIMEOUT_MS, remaining);
        set(CURLOPT_CONNECTTIMEOUT_MS, std::min<long>(5000, remaining));
        set(CURLOPT_LOW_SPEED_LIMIT, 1L);
        set(CURLOPT_LOW_SPEED_TIME, 10L);
        set(CURLOPT_WRITEFUNCTION, &Operation::Body);
        set(CURLOPT_WRITEDATA, &operation);
        set(CURLOPT_HEADERFUNCTION, &Operation::Header);
        set(CURLOPT_HEADERDATA, &operation);
        const auto append = [&](const std::string& value) {
            auto* updated = curl_slist_append(headers.get(), value.c_str());

            if (!updated)
                throw Error("HTTP header allocation failed.");
        ...
        if (!request.parts.empty()) {
            mime.reset(curl_mime_init(easy.get()));

            if (!mime)
                throw Error("HTTP form allocation failed.");

            for (const auto& value : request.parts) {
                auto* part = curl_mime_addpart(mime.get());

                if (!part)
                    throw Error("HTTP form field allocation failed.");

                Curl(curl_mime_name(part, value.name.c_str()));
                Curl(curl_mime_data(part, value.data.data(), value.data.size()));

                if (!value.filename.empty())
                    Curl(curl_mime_filename(part, value.filename.c_str()));

                if (!value.content_type.empty())
                    Curl(curl_mime_type(part, value.content_type.c_str()));
            }
        ...
        } attachment{multi.get(), easy.get()};
        int running = 0;
        do {
            operation.Check();
            MultiCheck(curl_multi_perform(multi.get(), &running));

            if (operation.failure)
                std::rethrow_exception(operation.failure);

            if (running) {
                int events = 0;
                MultiCheck(curl_multi_poll(multi.get(), nullptr, 0, 50, &events));
            }
        } while (running);
        int pending = 0;
        auto* message = curl_multi_info_read(multi.get(), &pending);

        if (!message || message->msg != CURLMSG_DONE)
            throw Error("HTTP transfer completion is missing.");

        if (message->data.result != CURLE_OK && detail.front())
            throw Error("HTTP transfer failed: " + std::string(detail.data()));

        Curl(message->data.result);
        operation.Check();
        long status = 0;
        Curl(curl_easy_getinfo(easy.get(), CURLINFO_RESPONSE_CODE, &status));
        operation.response.status = status;
        operation.response.url = address.normalized;
        ...
        Curl(curl_easy_getinfo(easy.get(), CURLINFO_REDIRECT_URL, &location));

        if (!location)
            return std::move(operation.response);

        if (hop >= request.redirects)
            throw Error("HTTP redirect limit reached.");

        auto next = Parse(location);

        if (address.scheme == "https" && next.scheme != "https")
            throw Error("HTTP redirect cannot downgrade HTTPS.");

        if ((status == 303 && request.method != Method::Head) ||
            ((status == 301 || status == 302) && request.method == Method::Post)) {
            request.method = Method::Get;
            request.body.clear();
            request.parts.clear();
            std::erase_if(request.headers, [](const auto& header) {
                return Lower(header.first).starts_with("content-");
            });
        }

        if (next.origin != address.origin) {
            if (request.method != Method::Get && request.method != Method::Head)
                throw Error("HTTP upload redirects must remain on the same origin.");
