    ...
    return std::visit(
        [](auto number) -> T {
            if (!std::in_range<T>(number))
                throw Error("Integer is outside the hook value's range.");
...
void WriteInteger(KeelHookValue& value, const IntegerValue& source) {
    switch (value.type) {
        case KH_VALUE_BOOL: {
            ...
            if (number > 1)
                throw Error("Boolean hook value requires zero or one.");

            value.scalar.boolean = number;
            break;
        }

        case KH_VALUE_INT8:
            value.scalar.int8 = Narrow<std::int8_t>(source);
            break;

        case KH_VALUE_UINT8:
            value.scalar.uint8 = Narrow<std::uint8_t>(source);
            break;

        case KH_VALUE_INT16:
            value.scalar.int16 = Narrow<std::int16_t>(source);
            break;

        case KH_VALUE_UINT16:
            value.scalar.uint16 = Narrow<std::uint16_t>(source);
            break;

        case KH_VALUE_INT32:
            value.scalar.int32 = Narrow<std::int32_t>(source);
            break;

        case KH_VALUE_UINT32:
            value.scalar.uint32 = Narrow<std::uint32_t>(source);
            break;

        case KH_VALUE_INT64:
            value.scalar.int64 = Narrow<std::int64_t>(source);
            break;

        case KH_VALUE_UINT64:
            value.scalar.uint64 = Narrow<std::uint64_t>(source);
            break;
...
double ReadNumber(const KeelHookValue& value) {
    if (value.type == KH_VALUE_FLOAT32)
        return value.scalar.float32;

    if (value.type == KH_VALUE_FLOAT64)
...
void WriteNumber(KeelHookValue& value, double number) {
    if (!std::isfinite(number))
        throw Error("Hook float setter requires a finite value.");

    if (value.type == KH_VALUE_FLOAT64)
        value.scalar.float64 = number;
    else if (value.type == KH_VALUE_FLOAT32) {
        if (std::abs(number) > std::numeric_limits<float>::max())
            throw Error("Hook float32 value is out of range.");
...
Frame::Frame(const Definition& definition) : phase_(KH_PHASE_PRE), flags_(0), result_{} {
    ...
    for (unsigned i = 0; i < arguments_.size(); ++i)
        arguments_[i].type = definition.arguments[i];
}

Frame::Frame(KeelHookFrame& frame, const Definition& definition)
    : phase_(frame.phase), flags_(frame.flags), result_(frame.result) {
    if (frame.size != sizeof(frame) || (frame.phase != KH_PHASE_PRE && frame.phase != KH_PHASE_POST) ||
        ...
        frame.result.type != definition.result || frame.result.reserved ||
    ...
    if (frame.argument_count)
        arguments_.assign(frame.arguments, frame.arguments + frame.argument_count);
...
const KeelHookValue& Frame::Value(unsigned slot) const {
    if (slot > arguments_.size())
        throw Error("Hook value slot is out of range.");
...
KeelHookValue& Frame::Writable(unsigned slot) {
    ...
    if (slot && phase_ != KH_PHASE_PRE)
        throw Error("Post hooks cannot change arguments.");

    return slot ? arguments_[slot - 1] : result_;
}

unsigned Frame::Type(unsigned slot) const {
    return Value(slot).type;
}

std::int32_t Frame::Integer(unsigned slot) const {
    return Narrow<std::int32_t>(ReadInteger(Value(slot)));
}
...
float Frame::Number(unsigned slot) const {
    ...
    if (!std::isfinite(number) || std::abs(number) > std::numeric_limits<float>::max())
        throw Error("Hook float is not representable as finite float32.");
...
std::string Frame::NumberText(unsigned slot) const {
    ...
    const auto [end, error] = std::to_chars(buffer.data(),
                                            ...
                                            ReadNumber(Value(slot)),
                                            std::chars_format::general,
                                            std::numeric_limits<double>::max_digits10);

    if (error != std::errc{})
        throw Error("Cannot format hook float.");
...
bool Frame::IsNull(unsigned slot) const {
    ...
    if (value.type != KH_VALUE_POINTER)
        throw Error("Hook value is not a pointer.");

    return value.scalar.pointer == nullptr;
}

void Frame::SetInteger(unsigned slot, std::int32_t value) {
    WriteInteger(Writable(slot), std::int64_t{value});
}

void Frame::SetIntegerText(unsigned slot, const std::string& text) {
    if (text.empty() || text.size() > 20)
        throw Error("Hook integer text is invalid.");
    ...
    const auto parse = [&]<typename T>() -> IntegerValue {
        ...
        if (error != std::errc{} || end != text.data() + text.size())
            throw Error("Hook integer text is invalid or out of range.");

        return value;
    };
    const auto value =
        text.front() == '-' ? parse.template operator()<std::int64_t>() : parse.template operator()<std::uint64_t>();

    WriteInteger(Writable(slot),value);
}

void Frame::SetNumber(unsigned slot, float value) {
    WriteNumber(Writable(slot), value);
}

void Frame::SetNumberText(unsigned slot, const std::string& text) {
    if (text.empty() || text.size() > 64)
        throw Error("Hook float text is invalid.");
    ...
    if (error != std::errc{} || end != text.data() + text.size())
        throw Error("Hook float text is invalid or out of range.");
...
void Frame::Copy(unsigned destination, unsigned source) {
    ...
    if (value.type == KH_VALUE_VOID || value.type != copy.type)
        throw Error("Hook copy requires identical non-void types.");

    value = copy;
}

void Frame::Commit(KeelHookFrame& frame, unsigned action) const {
    if (phase_ == KH_PHASE_PRE)
        std::copy(arguments_.begin(), arguments_.end(), frame.arguments);

    if (action != KH_ACTION_CONTINUE)
        frame.result = result_;
}
}
