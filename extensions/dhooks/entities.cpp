...
EntityLease::~EntityLease() {
    if (handle) { service->entities_.release(service->owner_, handle); --service->entity_count_; }
}
void Service::EntityServices(const KeelEntityAccessApi& access, const KeelEntitiesApi& entities, const KeelPlayersApi& players) {
    Thread();
    if (access_.visit || entity_count_) throw Error("Entity services are already configured.");
    ...
    access_ = access; entities_ = entities; players_ = players;
}
std::shared_ptr<EntityLease> Service::AcquireEntity(std::uint32_t source) {
    Thread();
    if (!access_.visit || source == KEELS2_INVALID_SOURCE2_ENTITY_HANDLE) throw Error("Entity reference is invalid or unavailable.");
    if (entity_count_ == 256) throw Error("SDKCall entity limit (256) reached.");
    auto lease = std::make_shared<EntityLease>(); lease->service = shared_from_this();
    ...
    lease->handle = handle; ++entity_count_;
...
void Service::ValidateEntity(const EntityLease& entity) const {
    ...
    if (!Consistent(current) || current.index != entity.identity.index || current.epoch != entity.identity.epoch ||
        current.source2_handle != entity.identity.source2_handle) throw Error("SDKCall entity identity changed.");
}
KeelPlayerInfo Service::Player(const KeelPlayerConnection& player) const {
    ...
    if (info.size != sizeof(info) || info.reserved || info.slot != player.slot || info.connection != player.generation ||
        !(info.flags & KEELS2_PLAYER_CONNECTED)) throw Error("SDKCall player connection changed.");
    return info;
}
void Service::InvokeEntities(const TargetData& target, const std::vector<KeelHookValue>& arguments,
    ...
    Thread();
    if (!access_.visit || entities.empty() || entities.size() != slots.size()) throw Error("Checked entity calls are unavailable.");
    struct Context {
        Service& service;
    ...
    const auto invoke = [](void* raw, void* const* pointers, unsigned count) noexcept -> KeelResult {
        ...
        try {
            if (value.called || !pointers || count != value.slots.size()) throw Error("Invalid entity callback from host.");
            value.called = true;
            for (unsigned i = 0; i < count; ++i) {
                if (!pointers[i]) throw Error("Host returned a null entity pointer.");
        ...
        } catch (...) { value.error = std::current_exception(); return KEEL_RESULT_ENGINE_FAILURE; }
    };
    ...
    if (!context.called) throw Error("Host omitted the entity call callback.");
}
}
