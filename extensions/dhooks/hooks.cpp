...
bool Released(KeelResult result) {
...
bool Hook::Active() const {
    service_->Thread();
    return !registration_->closing && registration_->handle;
}

void Hook::Enable(bool enabled) {
    service_->Thread();

    if (registration_->closing || !registration_->handle)
        throw Error("Hook is closing.");

    Check(service_->hooks_.set_callback_enabled(
              service_->owner_, registration_->handle, enabled ? KEEL_TRUE : KEEL_FALSE),
          "Enable hook");
...
Service::Service(KeelPluginHandle owner,
    ...
    if (!owner || hooks.size != sizeof(hooks) || hooks.api_version != KEELHOOK_API_VERSION || !hooks.resolve_target ||
        !hooks.release_target || !hooks.add_callback || !hooks.remove_callback || !hooks.set_callback_enabled ||
    ...
    targets_.reserve(64);
    registrations_.reserve(256);
}

void Service::Thread() const {
    Check(runtime_.check_game_thread(owner_), "Hook operation");
}
...
std::unique_ptr<Target> Service::Open(const Definition& definition) {
    ...
    if (targets_.size() >= 64)
        throw Error("Hook target limit (64) reached.");

    auto data = std::make_shared<TargetData>();
    data->definition = definition;
    auto result = std::unique_ptr<Target>(new Target(shared_from_this(),data,definition));
    const KeelHookTargetSpec spec{sizeof(spec),
                                  definition.source,
                                  KH_MECHANISM_DETOUR,
                                  definition.method ? KH_TARGET_METHOD : 0u,
                                  definition.module.empty() ? nullptr : definition.module.c_str(),
                                  definition.symbol.empty() ? nullptr : definition.symbol.c_str(),
                                  definition.pattern.empty() ? nullptr : definition.pattern.c_str(),
                                  definition.profile.empty() ? nullptr : definition.profile.c_str(),
                                  nullptr,
                                  definition.offset,
    ...
    if (!data->handle)
        throw Error("Host returned an empty hook target.");
    ...
    const auto existing = std::find_if(targets_.begin(), targets_.end(), [&](const auto& value) {
        return value->handle == data->handle;
    });

    if (existing != targets_.end()) {
        if ((*existing)->definition.result != definition.result ||
            (*existing)->definition.arguments != definition.arguments ||
            (*existing)->definition.method != definition.method)
            throw Error("Host reused a hook target with a different prototype.");

        result->data_ = *existing;
    } else
        targets_.push_back(std::move(data));
...
std::unique_ptr<Call> Service::Prepare(const Target& target) {
    ...
    if (!calls_.invoke)
        throw Error("Direct-call host service is unavailable.");

    if (target.service_.get() != this || !target.definition_.allow_calls)
        throw Error("This configured target does not allow direct calls.");

    if (target.definition_.method &&
        std::none_of(target.definition_.entities.begin(), target.definition_.entities.end(), [](const auto& entity) {
            return entity.argument == 1;
        }))
        throw Error("Method calls require a configured entity adapter for argument 1.");

    if (!target.definition_.entities.empty() && !access_.visit)
        throw Error("Checked entity calls are unavailable on this host.");
...
void Service::Invoke(const TargetData& target, unsigned flags, const std::vector<KeelHookValue>& arguments,
    ...
    struct Scope {
        std::vector<BufferScope>& values;
        ~Scope() {
            values.pop_back();
        }
    } scope{active_buffers_};
    Check(
        calls_.invoke(owner_, target.handle, flags, arguments.data(), static_cast<unsigned>(arguments.size()), &result),
        "Invoke configured target");

    if (result.type != target.definition.result || result.reserved ||
        (result.type == KH_VALUE_BOOL && result.scalar.boolean > 1))
        throw Error("Invalid direct-call result from host.");
}

void Service::CheckBufferEdits(const KeelHookFrame& before, const Frame& after) const {
    if (before.phase != KH_PHASE_PRE)
        return;

    for (const auto& active : active_buffers_) {
        if (active.target != before.target)
            continue;
        ...
        for (const auto& bound : *active.bounds) {
            ...
            if (before.arguments[bound.argument - 1].scalar.pointer != pointer)
                continue;
            ...
            if (bound.length_argument) {
                ...
                if ((value.type == KH_VALUE_INT32 &&
                     (value.scalar.int32 < 0 || static_cast<unsigned>(value.scalar.int32) > bound.capacity)) ||
...
std::unique_ptr<Hook> Service::Attach(const Target& target, unsigned phases, std::int32_t priority,
    ...
    if (registrations_.size() >= 256)
        throw Error("Hook callback limit (256) reached.");

    auto registration = std::make_shared<Registration>();
    registration->service = this;
    registration->target = target.data_;
    registration->callback = std::move(callback);
    registration->retire = std::move(retire);
    ...
    try {
        ...
        if (!registration->handle)
            throw Error("Host returned an empty hook callback.");
    } catch (...) {
        Close(*registration);
        Collect();
        throw;
...
void Service::Collect() {
    ...
    for (auto it = registrations_.begin(); it != registrations_.end();) {
        ...
        if (!registration.closing || registration.active) {
            ++it;
            continue;
        }
    ...
    for (auto it = targets_.begin(); it != targets_.end();) {
        ...
        if (!Released(hooks_.release_target(owner_, (*it)->handle))) {
            ++it;
            continue;
        }
...
KeelHookAction Service::Dispatch(KeelHookFrame* frame, void* raw) noexcept {
    ...
    try {
        if (self->runtime_.check_game_thread(self->owner_) != KEEL_RESULT_OK)
            return KH_ACTION_CONTINUE;

        auto service = self->shared_from_this();

        if (registration->closing || !registration->enabled || self->depth_ >= 8 ||
            frame->target != registration->target->handle)
            return KH_ACTION_CONTINUE;
