...
bool TextValid(const std::string& value, std::size_t maximum) {
    return value.size() <= maximum && std::none_of(value.begin(), value.end(), [](unsigned char c) {
               return c < 32 || c == 127;
           });
}
...
void Keys(const Json& value, const std::set<std::string>& allowed) {
    if (!value.is_object())
        throw Error("Hook configuration requires objects.");

    for (const auto& [key, entry] : value.items())
        if (!allowed.contains(key))
            throw Error("Unknown hook configuration field.");
}
...
std::string Text(const Json& value, const char* key) {
    ...
    if (!value.at(key).is_string())
        throw Error("Hook configuration field requires text.");
...
unsigned Type(const Json& value) {
    ...
    if (!value.is_string())
        throw Error("Hook types require scalar type names.");

    const auto found = types.find(value.get<std::string>());

    if (found == types.end())
        throw Error("Unknown hook scalar type.");
...
void Validate(const Definition& value) {
    if (value.result > KH_VALUE_FLOAT64 || value.arguments.size() > KEELHOOK_MAX_ARGUMENTS ||
        std::any_of(value.arguments.begin(),
                    value.arguments.end(),
                    [](unsigned type) {
                        return !type || type > KH_VALUE_FLOAT64;
                    }) ||
    ...
    for (const auto& buffer : value.buffers) {
        ...
        switch (buffer.kind) {
            case BufferKind::string:
                if (!buffer.capacity || buffer.capacity > 4096)
                    throw Error("String buffer capacity must be 1..4096 bytes.");
            ...
            case BufferKind::vector3:
                if (buffer.capacity != 3 || buffer.length_argument)
                    throw Error("Vector adapter requires exactly three floats and no length argument.");
        ...
        if (buffer_bytes > 16384)
            throw Error("Configured SDKCall buffers exceed 16 KiB per call.");
    }

    std::set<unsigned> entity_arguments;

    for (const auto& entity : value.entities) {
        if (!entity.argument || entity.argument > value.arguments.size() ||
            value.arguments[entity.argument - 1] != KH_VALUE_POINTER || buffer_arguments.contains(entity.argument) ||
            !entity_arguments.insert(entity.argument).second)
    ...
    if (!TextValid(value.module, 4095) || !TextValid(value.symbol, 512) || !TextValid(value.pattern, 16384) ||
        !TextValid(value.profile, 512))
        throw Error("Invalid hook resolver text.");

    if (value.source == KH_TARGET_PROFILE) {
        if (value.symbol.empty() || value.symbol.size() > 127 || !value.module.empty() || !value.pattern.empty() ||
            !value.profile.empty() || value.offset || value.occurrence)
            throw Error("Profile hook target requires only its symbol name.");
    } else if (value.source == KH_TARGET_SYMBOL) {
        if (value.module.empty() || value.symbol.empty() || !value.pattern.empty() || !value.profile.empty() ||
            value.occurrence)
        ...
        throw Error("Unsupported hook target source.");
}

Definition ReadDefinition(const std::filesystem::path& file, const std::string& name, const std::string& script) {
    if (name.empty() || !TextValid(name, 64) || script.empty() || !TextValid(script, 64))
        throw Error("Invalid hook target or script name.");

    std::ifstream stream(file,std::ios::binary);

    if (!stream)
        throw Error("Hook target configuration is unavailable.");

    std::vector<char> buffer(131073);
    stream.read(buffer.data(), buffer.size());
    const auto length = stream.gcount();

    if (length > 131072 || stream.bad())
        throw Error("Hook configuration is unreadable or exceeds128KiB.");

    try {
        std::vector<std::set<std::string>> keys;
        const auto json =
            Json::parse(buffer.data(), buffer.data() + length, [&](int depth, Json::parse_event_t event, Json& value) {
                if (depth > 16)
                    throw Error("Hook configuration nesting exceeds limit.");
        ...
        if (!json.at("targets").contains(name))
            throw Error("Hook target was not found.");
        ...
        if (!allowed.is_array() || allowed.empty() || allowed.size() > 128)
            throw Error("Hook target requires a plugin allow list.");
        ...
        for (const auto& plugin : allowed) {
            ...
            if (plugin == script || plugin == "*")
                permitted = true;
        }

        if (!permitted)
            throw Error("This script is not allowed to use the hook target.");

        Definition result;
        const auto source = Text(target,"source");

        if (source == "profile")
            result.source = KH_TARGET_PROFILE;
        else if (source == "symbol")
            result.source = KH_TARGET_SYMBOL;
        else if (source == "pattern")
            result.source = KH_TARGET_PATTERN;
        else
            throw Error("Unknown hook target source.");

        result.module = Text(target, "module");
        result.symbol = Text(target, "symbol");
        result.pattern = Text(target, "pattern");
        result.profile = Text(target, "profile");
        result.offset = Number(target,"offset");
        const auto occurrence = Number(target,"occurrence");

        if (occurrence < 0 || occurrence > UINT32_MAX)
            throw Error("Hook pattern occurrence is out of range.");

        result.occurrence = static_cast<unsigned>(occurrence);

        if (target.contains("method") && !target.at("method").is_boolean())
            throw Error("Hook method flag must be boolean.");

        if (target.contains("allow_calls") && !target.at("allow_calls").is_boolean())
            throw Error("Hook direct-call flag must be boolean.");

        result.allow_calls = target.value("allow_calls",false);
        result.method = target.value("method", false);
        result.result = Type(target.at("return"));
        const auto& arguments = target.at("arguments");

        if (!arguments.is_array() || arguments.size() > KEELHOOK_MAX_ARGUMENTS)
            throw Error("Hook arguments require at most32 scalar types.");

        for (const auto& type : arguments)
            result.arguments.push_back(Type(type));

        if (target.contains("buffers")) {
            const auto& buffers = target.at("buffers");

            if (!buffers.is_array() || buffers.size() > KEELHOOK_MAX_ARGUMENTS)
                throw Error("SDKCall buffers require an array of at most 32 adapters.");
            ...
            for (const auto& entry : buffers) {
                ...
                if (kind == "string")
                    buffer.kind = BufferKind::string;
                else if (kind == "int32")
                    buffer.kind = BufferKind::int32;
                else if (kind == "vector3") {
                    Keys(entry, {"argument", "kind"});
                    buffer.kind = BufferKind::vector3;
                } else
                    throw Error("Unknown SDKCall buffer kind.");
        ...
        if (target.contains("entities")) {
            ...
            if (!entities.is_array() || entities.size() > KEELHOOK_MAX_ARGUMENTS)
                throw Error("SDKCall entities require at most 32 adapters.");

            for (const auto& entry : entities) {
                Keys(entry, {"argument", "class"});
                const auto argument = Number(entry,"argument");

                if (argument < 1 || argument > KEELHOOK_MAX_ARGUMENTS)
                    throw Error("Entity argument is out of range.");
        ...
        if (target.contains("sdkhook")) {
            ...
            if (result.entity_hook.kind.empty() || result.entity_hook.kind.size() > 32 ||
                ...
                result.entity_hook.block.size() > 32)
                throw Error("Invalid SDKHooks target policy.");
        }
        ...
        throw Error("Invalid hook configuration.");
    }
}
}
