...
std::string CoreSettings::Value(std::string_view name) const {
    if (name == "sr_show_activity") return std::to_string(activity);
    if (name == "sr_chat_public_trigger") return public_trigger;
    if (name == "sr_chat_silent_trigger") return silent_trigger;
    if (name == "sr_debug") return std::to_string(debug);
...
void CoreSettings::SetField(std::string_view name, const std::string& value) {
    if (name == "sr_chat_public_trigger" || name == "sr_chat_silent_trigger") {
        if (value.size() > 8) throw std::runtime_error("chat triggers accept at most 8 characters");
    ...
    } else throw std::runtime_error("unknown core setting: " + std::string(name));
}
...
void CoreSettings::SetAll(const std::array<std::string, 4>& values) {
    auto candidate = *this;
    for (std::size_t i = 0; i < Names.size(); ++i) candidate.SetField(Names[i], values[i]);
    candidate.Validate();
    *this = std::move(candidate);
}
...
std::vector<std::string> CoreSettings::ParseLine(const std::string& line) {
    ...
    while (position < line.size()) {
        while (position < line.size() && (line[position] == ' ' || line[position] == '\t' || line[position] == '\r')) ++position;
        if (position == line.size() || line.compare(position, 2, "//") == 0) break;
        ...
        while (position < line.size()) {
            ...
            if (quoted && c == '"') { ++position; closed = true; break; }
            if (!quoted && (c == ' ' || c == '\t' || c == '\r' || line.compare(position, 2, "//") == 0)) break;
            if (c == ';' || static_cast<unsigned char>(c) < 32 || c == '"')
                throw std::runtime_error("invalid configuration syntax");
            word += c;
            ++position;
        }
        if (!closed) throw std::runtime_error("unterminated quoted value");
        ...
        if (words.size() > 2) throw std::runtime_error("expected one setting and one value");
    }
    if (!words.empty() && words.size() != 2) throw std::runtime_error("expected one setting and one value");
    return words;
}

CoreSettings CoreSettings::Read(const std::filesystem::path& path, CoreSettings initial) {
    if (!std::filesystem::exists(path)) return initial;
    const auto size = std::filesystem::file_size(path);
    if (size > 65536) throw std::runtime_error("core configuration exceeds 64 KiB");
    std::ifstream input(path, std::ios::binary);
    std::string source(static_cast<std::size_t>(size), '\0');
    if (!input.read(source.data(), static_cast<std::streamsize>(size))) throw std::runtime_error("cannot read complete core configuration");
    if (input.peek() != std::char_traits<char>::eof()) throw std::runtime_error("core configuration changed while reading");
    ...
    while (std::getline(lines, line)) {
        ...
        try {
            ...
            if (!words.empty()) initial.SetField(words[0], words[1]);
