...
bool Failure(std::string& error, const char* operation, KeelResult result) {
    if (error.empty()) error = std::string(operation) + " (KeelResult " + std::to_string(result) + ").";
    return false;
}
}
...
KeelResult Voice::Current(const Pair& pair) {
    ...
    if (receiver == KEEL_RESULT_NOT_FOUND || sender == KEEL_RESULT_NOT_FOUND) return KEEL_RESULT_NOT_FOUND;
...
bool Voice::Muted(const Player& player) const {
    for (const auto& [owner, players] : owners_) {
        ...
        if (found != players.end() && found->second.SameConnection(player)) return true;
    }
    return false;
}

KeelResult Voice::Filter(int receiver, int sender, bool& listening) {
    if (writing_) return KEEL_RESULT_OK;
    Pair pair;
    ...
    if (!muted && (pending == pairs_.end() || !pending->second.Same(pair))) return KEEL_RESULT_OK;
    pair.requested = listening;
    if (pending != pairs_.end() && pending->second.Same(pair)) pair.restore_owner = pending->second.restore_owner;
    pairs_[key] = pair;
    if (muted) listening = false;
    return KEEL_RESULT_OK;
}

KeelResult Voice::Write(const Pair& pair, bool listening) {
    const auto current = Current(pair);
    if (current != KEEL_RESULT_OK) return current;
    struct Writing {
        bool& state; bool previous;
        explicit Writing(bool& value) : state(value), previous(value) { state = true; }
        ~Writing() { state = previous; }
    } scope(writing_);
    try { return host_.SetListening(pair.receiver, pair.sender, listening); }
...
bool Voice::Apply(const Player& sender, std::string& error) {
    ...
    for (unsigned count = 0; count <= 128; ++count) {
        ...
        if (next == KEEL_RESULT_NOT_FOUND) return success;
        if (next != KEEL_RESULT_OK) return Failure(error, "Voice receiver lookup failed", next);
        if (receiver.slot <= after || count == 128) return Failure(error, "Voice receiver list is invalid", KEEL_RESULT_INVALID_ARGUMENT);
        after = receiver.slot;
        Pair pair{receiver, sender};
        auto result = Current(pair);
        if (result == KEEL_RESULT_NOT_FOUND) continue;
        if (result != KEEL_RESULT_OK) { success = Failure(error, "Voice connection lookup failed", result); continue; }
        const Key key{receiver.slot, sender.slot};
        const auto existing = pairs_.find(key);
        if (existing != pairs_.end() && existing->second.Same(pair)) pair = existing->second;
        else {
            result = host_.GetListening(receiver, sender, pair.requested);
            if (result != KEEL_RESULT_OK) { success = Failure(error, "Voice state lookup failed", result); continue; }
            result = Current(pair);
            if (result == KEEL_RESULT_NOT_FOUND) continue;
            if (result != KEEL_RESULT_OK) { success = Failure(error, "Voice connection lookup failed", result); continue; }
            pairs_[key] = pair;
        }
        if (!Muted(sender)) continue;
...
void Voice::Removed(std::uint64_t owner, const Player& sender) {
    ...
    for (auto& [key, pair] : pairs_)
        if (pair.sender.SameConnection(sender)) pair.restore_owner = owner;
}
...
bool Voice::Restore(std::uint64_t owner, const Player* sender, std::string& error) {
    ...
    for (const auto& [key, pair] : pending) {
        if (Muted(pair.sender) || (owner && pair.restore_owner != owner) ||
            (sender && !pair.sender.SameConnection(*sender))) continue;
        const auto found = pairs_.find(key);
        if (found == pairs_.end() || !found->second.Same(pair)) continue;
        auto result = Current(pair);
        if (result == KEEL_RESULT_OK) result = Write(pair, pair.requested);
        if (result == KEEL_RESULT_OK || result == KEEL_RESULT_NOT_FOUND) {
        ...
        } else success = Failure(error, "Voice restoration failed; retry required", result);
    }
    return success;
}
...
bool Voice::Set(std::uint64_t owner, const Player& player, bool muted, std::string& error) {
    ...
    if (current == KEEL_RESULT_NOT_FOUND) { error = "Player is no longer available."; return false; }
    if (current != KEEL_RESULT_OK) return Failure(error, "Voice connection lookup failed", current);
    if (muted) {
        bool listening;
        const auto result = host_.GetListening(player, player, listening);
        if (result != KEEL_RESULT_OK) return Failure(error, "Voice control is unavailable", result);
        const auto checked = Current(player);
        if (checked == KEEL_RESULT_NOT_FOUND) { error = "Player is no longer available."; return false; }
        if (checked != KEEL_RESULT_OK) return Failure(error, "Voice connection lookup failed", checked);
        owners_[owner][player.slot] = player;
        if (!Apply(player, error)) { error = "Mute recorded; " + error; return false; }
        return true;
    }
    auto found = owners_.find(owner);
    if (found != owners_.end()) {
        found->second.erase(player.slot);
        if (found->second.empty()) owners_.erase(found);
    }
...
bool Voice::Release(std::uint64_t owner, std::string& error) {
    error.clear();
    if (owner) suspended_.erase(owner);
    else { suspended_.clear(); suspended = false; }
    std::vector<std::pair<std::uint64_t, Player>> removed;
    for (auto it = owners_.begin(); it != owners_.end();) {
        if (owner && it->first != owner) { ++it; continue; }
        for (const auto& [slot, player] : it->second) removed.emplace_back(it->first, player);
        it = owners_.erase(it);
    }
    for (const auto& [id, player] : removed) Removed(id, player);
...
bool Voice::Refresh(std::string& error) {
    ...
    for (auto owner = owners_.begin(); owner != owners_.end();) {
        for (auto it = owner->second.begin(); it != owner->second.end();) {
            ...
            if (result == KEEL_RESULT_NOT_FOUND) it = owner->second.erase(it);
            else {
                if (result == KEEL_RESULT_OK) senders[it->first] = it->second;
                else success = Failure(error, "Voice connection lookup failed", result);
                ++it;
            }
        }
        if (owner->second.empty()) owner = owners_.erase(owner); else ++owner;
    }
    if (!Restore(0, nullptr, error)) success = false;
    for (const auto& [slot, sender] : senders) if (!Apply(sender, error)) success = false;
    return success;
}

bool Voice::Suspend(std::string& error) {
    error.clear();
    if (suspended) return Restore(0, nullptr, error);
...
void Voice::Disconnected(int slot, std::uint64_t connection) {
    for (auto owner = suspended_.begin(); owner != suspended_.end();) {
        ...
        if (found != owner->second.end() && found->second.connection == connection) owner->second.erase(found);
        if (owner->second.empty()) owner = suspended_.erase(owner); else ++owner;
    }
    for (auto owner = owners_.begin(); owner != owners_.end();) {
        auto found = owner->second.find(slot);
        if (found != owner->second.end() && found->second.connection == connection) owner->second.erase(found);
