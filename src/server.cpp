...
void Foundation::CancelMap(std::uint64_t owner) {
    if (!pending_map_ || (owner && pending_map_->owner != owner))
        return;
...
void Foundation::BindServer(Script& script) {
    ...
    const auto result = [&](KeelResult value, const char* action) {
        ...
        if (settings_.debug)
            host_.Log(script.manifest.id + ": " + script.error + " (KeelResult " + std::to_string(value) + ")");
    ...
    bind("IsMapInstalled", 1, [&, map_name, result](const Arguments& args) {
        ...
        if (!result(host_.MapInstalled(name, installed), "Map lookup"))
            return false;

        if (!installed)
            script.error = "Map is not installed.";

        return installed;
    });
    bind("ChangeMap", 1, [&, map_name, result](const Arguments& args) {
        if (script.state != PluginState::Running) {
            script.error = "Change maps from a running callback.";
            return false;
        }
        ...
        if (pending_map_) {
            script.error = "A map change is already pending.";
            return false;
        }
    ...
    bind("RestartRound", 1, [&, result](const Arguments& args) {
        if (script.state != PluginState::Running) {
            script.error = "Restart rounds from a running callback.";
            return false;
        }
        ...
        if (seconds < 1 || seconds > 60) {
            script.error = "Restart delay must be from 1 to 60 seconds.";
            return false;
        }
    ...
    bind("OpenConfigFile", 1, [&](const Arguments& args) -> Cell {
        ...
        if (name.empty() || name.size() > 64 || name.front() == '.' || name.find("..") != std::string::npos ||
            ...
            script.error = "Use a filename in the Source2Root configs directory.";
            return 0;
        }

        if (handles_.Owned(script.owner, ConfigFileType).size() >= 8) {
            script.error = "Close a configuration file before opening another (limit 8).";
            return 0;
        }
        ...
        try {
            ...
            if (size > 131072)
                throw std::runtime_error("file exceeds 128 KiB");
    ...
    bind("ReadConfigLine", 3, [&](const Arguments& args) {
        ...
        if (end > file.cursor && file.text[end - 1] == '\r')
            --end;

        if (end - file.cursor >= static_cast<std::size_t>(capacity)) {
            script.error = "Configuration line exceeds output capacity.";
            return -1;
        }
