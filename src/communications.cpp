...
bool Foundation::PreparePause() {
    ...
    if (!runtime_.Idle())
        return Fail("Cannot pause Source2Root during a script callback.");

    bool success = true;

    for (auto& [id, slot] : scripts_)
        for (auto* script : {slot.current.get(), slot.replacement.get()})
            if (script && !ClearMenus(*script))
                success = false;
    ...
    if (!voice_.Suspend(voice_error_))
        return Fail(voice_error_);
...
bool Foundation::Gagged(const Player& player) const {
    for (const auto& [id, slot] : scripts_)
        for (const auto* script : {slot.current.get(), slot.replacement.get()}) if (script) {
            ...
            if (found != script->gags.end() && found->second.SameConnection(player))
                return true;
        }
...
void Foundation::BindCommunications(Script& script) {
    ...
    const auto set = [&](const Arguments& args, bool voice) {
        if (script.state != PluginState::Running) {
            script.error = "Change communication restrictions from a running callback.";
            return false;
        }

        if (args.Int(2) != 0 && args.Int(2) != 1) {
            script.error = "Restriction state must be true or false.";
            return false;
        }
        ...
        if (voice)
            return voice_.Set(script.owner, player, args.Int(2) != 0, script.error);

        if (args.Int(2))
            script.gags[player.slot] = player;
        else
            script.gags.erase(player.slot);

        return true;
    };
    bind("SetPlayerMuted", 2, [set](const Arguments& args) {
        return set(args, true);
    });
    bind("SetPlayerGagged", 2, [set](const Arguments& args) {
