...
bool ParseSteamId(std::string_view text, std::uint64_t& result) {
    try {
        result = ParseSteamIdentity(std::string(text));
        return true;
...
bool SafeText(std::string_view text, std::size_t maximum) {
    return !text.empty() && text.size() <= maximum && std::none_of(text.begin(), text.end(), [](unsigned char c) {
        return c < 32 || c == 127;
    });
}
...
bool ParseUtc(std::string_view text, Timestamp &time, std::string &error)
{
    ...
    const year_month_day date{year{static_cast<int>(year_value)},
...
bool BanStore::Load(std::string& error) {
    ...
    if (status) {
        error = "Cannot inspect ban database: " + status.message();
        return false;
    }
    ...
    if (exists) {
        if (!Read(ban_file_, contents, error) || !ParseBans(contents, candidate, error))
            return false;
    } else {
        contents = "{\n  \"bans\": [],\n  \"schema\": 1\n}\n";
...
bool BanStore::AddBan(std::uint64_t account,
