...
void Foundation::ReconcilePlayers() {
    ...
    for (const auto& [id, entry] : scripts_)
        for (auto* script : {entry.current.get(), entry.replacement.get()}) if (script)
            for (auto handle : handles_.Owned(script->owner, PlayerType)) {
                ...
                if (!observed.insert(connection).second)
                    continue;
    ...
    for (const auto& [slot, generation] : stale)
        Disconnected(slot, generation);
}
...
bool Foundation::ReadPlayers(Script& script, std::vector<Player>& players) {
    ...
    for (;;) {
        ...
        if (next != KEEL_RESULT_OK) {
            script.error = "Could not list players (KeelResult " + std::to_string(next) + ").";
            return false;
        }

        if (candidate.slot <= after || !candidate.connection || players.size() >= 1024) {
            script.error = "Invalid player enumeration.";
            return false;
        }
        ...
        if (result != KEEL_RESULT_OK) {
            script.error = "Could not read a current player (KeelResult " + std::to_string(result) + ").";
            return false;
        }

        if (current.SameConnection(candidate))
            players.push_back(std::move(current));
    }
}

bool Foundation::MakePlayerHandles(Script& script,
                                   const std::vector<Player>& players,
                                   int capacity,
                                   std::vector<Cell>& output) {
    if (players.size() > static_cast<std::size_t>(capacity)) {
        script.error = "Player output array is too small.";
        return false;
    }
    ...
    if (handles_.Owned(script.owner).size() + script.commands.size() + script.events.size() + missing > 128) {
        script.error = "Plugin resource limit (128) reached.";
        return false;
    }
    ...
    try {
        for (const auto& player : players) {
            ...
            if (std::find(owned.begin(), owned.end(), handle) == owned.end())
...
void Foundation::BindPlayers(Script& script) {
    ...
    const auto action = [&](const Arguments& args, bool slap) {
        if (script.state != PluginState::Running) {
            script.error = "Player actions require a running plugin callback.";
            return false;
        }
        ...
        if (damage < 0 || damage > 1000) {
            script.error = "Slap damage must be from 0 to 1000.";
            return false;
        }
        ...
        if (!player.alive) {
            script.error = "Target is not alive.";
            return false;
        }
    ...
    bind("SlapPlayer", 2, [action](const Arguments& args) {
        return action(args, true);
    });
    bind("SlayPlayer", 1, [action](const Arguments& args) {
    ...
    bind("KickPlayer", 2, [&](const Arguments& args) {
        ...
        if (reason.empty() || std::any_of(reason.begin(), reason.end(), [](unsigned char c) {
            ...
            script.error = "Kick reason requires a single line of text.";
            return false;
        }
        ...
        if (self && script.self_kicks.contains(args.Int(1))) {
            script.error = "A disconnect was already requested for this caller.";
            return false;
        }

        if (self)
            script.self_kicks.emplace(args.Int(1), player);
        ...
        struct Pending {
            ...
            ~Pending() {
                if (armed)
                    script.self_kicks.erase(handle);
            }
    ...
    bind("GetPlayers", 2, [&](const Arguments& args) {
        ...
        if (!ReadPlayers(script, players) || !MakePlayerHandles(script, players, capacity, output))
            return Cell{-1};
    ...
    bind("FindTargets", 5, [&](const Arguments& args) {
        ...
        if (args.Int(5) != 0 && args.Int(5) != 1)
            throw NativeError("invalid multiple-target flag");
        ...
        if (selector.empty() || selector.size() > 256 ||
            std::any_of(selector.begin(), selector.end(), [](unsigned char c) {
                return c < 32 || c == 127;
            })) {
            script.error = "A valid target is required.";
            return Cell{-1};
        }
        ...
        if (group && selector != "@me" && selector != "@all" && selector != "@alive" && selector != "@dead" &&
            ...
            script.error = "Unknown target selector \"" + selector + "\".";
            return Cell{-1};
        }

        if (group && selector != "@me" && !args.Int(5)) {
            script.error = "This command requires one player.";
            return Cell{-1};
        }

        if (selector == "@me" && !args.Int(1)) {
            script.error = "The server console has no player target.";
            return Cell{-1};
        }
        ...
        try {
            steam = ParseSteamIdentity(selector);
        } catch (const std::exception&) {
        }

        const bool numeric = std::all_of(selector.begin(), selector.end(), [](unsigned char c) {
            return c >= '0' && c <= '9';
        });

        if (!steam && (selector.starts_with("STEAM_") || selector.front() == '[' || numeric)) {
            script.error = "Invalid Steam identity. Use #userid for numeric player names.";
            return Cell{-1};
        }
        ...
        if (user) {
            ...
            if (result != std::errc{} || end != selector.data() + selector.size() || user_id < 0) {
                script.error = "Use # followed by a user ID from sr_who.";
                return Cell{-1};
            }
        }
        ...
        if (selected.empty()) {
            script.error = "No players match \"" + quoted + "\".";
            return Cell{-1};
        }

        if ((!group || !args.Int(5)) && selected.size() != 1) {
            script.error = "More than one player matches \"" + quoted + "\". Use a #userid from sr_who.";
            return Cell{-1};
        }

        std::vector<Cell> output;

        if (!MakePlayerHandles(script, selected, capacity, output))
            return Cell{-1};
    ...
    bind("GetPlayerUserID", 2, [&](const Arguments& args) {
        ...
        if (ResolvePlayer(script, args.Int(1), player) != KEEL_RESULT_OK || player.user_id < 0)
            return 0;

        args.OutputCell(2, player.user_id);
        return 1;
    });
    ...
    bind("IsPlayerConnected", 1, [&](const Arguments& args) {
        if (!handles_.Contains(args.Int(1), script.owner, PlayerType))
            return false;
