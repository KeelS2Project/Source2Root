...
public:
    ...
    bool Load() override {
        try {
            if (runtime_.Connect(HostContext()) != KEEL_RESULT_OK || runtime_.CheckGameThread() != KEEL_RESULT_OK ||
                services_.Connect(HostContext()) != KEEL_RESULT_OK || source2_.Connect(HostContext()) != KEEL_RESULT_OK)
                return false;

            const void* input_service = nullptr;

            if (HostContext().QueryService(KEELS2_PLAYER_INPUT_SERVICE_NAME,
                                           KEELS2_PLAYER_INPUT_API_VERSION,
                                           &input_service) != KEEL_RESULT_OK)
                throw std::runtime_error("player input service is unavailable");
            ...
            if (root_.empty())
                throw std::runtime_error("module must be installed under addons/keels2");
            ...
            try {
                settings = sr::CoreSettings::Read(root_.parent_path().parent_path() / "cfg/source2root/source2root.cfg",
                                                  settings);
            } catch (const std::exception& error) {
                Log("Could not read source2root.cfg: " + std::string(error.what()) + ". Using defaults.");
            }
            ...
            callback_api_ = {sizeof(callback_api_),
                             SR_CALLBACK_API_VERSION,
                             this,
                             &RetainCallback,
                             &InvokeCallback,
                             &CancelCallback};

            if (services_.Publish(
                    SR_CALLBACK_SERVICE, SR_CALLBACK_API_VERSION, &callback_api_, callback_publication_) !=
                KEEL_RESULT_OK)
                throw std::runtime_error("could not publish persistent callback API");

            consumer_api_ = {
                sizeof(consumer_api_), SR_CONSUMER_API_VERSION, this, &ConsumerPlayer, &ConsumerPermission};

            if (services_.Publish(
                    SR_CONSUMER_SERVICE, SR_CONSUMER_API_VERSION, &consumer_api_, consumer_publication_) !=
                KEEL_RESULT_OK)
            ...
            if (messages && events) {
                ...
                Log("menu transport is unavailable; " + std::string(LastError()));
    ...
    bool PrepareUnload() override {
        if (runtime_.CheckGameThread() != KEEL_RESULT_OK || !foundation_)
            return !foundation_;

        if (!foundation_->Shutdown())
            return false;
    ...
    void OnGameFrame(bool, bool, bool) override {
        ...
        if (foundation_)
            foundation_->Tick(sr::Foundation::Clock::now());
    }

    void OnPluginResumed(const PluginSnapshot& plugin) override {
        if (plugin.id != HostContext().PluginHandle() || !foundation_)
            return;
        ...
        for (std::size_t i = 0; i < sr::CoreSettings::Names.size(); ++i) {
            ...
            if (const auto* text = std::get_if<std::string>(&value))
                ...
                values[i] = std::to_string(*number);
            else {
                Log("Unexpected core ConVar type after resume.");
                return;
            }
        }

        if (foundation_->SetSettings(values))
            return;
        ...
        try {
            for (const auto name : sr::CoreSettings::Names) {
                ...
                if (name == "sr_show_activity")
                    value = static_cast<std::int32_t>(settings.activity);
                else if (name == "sr_debug")
                    value = static_cast<std::int32_t>(settings.debug);
                else
    ...
    Action ChatCommand(ConCommandRef, const CCommandContext& context, const CCommand& command) {
        ...
        if (!foundation_ || !slot.IsValid() || command.ArgC() < 2)
    ...
    KeelResult Lookup(int slot, sr::Player& result) override {
        ...
        if (!GetPlayer(CPlayerSlot(slot), player))
            return LastResult();
    ...
    KeelResult NextPlayer(int after, sr::Player& result) override {
        ...
        if (!GetNextPlayer(CPlayerSlot(after), player))
            return LastResult();
    ...
    KeelResult PlayerHealth(const sr::Player& expected, std::int32_t& health) override {
        ...
        if (!GetPlayer(PlayerConnection{CPlayerSlot(expected.slot), expected.connection}, player))
            return LastResult();
        ...
        if (!entities || entities->size != sizeof(*entities) || entities->api_version != KEELS2_ENTITIES_API_VERSION ||
            !entities->find_by_source2_handle || !entities->read_field || !entities->release)
        ...
        if (!schema || schema->size != sizeof(*schema) || schema->api_version != KEELS2_SCHEMA_API_VERSION ||
            !schema->resolve_field || !schema->release_field)
            return KEEL_RESULT_INCOMPATIBLE;
        ...
        if (result == KEEL_RESULT_OK)
            result = entities->read_field(owner, entity, field, &health, sizeof(health));

        if (field) {
            const auto cleanup = schema->release_field(owner, field);

            if (cleanup != KEEL_RESULT_OK && cleanup != KEEL_RESULT_NOT_FOUND)
                result = cleanup;
        }
    ...
    KeelResult Reply(const sr::Player* player, const std::string& text) override {
        ...
        if (!GetPlayer(PlayerConnection{CPlayerSlot(player->slot), player->connection}, current))
            return LastResult();
    ...
    KeelResult GetListening(const sr::Player& receiver, const sr::Player& sender, bool& listening) override {
        ...
        if (!GetPlayer(PlayerConnection{CPlayerSlot(receiver.slot), receiver.connection}, current) ||
            !GetPlayer(PlayerConnection{CPlayerSlot(sender.slot), sender.connection}, current))
            return LastResult();
    ...
    KeelResult ChangeMap(const std::string& name) override {
        ...
        if (!engine->IsMapValid(name.c_str()))
    ...
    KeelResult ReadConVar(std::uint64_t handle, sr::ConVarValue& value) override {
        ...
        case KEELS2_CONVAR_INT32:
            value = native.value.int32_value;
            break;

        case KEELS2_CONVAR_FLOAT32:
            value = native.value.float32_value;
            break;

        case KEELS2_CONVAR_STRING:
            if (!native.value.string_value)
                return KEEL_RESULT_INCOMPATIBLE;

            value = std::string(native.value.string_value);
            break;
    ...
    KeelResult RenderMenu(const sr::Player& player, const std::string& html, int duration_ms) override {
        ...
        if (result != KEEL_RESULT_OK) {
            if (last_menu_error_ != menu_->Error())
                Log(menu_->Error());

            last_menu_error_ = menu_->Error();
        } else
            last_menu_error_.clear();
    ...
    KeelResult ReadPlayerInput(const sr::Player& player, KeelPlayerInput& input) override {
        ...
        const auto result = player_input_ ? player_input_->read(HostContext().PluginHandle(), &connection, &input)
                                          : KEEL_RESULT_NOT_READY;

        if (result == KEEL_RESULT_OK)
            input_errors_.erase(player.slot);
        else {
...
private:
    void StartScripts() {
        if (!foundation_ || scripts_started_)
            return;
    ...
    Action Listening(HookCall<bool>& call, CPlayerSlot receiver, CPlayerSlot sender, bool& listening) {
        ...
        if (result == KEEL_RESULT_OK) {
            try {
                result = foundation_->FilterVoice(receiver.Get(), sender.Get(), listening);
            } catch (...) {
    ...
    KeelResult PlayerPawn(const sr::Player& expected, Entity& pawn) {
        ...
        if (!GetPlayer(connection, current))
            return LastResult();

        if (!current.alive)
            return KEEL_RESULT_NOT_FOUND;

        if (!FindEntity(current.pawn, pawn))
            return LastResult();

        if (!GetPlayer(connection, checked))
            return LastResult();

        if (!checked.alive || checked.pawn.ToInt() != current.pawn.ToInt())
            return KEEL_RESULT_NOT_FOUND;
    ...
    void CreateCoreSettings(const sr::CoreSettings& settings) {
        ...
        for (std::size_t i = 0; i < sr::CoreSettings::Names.size(); ++i) {
            ...
            if (numeric) {
                ...
                spec.default_value.value.string_value = text.c_str();
            ...
            if (numeric)
                initial.value.int32_value = name == "sr_show_activity" ? settings.activity : settings.debug;
            else
                initial.value.string_value = configured.c_str();
    ...
    static void SettingChanged(const KeelConVarChange* change, void* data) noexcept {
        ...
        if (!self.foundation_ || !change || self.restoring_settings_)
            return;
        ...
        try {
            ...
            if (!self.foundation_->SetSetting(change->name, text) &&
                self.convars_->queue_set(
                    self.HostContext().PluginHandle(), change->convar, change->slot, &change->old_value) !=
                    KEEL_RESULT_OK)
                self.Log("Could not restore rejected core setting " + std::string(change->name) + ".");
        } catch (...) {
            self.Log("Could not apply a core setting change.");
        }
    }
    ...
    void ConfigCommand(const CCommand& command) {
        ...
        if (command.ArgC() > 4) {
            Log("Usage: sr config [setting [value]]");
            return;
        }
        ...
        if (found == core_settings_.end()) {
            Log("Unknown core setting \"" + name + "\". Use sr config to list settings.");
            return;
        }
        ...
        if (command.ArgC() == 4) {
            try {
                ...
                if (name == "sr_show_activity" || name == "sr_debug") {
                    ...
                    value.value.string_value = command[3];

                if (convars_->queue_set(
                        HostContext().PluginHandle(), found->second, KEELS2_CONVAR_GLOBAL_SLOT, &value) !=
                    KEEL_RESULT_OK) {
    ...
    static std::filesystem::path ModuleDirectory() {
        ...
        if (!count || count == path.size())
            throw std::runtime_error("cannot locate module path");
    ...
    void GameEvent(IGameEvent* event) {
        if (foundation_ && event)
            foundation_->Event(event->GetName());
    }
    ...
    void ExtensionCommand(const std::vector<std::string>& arguments) {
        ...
        for (auto owner : foundation_->Extensions()) {
            ...
            if (GetPlugin(owner, details))
                extensions.push_back(details);
        }
        ...
        if (arguments[0] == "list") {
            ...
            for (const auto& extension : extensions)
                Log("  " + std::to_string(extension.handle) + " | " + extension.file + " | " + extension.name + " | " +
                    extension.version);

            if (extensions.empty())
                Log("  No registered native extensions.");
        ...
        for (const auto& extension : extensions) {
            if (arguments[1] != extension.file && arguments[1] != extension.name &&
                arguments[1] != std::to_string(extension.handle))
                continue;

            if (selected) {
                Log("More than one extension matches. Use its listed ID.");
                return;
            }
        ...
        if (!selected) {
            Log("Unknown Source2Root extension. Use sr extensions list.");
            return;
        }
    ...
    void Manage(const CCommandContext& context, const CCommand& command) {
        if (context.GetPlayerSlot().Get() != -1 || !foundation_)
            return;
        ...
        if (!requested_help)
            for (int i = 2; i < command.ArgC(); ++i)
                arguments.emplace_back(command[i]);

        if (name == "plugins")
            foundation_->ManagePlugins(arguments);
        else if (name == "extensions")
            ExtensionCommand(arguments);
        else if (name == "config") {
            if (requested_help)
            ...
            Log("Unknown command or invalid arguments. Use sr help.");
            help();
        }
    }

    void MenuCommand(const CCommandContext& context, const CCommand& command) {
        if (!foundation_ || command.ArgC() < 2 || command.ArgC() > 3)
            return;

        sr::Player player;

        if (Lookup(context.GetPlayerSlot().Get(), player) != KEEL_RESULT_OK)
            return;
        ...
        if (value == "up")
            input = sr::MenuInput::Up;
        else if (value == "down")
            input = sr::MenuInput::Down;
        else if (value == "select")
            input = sr::MenuInput::Select;
        else if (value == "back")
            input = sr::MenuInput::Back;

        if (input)
            foundation_->MenuInput(player, session, *input);
    }
    ...
    static KeelResult Call(void* context, Operation operation, bool cleanup = false) noexcept {
        ...
        try {
            return operation(*self.foundation_);
        } catch (...) {
    ...
    static KeelResult
    RegisterNative(void* context, KeelPluginHandle owner, const SrNativeSpec* spec, SrRegistration* result) {
        if (!spec || !result)
            return KEEL_RESULT_INVALID_ARGUMENT;

        return Call(context, [&](auto& core) {
            return core.RegisterNative(owner, *spec, *result);
        });
    }

    static KeelResult RegisterContextNative(void* context,
                                            KeelPluginHandle owner,
                                            const SrContextNativeSpec* spec,
                                            SrRegistration* result) {
        if (!spec || !result)
            return KEEL_RESULT_INVALID_ARGUMENT;

        return Call(context, [&](auto& core) {
            return core.RegisterContextNative(owner, *spec, *result);
        });
    }
    ...
    static KeelResult UnregisterNative(void* context, KeelPluginHandle owner, SrRegistration registration) {
        return Call(
            context,
            [&](auto& core) {
                return core.UnregisterNative(owner, registration);
            },
            true);
    }
    ...
    static KeelResult DeliverCallback(void* context, KeelPluginHandle owner, SrCallback callback,
        ...
        return Call(context, [&](auto& core) {
            return core.DeliverCallback(owner, callback, cells, count, text);
        });
    }
    ...
    static KeelResult CancelCallback(void* context, KeelPluginHandle owner, SrCallback callback) {
        return Call(
            context,
            [&](auto& core) {
                return core.CancelCallback(owner, callback);
            },
            true);
    }
    ...
    static KeelResult RetainCallback(void* context, KeelPluginHandle owner, SrCallback callback) {
        return Call(context, [&](auto& core) {
            return core.RetainCallback(owner, callback);
        });
    }
    ...
    static KeelResult InvokeCallback(void* context, KeelPluginHandle owner, SrCallback callback,
        ...
        return Call(context, [&](auto& core) {
            return core.InvokeCallback(owner, callback, arguments, count, result);
        });
    }

    static KeelResult
    PlayerSnapshot(void* context, SrPlayerIdentity* players, std::uint32_t capacity, std::uint32_t* count) {
        return Call(context, [&](auto& core) {
            return core.NativePlayerSnapshot(players, capacity, count);
        });
    }
    ...
    static KeelResult OpenMenu(void* context, KeelPluginHandle owner, const KeelPlayerConnection* player,
        ...
        if (!player || !spec || !session)
            return KEEL_RESULT_INVALID_ARGUMENT;

        return Call(context, [&](auto& core) {
            return core.OpenNativeMenu(owner, *player, *spec, *session);
        });
    }
    ...
    static KeelResult CloseMenu(void* context, KeelPluginHandle owner, SrMenuSession session) {
        return Call(
            context,
            [&](auto& core) {
                return core.CloseNativeMenu(owner, session);
            },
            true);
    }
    ...
    static KeelResult MenuStatus(void* context, KeelPluginHandle owner, SrMenuSession session) {
        return Call(
            context,
            [&](auto& core) {
                return core.NativeMenuStatus(owner, session);
            },
            true);
    }
    ...
    static KeelResult ConsumerStatus(void* context, KeelPluginHandle provider, std::uint64_t owner) {
        return Call(
            context,
            [&](auto& core) {
                return core.NativeConsumerStatus(provider, owner);
            },
            true);
    }
    ...
    static KeelResult ConsumerPlayer(void* context, KeelPluginHandle provider, std::uint64_t owner,
        ...
        return Call(context, [&](auto& core) {
            return core.ConsumerPlayer(provider, owner, player, handle);
        });
    }
    ...
    static KeelResult ConsumerPermission(void* context, KeelPluginHandle provider, std::uint64_t owner,
        ...
        return Call(context, [&](auto& core) {
            return core.ConsumerPermission(provider, owner, player, permission, allowed);
        });
    }
};
}
