...
std::vector<std::string> ParseArguments(const std::string& text) {
    if (text.size() > 1024 || std::any_of(text.begin(), text.end(), [](unsigned char c) {
            return c < 32 || c == 127 || c == ';';
        }))
    ...
    while (position < text.size()) {
        ...
        if (result.size() == 32)
            throw std::runtime_error("Too many arguments (maximum 32).");
        ...
        while (position < text.size()) {
            ...
            if (c == '\\' && position < text.size() && (text[position] == '\\' || text[position] == '"'))
                c = text[position++];

            token += c;

            if (token.size() > 256)
                throw std::runtime_error("An argument exceeds 256 bytes.");
        }
...
void Foundation::BindText(Script& script) {
    ...
    auto parse = [&](const Arguments& args, std::vector<std::string>& result) {
        try {
            result = ParseArguments(args.String(1, 1024));
            return true;
    ...
    auto bind_argument = [&](const char* name, bool remaining) {
        runtime_.Bind(*script.vm, name, 4, [&, remaining](const Arguments& args) -> Cell {
            ...
            if (index < 0 || static_cast<std::size_t>(index) > values.size() ||
                (!remaining && static_cast<std::size_t>(index) == values.size())) {
                script.error = "Argument index is out of range.";
                return 0;
            }
            ...
            if (output.size() >= static_cast<std::size_t>(args.Int(4))) {
                script.error = "Argument output buffer is too small.";
                return 0;
            }

            args.Output(3, args.Int(4), output);
            return 1;
        });
    };
    ...
    runtime_.Bind(*script.vm, "ParseInt", 4, [&](const Arguments& args) -> Cell {
        ...
        if (args.Int(3) > args.Int(4))
            throw NativeError("invalid integer bounds");
        ...
        if (error != std::errc{} || end != text.data() + text.size() || value < args.Int(3) || value > args.Int(4)) {
            script.error =
                "Expected an integer from " + std::to_string(args.Int(3)) + " to " + std::to_string(args.Int(4)) + ".";

            return 0;
        }

        args.OutputCell(2, value);
        return 1;
    });
}
