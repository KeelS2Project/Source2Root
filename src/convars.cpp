...
bool Foundation::ConVarError(Script& script, const std::string& name, KeelResult result) {
    ...
    if (settings_.debug) host_.Log(script.manifest.id + ": " + script.error + " (KeelResult " + std::to_string(result) + ")");
    return false;
}
...
void Foundation::ValidateConVar(const ConVarDefinition& definition) {
    ...
    if (const auto* text = std::get_if<std::string>(&definition.initial)) {
        if (text->size() > 512) throw NativeError("ConVar strings accept at most 512 bytes");
        return;
    }
    if (definition.initial.index() != definition.minimum.index() || definition.initial.index() != definition.maximum.index())
...
Cell Foundation::CreateConVar(Script& script, ConVarDefinition definition) {
    ...
    if (script.convars.contains(name)) { ConVarError(script, name, KEEL_RESULT_ALREADY_EXISTS); return 0; }
    auto found = convars_.find(name);
    if (found != convars_.end()) {
        for (const auto* owner : found->second.owners)
            if (owner->manifest.id != script.manifest.id) { ConVarError(script, name, KEEL_RESULT_ALREADY_EXISTS); return 0; }
    ...
    try {
        if (create) found = convars_.emplace(name, Variable{std::move(definition), 0, {}}).first;
        found->second.owners.insert(&script);
        script.convars.emplace(name, handle);
    } catch (...) {
        if (found != convars_.end()) {
            found->second.owners.erase(&script);
            if (found->second.owners.empty()) convars_.erase(found);
        }
...
bool Foundation::ReadConVar(Script& script, Cell handle, ConVarValue& value) {
    ...
    if (result != KEEL_RESULT_OK) return ConVarError(script, name, result);
...
bool Foundation::WriteConVar(Script& script, Cell handle, const ConVarValue& value) {
    ...
    if (value.index() != variable.definition.initial.index()) return ConVarError(script, name, KEEL_RESULT_INCOMPATIBLE);
    auto bounded = value;
    if (const auto* number = std::get_if<float>(&value)) {
        if (!std::isfinite(*number)) throw NativeError("ConVar value must be finite");
        bounded = std::clamp(*number, std::get<float>(variable.definition.minimum), std::get<float>(variable.definition.maximum));
    ...
    } else if (std::get<std::string>(value).size() > 512) throw NativeError("ConVar strings accept at most 512 bytes");
...
bool Foundation::CloseConVar(Script& script, const std::string& name) {
    ...
    if (owned == script.convars.end()) return ConVarError(script, name, KEEL_RESULT_NOT_FOUND);
    auto found = convars_.find(name);
    if (found == convars_.end()) return ConVarError(script, name, KEEL_RESULT_NOT_FOUND);
    auto& variable = found->second;
    if (variable.owners.size() == 1 && variable.native) {
        const auto result = host_.ReleaseConVar(variable.native);
        if (result != KEEL_RESULT_OK && result != KEEL_RESULT_NOT_FOUND) return ConVarError(script, name, result);
    }
    variable.owners.erase(&script);
    if (variable.owners.empty()) convars_.erase(found);
    if (handles_.Contains(owned->second, script.owner, ConVarType)) handles_.Remove(owned->second, script.owner, ConVarType);
    script.convars.erase(owned);
    return true;
}

std::string Foundation::ConVarText(const ConVarValue& value) {
    if (const auto* number = std::get_if<Cell>(&value)) return std::to_string(*number);
    ...
    if (const auto* number = std::get_if<float>(&value)) {
        ...
        for (unsigned char c : std::get<std::string>(value)) {
            ...
            else if (c < 32 || c == 127) output << "\\x" << std::hex << std::setw(2) << std::setfill('0') << static_cast<unsigned>(c);
...
void Foundation::BindConVars(Script& script) {
    ...
    bind("GetConVarInt", 2, [&](const Arguments& args) {
        args.OutputCell(2, 0);
        ConVarValue value;
        if (!ReadConVar(script, args.Int(1), value)) return 0;
        const auto* number = std::get_if<Cell>(&value);
        if (!number) { script.error = "ConVar is not an integer"; return 0; }
    ...
    bind("GetConVarFloat", 2, [&](const Arguments& args) {
        ...
        if (!number) { script.error = "ConVar is not a float"; return 0; }
    ...
    bind("GetConVarString", 3, [&](const Arguments& args) {
        ...
        if (!text) { script.error = "ConVar is not a string"; return 0; }
        args.Output(2, args.Int(3), *text);
        return 1;
    });
    bind("SetConVarInt", 2, [&](const Arguments& args) { return WriteConVar(script, args.Int(1), args.Int(2)); });
    bind("SetConVarFloat", 2, [&](const Arguments& args) { return WriteConVar(script, args.Int(1), std::bit_cast<float>(args.Int(2))); });
    bind("SetConVarString", 2, [&](const Arguments& args) { return WriteConVar(script, args.Int(1), args.String(2)); });
