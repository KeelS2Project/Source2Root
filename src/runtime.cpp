...
void CheckMemoryBudget(const std::vector<std::uint8_t>& file) {
    ...
    if (header.magic != sp::SmxConsts::FILE_MAGIC || header.imagesize > 4 * 1024 * 1024 ||
        header.imagesize < sizeof(header)) throw std::runtime_error("invalid or oversized SMX image");
    ...
    if (header.compression == sp::SmxConsts::FILE_COMPRESSION_GZ) {
        ...
        if (header.imagesize > file.size()) throw std::runtime_error("truncated SMX image");
        image = file;
    } else throw std::runtime_error("unsupported SMX compression");
    ...
    for (unsigned i = 0; i < header.sections; ++i) {
        ...
        if (!end) throw std::runtime_error("SMX section name is not terminated");
        if (std::string_view(name, end - name) != ".data") continue;
        if (found_data || section.size < sizeof(sp::sp_file_data_t)) throw std::runtime_error("invalid SMX data section");
        found_data = true;
    ...
    if (!found_data) throw std::runtime_error("SMX has no data section");
}
...
std::string Arguments::String(int index, std::size_t limit) const {
    ...
    std::string text;
    if (limit > 4095) throw NativeError("string limit exceeded");
...
public:
    ...
    void AddRef() override { ++references_; }
    void Release() override { if (--references_ == 0) delete this; }
...
#endif
    try {
        if (!factory_fn) throw std::runtime_error("SourcePawn runtime library/factory unavailable");
        ...
        if (!environment_) throw std::runtime_error("SourcePawn environment creation failed");
...
std::unique_ptr<SourcePawn::IPluginRuntime> PawnRuntime::Load(const std::filesystem::path& path,
    ...
    if (input.peek() != std::char_traits<char>::eof()) throw std::runtime_error("SMX changed while reading");
    ...
    std::unique_ptr<SourcePawn::IPluginRuntime> vm(
        environment_->APIv2()->LoadBinaryFromMemory(path.string().c_str(), owned, file.size(),
            [](std::uint8_t* bytes) { delete[] bytes; }, error, sizeof(error)));
    if (!vm) throw std::runtime_error("invalid SMX: " + std::string(error));
    if (vm->GetMemUsage() > 16 * 1024 * 1024)
        throw std::runtime_error("VM exceeds 16 MiB memory limit");
    if (source) *source = std::move(file);
    return vm;
}
...
void PawnRuntime::Bind(SourcePawn::IPluginRuntime& vm, const char* name, int argc,
    ...
    std::uint32_t index;
    if (vm.FindNativeByName(name, &index) != SP_ERROR_NONE) return;
    ...
    binding->Release();
    if (result != SP_ERROR_NONE) throw std::runtime_error("native binding failed: " + std::string(name));
}
...
void PawnRuntime::ReportError(const SourcePawn::IErrorReport& report, SourcePawn::IFrameIterator& frames) {
    ...
    for (; !frames.Done(); frames.Next()) {
        if (frames.IsInternalFrame()) continue;
        message += "\n  ";
