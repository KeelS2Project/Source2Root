...
KeelResult Cs2MenuBackend::Render(IGameEventManager2* manager, int slot, const std::string& html, int duration_ms) {
    ...
    try {
        ...
        if (!event)
            throw std::runtime_error("CS2 menu game event is unavailable");
        ...
        if (!definition)
            throw std::runtime_error("legacy game-event network message unavailable");

        allocated = definition->AllocateMessage();

        if (!allocated)
            throw std::runtime_error("legacy game-event allocation failed");
    ...
    if (event) {
        try {
            ...
            error_ = "CS2 menu event release failed";
            result = KEEL_RESULT_ENGINE_FAILURE;
        }
    }
    ...
    if (allocated) {
        try {
            messages_->DeallocateNetMessageAbstract(definition, allocated);
        } catch (...) {
            error_ = "CS2 menu message release failed";
            result = KEEL_RESULT_ENGINE_FAILURE;
        }
    }
