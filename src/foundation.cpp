...
Manifest Manifest::Read(const std::filesystem::path& path) {
    ...
    if (size > 16384)
        throw std::runtime_error("manifest exceeds 16 KiB");
...
Foundation::Foundation(GameHost& host,
                       const std::filesystem::path& runtime_library,
                       std::filesystem::path root,
                       CoreSettings settings)
    : host_(host), runtime_(runtime_library,
                            [&](const auto& message) {
                                host_.Log(message);
                            }),
      root_(std::move(root)), bans_(root_ / "data/bans.json"), voice_(host_), thread_(std::this_thread::get_id()),
...
bool Foundation::CheckDependencies(const Manifest& manifest) {
    ...
    while (!pending.empty()) {
        ...
        if (id == manifest.id)
            return Fail(manifest.id + ": dependency cycle; replacement refused");
        ...
        for (const auto* script : {found->second.current.get(), found->second.replacement.get()}) {
            ...
            for (const auto& [dependency, version] : script->manifest.dependencies)
...
std::string Foundation::Dependent(const std::string& id, bool running_only) const {
    for (const auto& [other_id, slot] : scripts_) {
        ...
        for (const auto* script : {slot.current.get(), slot.replacement.get()}) {
            if (!script || !script->vm || !script->manifest.dependencies.contains(id))
                continue;
...
std::unique_ptr<Foundation::Script> Foundation::Prepare(const std::filesystem::path& manifest_path,
    ...
    try {
        ...
        if (Cleanup(*candidate))
            candidate->state = PluginState::Failed;
...
bool Foundation::Load(const std::filesystem::path& manifest) {
    ...
    if (!scope)
        return Fail("plugin management is already in progress");
...
bool Foundation::UnloadScript(const std::string& id) {
    ...
    if (found == scripts_.end() || !found->second.current)
        return Fail("unknown plugin ID: " + id);

    const auto dependent = Dependent(id);

    if (!dependent.empty())
        return Fail(id + ": in use by " + dependent);
...
bool Foundation::ReloadScript(const std::string& id) {
    ...
    if (slot.current->active)
        return Fail(id + ": callback active; replacement refused");
    ...
    try {
        ...
        if (!CheckDependencies(slot.replacement->manifest))
            return false;
        ...
        return error_ == error.what() ? false : Fail(id + ": " + error.what());
    }
}
...
bool Foundation::ClearMenus(Script& script) {
    ...
    for (const auto& [slot, display] : displays_)
        if (display.script == &script)
            return Fail(script.manifest.id + ": menu clear pending; retry required");
...
bool Foundation::Pause(const std::string& id) {
    ...
    if (found->second.replacement)
        return Fail(id + ": replacement pending; finish reload or unload first");

    if (script.state == PluginState::Paused)
        return ClearMenus(script);

    if (script.state != PluginState::Running)
        return Fail(id + ": only running plugins can be paused");

    if (script.active)
        return Fail(id + ": callback active; pause refused");
...
bool Foundation::Resume(const std::string& id) {
    ...
    if (script.state != PluginState::Paused)
        return Fail(id + ": only paused plugins can be resumed");

    if (!CheckDependencies(script.manifest) || !ClearMenus(script))
        return false;
...
bool Foundation::UnloadAll() {
    ...
    if (!runtime_.Idle())
        return Fail("cannot unload all plugins while a script callback is active");
...
bool Foundation::UnloadAllScripts() {
    ...
    for (const auto& [id, slot] : scripts_)
        if (slot.current)
            order.emplace_back(slot.current->load_order, id);
    ...
    for (const auto& [id, slot] : scripts_) {
        if ((!slot.current || !slot.current->vm) && (!slot.replacement || !slot.replacement->vm))
            continue;
...
bool Foundation::Shutdown() {
    ...
    if (!runtime_.Idle())
        return Fail("cannot shut down SourcePawn while a callback is active");
    ...
    if (!voice_.Release(0, voice_error_)) {
        Fail(voice_error_);
        success = false;
    }
...
std::vector<KeelPluginHandle> Foundation::Extensions() const {
    ...
    for (const auto& [registration, provider] : providers_)
        owners.insert(provider.owner);
...
bool Foundation::Cleanup(Script& script) {
    ...
    if (script.active)
        return Fail(script.manifest.id + ": callback active; retained for unload retry");

    if (script.vm && !script.stop_notified) {
        script.stop_notified = true;

        if (auto* stop = script.vm->GetFunctionByName("OnPluginStop"))
            Invoke(script, stop);
    }
    ...
    for (const auto& [slot, display] : displays_)
        if (display.script == &script)
            return Fail(script.manifest.id + ": menu clear pending; retained for retry");
    ...
    if (!success)
        return Fail(script.manifest.id + ": native resource release failed; retained for retry");
...
unsigned Foundation::ServiceUsers(const std::string& service, unsigned version) const {
    ...
    for (const auto& [id, other] : providers_)
        if (other.service == service && other.version == version)
            users += other.users;

    for (const auto& [slot, display] : native_displays_)
        if (display.service == service && display.version == version)
            ++users;
...
bool Foundation::Invoke(Script& script, SourcePawn::IPluginFunction* function,
    ...
    if (!success) {
        ...
        if (++script.faults >= 3)
            script.state = PluginState::Retiring;
    }
...
std::vector<PluginStatus> Foundation::Status() const {
    ...
    for (const auto& [id, slot] : scripts_) if (slot.current)
            status.push_back({id,
                              slot.current->state,
                              slot.current->error,
                              slot.current->commands.size(),
                              slot.current->events.size(),
                              handles_.Owned(slot.current->owner).size(),
                              slot.current->convars.size()});
...
std::string Foundation::SelectPlugin(const std::string& selector) {
    ...
    for (const auto& [id, slot] : scripts_) if (slot.current) {
        ...
        if (entry.empty() || (selector != entry && selector != id + "/" + entry))
            continue;

        if (!match.empty()) {
            Fail("More than one script uses \"" + selector + "\". Use a plugin ID.");
            return {};
        }
    ...
    if (match.empty())
        Fail("Unknown script plugin \"" + selector + "\". Use sr plugins list.");
...
std::filesystem::path Foundation::SelectFile(const std::string& selector) {
    if (ValidIdentifier(selector))
        return root_ / "plugins" / selector / "plugin.json";
    ...
    if (!file.parent_path().empty()) {
        if (!ValidIdentifier(file.parent_path().string()))
            throw std::runtime_error("invalid script directory ID");
    ...
    if (std::filesystem::exists(directory)) for (const auto& entry : std::filesystem::directory_iterator(directory)) {
            ...
            if (manifest.entry != selector)
                continue;

            if (!match.empty())
                throw std::runtime_error("more than one plugin uses this file name; include the plugin directory");

            match = path;
    }

    if (match.empty())
        throw std::runtime_error("no plugin manifest names this file");
...
void Foundation::ManagePlugins(const std::vector<std::string>& arguments) {
    ...
    if (arguments.empty() || (arguments.size() == 1 && arguments[0] == "help")) {
        help();
        return;
    }
    ...
    if (operation == "list" && arguments.size() == 1) {
        ...
        output << "Source2Root Script Plugins:\n  " << std::left << std::setw(22) << "ID" << std::setw(11) << "Status"
               << "File\n";

        for (const auto& [id, slot] : scripts_) if (slot.current)
                output << "  " << std::setw(22) << id << std::setw(11) << StateName(slot.current->state)
                       << slot.current->manifest.entry << '\n';

        if (scripts_.empty())
            output << "  No known script plugins.\n";
    ...
    if (operation == "unload_all" && arguments.size() == 1) {
        if (UnloadAll())
            host_.Log("Unloaded all script plugins.");

        return;
    }

    if (operation == "refresh" && arguments.size() == 1) {
        Refresh();
        return;
    }
    ...
    if (operation == "cvars" && arguments.size() <= 2) {
        ...
        if (arguments.size() == 2 && selected.empty())
            return;
        ...
        for (const auto& [name, variable] : convars_) {
            ...
            for (const auto* script : ordered) {
                if (!selected.empty() && script->manifest.id != selected)
                    continue;
    ...
    if (operation == "load") {
        try {
            ...
            if (Load(file))
                host_.Log("Loaded " + scripts_.at(file.parent_path().filename().string()).current->manifest.entry +
                          ".");
        } catch (const std::exception& error) {
            Fail("Could not load " + arguments[1] + ": " + error.what() + ".");
        }
    ...
    if (operation != "info" && operation != "unload" && operation != "reload" && operation != "pause" &&
        operation != "resume" && operation != "retry") {
        help();
        return;
    }
    ...
    if (operation == "info") {
        ...
        for (const auto& [dependency, version] : manifest.dependencies)
            output << "\n  " << dependency << " >= " << version;

        if (scripts_.at(id).replacement)
...
void Foundation::Discover() {
    ...
    if (!scope) {
        Fail("plugin management is already in progress");
        return;
    }
...
bool Foundation::Refresh() {
    ...
    if (!runtime_.Idle())
        return Fail("cannot refresh plugins while a script callback is active");
...
void Foundation::RecordFailure(const std::filesystem::path& path, const std::string& error) {
    ...
    if ((slot.current && slot.current->vm) || slot.replacement)
        return;

    if (!slot.current)
        slot.current = std::make_unique<Script>();
    ...
    try {
        script.manifest = Manifest::Read(path);
    } catch (const std::exception&) {
    }
}
...
bool Foundation::SameBytecode(const std::filesystem::path& path, const std::vector<std::uint8_t>& expected) {
    ...
    if (!std::filesystem::is_regular_file(path))
        throw std::runtime_error("bytecode file is missing or is not a regular file");
    ...
    for (std::size_t offset = 0; offset < expected.size();) {
        ...
        if (!std::equal(buffer.begin(), buffer.begin() + count, expected.begin() + offset))
            return false;
    ...
    if (input.bad())
        throw std::runtime_error("cannot read bytecode file");
...
bool Foundation::DiscoverScripts(bool refresh) {
    ...
    if (refresh) for (auto& [id, slot] : scripts_) {
            ...
            if (script.state == PluginState::Disabled || script.state == PluginState::Failed)
                continue;
        ...
        try {
            ...
            if (manifest.source == script.manifest.source && same)
                ++unchanged;
            else
                pending.emplace(id, Pending{std::move(manifest), script.manifest_path, true});
        } catch (const std::exception& error) {
        ...
        if (std::filesystem::exists(directory)) for (const auto& entry : std::filesystem::directory_iterator(directory)) {
                ...
                try {
                    ...
                    if (!manifest.enabled) {
                        ...
                        pending.emplace(id, Pending{std::move(manifest), path, false});
            } catch (const std::exception& error) {
    ...
    while (progress && !pending.empty()) {
        ...
        for (auto it = pending.begin(); it != pending.end();) {
            ...
            if (std::any_of(
                    item.manifest.dependencies.begin(), item.manifest.dependencies.end(), [&](const auto& dependency) {
                        return pending.contains(dependency.first);
                    })) {
                ++it;
                continue;
            }
            ...
            if (ready) {
                ...
                if (item.replacement)
                    scripts_.at(it->first).current->error = error_;
    ...
    for (const auto& [id, item] : pending) {
        ...
        if (item.replacement)
            scripts_.at(id).current->error = message;
        else
            RecordFailure(item.path, message);
...
void Foundation::Limit(Script& script) {
    ...
    if (handles_.Owned(script.owner).size() + script.commands.size() + script.events.size() + script.callbacks.size() >=
        128)
...
bool Foundation::RegisterCommand(Script& script, const std::string& name, const std::string& permission,
    ...
    if ((!usage.empty() && usage != name && !usage.starts_with(name + " ")) ||
        std::any_of(usage.begin(), usage.end(), [](unsigned char c) {
            return c < 32 || c == 127;
        }))
        throw NativeError("usage must start with the command name and stay on one line");

    if (script.commands.contains(name)) {
        script.error = "duplicate command registration: " + name;
        return false;
    }
    ...
    if (existing != commands_.end()) {
        for (const auto* owner : existing->second)
            if (owner->manifest.id != script.manifest.id) {
                script.error = "command owned by another plugin: " + name;
                return false;
            }
    } else if (host_.RegisterCommand(name) != KEEL_RESULT_OK) {
        script.error = "native command registration failed: " + name;
        return false;
    }
...
bool Foundation::ListenEvent(Script& script, const std::string& name, SourcePawn::IPluginFunction* callback) {
    ...
    if (name != "round_start")
        throw NativeError("foundation supports the round_start game event");

    if (script.events.contains(name)) {
        script.error = "duplicate event registration";
        return false;
    }

    if (!events_.contains(name) && host_.ListenEvent(name) != KEEL_RESULT_OK) {
        script.error = "native event registration failed";
        return false;
    }
...
Cell Foundation::PlayerHandle(Script& script, const Player& player) {
    for (auto id : handles_.Owned(script.owner, PlayerType))
        if (std::get<Player>(handles_.Get(id, script.owner, PlayerType)).SameConnection(player))
            return id;
...
KeelResult Foundation::ResolvePlayer(Script& script, Cell handle, Player& player) {
    if (!handles_.Contains(handle, script.owner, PlayerType)) {
        script.error = "Player is no longer available.";
    ...
    if (result != KEEL_RESULT_OK) {
        script.error = "player lookup failed (KeelResult " + std::to_string(result) + ")";
        return result;
    }

    if (!player.SameConnection(expected)) {
        script.error = "Player is no longer available.";
        return KEEL_RESULT_NOT_FOUND;
    }
...
bool Foundation::Allowed(Script& script, Cell player, const std::string& permission) {
    if (!ValidPermission(permission))
        throw NativeError("invalid permission name");
    ...
    if (ResolvePlayer(script, player, current) != KEEL_RESULT_OK)
        return false;

    if (!permissions_.Allows(current, permission)) {
        script.error = "permission denied: " + permission;
        return false;
    }
