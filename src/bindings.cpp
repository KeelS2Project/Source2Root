...
void Foundation::Bind(Script& script) {
    ...
    bind(
        ...
        [&](const Arguments& args) {
            return RegisterCommand(script,
                                   args.String(1, 64),
                                   args.String(2, 96),
                                   args.Callback(3),
                                   args.Count() == 4 ? args.String(4, 256) : "");
        },
        4);
    ...
    bind("ReplyToCommand", 2, [&](const Arguments& args) {
        ...
        if (args.Int(1) && ResolvePlayer(script, args.Int(1), player) != KEEL_RESULT_OK)
            return 0;

        const auto result = host_.Reply(args.Int(1) ? &player : nullptr, text);

        if (result != KEEL_RESULT_OK)
            script.error = "reply failed (KeelResult " + std::to_string(result) + ")";
    ...
    bind("GetPlayerSteamID", 3, [&](const Arguments& args) {
        ...
        if (format < 0 || format > 2)
            throw NativeError("invalid Steam identity format");
    ...
    bind("CanTarget", 3, [&](const Arguments& args) {
        ...
        if (!args.Int(2)) {
            script.error = "A player target is required.";
            return 0;
        }

        if ((args.Int(1) && ResolvePlayer(script, args.Int(1), caller) != KEEL_RESULT_OK) ||
            ResolvePlayer(script, args.Int(2), target) != KEEL_RESULT_OK)
            return 0;
    ...
    bind("CanTargetIdentity", 3, [&](const Arguments& args) {
        ...
        try {
            target = ParseSteamIdentity(args.String(2, 64));
        } catch (const std::exception&) {
            script.error = "Invalid Steam identity.";
            return 0;
        }
        ...
        if (!permissions_.CanTargetIdentity(args.Int(1) ? &caller : nullptr, target, permission)) {
            script.error = "You cannot target an account with equal or higher immunity.";
            return 0;
        }
    ...
    bind("ReloadAdmins", 1, [&](const Arguments& args) {
        if (!Allowed(script, args.Int(1), "admin.reloadadmins"))
            return 0;
    ...
    bind("CreateTimer", 3, [&](const Arguments& args) {
        ...
        if (milliseconds < 1 || milliseconds > 3600000)
            throw NativeError("timer delay must be 1..3600000 milliseconds");

        auto* callback = args.Callback(2);
        Player player;

        if (args.Int(3) && ResolvePlayer(script, args.Int(3), player) != KEEL_RESULT_OK)
            return Cell{0};
        ...
        return handles_.Add(
            script.owner,
            TimerType,
            Timer{now_ + std::chrono::milliseconds(milliseconds), callback, args.Int(3), across_maps == 1});
    }, 4);
    ...
    bind("CreateMenu", 3, [&](const Arguments& args) {
        ...
        if (title.empty() || !ValidPermission(permission))
            throw NativeError("invalid menu title or permission");
    ...
    bind("AddMenuItem", 3, [&](const Arguments& args) {
        ...
        if (menu.menu.items.size() >= 128)
            throw NativeError("menu item limit (128) reached");

        for (const auto& [slot, display] : displays_)
            if (display.script == &script && display.menu == args.Int(1))
                throw NativeError("cannot edit a displayed menu");

        auto text = args.String(2, 96);

        if (text.empty() || (args.Int(3) != 0 && args.Int(3) != 1))
            throw NativeError("invalid menu item");

        const auto value = args.Count() == 4 ? args.Int(4) : -1;

        if (value < -1)
            throw NativeError("menu item value must be -1 or nonnegative");
    ...
    bind("ShowMenu", 3, [&](const Arguments& args) {
        ...
        if (ResolvePlayer(script, args.Int(2), player) != KEEL_RESULT_OK ||
            !Allowed(script, args.Int(2), menu.menu.permission))
            return 0;

        if (args.Int(3) < 250 || args.Int(3) > 120000 || menu.menu.items.empty())
            throw NativeError("menu requires items and a 250..120000 ms timeout");
        ...
        if (script.state != PluginState::Running) {
            script.error = "display menus after initialization, in a callback";
            return 0;
        }

        if (!CloseDisplay(player.slot)) {
            script.error = "previous menu clear failed";
            return 0;
        }

        if (next_session_ == std::numeric_limits<std::uint64_t>::max())
            throw NativeError("menu session IDs exhausted");

        menu.menu.selected = 0;
        const auto result = host_.RenderMenu(player, menu.menu.Html(menu.back != nullptr), args.Int(3));

        if (result != KEEL_RESULT_OK) {
            script.error = "menu renderer unavailable (KeelResult " + std::to_string(result) + ")";
            return 0;
        }

        const auto session = next_session_++;
        displays_.emplace(player.slot,
                          Display{session,
                                  &script,
                                  args.Int(1),
                                  args.Int(2),
                                  ...
                                  std::chrono::milliseconds(args.Int(3)),
                                  {}});
    ...
    bind("CloseMenu", 1, [&](const Arguments& args) {
        ...
        for (auto it = displays_.begin(); it != displays_.end();) {
            ...
            if (owned && !CloseDisplay(slot)) {
                script.error = "menu clear failed; retry close";
                return 0;
            }
        }

        if (handles_.Contains(args.Int(1), script.owner, MenuType))
            handles_.Remove(args.Int(1), script.owner, MenuType);
    ...
    for (auto& [registration, provider] : providers_) {
        ...
        if (script.vm->FindNativeByName(provider.name.c_str(), &index) != SP_ERROR_NONE)
            continue;
        ...
        runtime_.Bind(*script.vm, provider.name.c_str(), static_cast<int>(provider.argc),
            [this, &script, &provider](const Arguments& args) -> Cell {
                ...
                if (provider.context_invoke)
                    return InvokeContextNative(script, provider, args);

                std::vector<Cell> arguments;

                for (unsigned i = 1; i <= provider.argc; ++i)
                    arguments.push_back(args.Int(static_cast<int>(i)));
...
KeelResult Foundation::RegisterNative(KeelPluginHandle owner, const SrNativeSpec& spec, SrRegistration& registration) {
    ...
    if (spec.size != sizeof(spec) || spec.api_version != SR_EXTENSION_API_VERSION)
    ...
    if (!owner || !spec.name || !spec.provider_service || !spec.invoke || !spec.provider_version ||
        ...
        std::strlen(spec.provider_service) > 96 || !*spec.provider_service)
    ...
    const auto first = [](unsigned char c) {
        return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c == '_';
    };

    if (name.empty() || !first(name.front()) ||
        ...
        next_provider_ == std::numeric_limits<std::uint64_t>::max())
        return KEEL_RESULT_INVALID_ARGUMENT;
...
KeelResult Foundation::UnregisterNative(KeelPluginHandle owner, SrRegistration registration) {
    ...
    if (found->second.users || found->second.active)
