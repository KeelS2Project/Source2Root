...
public:
    ...
    std::vector<KeyValue> Parse(unsigned depth = 0) {
        if (depth > 16)
            Error("objects are nested too deeply");
...
private:
    ...
    std::string Token() {
        ...
        while (position_ < source_.size()) {
            ...
            if (static_cast<unsigned char>(c) < 32 || c == 127 || (!quoted && c == '"'))
            ...
            if (text.size() > 4096)
                Error("token exceeds 4096 bytes");
        }
...
std::vector<KeyValue> ReadKeyValues(const std::filesystem::path& file) {
    try {
        ...
        if (!input.read(source.data(), static_cast<std::streamsize>(size)))
            throw std::runtime_error("cannot read complete configuration");

        if (input.peek() != std::char_traits<char>::eof())
            throw std::runtime_error("configuration changed while reading");
