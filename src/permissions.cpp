...
Fields Properties(const KeyValue& entry, std::initializer_list<std::string_view> allowed) {
    if (!entry.object)
        throw std::runtime_error(entry.name + " must be an object");
    ...
    for (const auto& child : entry.children) {
        ...
        if (!fields.emplace(name, &child).second)
            throw std::runtime_error(entry.name + ": duplicate field " + child.name);
    }
...
const std::string& Scalar(const Fields& fields, const char* name) {
    ...
    if (found->second->object)
        throw std::runtime_error(std::string(name) + " must be a string value");

    return found->second->value;
}

int ImmunityValue(const Fields& fields, int fallback) {
    if (!fields.contains("immunity"))
        return fallback;
...
bool Authenticated(const Player& player) {
    return player.slot >= 0 && player.connection && player.authenticated && !player.bot &&
           ValidSteamIdentity(player.steam_id);
}
...
void Permissions::Load(const std::filesystem::path& directory) {
    ...
    if (!has_admins || !has_groups)
        throw std::runtime_error("admins.cfg and admin_groups.cfg must both be present");
    ...
    for (const auto& entry : Root(group_file, "Groups")) {
        ...
        if (key.empty() || key.size() > 64 || std::any_of(key.begin(), key.end(), [](unsigned char c) {
                return c < 32 || c == 127;
            }))
        ...
        if (const auto found = fields.find("permissions"); found != fields.end()) {
            if (!found->second->object)
                throw std::runtime_error(entry.name + ": permissions must be an object");
            ...
            for (const auto& permission : found->second->children) {
                ...
                if (!group.permissions.insert(permission.name).second)
                    throw std::runtime_error(entry.name + ": duplicate permission " + permission.name);
            }
        }

        if (!groups.emplace(key, std::move(group)).second)
            throw std::runtime_error("duplicate group name: " + entry.name);

        if (groups.size() > 128)
            throw std::runtime_error("administrator configuration exceeds 128 groups");
    }

    std::map<std::uint64_t, Admin> admins;

    for (const auto& entry : Root(admin_file, "Admins")) {
        if (entry.name.size() > 128 || std::any_of(entry.name.begin(), entry.name.end(), [](unsigned char c) {
                return c < 32 || c == 127;
            }))
        ...
        if (found == groups.end())
            throw std::runtime_error(entry.name + ": unknown administrator group");
        ...
        if (!admins
                 .emplace(
                     identity,
                     Admin{
                         entry.name, group.name, group.permissions, ImmunityValue(fields, group.immunity), group.root})
                 .second)
            throw std::runtime_error("duplicate administrator identity: " + std::to_string(identity));

        if (admins.size() > 4096)
            throw std::runtime_error("administrator configuration exceeds 4096 administrators");
    }
...
bool Permissions::CanTarget(const Player* caller, const Player& target, const std::string& permission) const {
    if (!ValidPermission(permission) || target.slot < 0 || !target.connection)
        return false;

    if (!caller)
        return true;

    if (!Authenticated(*caller) || !Allows(*caller, permission))
        return false;

    if (!target.bot && !Authenticated(target))
        return false;

    if (caller->SameConnection(target) && caller->steam_id == target.steam_id)
        return true;
...
bool Permissions::CanTargetIdentity(const Player* caller, std::uint64_t target, const std::string& permission) const {
    if (!ValidPermission(permission) || !ValidSteamIdentity(target))
        return false;
