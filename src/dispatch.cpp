...
bool Foundation::Dispatch(Origin origin, int slot, const std::string& text) {
    ...
    if (chat && !public_match && !silent_match)
        return gagged;

    const bool silent =
        silent_match && (!public_match || settings_.silent_trigger.size() > settings_.public_trigger.size());
    ...
    for (auto* script : found->second) {
        ...
        try {
            ...
            Fail(script->manifest.id + ": " + error.what());
        }
...
void Foundation::Event(const std::string& name) {
    ...
    for (auto* script : found->second)
        owners.emplace_back(script->manifest.id, script->owner);
...
bool Foundation::CloseDisplay(int slot) {
    ...
    if (result != KEEL_RESULT_OK || host_.RenderMenu(current, "") != KEEL_RESULT_OK)
        return false;
...
void Foundation::Tick(Clock::time_point now) {
    ...
    if (now_ >= next_voice_) {
        ...
        if (!voice_.Refresh(error) && error != voice_error_)
            Fail(error);
    ...
    for (auto& [id, slot] : scripts_) {
        ...
        if (!script || script->state != PluginState::Running)
            continue;

        for (auto handle : handles_.Owned(script->owner, TimerType)) {
            if (!handles_.Contains(handle, script->owner, TimerType))
                continue;
    ...
    for (auto it = displays_.begin(); it != displays_.end();) {
        ...
        if (display.expires <= now_ || display.script->state != PluginState::Running) {
            CloseDisplay(slot);
            continue;
        }
...
MenuControls Foundation::InitialMenuControls(const Player& player) {
    ...
    try {
        if (host_.ReadPlayerInput(player, input) == KEEL_RESULT_OK)
            controls.Baseline(input);
    } catch (...) {}
...
void Foundation::PollMenuInput() {
    ...
    for (const auto& [slot, display] : displays_)
        if (display.script->state == PluginState::Running)
            sessions.emplace_back(display.player, display.session);

    for (const auto& [slot, display] : native_displays_)
        if (!display.active && !display.closing)
            sessions.emplace_back(display.player, display.session);
    ...
    for (const auto& [player, session] : sessions) {
        ...
        else if (auto found = native_displays_.find(player.slot);
                 found != native_displays_.end() && found->second.session == session)
            controls = &found->second.controls;
        ...
        if (controls) {
            ...
            if (action)
                MenuInput(player, session, *action);
        }
    }
}
...
void Foundation::MapChanged() {
    ...
    for (auto& [id, slot] : scripts_)
        for (auto* script : {slot.current.get(), slot.replacement.get()}) if (script) {
            ...
            for (auto type : {PlayerType, MenuType})
                handles_.Retire(script->owner, type);
...
void Foundation::Disconnected(int slot, std::uint64_t generation) {
    ...
    for (auto& [id, entry] : scripts_) for (auto* script : {entry.current.get(), entry.replacement.get()}) if (script) {
        ...
        if (gag != script->gags.end() && gag->second.connection == generation)
            script->gags.erase(gag);
        ...
        for (auto handle : handles_.Owned(script->owner, PlayerType)) {
            ...
            if (player.slot != slot || player.connection != generation)
                continue;
...
bool Foundation::MenuInput(const Player& player, std::uint64_t session, sr::MenuInput input) {
    ...
    if (native_displays_.contains(player.slot))
        return NativeMenuInput(player, session, input);

    auto found = displays_.find(player.slot);

    if (found == displays_.end() || found->second.session != session || !found->second.player.SameConnection(player))
        return false;

    auto display = found->second;

    if (display.script->state != PluginState::Running || display.expires <= now_) {
    ...
    if (!Allowed(*display.script, display.player_handle, menu.menu.permission)) {
        CloseDisplay(player.slot);
        return false;
    }
    ...
    if (action == MenuAction::Cancelled)
        return CloseDisplay(player.slot);
    ...
    if (action == MenuAction::Changed) {
        if (host_.RenderMenu(
                player,
                menu.menu.Html(menu.back != nullptr),
                static_cast<int>(std::chrono::ceil<std::chrono::milliseconds>(display.expires - now_).count())) ==
            KEEL_RESULT_OK)
            return true;
