...
public:
    ...
    Id Add(std::uint64_t owner, std::uint32_t type, Value value) {
        if (!owner || !type) throw std::runtime_error("invalid handle owner/type");
        for (std::size_t i = 0; i < entries_.size(); ++i) {
            auto& entry = entries_[i];
            if (entry.owner || entry.generation == MaxGeneration) continue;
            ++entry.generation;
            entry.owner = owner;
            entry.type = type;
        ...
        if (entries_.size() >= 4095) throw std::runtime_error("handle capacity exhausted");
    ...
    bool Contains(Id id, std::uint64_t owner, std::uint32_t type) {
        try { Check(id, owner, type); return true; }
    ...
    std::size_t Count() const {
        std::size_t count = 0;
        for (const auto& entry : entries_) if (entry.owner) ++count;
        return count;
    }
private:
    ...
    Entry& Check(Id id, std::uint64_t owner, std::uint32_t type) {
        ...
        if (id <= 0 || !slot || slot > entries_.size()) throw std::runtime_error("invalid handle");
