...
public:
    ...
    public:
        ~Ticket() { Cancel(); }
        void Cancel() noexcept { job_->canceled.store(true, std::memory_order_relaxed); }
    ...
    std::unique_ptr<Ticket> Submit(std::function<void(const std::atomic_bool&)> work,
        std::function<bool(std::exception_ptr)> completion) {
        Thread();
        if (!work || !completion) throw std::invalid_argument("Missing job function.");
        ...
        auto ticket = std::unique_ptr<Ticket>(new Ticket(job));
        {
            ...
            if (stopping_) throw std::logic_error("Worker queue is stopping.");
            if (jobs_.size() >= capacity_) return {};
            jobs_.push_back(job);
            try { waiting_.push_back(job); }
    ...
    void Dispatch() {
        Thread();
        if (dispatching_) throw std::logic_error("Worker completions cannot dispatch recursively.");
        struct Scope { bool& active; ~Scope() { active = false; } } scope{dispatching_};
        dispatching_ = true;
        std::size_t remaining;
        { std::lock_guard lock(mutex_); remaining = ready_.size(); }
        while (remaining--) {
            std::shared_ptr<Job> job;
            { std::lock_guard lock(mutex_); job = ready_.front(); ready_.erase(ready_.begin()); }
            bool consumed = job->canceled.load(std::memory_order_relaxed);
            std::exception_ptr failure;
            if (!consumed) {
                try { consumed = job->completion(job->error); }
                catch (...) { consumed = true; failure = std::current_exception(); }
            }
...
private:
    void Thread() const {
        if (owner_ != std::this_thread::get_id()) throw std::logic_error("Worker queue requires its owner thread.");
    }
    void Run() noexcept {
        for (;;) {
            std::shared_ptr<Job> job;
    ...
    void Stop() noexcept {
        {
            std::lock_guard lock(mutex_);
            stopping_ = true;
            for (const auto& job : jobs_) job->canceled.store(true, std::memory_order_relaxed);
        }
        wake_.notify_all();
        for (auto& worker : workers_) if (worker.joinable()) worker.join();
        // All closures are now destroyed by the owner, after workers have left.
        for (const auto& job : jobs_) { job->work = {}; job->completion = {}; job->error = {}; }
        waiting_.clear(); ready_.clear(); jobs_.clear();
    }
