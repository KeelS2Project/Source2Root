...
public:
    bool Load() final {
        if (api_ || native_api_ || callback_api_ || consumer_api_ || publication_ || queried_ || native_queried_ ||
            callback_queried_ || consumer_queried_)
            return false;
        ...
        try {
            ...
            if (std::any_of(bindings_.begin(), bindings_.end(), [](const auto& binding) {
                    return bool(binding->context_call);
                })) {
                raw = nullptr;
            ...
            if (services_.Publish(
                    service_.c_str(), version_, service_value_ ? service_value_ : &marker_, publication_) !=
                KEEL_RESULT_OK)
            ...
            for (auto& binding : bindings_) {
                ...
                if (binding->context_call) {
                    ...
                    registered = native_api_->register_native(
                        native_api_->context, HostContext().PluginHandle(), &spec, &binding->registration);
                } else {
            ...
            return SetupFailed(error.what());
        } catch (...) {
            return SetupFailed("Extension initialization threw an exception.");
        }
    }

    bool PrepareUnload() final {
        return PrepareExtensionUnload() && Release();
    }
...
protected:
    ...
    virtual bool PrepareExtensionUnload() {
        return true;
    }
    ...
    KeelResult RetainCallback(SrCallback callback) {
        ...
        if (!callback_api_) {
            ...
            if (!api || api->size != sizeof(*api) || api->api_version != SR_CALLBACK_API_VERSION || !api->retain ||
                !api->invoke || !api->cancel)
                return KEEL_RESULT_INCOMPATIBLE;
    ...
    KeelResult InvokeCallback(SrCallback callback, const std::vector<SrCallbackArgument>& arguments,
        ...
        if (!ready_ || !callback_api_)
            return KEEL_RESULT_NOT_READY;

        if (arguments.size() > SR_CALLBACK_MAX_ARGUMENTS)
    ...
    KeelResult PlayerSnapshot(std::vector<SrPlayerIdentity>& players) {
        ...
        if (!native_api_ || !native_api_->player_snapshot)
            return KEEL_RESULT_NOT_READY;
        ...
        if (result == KEEL_RESULT_OK && count <= snapshot.size())
            players.assign(snapshot.begin(), snapshot.begin() + count);
        else if (result == KEEL_RESULT_OK)
    ...
    KeelResult OpenNativeMenu(const KeelPlayerConnection& player, SrMenuSpec spec, SrMenuSession& session) {
        ...
        if (!api_ || !api_->open_menu)
            return KEEL_RESULT_NOT_READY;

        spec.provider_service = service_.c_str();
        spec.provider_version = version_;
    ...
    KeelResult NativeMenuStatus(SrMenuSession session) {
        if (!native_api_ || !native_api_->menu_status)
            return KEEL_RESULT_NOT_READY;
    ...
    KeelResult ConsumerStatus(std::uint64_t owner) {
        if (!native_api_ || !native_api_->consumer_status)
            return KEEL_RESULT_NOT_READY;
    ...
    KeelResult ConsumerPermission(std::uint64_t owner,
                                  const KeelPlayerConnection& player,
        ...
        return consumer_api_->check_permission(
            consumer_api_->context, HostContext().PluginHandle(), owner, &player, permission, &allowed);
    }
    ...
    bool RegisterNative(const char* name, std::int32_t (Owner::*callback)(Arguments...)) {
        ...
        if (!staging_ || !name || !*name || !callback)
            return false;
    ...
    bool RegisterNative(const char* name, std::uint32_t count, std::int32_t (Owner::*callback)(NativeCall&)) {
        ...
        if (!staging_ || !name || !*name || !callback || count > 16)
            return false;
        ...
        binding->name = name;
        binding->count = count;
        binding->context_call = [owner, callback](NativeCall& call) {
            return (owner->*callback)(call);
        };
        bindings_.push_back(std::move(binding));
        return true;
    }
...
private:
    KeelResult ConnectConsumers() {
        ...
        if (!api || api->size != sizeof(*api) || api->api_version != SR_CONSUMER_API_VERSION || !api->player_handle ||
    ...
    struct Binding {
        ...
        static KeelResult InvokeContext(void* data, const SrNativeCall* call, std::int32_t* result,
            ...
            if (!binding->extension->ready_)
                return fail(KEEL_RESULT_NOT_READY, "Extension initialization is incomplete.");

            try {
                NativeCall context(*call);
                *result = binding->context_call(context);
                return KEEL_RESULT_OK;
            } catch (const std::invalid_argument& exception) {
                return fail(KEEL_RESULT_INVALID_ARGUMENT, exception.what());
            } catch (const std::exception& exception) {
                return fail(KEEL_RESULT_ENGINE_FAILURE, exception.what());
            } catch (...) {
                return fail(KEEL_RESULT_ENGINE_FAILURE, "Native callback threw an exception.");
            }
        }
    ...
    bool ReleaseResources() {
        for (auto& binding : bindings_) if (binding->registration) {
                ...
                const auto result =
                    api_->unregister_native(api_->context, HostContext().PluginHandle(), binding->registration);
