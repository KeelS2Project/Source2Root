...
public:
    ...
    float Float(unsigned index) const {
        return std::bit_cast<float>(Int(index));
    }
    ...
    std::vector<std::int32_t> Array(unsigned index, unsigned count) const {
        if (count > SR_NATIVE_BUFFER_LIMIT / sizeof(std::int32_t))
            throw std::invalid_argument("Array is too large.");
    ...
    std::int32_t Own(std::uint32_t type, std::unique_ptr<T> value) const {
        if (!value)
            throw std::invalid_argument("Resource is empty.");
...
private:
    ...
    void Check(KeelResult result) const {
        if (result != KEEL_RESULT_OK)
            throw std::invalid_argument(call_.error && *call_.error ? call_.error : "Native call failed.");
    }
};
