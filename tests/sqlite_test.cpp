...
int main(int argc, char** argv) {
    try {
        ...
        Run(database, "CREATE TABLE values_test(id INTEGER PRIMARY KEY, n INTEGER, text TEXT, real REAL, empty TEXT)");
        {
            ...
            Reject([&] { insert.Step(); }, "step after done must not repeat writes");
            Reject([&] { insert.BindInt(1, 9); }, "bind requires reset");
            insert.Reset();
            insert.BindInt(1, 9);
        ...
        }
        {
            ...
            Reject([&] { query.Int(0); }, "get requires row");
            ...
            Reject([&] { query.String(4); }, "column bounds");
            Check(!query.Step(), "query complete");
        }
        Reject([&] { Statement invalid(database, "SELECT 1; SELECT 2"); }, "reject multiple statements");
        Reject([&] { Statement invalid(database, "ATTACH DATABASE 'outside.sqlite' AS other"); }, "reject file escape via attach");
        Reject([&] { Statement invalid(database, "PRAGMA writable_schema=ON"); }, "reject dangerous pragma");
        Reject([&] { Statement invalid(database, "SELECT load_extension('anything')"); }, "extension loading absent");
        Reject([&] { Statement invalid(database, "SELECT * FROM missing_table"); }, "SQL errors reported");
        {
            ...
            Reject([&] { big.Int(0); }, "cell overflow rejected");
            Reject([&] { big.String(1); }, "embedded NUL not silently truncated");
        }
        ...
        }
        {
            ...
            Reject([&] { other->Execute("BEGIN IMMEDIATE"); }, "busy connection reports failure");
            Check(std::chrono::steady_clock::now() - start < std::chrono::seconds(2), "busy wait bounded");
            database->Execute("COMMIT");
        }
        {
            Statement runaway(database, "WITH RECURSIVE loop(x) AS (SELECT 1 UNION ALL SELECT x+1 FROM loop) SELECT sum(x) FROM loop");
            const auto start = std::chrono::steady_clock::now();
            Reject([&] { runaway.Step(); }, "runaway query interrupted");
        ...
        Check(restored.Step() && restored.Int(0) == 2, "connection destruction rolls back transaction and retains committed data");
#if !defined(_WIN32)
        const auto link = root / "link.sqlite";
        std::filesystem::remove(link);
        std::filesystem::create_symlink(path, link);
        Reject([&] { Database linked(link); }, "do not follow database symlinks");
#endif
