...
int main() {
    try {
        ...
        auto second = queue.Submit(
            [&](const auto&) {
                canceled_work = true;
            },
            [&](auto) {
                canceled_completion = true;
                return true;
            });

        second.reset();
        const bool bounded = !queue.Submit(
            [](const auto&) {
            },
            [](auto) {
                return true;
        ...
        Await([&] {
            queue.Dispatch();
            return completions != 0 && queue.Pending() == 1;
        });
        ...
        auto failed = queue.Submit(
            ...
            [&](auto error) {
                try {
                    ...
                    exception_seen = std::string(e.what()) == "network failed";
                }
