...
int main(int argc, char** argv) {
    try {
        ...
        Reject([&] { Query(path, "CREATE TABLE items(value)", canceled); }, "canceled job rejected before opening database");
        ...
        bound.Bind(1, std::int32_t{42}); bound.Bind(2, 1.25); bound.Bind(3, std::string("quote'; DROP TABLE items; --"));
        ...
        Check(result.changes == 1 && result.inserted == "1" && result.rows[0][0].integer == 7, "write commits with result metadata");
        for (const auto* sql : {"COMMIT", "BEGIN", "ROLLBACK", "SAVEPOINT escape", "SELECT ?", "SELECT 1; SELECT 2",
            ...
            Reject([&] { Query(path, sql, canceled); }, "unsupported or unsafe async statement rejected");
        Reject([&] { Query(path, "INSERT INTO items WITH RECURSIVE numbers(x) AS (SELECT 1 UNION ALL SELECT x+1 FROM numbers WHERE x<257) SELECT x FROM numbers RETURNING value", canceled); },
        ...
        Reject([&] { Query(path, "WITH RECURSIVE numbers(x) AS (SELECT 1 UNION ALL SELECT x+1 FROM numbers WHERE x<256) SELECT hex(zeroblob(1000)) FROM numbers", canceled); },
            "total result byte limit enforced");
        std::string columns = "SELECT 1";
        for (int i = 1; i < 33; ++i) columns += ",1";
        Reject([&] { Query(path, columns, canceled); }, "column limit enforced");
        const auto start = std::chrono::steady_clock::now();
        Reject([&] { Query(path, "WITH RECURSIVE forever(x) AS (SELECT 1 UNION ALL SELECT x+1 FROM forever) SELECT sum(x) FROM forever", canceled); },
