...
int main() {
    ...
    const auto read = [&](std::uint64_t buttons) {
        input.buttons = buttons;
        return controls.Read(input);
    };
    ...
    Require(!read(0) && read(KEELS2_BUTTON_USE | KEELS2_BUTTON_BACK) == sr::MenuInput::Select,
            "select takes priority over navigation");

    Require(!read(0) && read(KEELS2_BUTTON_RELOAD | KEELS2_BUTTON_USE) == sr::MenuInput::Back,
            "back takes priority over selection");

    Require(!read(0) && !read(KEELS2_BUTTON_ATTACK | KEELS2_BUTTON_JUMP), "unrelated gameplay input ignored");
    input.context = 2;
    Require(!read(KEELS2_BUTTON_USE), "changed input context resets baseline");
    input.reserved = 1;
    Require(!read(0), "malformed snapshot rejected");
    input.reserved = 0;
    Require(!read(KEELS2_BUTTON_USE), "malformed snapshot resets later held input");
    input.buttons = UINT64_MAX;
    Require(!controls.Read(input), "unknown bits rejected");
    Require(!read(KEELS2_BUTTON_USE), "unknown bits reset later held input");
    input.context = 0;
    Require(!read(0), "zero context rejected");
    input.context = 2;
    Require(!read(KEELS2_BUTTON_USE), "zero context resets later held input");
    input.size = 0;
    Require(!read(0), "wrong snapshot size rejected");
    input.size = sizeof(input);
    Require(!read(KEELS2_BUTTON_USE), "wrong size resets later held input");
