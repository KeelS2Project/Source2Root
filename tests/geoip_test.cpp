...
static const auto& Text(const Record& record, Field field) { return record.text[static_cast<unsigned>(field)]; }
int main(int argc, char** argv) {
    try {
        ...
        for (const auto* bad : {"", "../city.mmdb", "dir/city.mmdb", ".hidden.mmdb", "city", "city..mmdb"})
            Reject([&] { ValidateName(bad); }, "invalid database filename refused");
        ValidateName("GeoLite2-City.mmdb");
        Reject([&] { Reader reader(data / "GeoIP2-City-Test.mmdb", snapshots, stopped); }, "pre-canceled load does no work");
        Reject([&] { Reader reader(root / "missing.mmdb", snapshots, canceled); }, "missing database refused");
        std::ofstream(input) << "malformed";
        Reject([&] { Reader reader(input, snapshots, canceled); }, "malformed database refused");
        ...
        std::optional<Record> retained;
        {
            ...
            Check(london && Text(*london, Field::CountryCode) == "GB" && Text(*london, Field::RegisteredCountryCode) == "US" &&
                Text(*london, Field::City) == "Londres" && Text(*london, Field::RegionCode) == "GB-ENG", "localized IPv4 and distinct country fields");
            ...
            Check(Text(*reader.Lookup("[2001:218::1]:27015", "ja"), Field::CountryName) == "日本", "IPv6 bracketed port and UTF8 locale");
            Check(Text(*reader.Lookup("2001:218::1:1234"), Field::CountryCode) == "JP", "bare IPv6 suffix remains part of address");
            Check(Text(*reader.Lookup("::ffff:81.2.69.160", "unavailable"), Field::City) == "London", "mapped IPv4 and English fallback");
            ...
            for (const auto* bad : {"localhost", "https://81.2.69.160", "81.2.69.160:65536", "81.2.69.160:", "81.2.69.160:bad",
                ...
                Reject([&] { reader.Lookup(bad); }, "hostname or invalid address/port refused");
            Reject([&] { reader.Lookup("81.2.69.160", "en/../fr"); }, "invalid locale refused");
            std::ofstream(input, std::ios::trunc) << "replacement";
            retained = reader.Lookup("81.2.69.160");
            Check(retained && Text(*retained, Field::City) == "London", "original file truncation cannot invalidate private mapping");
            std::atomic<unsigned> successful{0};
            std::vector<std::thread> threads;
            for (int i = 0; i < 4; ++i) threads.emplace_back([&] {
                for (int j = 0; j < 100; ++j) if (Text(*reader.Lookup("81.2.69.160"), Field::City) == "London") ++successful;
            });
            for (auto& thread : threads) thread.join();
            Check(successful == 400, "immutable reader supports independent concurrent lookups");
        }
        Check(std::filesystem::is_empty(snapshots) && Text(*retained, Field::City) == "London", "records outlive reader and snapshot is removed");
        {
            Reader reader(data / "GeoIP2-Country-Test.mmdb", snapshots, canceled);
            const auto value = reader.Lookup("81.2.69.160");
            Check(value && Text(*value, Field::CountryCode) == "GB" && !Text(*value, Field::City) && !value->numbers[0], "partial country database leaves unsupported fields absent");
        }
        {
            Reader reader(data / "GeoLite2-ASN-Test.mmdb", snapshots, canceled);
            const auto value = reader.Lookup("1.0.0.1");
            Check(value && value->asn == 15169 && Text(*value, Field::Organization) == "Google Inc." && !Text(*value, Field::CountryCode), "ASN database and absent geography");
        }
        ...
        Reject([&] { Distance(91, 0, 0, 0, false); }, "latitude bound");
        Reject([&] { Distance(0, 0, 0, std::numeric_limits<double>::infinity(), false); }, "nonfinite distance refused");
        Check(std::filesystem::is_empty(snapshots), "all successful readers release private snapshots");
        {
            const auto large = root / "large.mmdb";
            std::ofstream(large).close(); std::filesystem::resize_file(large, 512ull * 1024 * 1024 + 1);
            Reject([&] { Reader reader(large, snapshots, canceled); }, "oversized sparse database refused before copy");
            std::filesystem::remove(large);
        }
#if !defined(_WIN32)
        const auto link = root / "linked.mmdb";
        std::filesystem::create_symlink(data / "GeoIP2-City-Test.mmdb", link);
        Reject([&] { Reader reader(link, snapshots, canceled); }, "database symlinks refused");
#endif
        unsigned corrupt = 0;
        for (const auto& file : std::filesystem::recursive_directory_iterator(data.parent_path() / "bad-data")) {
            if (!file.is_regular_file() || file.path().extension() != ".mmdb") continue;
            try {
                ...
                for (const auto* address : {"1.1.1.1", "81.2.69.160", "2001:218::1", "ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff"}) {
                    try { reader.Lookup(address); } catch (const Error&) { /* malformed entries are bounded errors */ }
                }
