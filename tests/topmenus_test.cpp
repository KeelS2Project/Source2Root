...
int main() {
    try {
        ...
        Check(catalog.Open("admin", "Ignored title") == menu && menu->title == "Administration",
              "named menus share one catalog");

        std::set<std::uint64_t> running{1, 2};
        auto live = [&](auto owner) {
            return running.contains(owner);
        };
        auto access = [](const auto& object) {
            if (object->permission == "deny")
        ...
        Check(root.rows.size() == 2 && root.rows[0].label == "Players" && !root.rows[1].enabled,
              "categories sorted with disabled root items");

        auto page = menu->Build("players", 0, live, access);
        Check(page.rows.size() == 2 && page.rows[0].object.lock() == item && page.rows[1].back,
              "shared contribution and explicit back");

        Reject(
            [&] {
                menu->Add(top::Kind::Item, "test", "players", "Other", "", "first", 1, 0);
            },
            "foreign duplicate rejected");

        Reject(
            [&] {
                menu->Add(top::Kind::Item, "test", "players", "Duplicate", "", "second", 2, 0);
            },
            "same generation duplicate rejected");

        auto replacement = menu->Add(top::Kind::Item, "test", "players", "Reloaded", "", "second", 3, -1);
        Check(menu->Build("players", 0, live, access).rows[0].object.lock() == item,
              "loading generation does not mask running item");

        running.insert(3);
        Check(menu->Build("players", 0, live, access).rows[0].object.lock() == replacement, "latest running generation wins");
        replacement->Close();
        Check(menu->Build("players", 0, live, access).rows[0].object.lock() == item,
              "aborted replacement restores existing registration");
        ...
        Reject(
            [&] {
                menu->Build("players", 0, live, access);
            },
            "paused category closes its submenu");
        ...
        item->SetOrder(-9);
        Check(menu->revision == revision + 3 && menu->Build("", 0, live, access).title == "New title",
              "title label order invalidate cached display");

        Reject(
            [&] {
                menu->Build("", 0, live, [&](const auto&) {
                    item->SetLabel("Mutation");
                    return top::Access::Enabled;
                });
            },
            "access callback mutation discards stale page");
        ...
        Check(page.rows.size() == 30 && page.rows[28].page == 1 && page.rows[29].back,
              "first page has bounded content and navigation");

        page = menu->Build("players", 1, live, access);
        Check(page.rows.size() == 31 && page.rows[28].page == 0 && page.rows[29].page == 2 && page.rows[30].back,
              "middle page has both directions and back");
        ...
        Reject(
            ...
            "closed category cannot dispatch child");

        Reject(
            [&] {
                catalog.Open("../escape", "Invalid");
            },
            "invalid menu identifier rejected");

        Reject(
            [&] {
                disabled->SetLabel("line\nbreak");
            },
            "control characters rejected");

        Reject(
            [&] {
                menu->Add(top::Kind::Item, "invalid", "", "Invalid", "Admin..root", "first", 1, 0);
            },
        ...
        for (int i = 0; i < 256; ++i)
            many.push_back(menu->Add(top::Kind::Item, "bounded" + std::to_string(i), "", "Bounded", "", "first", 1, 0));

        Reject(
            [&] {
                menu->Add(top::Kind::Item, "overflow", "", "Overflow", "", "first", 1, 0);
            },
            "object catalog enforces aggregate limit");

        many.clear();
        std::vector<std::shared_ptr<top::Menu>> menus;

        for (int i = 0; i < 31; ++i)
            menus.push_back(catalog.Open("menu" + std::to_string(i), "Menu"));

        Reject(
            [&] {
                catalog.Open("overflow", "Too many");
            },
            "catalog bounds retained menus");
