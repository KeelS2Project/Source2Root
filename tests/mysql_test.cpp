...
int main(int argc, char** argv) {
    try {
        ...
        auto run = [&](const std::string& sql) {
            return mysql::Query(config, db::QueryInput{sql, {}}, canceled);
        };
        ...
        insert.Bind(2, std::string("quote'; DROP TABLE source2root_driver_checks; --"));
        insert.Bind(3, 1.25);
        ...
        for (const auto* sql : {"SELECT ?",
                                "SELECT 1; SELECT 2",
                                "SELECT * FROM missing_table",
                                "SELECT CONCAT('a',CHAR(0),'b')",
                                "SELECT REPEAT('x',4096)"})
            Reject(
                [&] {
                    run(sql);
                },
                "invalid query or oversized result rejected");

        Reject(
            [&] {
                run("WITH RECURSIVE numbers(x) AS (SELECT 1 UNION ALL SELECT x+1 FROM numbers WHERE x<257) SELECT x FROM numbers");
            },
            "row limit enforced");
        ...
        Reject(
            [&] {
                run("WITH RECURSIVE numbers(x) AS (SELECT 1 UNION ALL SELECT x+1 FROM numbers WHERE x<1000) SELECT CASE WHEN x<=257 THEN x ELSE SLEEP(0.01) END, REPEAT('x',1024) FROM numbers");
            },
            "bounded streaming result rejected");

        Check(std::chrono::steady_clock::now() - streaming < std::chrono::seconds(3),
              "aborted result closes the connection without draining slow remaining rows");

        auto invalid = config;
        invalid.ca = argv[2];
        Reject(
            [&] {
                mysql::Query(invalid, {"SELECT 1", {}}, canceled);
            },
            "untrusted TLS issuer rejected");
        ...
        Reject(
            ...
            "private issuer requires its CA even on loopback");
        ...
        try {
            ...
            if (!hostname_rejected)
                std::cerr << "Unexpected hostname rejection: " << error.what() << '\n';
        }
        ...
        try {
            ...
            hidden = std::string(error.what()).find(config.password) == std::string::npos;
        }
        ...
        Reject(
            [&] {
                mysql::Query(config, insert, canceled);
            },
            "canceled query never executes");
        ...
        Check(run("SELECT count(*) FROM source2root_driver_checks").rows[0][0].integer == 3,
              "multi-statement transaction commits atomically");
        ...
        Check(run("SELECT count(*) FROM source2root_driver_checks").rows[0][0].integer == 3,
              "destroying uncommitted session rolls back");

        {
            ...
            Reject(
                ...
                "query failure poisons transaction");

            Reject(
                [&] {
                    session.Commit();
                },
                "failed session cannot commit earlier writes");
        }

        Check(run("SELECT count(*) FROM source2root_driver_checks").rows[0][0].integer == 3,
              "failed transaction rolls back earlier writes");

        {
            ...
            Reject(
                ...
                "cancel before commit aborts session");

            canceled = false;
        }

        Check(run("SELECT count(*) FROM source2root_driver_checks").rows[0][0].integer == 3,
              "canceled transaction rolled back");
        ...
        Reject(
            [&] {
                mysql::Query(short_timeout, {"SELECT SLEEP(4)", {}}, canceled);
            },
            "finite network read timeout");
