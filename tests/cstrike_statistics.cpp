...
struct Fixture {
    ...
    static KeelResult Player(KeelPluginHandle plugin, const KeelPlayerConnection* player, KeelPlayerInfo* info) {
        ...
        if (player->slot != 2 || player->generation != current->connection)
    ...
    static KeelResult Describe(KeelPluginHandle plugin, KeelEntityHandle entity, KeelEntityInfo* out) {
        ...
        if (plugin != owner || found == current->entities.end() || found->second != current->epoch)
            return KEEL_RESULT_NOT_FOUND;

        *out = {sizeof(*out), 3, 0x12003, 0, current->epoch};
        return KEEL_RESULT_OK;
    }
    ...
    static KeelResult Equal(KeelPluginHandle plugin, KeelEntityHandle a, KeelEntityHandle b, KeelBool* out) {
        ...
        if (Describe(plugin, a, &x) || Describe(plugin, b, &y))
            return KEEL_RESULT_NOT_FOUND;
    ...
    static KeelResult Resolve(KeelPluginHandle plugin, const KeelSchemaFieldSpec* spec, KeelSchemaFieldHandle* out) {
        ...
        if (f.missing_field || std::string(spec->class_name) != "CCSPlayerController" ||
            (name != "m_iScore" && name != "m_iMVPs"))
            return KEEL_RESULT_NOT_FOUND;

        if (spec->value_type != KEELS2_SCHEMA_INT32)
    ...
    static KeelResult DescribeField(KeelPluginHandle plugin, KeelSchemaFieldHandle field, KeelSchemaFieldInfo* out) {
        ...
        if (plugin != owner || found == current->fields.end())
            return KEEL_RESULT_NOT_FOUND;
        ...
        *out = {sizeof(*out),KEELS2_SCHEMA_MODULE_SERVER,KEELS2_SCHEMA_INT32,4,4,16,0,
        ...
        if (current->bad_type)
            out->value_type = KEELS2_SCHEMA_UINT32;

        return KEEL_RESULT_OK;
    }

    static KeelResult
    Read(KeelPluginHandle plugin, KeelEntityHandle entity, KeelSchemaFieldHandle field, void* value, unsigned size) {
        KeelEntityInfo info{};

        if (Describe(plugin, entity, &info))
            return KEEL_RESULT_NOT_FOUND;

        const auto found = current->fields.find(field);

        if (found == current->fields.end() || size != 4)
            return KEEL_RESULT_INCOMPATIBLE;
    ...
    static KeelResult Write(KeelPluginHandle plugin,
    ...
    static inline const KeelEntitiesApi entities_api{
    ...
    std::shared_ptr<source2root::sdktools::Service> Service(bool writable = true) {
        return std::make_shared<source2root::sdktools::Service>(
            owner, entities_api, schema_api, players_api, runtime_api, writable ? &writes_api : nullptr);
    }

    void Empty() const {
        Check(entities.empty() && fields.empty(), "statistics temporary resources released");
    }
};
...
void RunStatisticsChecks() {
    ...
    Check(statistics.Get(player, Statistic::score) == -3 && statistics.Get(player, Statistic::mvps) == 2,
    ...
    for (const auto value : {std::numeric_limits<std::int32_t>::min(),0,std::numeric_limits<std::int32_t>::max()}) {
        ...
        Check(statistics.Get(player, Statistic::score) == value && statistics.Get(player, Statistic::mvps) == 2,
              "score changes only score and preserves signed range");
    }

    for (const auto value : {0,std::numeric_limits<std::int32_t>::max()}) {
        statistics.Set(player, Statistic::mvps, value);
        Check(statistics.Get(player, Statistic::mvps) == value, "MVP boundaries");
    }

    fixture.Empty();
    const auto written = fixture.writes;
    Reject([&] {
        statistics.Set(player, Statistic::mvps, -1);
    });
    Reject([&] {
        statistics.Set(player, static_cast<Statistic>(99), 1);
    });
    Reject([&] {
        statistics.Get(player, static_cast<Statistic>(99));
    });
    fixture.missing_field = true;
    Reject([&] {
        statistics.Get(player, Statistic::score);
    });
    Reject([&] {
        statistics.Set(player, Statistic::score, 0);
    });
    fixture.missing_field = false;
    fixture.bad_type = true;
    Reject([&] {
        statistics.Get(player, Statistic::score);
    });
    fixture.bad_type = false;
    fixture.caps = 0;
    Reject([&] {
        statistics.Set(player, Statistic::score, 0);
    });
    fixture.caps = 1;
    Statistics readonly(fixture.Service(false));
    Check(readonly.Get(player, Statistic::score) == fixture.score, "old host supports statistic reads");
    Reject([&] {
        readonly.Set(player, Statistic::score, 0);
    });
    Reject([&] {
        statistics.Get({2, 0, 20}, Statistic::score);
    });
    fixture.mutation = 1;
    Reject([&] {
        statistics.Set(player, Statistic::score, 0);
    ...
    Check(fixture.writes == written, "invalid statistics calls do not write");
    fixture.Empty();
    fixture.write_status = KEEL_RESULT_ENGINE_FAILURE;
    Reject([&] {
        statistics.Set(player, Statistic::score, 7);
    });
    Check(fixture.score == 7, "failure after mutation does not promise rollback");
    fixture.write_status = KEEL_RESULT_OK;
    const auto before_recursion = fixture.writes;
    fixture.callback = [&] {
        statistics.Set(player, Statistic::score, 8);
    };
    Reject([&] {
        statistics.Set(player, Statistic::score, 8);
    });
    fixture.callback = {};
    Check(fixture.writes == before_recursion + 8, "statistic notification recursion is bounded");
    fixture.Empty();
    ...
    destroyable->Set(player, Statistic::score, 10);
    fixture.callback = {};
    Check(!destroyable && fixture.score == 10, "temporary resources retain service after facade destruction");
    fixture.Empty();
