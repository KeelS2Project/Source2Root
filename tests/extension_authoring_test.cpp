...
KeelResult Release(KeelPluginHandle owner, const char* name, std::uint32_t version) {
    Check(owner == Owner && std::string(name) == SR_EXTENSION_SERVICE && version == 1 &&
        host.natives.empty() && !host.publications, "lease released after provider cleanup");
    if (host.reject_release) return KEEL_RESULT_BUSY;
    Check(host.leases == 1, "extension API lease released exactly once");
...
void Empty() {
    Check(host.natives.empty() && !host.leases && !host.publications && !host.prepare,
