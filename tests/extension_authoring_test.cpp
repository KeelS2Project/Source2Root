...
KeelResult Register(void*, KeelPluginHandle owner, const SrNativeSpec* spec, SrRegistration* id) {
    ...
    *id = 0;
    ...
    if (++host.registrations == host.reject_registration)
...
KeelResult Unregister(void*, KeelPluginHandle owner, SrRegistration id) {
    ...
    if (host.throw_unregister)
        throw std::runtime_error("provider cleanup exception");

    if (host.reject_unregister)
...
SrCallbackApi callback_api{sizeof(SrCallbackApi),
                           1,
                           nullptr,
                           [](void*, KeelPluginHandle owner, SrCallback token) {
                               Check(owner == Owner && token == 9, "persistent callback owner and token");
                               ++host.callback_retains;
                               return KEEL_RESULT_OK;
                           },
                           [](void*,
                              KeelPluginHandle owner,
                              SrCallback token,
                              const SrCallbackArgument*,
                              std::uint32_t count,
                              std::int32_t* result) {
                               Check(owner == Owner && token == 9 && !count && result,
                                     "persistent invocation envelope");

                               *result = 42;
                               return KEEL_RESULT_OK;
                           },
                           [](void*, KeelPluginHandle, SrCallback) {
                               return KEEL_RESULT_OK;
                           }};
...
KeelResult Query(KeelPluginHandle owner, const char* name, std::uint32_t version, const void** result) {
    ...
    *result = nullptr;
    ...
    if (std::string(name) == KEELS2_UNLOAD_SERVICE_NAME)
        *result = &unload;
    else if (std::string(name) == KEELS2_SERVICES_SERVICE_NAME)
        *result = &services;
...
KeelHostApi api{sizeof(api),
                KEELS2_PLUGIN_ABI_VERSION,
                [](KeelPluginHandle, KeelLogLevel, const char* message) {
                    host.logs += std::string(message) + '\n';
                },
                [](KeelPluginHandle, const KeelCommandSpec*, KeelCommandHandle*) {
                    return KEEL_RESULT_UNSUPPORTED;
                },
                [](KeelPluginHandle, KeelCommandHandle) {
                    return KEEL_RESULT_NOT_FOUND;
                },
                Query};
...
public:
    ...
    Probe() : Extension("test.extension") {
        active_probe = this;
    }
...
private:
    ...
    bool PrepareExtensionUnload() override {
        if (host.prepare_throws)
            throw std::runtime_error("prepare rejected");

        return !host.prepare_refuses;
    }

    std::int32_t NoArguments() {
        return 17;
    }
...
int main() {
    try {
        for (int failure = 0; failure < 7; ++failure) {
            ...
            if (failure == 0)
                host.start_refuses = true;

            if (failure == 1)
                host.start_throws = true;

            if (failure == 2)
                host.missing_api = true;

            if (failure == 3)
                host.invalid_api = true;

            if (failure == 4)
                host.reject_publish = true;

            if (failure == 5)
                host.reject_registration = 1;

            if (failure == 6)
                host.reject_registration = 2;

            Check(!KeelPlugin_Load(&api, Owner), "failed setup rolls back before ABI load refusal");
            Empty();

            if (failure < 2)
                Check(!host.registrations, "failed start never publishes staged callbacks");
        }
        ...
        Reset();
        Check(KeelPlugin_Load(&api, Owner), "static extension instance reloads after cleanup");
        Finish();

        for (int mode = 0; mode < 3; ++mode) {
            Reset();
            host.missing_callbacks = mode == 1;
            host.invalid_callbacks = mode == 2;
            Check(KeelPlugin_Load(&api, Owner) && !host.callback_queries,"legacy extension needs no persistent callback service");
            result = 99;
            Check(active_probe->InvokeCallback(9, {}, result) == KEEL_RESULT_NOT_READY && !result,
                  "invocation waits for lazy service acquisition");
