...
void Inputs()
{
    ...
    for (unsigned type = 0; type <= KEELS2_INPUT_ENTITY; ++type) {
        ...
        value.string_value = "typed";
        value.vector_value[1] = 7;
        ...
        if (type == KEELS2_INPUT_COLOR) {
            Reject(
                [&] {
                    entity->Input("Enable", value, invoked, nullptr, nullptr, nullptr, true, 1.25f);
                },
                "queued color refused");
    ...
    const auto invalid = [&](auto action) {
        invoked = true;
        Reject(action, "invalid input rejected");
        Check(!invoked, "invalid input clears invocation");
    };
    invalid([&] {
        entity->Input("", value, invoked);
    });
    invalid([&] {
        entity->Input("bad\nname", value, invoked);
    });
    invalid([&] {
        entity->Input("Enable", value, invoked, nullptr, nullptr, nullptr, false, 1);
    });
    invalid([&] {
        entity->Input("Enable", value, invoked, nullptr, nullptr, nullptr, true, -1);
    });
    invalid([&] {
        entity->Input("Enable", value, invoked, nullptr, nullptr, participant.get());
    });
    value.type = KEELS2_INPUT_ENTITY;
    invalid([&] {
        entity->Input("Enable", value, invoked);
    });
    value.type = KEELS2_INPUT_VOID;
    auto foreign_service = f.ServiceFor(2);
    auto foreign = foreign_service->Find(3);
    invalid([&] {
        entity->Input("Enable", value, invoked, foreign.get());
    });
    foreign.reset();
    foreign_service.reset();
    auto pending = service->Create("prop_dynamic");
    invalid([&] {
        entity->Input("Enable", value, invoked, pending.get());
    });
    invalid([&] {
        pending->Input("Enable", value, invoked);
    });
    pending.reset();
    ++f.epoch;
    invalid([&] {
    ...
    Reject(
        ...
        "invoked input failure propagated");

    Check(invoked, "invocation survives engine failure");
    f.input_result = KEEL_RESULT_OK;
    f.input_invoked = 7;
    Reject(
        [&] {
            entity->Input("Enable", value, invoked);
        },
        "invalid invocation marker rejected");

    Check(invoked, "unknown nonzero invocation is conservatively true");
    f.input_invoked = KEEL_TRUE;
    ...
    f.on_input = [&] {
        ...
        if (depth == 8)
            Reject(
                [&] {
                    entity->Input("Enable", value, nested);
                },
                "input recursion bound");
        else
            entity->Input("Enable", value, nested);
    ...
    f.on_input_caps = [&] {
        ...
        if (depth == 8)
            Reject(
                [&] {
                    service->InputCapabilities();
                },
                "capability recursion bound");
        else
            service->InputCapabilities();
    ...
    invalid([&] {
        entity->Input("Enable", value, invoked, participant.get());
    });
    f.on_describe = {};
    ...
    entity->Input("Enable", value, invoked, participant.get());
    f.on_input = {};
    Check(invoked && weak.expired() && f.entities.empty(),
          "input retains provider until callbacks and then releases all resources");
    ...
    Reject(
        [&] {
            legacy->InputCapabilities();
        },
        "optional input absence supported");

    auto bad = Fixture::input_api;
    bad.dispatch = nullptr;
    Reject(
        [&] {
        ...
        "incomplete optional input table refused");
}
