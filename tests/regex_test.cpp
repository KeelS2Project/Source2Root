...
int main() {
    try {
        Result retained;
        {
            ...
            Check(p.Match("123").Count() == 0 && p.Match("one two", false, 4).Capture(0, 0) == "two",
                  "no match and starting offset");
        }

        Check(retained.Capture(1, 1) == "two", "result survives pattern destruction");
        Reject(
            [&] {
                retained.Capture(2, 0);
            },
            "invalid match index refused");

        Reject(
            [&] {
                retained.Capture(0, 3);
            },
            "invalid group index refused");

        Reject(
            [&] {
                retained.Named(0, "absent");
            },
            "unknown capture name refused");

        Pattern empty("()|(a)");
        const auto empties = empty.Match("a", true);
        Check(empties.Count() == 3 && empties.Capture(0, 1) == "" && !empties.Capture(0, 2) &&
                  empties.Capture(1, 2) == "a" && empties.Offsets(2, 0).start == 1,
              "global empty matches make progress and retain nonempty alternative");

        auto dup = Pattern("(?J)(?<v>a)|(?<v>b)").Match("ab", true);
        Check(dup.Capture(0, dup.Named(0, "v")) == "a" && dup.Capture(1, dup.Named(1, "v")) == "b",
              "duplicate names select participating group");
        ...
        Check(Pattern("a", Anchored).Match("ba").Count() == 0 &&
                  Pattern("a", Anchored).Match("ba", false, 1).Count() == 1,
              "anchoring honors offset");

        Check(Pattern("a$", DollarEndOnly).Match("a\n").Count() == 0 &&
                  Pattern("a.*b", Ungreedy).Match("abxb").Capture(0, 0) == "ab",
              "endonly and ungreedy flags");

        Pattern utf("\\w+", Utf | Ucp);
        Check(utf.Match("é世界").Capture(0, 0) == "é世界", "Unicode properties");
        auto unicode = Pattern("", Utf).Match("é界", true);
        Check(unicode.Count() == 3 && unicode.Offsets(1, 0).start == 2 && unicode.Offsets(2, 0).start == 5,
              "empty UTF matches advance whole characters");

        Reject(
            [&] {
                utf.Match("é", false, 1);
            },
            "UTF continuation offset refused");

        Reject(
            [&] {
                utf.Match(std::string(1, '\xff'));
            },
            "invalid UTF subject refused");

        Reject(
            [&] {
                utf.Replace("é", std::string(1, '\xff'));
            },
            "invalid UTF replacement refused");

        Reject(
            [&] {
                Pattern bad(std::string(1, '\xff'), Utf);
            },
            "invalid UTF pattern refused");

        Reject(
            [&] {
                Pattern("(*UTF)\\C");
            },
            "raw-byte UTF matching refused");

        Reject(
            [&] {
                Pattern("(?C1)a");
            },
            "explicit callout refused");
        ...
        Reject(
            ...
            "out-of-range start refused");

        Reject(
            [&] {
                utf.Match(std::string(4096, 'a'));
            },
            "large subject refused");

        Reject(
            [&] {
                Pattern(std::string(4096, 'a'));
            },
            "large pattern refused");

        Reject(
            [&] {
                Pattern(std::string("a\0b", 3));
            },
            "NUL pattern refused");
        ...
        Reject(
            ...
            "too many captures refused");

        Reject(
            [&] {
                Pattern p(std::string(65, '(') + "a" + std::string(65, ')'));
            },
            "deep compile refused");

        Reject(
            [&] {
                Pattern p("(?:abcdefghij){65535}");
            },
            "expanded compile size/memory refused");

        Pattern one(".");
        Check(one.Match(std::string(256, 'a'), true).Count() == 256, "exact global match cap accepted");
        Reject(
            [&] {
                one.Match(std::string(257, 'a'), true);
            },
            "excessive global matches fail without partial result");
        ...
        Reject(
            [&] {
                one.Replace(std::string(257, 'a'), "", true);
            },
            "replacement callout refuses excessive substitutions");

        Reject(
            [&] {
                replace.Replace("a1", "$unknown");
            },
            "unknown replacement group refused");

        Reject(
            [&] {
                replace.Replace("a1", "$1", false, 0, 1);
            },
            "output overflow refused");

        Reject(
            [&] {
                replace.Replace("a1", std::string(4096, 'a'));
            },
            "large replacement refused");

        Reject(
            [&] {
                replace.Replace("a1", "x", false, 0, 4097);
            },
            "invalid output capacity refused");
        ...
        Reject(
            [&] {
                expensive.Match(std::string(40, 'a') + '!');
            },
            "pathological backtracking must fail within budget");

        Reject(
            [&] {
                expensive.Replace(std::string(40, 'a') + '!', "x", true);
            },
            "replacement matching uses same work limits");
        ...
        const auto limit = Reject(
            [&] {
                Pattern("(*NO_START_OPT)(*NO_AUTO_POSSESS)a+b").Match(std::string(4095, 'a'));
            },
            "unanchored aggregate work bounded");
        ...
        for (unsigned i = 0; i < 4; ++i)
            threads.emplace_back([&] {
                for (unsigned j = 0; j < 50; ++j)
                    if (replace.Match("a1 b2", true).Count() == 2)
                        ++successes;
            });
