...
public:
    ...
    KeelResult Reply(const sr::Player* player, const std::string& text) override {
        ...
        if (player && (!players.contains(slot) || !players.at(slot).SameConnection(*player)))
            return KEEL_RESULT_NOT_FOUND;
...
int main(int argc, char** argv) {
    ...
    std::ofstream(plugin / "plugin.json")
        << R"({"schema":1,"id":"activity","name":"Activity fixture","author":"tests","version":"1.0.0","api":2,"entry":"activity.smx","enabled":true,"dependencies":[]})";
    ...
    auto verify = [&](int mask, int issuer, const std::string& name) {
        ...
        Require(host.logs.size() == 1 && host.logs.back() == "activity: activity delivered",
                "shared API creates no separate successful audit log");
    };
    ...
    for (int mask = 0; mask <= 15; ++mask) {
        ...
        for (auto origin : {sr::Origin::ClientConsole, sr::Origin::PublicChat, sr::Origin::SilentChat}) {
            host.Clear();
            dispatch(origin, 3);
            verify(mask, 3, "Peter Brev");
        }

        host.Clear();
        dispatch(sr::Origin::ServerConsole, -1);
        verify(mask, -1, "Server");
        host.Clear();
        dispatch(sr::Origin::ClientConsole, 5);
        verify(mask, 5, "Recipient");

        if (!(mask & 5))
            Require(host.enumeration_calls == 0, "disabled audience flags do not query recipients");
    }
    ...
    for (int mask = 0; mask <= 15; ++mask) {
        ...
        host.Clear();
        dispatch(sr::Origin::ClientConsole, 3, "override");
        Require(host.Text(3) == "Slapped Kiddo (10 damage); 1 target failed: private detail",
                "custom confirmation reaches issuer once");
    ...
    Require(app.MenuInput(host.players.at(3), app.CurrentMenu(host.players.at(3)), sr::MenuInput::Select),
            "actual SourcePawn menu callback");
    ...
    Require(!app.MenuInput(host.players.at(3), app.CurrentMenu(host.players.at(3)), sr::MenuInput::Select),
            "revoked menu permission prevents announcement");
    ...
    app.SetSetting("sr_show_activity", "13");
    host.Clear();
    dispatch(sr::Origin::ClientConsole, 3, "remember");
    ...
    Require(host.Text(5).empty() && host.Text(4) == "Peter Brev slapped Kiddo (10 damage)",
            "skip recycled recipient while continuing other recipients");
    ...
    app.ReloadPermissions();
    dispatch(sr::Origin::ClientConsole, 3);
    Require(host.Text(4) == anonymous && host.Text(8) == anonymous,
            "recipient classification follows current administrator assignments");
    ...
    Require(host.logs.back().find("Activity announcement failed") != std::string::npos,
            "partial delivery returns failure with a diagnostic");

    host.fail_reply = 3;
    host.Clear();
    dispatch(sr::Origin::ClientConsole, 3);
    Require(host.Text(3).empty() && host.Text(4) == "Peter Brev slapped Kiddo (10 damage)",
            "failed confirmation is attempted once and other recipients continue");

    unsigned attempts = 0;

    for (const auto& message : host.attempts)
        if (message.slot == 3)
            ++attempts;

    Require(attempts == 1 && host.logs.back().find("Activity confirmation failed") != std::string::npos,
            "no automatic retry of a failed confirmation");

    host.fail_reply = -2;
    host.fail_lookup = 4;
    host.Clear();
    dispatch(sr::Origin::ClientConsole, 3);
    Require(host.Text(4).empty() && host.Text(5) == anonymous &&
                host.logs.back().find("recipient lookup failed") != std::string::npos,
    ...
    host.Clear();
    dispatch(sr::Origin::ClientConsole, 3);
    Require(host.messages.size() == 1 && host.Text(3) == confirmation &&
                host.logs.back().find("recipient lookup failed") != std::string::npos,
            "unavailable enumeration still permits the single issuer confirmation");

    host.enumeration = KEEL_RESULT_OK;
    host.invalid_next = true;
    host.Clear();
    dispatch(sr::Origin::ClientConsole, 3);
    Require(host.enumeration_calls == 1 && host.logs.back().find("Invalid activity recipient") != std::string::npos,
            "invalid enumeration cannot loop forever");

    host.invalid_next = false;

    for (const auto& argument : {"invalid", "empty"}) {
        host.Clear();
        dispatch(sr::Origin::ClientConsole, 3, argument);
        Require(host.messages.empty() && host.logs.back().find("single line") != std::string::npos,
                "invalid action text sends nothing without retiring the script");
    }

    host.Clear();
    dispatch(sr::Origin::ClientConsole, 3, "percent");
    Require(host.Text(3) == "Set accuracy to 100% for %s.", "action text is not a format string");
    host.players.at(3).name = "Peter\n\x02";
    host.Clear();
    dispatch(sr::Origin::ClientConsole, 3);
    Require(host.Text(4) == "Peter   slapped Kiddo (10 damage)",
            "actor control bytes cannot inject extra lines or chat colors");

    Require(app.Status().front().state == sr::PluginState::Running, "ordinary activity failures preserve the plugin");
    host.Clear();
    Require(app.Pause("activity"), "pause script");
    dispatch(sr::Origin::ClientConsole, 3);
    Require(host.messages.empty(), "paused command cannot announce");
    Require(app.Resume("activity"), "resume script");
    host.Clear();
    Require(app.Shutdown(), "clean activity shutdown");
    Require(host.messages.empty() && host.logs.front() == "activity: stop activity refused",
            "retiring plugin cannot announce during cleanup");
