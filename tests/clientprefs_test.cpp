...
template <typename Predicate> static void Until(Service& service, Predicate done, unsigned seconds = 5) {
    ...
    while (!done()) {
        ...
        if (std::chrono::steady_clock::now() >= deadline)
            throw std::runtime_error("Timed out waiting for preferences jobs.");
...
static void StorageChecks(const std::filesystem::path& root) {
    const auto path = root / "prefs.sqlite";
    {
        ...
        store.Register(Protected);
        Reject(
            [&] {
                store.Register({Public.name, Public.description, Access::Private});
            },
            "access mode cannot silently change");

        Reject(
            [&] {
                store.Register({"bad/name", "", Access::Public});
            },
            "path-like cookie names rejected");

        Reject(
            [&] {
                store.Register({"bad", "", static_cast<Access>(99)});
            },
            "unknown access rejected");

        Reject(
            [&] {
                store.Register({"bad", std::string(256, 'd'), Access::Public});
            },
            "description bound");
        ...
        Reject(
            ...
            "unauthenticated identity rejected");

        Reject(
            [&] {
                store.Load(First | (2ull << 52));
            },
            "wrong account type rejected");
        ...
        Reject(
            [&] {
                store.Save(First, {{Public.name, {"changed", 10}}, {"zz_missing", {"bad", 10}}});
            },
            "unknown cookie fails batch");

        Check(store.Load(First).at(Public.name).text == text, "failed batch rolls back earlier values");
        Reject(
            [&] {
                store.Save(First, {{Public.name, {std::string(256, 'x'), 10}}});
            },
            "value bound");

        Reject(
            [&] {
                store.Save(First, {{Public.name, {std::string("a\0b", 3), 10}}});
            },
            "embedded NUL rejected");

        for (unsigned i = 3; i < MaxCookies; ++i)
            store.Register({"limit" + std::to_string(i), "", Access::Public});

        Reject(
            [&] {
                store.Register({"too_many", "", Access::Public});
            },
            "catalog bound persists");
    }
    ...
    Reject(
        [&] {
            Store future(root / "future.sqlite");
        },
...
static void Cache(const std::filesystem::path& root) {
    ...
    const Identity first{0, 1, First}, replacement{0, 2, Second}, rejoined{1, 3, First};
    {
        ...
        Reject(
            [&] {
                service.Get(first, music);
            },
            "read before cache/registration completion refused");

        Until(service, [&] {
            return service.Pending() == 0;
        });
        Check(service.Ready() && music->state == State::Ready && service.Status(first) == State::Ready,
              "async catalog, registration and account load");

        Check(service.Register(Public) == music && service.Get(first, music).text.empty(),
              "shared cookie registration and unset value");

        const auto visible = service.UserCookies();
        Check(visible.size() == 2 && visible[0].name == "music" && visible[1].name == "rank",
              "private cookies hidden, protected visible");

        Reject(
            [&] {
                service.UserSet(first, rank->definition.name, "1", 1);
            },
            "player cannot write protected value");

        Reject(
            [&] {
                service.UserSet(first, hidden->definition.name, "1", 1);
            },
            "player cannot write private value");
        ...
        Check(service.Get(first, music).text == "latest" && !service.Persisted(first),
              "cache acknowledgement is not durability acknowledgement");

        Until(service, [&] {
            return service.Persisted(first);
        });
        Check(Store(path).Load(First).at(Public.name).text == "latest",
              "newer edit survives an older in-flight save completion");
        ...
        Reject(
            [&] {
                service.Set(first, music, "during-refresh", 8);
            },
            "writes refused while refresh loads");

        Until(service, [&] {
            return service.Status(first) == State::Ready;
        });
        ...
        Reject(
            ...
            "refresh cannot discard accepted pending writes");
        ...
        Reject(
            [&] {
                service.Sync({first, first});
            },
            "duplicate slot snapshot rejected");
        ...
        Reject(
            ...
            "slot reuse invalidates previous connection");

        Until(service, [&] {
            return service.Status(replacement) == State::Ready;
        });
        ...
        Until(service, [&] {
            return service.Status(rejoined) == State::Ready;
        });
        ...
        Check(!service.ErrorText(rejoined).empty() && !service.Persisted(rejoined),
              "failed database write stays dirty and reports its error");
...

    {
        ...
        Check(service.Ready() && service.Find(Private.name)->definition.access == Access::Private,
              "catalog restored on restart");

        Reject(
            [&] {
                service.Status(first);
            },
            "late load completion does not revive retired connection");
        ...
        Reject(
            [&] {
                service.Get(rejoined, foreign);
            },
            "foreign cookie object refused");

        Reject(
            [&] {
                service.Register({Public.name, Public.description, Access::Private});
            },
            "persisted access metadata cannot be silently downgraded");
...

    {
        ...
        Check(conflicting->state == State::Failed && service.Find(Public.name)->definition == Public &&
                  service.Register(Public)->state == State::Ready,
              "persisted definition survives conflicting early registration and remains usable");
...

    {
        ...
        Reject(
            [&] {
                service.Set(first, cookie, "not accepted", 6);
            },
            "failed cache never acknowledges writes");

        Check(service.CanStop(), "no accepted writes remain after initial failure");
        std::filesystem::remove(root / "not-a-directory");
        service.RetryCatalog();
        service.RetryLoad(first);
...
static void Menus(const std::filesystem::path& root) {
    ...
    Reject(
        [&] {
            ValidatePrefab({rank, PrefabType::YesNo, "Rank", 1});
        },
        "protected cookies have no editable prefab");

    Reject(
        [&] {
            ValidatePrefab({hidden, PrefabType::YesNo, "Hidden", 1});
        },
        "private cookies have no menu prefab");

    Reject(
        [&] {
            ValidatePrefab({music, static_cast<PrefabType>(99), "Bad", 1});
        },
        "unknown prefab rejected");
    ...
    Check(BuildSettings(service, player, staged, 0).rows[0].text.starts_with("Replacement"),
          "latest live staged registration wins");

    replacement.reset();
    Check(BuildSettings(service, player, staged, 0).rows[0].text.starts_with("Music"),
          "failed staged replacement preserves prior registration");
    ...
    for (unsigned i = 0; i < 32; ++i)
        service.Register({"extra" + std::to_string(i), "", Access::Public});
    ...
    for (const auto& row : menu.rows)
        Check(row.text.size() <= 96, "menu text respects renderer limit");
    ...
    Reject(
        [&] {
            BuildSettings(service, player, {}, 0);
        },
        "disconnected identity cannot create settings menu");
}
...
static void Offline(const std::filesystem::path& root) {
    ...
    Reject(
        [&] {
            service.SetIdentity(0, music, "invalid", 3);
        },
        "offline account must be a valid individual Steam ID");

    Reject(
        [&] {
            service.SetIdentity(First, music, "invalid", -1);
        },
        "offline negative timestamp rejected");
    ...
    Until(service, [&] {
        return service.IdentityPersisted(First);
    });
    service.Sync({});
    auto lock = std::make_shared<source2root::sqlite::Database>(path);
    lock->Execute("BEGIN IMMEDIATE");
    ...
    Check(service.IdentityPersisted(Second) && service.IdentityError(Second).empty() &&
              Store(path).Load(Second).at(Public.name).text == "offline-retry",
          "offline retry survives initial load failure and account eviction");
    ...
    for (unsigned i = 0; i < MaxAccounts; ++i)
        bounded.SetIdentity(First + i, bounded.Find(Public.name), "retained", 7);

    Reject(
        [&] {
            bounded.SetIdentity(First + MaxAccounts, bounded.Find(Public.name), "overflow", 7);
        },
        "offline accounts cannot grow beyond bounded retained cache");

    Until(bounded, [&] {
        return bounded.Pending() == 0;
    });
    ...
    Until(
        bounded,
        [&] {
            return bounded.CanStop();
        },
        20);

    Check(Store(path).Load(First).at(Public.name).text == "retained" &&
              Store(path).Load(First + MaxAccounts - 1).at(Public.name).text == "retained",
          "all bounded offline accounts drain after repair");
}
...
int main(int argc, char** argv) {
    try {
        ...
        if (std::string(argv[1]) == "storage")
            StorageChecks(root);
        else if (std::string(argv[1]) == "cache")
            Cache(root);
        else if (std::string(argv[1]) == "menus")
            Menus(root);
        else if (std::string(argv[1]) == "offline")
            Offline(root);
        else
