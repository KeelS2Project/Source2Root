...
struct Command {
    ...
    explicit Command(const std::vector<std::string>& values, const char* original = nullptr) : words(values) {
        ...
        if (words.size() > 64 || std::strlen(input) >= 512)
            throw std::runtime_error("fixture command limit");

        text.AddMultipleToTail(static_cast<int>(std::strlen(input) + 1), input);

        for (auto& value : words)
            arguments.AddToTail(value.data());

        if (words.size() > 1)
            argument_start = static_cast<int>(words[0].size() + 1);
    }
};
