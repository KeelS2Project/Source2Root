...
public:
    ...
    KeelResult RenderMenu(const sr::Player& player, const std::string& html, int duration_ms) override {
        Require(players.contains(player.slot) && player.SameConnection(players.at(player.slot)),
                "render cannot follow reused slot");

        if (html.empty() ? fail_clear : fail_render)
            return KEEL_RESULT_ENGINE_FAILURE;

        ++renders;
        durations[player.slot] = duration_ms;
        Require(html.empty() ? duration_ms == 0 : duration_ms > 0, "render carries a finite lifetime or explicit close");
        menus[player.slot] = html;
        return KEEL_RESULT_OK;
    }
    ...
    KeelResult ReadPlayerInput(const sr::Player& player, KeelPlayerInput& state) override {
        ...
        if (throw_input)
            throw std::runtime_error("input fixture failure");

        if (!players.contains(player.slot) || !players.at(player.slot).SameConnection(player))
            return KEEL_RESULT_NOT_FOUND;
    ...
    KeelResult ReleaseProvider(const std::string&, unsigned) override {
        ...
        Require(leases > 0, "provider lease underflow");
        --leases;
        return KEEL_RESULT_OK;
    }
};
...
int main(int argc, char** argv) {
    ...
    const std::string allowed =
        R"("Admins" { "Tester" { "identity" "[U:1:123]" "group" "root" } "Peer" { "identity" "[U:1:124]" "group" "root" } })";

    std::ofstream(admins) << allowed;
    std::ofstream(root / "configs/admin_groups.cfg") << R"("Groups" { "root" { "immunity" "100" } })";
    std::ofstream(manifest)
        << R"({"schema":1,"id":"input","name":"Input test","author":"tests","version":"1.0.0","api":2,"entry":"main.smx","enabled":true,"dependencies":[]})";
    ...
    for (const auto& [slot, player] : host.players)
        host.input[slot] = {sizeof(KeelPlayerInput), 0, 0, 1};
    ...
    const auto open = [&](int slot = 3) {
        ...
        Require(host.menus.at(slot).find("Forward/Back: move") != std::string::npos &&
                    host.menus.at(slot).find("Use: select") != std::string::npos &&
                    host.menus.at(slot).find("Reload: close") != std::string::npos,
                "menu explains actual action controls");
    };
    const auto press = [&](std::uint64_t button, int slot = 3) {
        host.input[slot].buttons = 0;
        tick();
        host.input[slot].buttons = button;
        tick();
    };
    tick();
    open();
    ...
    now += std::chrono::milliseconds(8688);
    app.Tick(now);
    ...
    now += std::chrono::milliseconds(1279);
    app.Tick(now);
    ...
    host.input[3].buttons = KEELS2_BUTTON_USE;
    open();
    const auto first = app.CurrentMenu(host.players.at(3));
    tick();
    tick();
    ...
    Require(submenu != first && submenu && host.menus.at(3).find("Submenu test") != std::string::npos,
            "key selects real SourcePawn submenu");
    ...
    press(KEELS2_BUTTON_USE);
    Require(!app.CurrentMenu(host.players.at(3)) && host.replies == std::vector<std::string>{"selected=20"},
            "fresh press selects submenu once");
    ...
    host.fail_clear = false;
    tick();
    Require(!app.CurrentMenu(host.players.at(3)) && host.menus.at(3).empty(),
            "clear retry closes without late parent callback");
    ...
    app.Tick(now);
    Require(!app.CurrentMenu(host.players.at(3)) && host.menus.at(3).empty(),
            "submenu timeout closes instead of invoking parent callback");

    host.input[3].buttons = 0;
    open();
    press(KEELS2_BUTTON_USE);
    std::ofstream(admins) << R"("Admins" {})";
    app.ReloadPermissions();
    press(KEELS2_BUTTON_RELOAD);
    Require(!app.CurrentMenu(host.players.at(3)) && host.menus.at(3).empty(),
            "parent callback rechecks current menu permission");
    ...
    press(KEELS2_BUTTON_RELOAD);
    Require(app.CurrentMenu(host.players.at(3)) && host.menus.at(3).find("Page 1/2") != std::string::npos,
            "reload goes back one page");
    ...
    tick();
    Require(host.menus.at(3).find("Submenu test") == std::string::npos, "replacement retains held-key baseline");
    std::ofstream(admins) << R"("Admins" {})";
    app.ReloadPermissions();
    press(KEELS2_BUTTON_USE);
    ...
    open();
    host.on_read = [&] {
        Require(app.Unload("input"), "unload between sample and dispatch");
    };
    host.input[3].buttons = KEELS2_BUTTON_USE;
    tick();
    ...
    host.input[3].buttons = KEELS2_BUTTON_USE;
    tick();
    Require(app.Resume("input"), "resume input script");
    open();
    session = app.CurrentMenu(host.players.at(3));
    tick();
    ...
    tick();
    tick();
    Require(!app.CurrentMenu(old_player) && !app.CurrentMenu(host.players.at(3)) && host.replies.empty(),
            "reused slot cannot select old menu");

    host.input[3].buttons = 0;
    open();
    app.Disconnected(3, host.players.at(3).connection);
    tick();
    ...
    host.fail_clear = false;
    tick();
    Require(host.menus.at(3).find("Submenu test") == std::string::npos,
            "held select after clear failure cannot retry action");
    ...
    tick();
    Require(!app.CurrentMenu(host.players.at(3)) && host.menus.at(3).empty(),
            "failed script update retries cleanup after transport recovery");
    ...
    host.input[3].buttons = 0;
    native_open(3, native);
    const auto peer_session = native_open(4, peer);
    Require(host.leases == 1, "native menus share one provider lease");
    native.callback = [&] {
        Require(app.CloseNativeMenu(50, peer_session) == KEEL_RESULT_OK,
                "native callback closes another sampled session");

        native_open(4, peer);
    };
    host.input[3].buttons = host.input[4].buttons = KEELS2_BUTTON_USE;
    tick();
    Require(native.calls == 1 && !peer.calls && app.CurrentMenu(host.players.at(4)) != peer_session,
            "callback replacement invalidates later input snapshot");

    tick();
    Require(!peer.calls, "replacement native menu cannot reuse held select");
    peer.callback = [&] {
        app.MapChanged();
    };
    ...
    native_open(3, native);
    Require(!app.CurrentMenu(host.players.at(3)) && !host.leases,
            "opening baseline does not retain display erased by map callback");
    ...
    host.fail_clear = false;
    tick();
    Require(!app.CurrentMenu(host.players.at(3)) && host.menus.at(3).empty() && !host.leases,
            "native update failure retries cleanup after transport recovery");

    Require(app.Shutdown(), "clean input shutdown");
    const auto reads = host.input_reads;
    tick();
