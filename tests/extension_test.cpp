...
public:
    ...
    std::vector<PluginDependency> Dependencies() const override { return {{"Source2Root", "1.0.0", DependencyRequirement::exact}}; }
    bool Load() override {
        if (services.Connect(HostContext()) != KEEL_RESULT_OK) return false;
        const void* value = nullptr;
        if (HostContext().QueryService(SR_EXTENSION_SERVICE, 999, &value) != KEEL_RESULT_INCOMPATIBLE || value) return false;
        if (HostContext().QueryService(SR_EXTENSION_SERVICE, 1, &value) != KEEL_RESULT_OK) return false;
        ...
        if (!api || api->size != sizeof(*api) || api->api_version != 1 || !api->register_native || !api->unregister_native)
            return false;
        SrNativeSpec wrong{};
        wrong.size = sizeof(wrong); wrong.api_version = 999;
        SrRegistration rejected = 0;
        if (api->register_native(api->context, HostContext().PluginHandle(), &wrong, &rejected) != KEEL_RESULT_INCOMPATIBLE || rejected)
            return false;
        if (services.Publish("source2root.example", 1, &marker, publication) != KEEL_RESULT_OK) return false;
...
private:
    ...
    static KeelResult Add(void*, const int32_t* arguments, uint32_t count, int32_t* result, char*, uint32_t) noexcept {
        if (count != 2 || !arguments || !result) return KEEL_RESULT_INVALID_ARGUMENT;
        if (during_native) during_native();
        const int64_t sum = static_cast<int64_t>(arguments[0]) + arguments[1];
        if (sum < std::numeric_limits<int32_t>::min() || sum > std::numeric_limits<int32_t>::max()) return KEEL_RESULT_INVALID_ARGUMENT;
...
extern "C" KEELS2_PLUGIN_EXPORT void SrFixtureDuringNative(void (*callback)()) { during_native = callback; }
