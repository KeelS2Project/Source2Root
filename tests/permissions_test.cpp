...
int main(int argc, char** argv) {
    ...
    auto write = [&](const char* name, const std::string& text) { std::ofstream(root / name, std::ios::binary) << text; };
    ...
    Require(access.Allows(actor, "any.plugin.permission") && !access.Allows(actor, "invalid.*"), "root grants every valid named permission");
    ...
    Require(access.Immunity(higher) == 200 && access.Immunity(maximum) == std::numeric_limits<int>::max(), "administrator immunity overrides group default");
    Require(access.Allows(moderator, "admin.kick") && !access.Allows(moderator, "admin.ban"), "named permissions are independent");
    Require(!access.Find(ordinary) && !access.Allows(ordinary, "admin.kick") && access.Allows(ordinary, ""), "display labels cannot authorize users; public permission stays public");
    Require(access.CanTarget(&actor, moderator, "admin.ban") && !access.CanTarget(&moderator, actor, "admin.kick"), "strict higher immunity permits targeting downward only");
    Require(!access.CanTarget(&actor, equal, "admin.kick") && !access.CanTarget(&actor, higher, "admin.kick"), "root cannot bypass equal or higher immunity");
    Require(access.CanTarget(&moderator, moderator, "admin.kick") && !access.CanTarget(&moderator, moderator, "admin.ban"), "self-targeting still requires command permission");
    ...
    Require(!access.CanTargetIdentity(&actor, equal.steam_id, "admin.ban") && access.CanTargetIdentity(&actor, actor.steam_id, "admin.ban") &&
        access.CanTargetIdentity(nullptr, maximum.steam_id, "admin.ban"), "offline identity targeting uses the same immunity and self/server rules");
    auto pending = actor;
    pending.authenticated = false;
    Require(!access.Find(pending) && !access.CanTarget(&pending, moderator, "admin.kick") && !access.CanTarget(&actor, pending, "admin.kick"),
        "unverified identities neither authorize nor bypass target immunity");
    pending.bot = true;
    pending.slot = 12;
    Require(access.CanTarget(&actor, pending, "admin.kick") && !access.Find(pending), "connected bots have zero immunity and no administrator identity");
    ...
    auto rejected = [&](const char* file, const std::string& contents) {
        write(file, contents);
        bool failed = false;
        try { access.Load(root); } catch (const std::exception&) { failed = true; }
        Require(failed && access.Allows(actor, "new.permission") && access.Immunity(higher) == 200, "invalid reload retains the complete previous configuration");
        write("admin_groups.cfg", groups); write("admins.cfg", admins);
    };
    rejected("admins.cfg", R"("Admins" { "Unknown" { "identity" "[U:1:123]" "group" "missing" } })");
    rejected("admins.cfg", R"("Admins" { "A" { "identity" "[U:1:123]" "group" "root" } "B" { "identity" "STEAM_1:1:61" "group" "root" } })");
    ...
    bool failed = false;
    try { access.Load(root); } catch (...) { failed = true; }
    Require(failed && access.Allows(actor, "admin.kick"), "missing half of configuration pair retains privileges on explicit reload");
    write("admin_groups.cfg", groups);
    write("admins.cfg", "Admins {}");
    access.Load(root);
    Require(!access.Find(actor) && !access.CanTarget(&actor, moderator, "admin.kick"), "valid empty administrator file revokes permissions");
    std::filesystem::remove(root / "admin_groups.cfg");
    ...
    std::cout << "KeyValues parsing, identities, groups, root permissions, numeric immunity, self/console exceptions and atomic reload passed.\n";
}
