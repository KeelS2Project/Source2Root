...
public:
    struct Variable {
        sr::ConVarDefinition definition;
    ...
    Variable& ByName(const std::string& name) {
        for (auto& [id, variable] : variables)
            if (variable.definition.name == name)
                return variable;
    ...
    KeelResult CreateConVar(const sr::ConVarDefinition& definition, std::uint64_t& handle) override {
        ...
        for (const auto& [id, variable] : variables)
            if (variable.definition.name == definition.name)
                return KEEL_RESULT_ALREADY_EXISTS;

        const auto found = definitions.find(definition.name);

        if (found != definitions.end() && !(found->second == definition))
    ...
    KeelResult ReleaseConVar(std::uint64_t handle) override {
        ...
        if (found->second.definition.name == busy_name)
...
int main(int argc, char** argv) {
    ...
    auto install = [&](const std::string& id, const std::string& bytecode) {
        ...
        std::filesystem::copy_file(
            scripts / (bytecode + ".smx"), dir / "main.smx", std::filesystem::copy_options::overwrite_existing);
    ...
    auto run = [&](const char* text) {
        Require(app.Dispatch(sr::Origin::ServerConsole, -1, text), text);
    };
    Require(app.Load(manifest) && host.variables.size() == 3 && state("variables").convars == 3,
            "create typed, owned ConVars");

    Require(std::get<std::int32_t>(host.definitions.at("sr_test_unbounded_int").minimum) ==
                    std::numeric_limits<std::int32_t>::min() &&
                std::get<float>(host.definitions.at("sr_test_unbounded_float").maximum) ==
    ...
    Require(host.replies.back() == "typed writes and bounds passed" &&
                std::get<std::string>(host.ByName("sr_test_text").value) == "changed",
            "typed access clamps numeric bounds and writes strings");

    run("sr_variables mismatch");
    Require(host.replies.back() == "type mismatch rejected" &&
                std::get<std::int32_t>(host.ByName("sr_test_count").value) == 10,
            "wrong-type operations clear outputs and preserve engine value");

    Require(!app.Load(install("foreign", "convars")) && host.variables.size() == 3,
            "another script cannot adopt existing definitions");
    ...
    Require(host.log.find("sr_test_count = 10 | variables | paused") <
                    host.log.find("sr_test_fraction = 0 | variables | paused") &&
    ...
    Require(app.Unload("variables") && host.variables.empty(),
            "retry releases remaining command without stale ConVar access");
    ...
    host.during_write = [&] {
        Require(!app.Unload("variables"), "active callback prevents immediate destruction");
    };
    run("sr_variables write");
    Require(state("variables").state == sr::PluginState::Retiring && host.variables.size() == 3,
            "active write retains VM and resources");
    ...
    run("sr_variables");
    Require(host.replies.size() == replies && !state("variables").error.empty(),
            "closed handle cannot read a reused resource");
    ...
    Require(!app.Load(manifest) && host.variables.empty() && state("variables").convars == 0,
            "failed backend registration leaves no owned resource");

    host.fail_create = false;
    Require(app.Retry("variables") && app.Unload("variables"), "backend recovery permits explicit retry");
    Require(!app.Load(install("partial", "convars_failed")) && host.variables.empty(),
            "failed initialization closes its ConVars");
    ...
    std::cout
        << "Script ConVar ownership, typed values, bounds, map lifetime, reload, refusal, retry and greeting example passed.\n";
}
