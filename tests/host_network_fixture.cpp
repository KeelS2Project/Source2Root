...
struct NetworkFixture {
    ...
    std::array<void*, 128> messages_table{}, definition_table{}, packet_table{}, events_table{}, manager_table{},
        event_table{};
...
void Check(bool value) {
    if (!value)
        throw std::runtime_error("native menu network fixture contract");
}
...
void Post(void*, CSplitScreenSlot slot, bool local, int count, const uint64* mask,
    ...
    for (unsigned i = 1; i < (ABSOLUTE_PLAYER_LIMIT + 63) / 64; ++i)
        Check(!mask[i]);
    ...
    const auto duration =
        lifetime.GetReflection()->GetInt32(lifetime, lifetime.GetDescriptor()->FindFieldByName("val_long"));
...
bool Serialize(void*, IGameEvent* event, CNetMessagePB<CMsgSource1LegacyGameEvent>* packet) {
    ...
    auto& output = *network->output;
    Integer(output, "eventid", 413);
    Text(output, "event_name", "show_survival_respawn_status");
    auto* duration = Add(output, "keys");
    Integer(*duration, "type", 3);
    Integer(*duration, "val_long", network->duration);
    auto* token = Add(output, "keys");
    Integer(*token, "type", 1);
    Text(*token, "val_string", network->token.c_str());
    auto* player = Add(output, "keys");
    Integer(*player, "type", 3);
    Integer(*player, "val_long", network->player);
    return true;
}
...
extern "C" bool SrNetworkInitialize(const char* path) {
    try {
        ...
        for (const auto& file : descriptors.file())
            Check(network->pool.BuildFile(file));
...
extern "C" void* SrNetworkInterface(const char* name) {
    ...
    if (std::strcmp(name, NETWORKMESSAGES_INTERFACE_VERSION) == 0)
        return &network->messages;

    if (std::strcmp(name, GAMEEVENTSYSTEM_INTERFACE_VERSION) == 0)
...
extern "C" void* SrNetworkGameEventManager() {
    return network ? &network->manager : nullptr;
}

extern "C" const char* SrNetworkMenuText() {
    return network ? network->html.c_str() : "";
}
