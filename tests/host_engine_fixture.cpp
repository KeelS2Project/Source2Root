...
extern "C" void* SrFixtureEngine(void (*disconnect)(int, unsigned, const char*, void*), void* data) {
    engine.disconnect = [disconnect, data](CPlayerSlot slot, ENetworkDisconnectionReason reason, const char* text) {
        if (reason != NETWORK_DISCONNECT_KICKED) std::abort();
...
extern "C" void SrEngineFailListening(bool fail) { engine.fail_listening = fail; }
extern "C" unsigned SrEngineListeningCalls() { return engine.listening_calls; }

extern "C" unsigned SrEngineMapChanges() { return engine.map_changes; }
extern "C" const char* SrEngineChangedMap() { return engine.changed_map.c_str(); }
