...
public:
    ...
    void Log(const std::string& text) override {
        if (text.find("ASYNC_CALLBACK_OK") != std::string::npos)
            ++completions;
    ...
    static KeelResult Call(void* raw, const SrNativeCall* api, int32_t* result, char* error, uint32_t capacity) noexcept {
        try {
            ...
            *result = 0;
            ...
            if (call.Int(2) == 4) {
                *result = self.register_commands ? 1 : 0;
                return KEEL_RESULT_OK;
            }
            ...
            if (call.Int(2) == 1) {
                ...
                while (api->capture_callback(api->context, 1, &token) == KEEL_RESULT_OK)
                ...
                for (const auto id : captured)
                    Check(self.foundation.CancelCallback(100, id) == KEEL_RESULT_OK, "cancel quota token");
                ...
                if (call.Int(2) == 0)
                    call.Own(1, std::make_unique<Owned>(&self, token));
            }
...
int main(int argc, char** argv) {
    try {
        ...
        sr::Cell value = 42;
        auto deliver = [&](SrCallback token) {
            return foundation.DeliverCallback(100, token, &value, 1, "ok");
        };
        ...
        Check(deliver(stale) == KEEL_RESULT_NOT_FOUND && probe->tokens.back() != stale,
              "stale completion cannot enter replacement VM");
        ...
        Check(refused && foundation.Unload("first") && host.destroyed == destroyed,
              "unload retry does not repeat resource destruction");
