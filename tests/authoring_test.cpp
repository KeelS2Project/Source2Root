...
public:
    ...
    KeelResult Lookup(int slot, sr::Player& result) override {
        if (slot == player.slot)
            result = player;
        else if (peers.contains(slot))
            result = peers.at(slot);
        else
    ...
    KeelResult RenderMenu(const sr::Player& target, const std::string& html, int) override {
        sr::Player current;
        Require(Lookup(target.slot, current) == KEEL_RESULT_OK && target.SameConnection(current),
                "menu must target the current connection");
...
int main(int argc, char** argv) {
    ...
    app.Dispatch(sr::Origin::ClientConsole, 3, "sr_status");
    Require(app.Unload("player_status") && host.menu.empty() && host.commands.empty(),
            "unload closes menu and releases command");
    ...
    std::ofstream(access_dir / "plugin.json")
        << R"({"schema":1,"id":"access","name":"Access fixture","author":"tests","version":"1.0.0","api":2,"entry":"access.smx","enabled":true,"dependencies":[]})";
    ...
    auto access = [&](int slot, const std::string& argument) {
        Require(app.Dispatch(
                    slot < 0 ? sr::Origin::ServerConsole : sr::Origin::ClientConsole, slot, "sr_access " + argument),
                "dispatch access fixture");
    };
    access(3, "group");
    Require(host.replies[host.replies.size() - 2] == "root" && host.replies.back() == "immunity 100",
            "public group and immunity output natives");
    ...
    access(4, "menu");
    Require(app.MenuInput(host.peers.at(4), app.CurrentMenu(host.peers.at(4)), sr::MenuInput::Select) &&
                host.replies.back() == "denied",
    ...
    groups(5);
    app.ReloadPermissions();
    Require(app.MenuInput(host.player, app.CurrentMenu(host.player), sr::MenuInput::Select) && !host.replies.empty() &&
                host.replies.back() == "denied",
    ...
    host.replies.clear();
    Require(app.MenuInput(host.player, app.CurrentMenu(host.player), sr::MenuInput::Select) &&
                host.replies.back() == "denied",
