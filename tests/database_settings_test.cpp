...
int main(int argc, char** argv) {
    try {
        ...
        Reject(
            [&] {
                ReadSettings(file, "local", "stranger");
            },
            "plugin allow list enforced");

        Reject(
            [&] {
                ReadSettings(file, "missing", "example");
            },
            "unknown profile rejected");

        auto mysql = [&](const std::string& fields) {
            write(
                "{\"schema\":1,\"connections\":{\"network\":{\"driver\":\"mysql\",\"database\":\"game\",\"user\":\"game\",\"allow_plugins\":[\"example\"]" +
                fields + "}}}");
        };
        mysql("");
        ...
        for (const auto* field : {",\"host\":\"remote.example\",\"tls\":false", ",\"port\":65536", ",\"timeout\":0",
            ...
            Reject(
                [&] {
                    ReadSettings(file, "network", "example");
                },
        ...
        auto postgresql = [&](const std::string& fields) {
            write(
                "{\"schema\":1,\"connections\":{\"network\":{\"driver\":\"postgresql\",\"database\":\"game\",\"user\":\"game\",\"allow_plugins\":[\"example\"]" +
                fields + "}}}");
        };
        postgresql("");
        ...
        for (const auto* fields : {",\"host\":\"one,two\"", ",\"host\":\"/tmp\"", ",\"socket\":\"/tmp/pgsql\"",
            ...
            Reject(
                ...
                "PostgreSQL route and TLS policy enforced");
        }
        ...
        Reject(
            ...
            "malformed secret configuration reported safely");
        ...
        Reject(
            ...
            "parameter indices start at one");
        ...
        Reject(
            [&] {
                input.Bind(1, std::numeric_limits<double>::infinity());
            },
            "nonfinite values refused");

        Reject(
            [&] {
                input.Bind(1, std::string("a\0b", 3));
            },
            "binary string not silently truncated");

        for (int i = 1; i <= 17; ++i)
            input.Bind(i, std::string(4095, 'a'));

        Reject(
            [&] {
                input.Validate();
            },
            "total parameter payload bounded");
