...
public:
    ...
    void OnGameFrame(bool, bool, bool) override {
        for (const auto& [owner, token] : callbacks_) {
            ...
            if (status == KEEL_RESULT_BUSY && !value)
                LogMessage("PERSISTENT_PAUSED_OK");
            else if (status == KEEL_RESULT_OK && value == 42)
                LogMessage("PERSISTENT_FRAME_OK");
            else
                LogError("PERSISTENT_FAILED frame invocation");
        }
    }
...
private:
    struct Owned {
        ...
        ~Owned() {
            ...
            if (canceled == KEEL_RESULT_NOT_FOUND)
                extension->LogMessage("PERSISTENT_CLEANUP_OK");
            else
                extension->LogError("PERSISTENT_FAILED cleanup ordering");
        }
    };
    ...
    std::int32_t Store(source2root::NativeCall& call) {
        if (callbacks_.contains(call.Owner()))
            throw std::runtime_error("Duplicate callback owner.");
