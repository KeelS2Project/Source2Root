...
int main(int argc, char** argv) {
    ...
    Require(settings.activity == 5 && settings.debug == 1 && settings.public_trigger == "/" && settings.silent_trigger == "!",
    ...
    for (const std::string source : {"sr_show_activity 16", "sr_debug 2", "sr_show_activity 1x", "unknown 1",
        ...
        Write(file, source);
        bool rejected = false;
        try { settings = sr::CoreSettings::Read(file, settings); }
    ...
    try { settings = sr::CoreSettings::Read(file, settings); }
    catch (const std::exception& error) { Require(std::string(error.what()).find("line 2") != std::string::npos, "error identifies configuration line"); }
