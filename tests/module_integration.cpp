...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (argc == 12 && std::string(argv[10]) == "topmenus") {
            ...
            std::ofstream(script / "plugins/top_other/plugin.json")
                << R"({"schema":1,"id":"top_other","name":"Top menu contributor","author":"tests","version":"1.0.0","api":2,"entry":"main.smx","enabled":true,"dependencies":[]})";
        }
        ...
        if (argc == 13 && std::string(argv[10]) == "construction") {
            ...
            std::ofstream(directory / "targets.json")
                << R"({"schema":1,"targets":{"spawn":{"allow_plugins":["hello"],"source":"symbol","module":")"
                << adapter_name
                << R"(","symbol":"SrFixtureConstructSpawn","method":true,"return":"void","arguments":["pointer","pointer"],"sdkhook":{"kind":"spawn","class":"CBaseEntity","block":"preserve_result"}}}})";
        }
        ...
        if (argc == 12 && std::string(argv[10]) == "sdkhooks") {
            ...
            std::filesystem::create_directories(directory);
            std::ofstream(directory / "targets.json")
                << R"({"schema":1,"targets":{"damage":{"allow_plugins":["hello"],"source":"symbol","module":")"
                << adapter_name
                << R"(","symbol":"SrFixtureDamageTarget","method":true,"return":"void","arguments":["pointer","pointer","pointer"],"sdkhook":{"kind":"damage","class":"CCSPlayerPawn","block":"preserve_result"}}}})";
        }
        ...
        if (argc == 12 && (std::string(argv[10]) == "dhooks" || std::string(argv[10]) == "sdkcall")) {
            ...
            std::filesystem::create_directories(directory);
            std::ofstream(directory / "targets.json")
                << R"({"schema":1,"targets":{"scalar":{"allow_calls":true,"allow_plugins":["hello"],"source":"symbol","module":")"
                << adapter_name
                << R"(","symbol":"SrFixtureHookScalar","return":"int32","arguments":["int32","float32"]},"observe_only":{"allow_plugins":["hello"],"source":"symbol","module":")"
                << adapter_name
                << R"(","symbol":"SrFixtureHookScalar","return":"int32","arguments":["int32","float32"]},"buffers":{"allow_calls":true,"allow_plugins":["hello"],"source":"symbol","module":")"
                << adapter_name
                << R"(","symbol":"SrFixtureHookBuffers","return":"int32","arguments":["pointer","uint32","pointer","int32","pointer"],"buffers":[{"argument":1,"kind":"string","capacity":32,"length_argument":2},{"argument":3,"kind":"int32","capacity":8,"length_argument":4},{"argument":5,"kind":"vector3"}]},"entity":{"allow_calls":true,"allow_plugins":["hello"],"source":"symbol","module":")"
                << adapter_name
                << R"(","symbol":"SrFixtureEntityMethod","method":true,"return":"int32","arguments":["pointer","int32","pointer","uint32"],"entities":[{"argument":1,"class":"CCSPlayerController"}],"buffers":[{"argument":3,"kind":"string","capacity":16,"length_argument":4}]},"pawn":{"allow_calls":true,"allow_plugins":["hello"],"source":"symbol","module":")"
                << adapter_name
                << R"(","symbol":"SrFixtureEntityMethod","method":true,"return":"int32","arguments":["pointer","int32","pointer","uint32"],"entities":[{"argument":1,"class":"CCSPlayerPawn"}],"buffers":[{"argument":3,"kind":"string","capacity":16,"length_argument":4}]}}})";
        }
        ...
        if (argc == 12 && std::string(argv[10]) == "database_configured") {
            ...
            if (const auto* config = std::getenv("SR_DATABASE_TEST_CONFIG"); config && *config) {
                ...
                std::ofstream(file)
                    << R"({"schema":1,"connections":{"acceptance":{"driver":"sqlite","database":"shared","allow_plugins":["hello"]}}})";
            }
        }

        std::ofstream(script / "configs/admin_groups.cfg")
            << R"("Groups" { "fixture" { "immunity" "10" "permissions" { "demo.hello" "1" "demo.status" "1" "admin.kick" "1" "admin.changemap" "1" "admin.restart" "1" } } })";
        ...
        if (argc >= 12)
            Check(network_init(argv[11]), "initialize native protocol fixture");
        ...
        auto contains = [&](const char* value) {
            return std::string(messages()).find(value) != std::string::npos;
        };
        ...
        if (argc == 12 && std::string(argv[10]) == "topmenus") {
            ...
            const auto occurrences = [&](const char* text) {
                ...
                while ((offset = log.find(text, offset)) != std::string::npos) {
                    ++total;
            ...
            auto press = [&](std::uint64_t button) {
                ...
                frame();
                input_state(button, 1, KEEL_RESULT_OK);
                frame();
            };
            ...
            press(KEELS2_BUTTON_USE);
            Check(occurrences("TOP_OTHER_SELECTED") == 1 && !*menu_text(),
                  "selection invokes contributor with its own player handle");
            ...
            run("sr_top_permissions");
            run("sr_top_show");
            player_lookup(KEEL_RESULT_ENGINE_FAILURE);
            frame();
            run("sr_top_close");
            frame();
            player_lookup(KEEL_RESULT_OK);
            frame();
            ...
            press(KEELS2_BUTTON_USE);
            run("sr plugins pause top_other");
            frame();
            Check(std::string(menu_text()).find("Contributed action") == std::string::npos &&
                std::string(menu_text()).find("Local action") != std::string::npos, "paused contributor disappears before selection");

            press(KEELS2_BUTTON_USE);
            Check(occurrences("TOP_LOCAL_SELECTED") == 1 && occurrences("TOP_OTHER_SELECTED") == 1,
                  "remaining owner still selects normally");

            run("sr plugins resume top_other");
            run("sr_top_show");
            press(KEELS2_BUTTON_USE);
            run("sr plugins reload top_other");
            frame();
            ...
            run("sr_top_show");
            press(KEELS2_BUTTON_USE);
            run("sr plugins unload top_other");
            frame();
            run("sr_top_hide");
            press(KEELS2_BUTTON_USE);
            ...
            run("sr_top_mutate");
            frame();
            Check(occurrences("TOP_MUTATION_REJECTED") == 1 && !*menu_text(),
                  "access callback mutation rejects display atomically");

            run("sr_top_show");
            press(KEELS2_BUTTON_USE);
            run("sr_top_selfclose");
            press(KEELS2_BUTTON_USE);
            ...
            run("sr plugins load top_other");
            run("sr_top_fault");
            run("sr_top_show");
            frame();
            Check(!*menu_text(), "script fault retires contribution and discards an invalidated display");
            run("sr_top_show");
            Check(std::string(menu_text()).find("Player actions") == std::string::npos,
                  "faulted callback cannot leave an active category");
        ...
        if (argc == 12 && std::string(argv[10]) == "persistent") {
            const auto occurrences = [&](const char* value) {
                ...
                while ((offset = log.find(value, offset)) != std::string::npos) {
                    ++found;
                    offset += std::strlen(value);
                }

                return found;
            };
            Check(occurrences("PERSISTENT_STORED_OK") == 1,"persistent service acquired at script startup");
            run("sr_persistent_module");
            frame();
            frame();
            ...
            Check(occurrences("PERSISTENT_CLEANUP_OK") == 3 && !contains("PERSISTENT_FAILED"),
                  "persistent lifecycle has no failures");
        ...
        if (argc == 12 && std::string(argv[10]) == "sdkhooks") {
            ...
            Check(damage() == 12 && contains("SDKHOOKS_PRE_OK") && contains("SDKHOOKS_POST_OK"),
                  "typed edit crosses actual native detour and SourcePawn callback");

            run("sr plugins pause hello");
            Check(damage() == 42, "paused script leaves original damage");
            run("sr plugins resume hello");
            run("sr_sdkhook_continue");
            Check(damage() == 42, "continue discards edits");
            run("sr_sdkhook_block");
            Check(damage() == -1, "block preserves output and skips original");
            run("sr_sdkhook_close");
            Check(damage() == 12, "active callback can close own registration and finish");
            frame();
            Check(damage() == 42,"closed registration becomes inert");
            run("sr_sdkhook_stale");
            Check(contains("SDKHooks frame is stale"), "stored frame cannot escape callback");
            run("sr plugins reload hello");
            frame();
            Check(damage() == 12, "reload creates fresh hook");
            run("sr_sdkhook_fault");
            Check(damage() == 42, "script fault discards staged damage");
            frame();
            Check(damage() == 42, "fault retires callback");
            run("sr plugins reload hello");
            frame();
            epoch();
            Check(damage() == 42, "old map identity never retargets");
            run("sr plugins reload hello");
            frame();
            Check(damage() == 12, "new registration captures current epoch");
            run("keel plugins unload 2");
            Check(contains("plugin unload is blocked"), "script retains hook provider");
            run("sr plugins unload hello");
            frame();
            Check(damage() == 42, "script unload restores original");
            run("keel plugins unload 2");
            run("keel plugins load sr_example");
            run("sr plugins load hello");
            frame();
            Check(damage() == 12, "provider reload reacquires typed native services");
            run("sr plugins unload hello");
            frame();
            Check(!contains("SDKHOOKS_FAILED"), "script checks succeed");
            Check(stop(), "SDKHooks host shutdown releases leases");
            Check(network_stop(), "SDKHooks fixture teardown");
        ...
        if (argc == 12 && std::string(argv[10]) == "sdkcall") {
            ...
            Check(contains("SDKCALL_BUFFERS_OK") && contains("SDKCALL_BUFFER_CLOSE_OK") &&
                      contains("SDKCALL_BUFFER_HOOK_BOUNDS_OK"),
            ...
            reconnect();
            run("sr_sdkcall_entity_reconnect");
            Check(contains("SDKCALL_ENTITY_RECONNECT_OK") && entity_calls() == 4,
                  "player reconnect refuses old call and supports fresh binding");

            entity_epoch();
            run("sr_sdkcall_entity_epoch");
            ...
            run("keel plugins unload 2");
            Check(contains("plugin unload is blocked"), "prepared call retains provider");
            run("sr plugins pause hello");
            Check(scalar(3, 2) == 8, "paused script hook bypasses callback");
            run("sr plugins resume hello");
            run("sr_sdkcall_close");
            Check(contains("SDKCALL_CLOSE_OK") && original() == 24, "call and hook can close inside callback");
            frame();
            Check(scalar(3, 2) == 8, "deferred cleanup restores original");
            run("sr_sdkcall_stale");
            Check(contains("stale, foreign or wrong-type handle"), "closed call cannot be reused");
            run("sr plugins reload hello");
            frame();
            const auto before = calls();
            run("sr_sdkcall");
            Check(calls() == before + 2, "new script generation owns fresh calls");
            run("sr_sdkcall_entities");
            Check(entity_calls() == 9, "reloaded script obtains fresh entity leases");
            run("sr plugins unload hello");
            frame();
            Check(scalar(3, 2) == 8, "script cleanup releases hook and prepared call");
            run("keel plugins unload 2");
            run("keel plugins load sr_example");
            run("sr plugins load hello");
            frame();
            const auto reloaded = calls();
            run("sr_sdkcall");
            Check(calls() == reloaded + 2, "provider reload reacquires optional direct-call service");
            run("sr plugins unload hello");
            frame();
            Check(!contains("SDKCALL_FAILED"),"SDKCall script reports no failure");
            Check(stop(), "SDKCall host stops after owned resource cleanup");
            Check(network_stop(), "SDKCall fixture teardown");
        ...
        if (argc == 12 && std::string(argv[10]) == "dhooks") {
            ...
            run("sr_dhook_disable");
            Check(scalar(3, 2) == 8, "disabled hook bypasses script");
            run("sr_dhook_enable");
            run("sr plugins pause hello");
            Check(scalar(3,2) == 8,"paused script cannot change native call");
            run("sr plugins resume hello");
            Check(scalar(3, 2) == 90, "resume reuses hook callback");
            const auto pre = occurrences("DHOOKS_PRE_OK");
            std::int32_t worker_result = 0;
            std::thread worker([&] {
                worker_result = scalar(3, 2);
            });
            worker.join();
            Check(worker_result == 8 && occurrences("DHOOKS_PRE_OK") == pre,"engine worker does not enter SourcePawn");
            run("keel plugins unload 2");
            Check(contains("plugin unload is blocked"), "script keeps detour provider loaded");
            run("sr plugins reload hello");
            frame();
            ...
            run("sr_dhook_mode");
            const auto originals = calls();
            Check(scalar(3,2) == 70 && calls() == originals,"pre supercede prevents original execution");
            run("sr_dhook_mode");
            Check(scalar(3, 2) == 60 && calls() == originals && contains("DHOOKS_SELF_CLOSE_OK"),
                  "self-removal retains callback until return");

            frame();
            Check(scalar(3, 2) == 8, "last hook removal restores original");
            run("sr_dhook_stale");
            Check(contains("Hook frame is stale, inactive or belongs to another script."),
                  "saved frame expires after callback");
            ...
            run("sr_dhook_faulted");
            Check(contains("DHOOKS_FAULT_RETIRED_OK"), "fault invalidates token and hook");
            frame();
            run("sr plugins reload hello");
            frame();
            ...
            run("sr plugins unload hello");
            frame();
            Check(scalar(3, 2) == 8, "unload restores original before provider removal");
            run("keel plugins unload 2");
            run("keel plugins load sr_example");
            run("sr plugins load hello");
            frame();
            Check(scalar(3, 2) == 90 && occurrences("DHOOKS_READY") == 5,
                  "native provider reload reacquires hook and callback services");

            run("sr plugins unload hello");
            frame();
            Check(stop(), "DHooks host stops after deferred hook cleanup");
            Check(network_stop(), "DHooks fixture teardown");
        ...
        if (argc == 12 && std::string(argv[10]) == "http") {
            ...
            auto await = [&](auto ready) {
                ...
                while (!ready()) {
                    if (std::chrono::steady_clock::now() >= deadline)
                        throw std::runtime_error("HTTP fixture completion timeout");
            ...
            await([&] {
                frame();
                return contains("HTTP_NATIVE_OK") || contains("HTTP_NATIVE_FAILED");
            });
            ...
            Check(!contains("HTTP_SCRIPT_HELLO") && !contains("HTTP_SCRIPT_BINARY") && !contains("HTTP_NATIVE_FAILED"),
                  "script callbacks wait while paused; native service works on worker");

            run("sr plugins resume hello");
            auto completed = [&](unsigned count) {
                frame();
                return occurrences("HTTP_SCRIPT_HELLO") == count && occurrences("HTTP_SCRIPT_BINARY") == count &&
                       occurrences("HTTP_SCRIPT_FORM") == count && occurrences("HTTP_SCRIPT_TLS") == count &&
                       occurrences("HTTP_SCRIPT_ERROR") == count;
            };
            await([&] {
                return completed(1);
            });
            run("sr_http_check");
            run("sr_http_stale");
            Check(contains("stale, foreign or wrong-type handle") && !contains("HTTP_SCRIPT_FAILED"),
                  "HTTP retained response and stale handle rejection");
            ...
            run("sr plugins unload hello");
            run("keel plugins unload 2");
            Check(contains("plugin unload is blocked") && contains("HTTP Native Fixture"),
                  "native consumer service lease retains HTTP provider");
            ...
            Check(!contains("HTTP_SCRIPT_FAILED") && !contains("HTTP_NATIVE_FAILED"),
                  "HTTP close/unload cancels outstanding callbacks and drains workers");
        ...
        if (argc == 12 && std::string(argv[10]) == "cstrike") {
            auto occurrences = [&](const char* value) {
                ...
                while ((at = log.find(value, at)) != std::string::npos) {
                    ++count;
                    at += std::strlen(value);
                }
            ...
            const auto stat_state =
                adapter.Get<void (*)(unsigned, unsigned, KeelResult, KeelResult, KeelResult, unsigned)>(
                    "SrFixtureStatisticsState");

            Check(stat_writes() == 0,"initialization does not change component statistics");
            run("sr_cs_components");
            Check(occurrences("CSTRIKE_COMPONENTS_OK") == 1 && stat_writes() == 8,
                  "compiled script component mappings and readback");

            stat_state(15, 0, KEEL_RESULT_OK, KEEL_RESULT_OK, KEEL_RESULT_OK, 0);
            run("sr_cs_components_readonly");
            Check(contains("CSTRIKE_COMPONENTS_READONLY_OK") && stat_writes() == 8,
                  "read-only component capabilities block every setter");

            stat_state(15, 15, KEEL_RESULT_ENGINE_FAILURE, KEEL_RESULT_OK, KEEL_RESULT_OK, 0);
            run("sr_cs_components_caps_error");
            Check(contains("CSTRIKE_COMPONENTS_CAPS_ERROR_OK") && stat_writes() == 8,
                  "component capability failure clears both script outputs");

            stat_state(15, 15, KEEL_RESULT_OK, KEEL_RESULT_ENGINE_FAILURE, KEEL_RESULT_OK, 0);
            run("sr_cs_components_read_error");
            Check(occurrences("CSTRIKE_COMPONENTS_READ_ERROR_OK") == 1,"component read failure suppresses partial host result");
            stat_state(15, 15, KEEL_RESULT_OK, KEEL_RESULT_OK, KEEL_RESULT_OK, 1);
            run("sr_cs_components_read_error");
            Check(occurrences("CSTRIKE_COMPONENTS_READ_ERROR_OK") == 2,"component read revalidates map epoch");
            stat_state(15, 15, KEEL_RESULT_OK, KEEL_RESULT_OK, KEEL_RESULT_ENGINE_FAILURE, 0);
            run("sr_cs_components_write_error");
            Check(contains("CSTRIKE_COMPONENTS_WRITE_ERROR_OK") && stat_writes() == 9,
                  "component error after mutation is visible to script");

            stat_state(15, 15, KEEL_RESULT_OK, KEEL_RESULT_OK, KEEL_RESULT_OK, 0);
            run("sr_cs_components_restore");
            Check(contains("CSTRIKE_COMPONENTS_RESTORE_OK") && stat_writes() == 10,
                  "component setter recovers after notification failure");
            ...
            round_state(0, KEEL_RESULT_OK, KEEL_RESULT_OK);
            run("sr_cs_round_unavailable");
            Check(contains("CSTRIKE_ROUND_UNAVAILABLE_OK") && round_count(nullptr) == 3,"unsupported round cannot dispatch");
            round_state(1, KEEL_RESULT_UNSUPPORTED, KEEL_RESULT_OK);
            run("sr_cs_round_caps_error");
            Check(contains("CSTRIKE_ROUND_CAPS_ERROR_OK") && round_count(nullptr) == 3,"round capability error clears output");
            round_state(1, KEEL_RESULT_OK, KEEL_RESULT_NOT_READY);
            run("sr_cs_round_error");
            ...
            Check(occurrences("CSTRIKE_SCRIPT_OK") == 1 && action_count(nullptr) == 0,
                  "Counter-Strike initialization has no player actions");

            Check(write_count() == 0, "Counter-Strike initialization has no statistic writes");
            run("sr_cs_stats");
            Check(occurrences("CSTRIKE_STATS_OK") == 1 && write_count() == 4,
                  "score/MVP mapping and readback through actual script");

            write_state(0, KEEL_RESULT_OK, false);
            run("sr_cs_stats_unavailable");
            Check(contains("CSTRIKE_STATS_UNAVAILABLE_OK") && write_count() == 4, "unavailable writer preserves statistic reads");
            write_state(1, KEEL_RESULT_ENGINE_FAILURE, false);
            run("sr_cs_stats_error");
            ...
            run("sr_cs_invalid");
            Check(contains("CSTRIKE_INVALID_OK") && action_count(nullptr) == 3 && write_count() == 4,
                  "invalid team/player/statistic cannot dispatch");
            ...
            Check(contains("CSTRIKE_CAPS_ERROR_OK") && action_count(nullptr) == 3,
                  "capability failure clears output and records error");
            ...
            reconnect();
            run("sr_cs_reconnected");
            Check(contains("CSTRIKE_RECONNECTED_OK") && action_count(nullptr) == 3 && write_count() == 4,
                  "saved player cannot target replacement connection");
            ...
            Check(action_count(nullptr) == 6 && occurrences("CSTRIKE_ACTIONS_OK") == 2,
                  "new player handle dispatches after reconnect");

            run("keel plugins unload 2");
            Check(contains("plugin unload is blocked"), "script retains Counter-Strike provider");
            run("sr plugins pause hello");
            run("sr_cs_check");
            run("sr_cs_stats");
            run("sr_cs_round");
            ...
            Check(stat_writes() == 10 && occurrences("CSTRIKE_COMPONENTS_OK") == 1,
                  "paused script cannot change component statistics");
            ...
            Check(occurrences("CSTRIKE_SCRIPT_OK") == 3 && action_count(nullptr) == 12 && !contains("CSTRIKE_FAILED"),
                  "provider reload rebinds actions");

            Check(occurrences("CSTRIKE_STATS_OK") == 3 && write_count() == 12, "provider reload rebinds statistics");
            run("sr plugins unload hello");
            Check(stop(), "Counter-Strike host stop");
            Check(network_stop(), "Counter-Strike fixture teardown");
        ...
        if (argc == 13 && std::string(argv[10]) == "construction") {
            ...
            run("sr_construct_block");
            run("sr_construct_close");
            run("sr_construct_close_hook");
            Check(occurrences("CONSTRUCTION_CASE_OK") == 4 && !tally(4) && tally(3),
                  "pending pre/live post, block and callback closure");

            mode(2);
            run("sr_construct_error");
            mode(0);
            Check(occurrences("CONSTRUCTION_CASE_OK") == 5 && !tally(4),"invoked engine failure consumes creation");
            mode(1);
            run("sr_construct_retry");
            Check(occurrences("CONSTRUCTION_RETRY_OK") == 1 && tally(4) == 1,"noninvoked failure retains pending ownership");
            mode(0);
            run("sr_construct_finish");
            Check(occurrences("CONSTRUCTION_FINISH_OK") == 1 && !tally(4) && tally(3),
                  "retry preserves copied staged values and teleport");

            mode(3);
            run("sr_construct_normal");
            mode(0);
            Check(occurrences("CONSTRUCTION_CASE_OK") == 6 && contains("active native operation"),
                  "construction call retains SDKTools module during unload attempt");

            mode(6);
            run("sr_construct_cancel");
            ...
            Check(tally(4) == 1, "pending retained for map test");
            epoch();
            run("sr_construct_stale");
            Check(occurrences("CONSTRUCTION_STALE_OK") == 1 && !tally(4),
                  "map epoch rejects pending and observed identities");

            run("sr_construct_leave");
            const auto before_reload = tally(1);
            run("sr plugins reload hello");
            frame();
            Check(occurrences("CONSTRUCTION_SCRIPT_READY") == 2 && tally(1) == before_reload + 1 && !tally(4),
                  "script reload cancels old generation pending ownership");

            run("sr_construct_leave");
            const auto before_unload = tally(1);
            run("sr plugins unload hello");
            frame();
            ...
            run("sr plugins load hello");
            run("sr_construct_normal");
            Check(occurrences("CONSTRUCTION_SCRIPT_READY") == 3 && occurrences("CONSTRUCTION_CASE_OK") == 7,
                  "provider reload rebinds construction service");
        ...
        if (argc == 12 && std::string(argv[10]) == "entity_input") {
            ...
            run("sr_input_invalid");
            Check(contains("INPUT_INVALID_OK") && tally(0) == 17, "invalid payloads/participants never dispatch");
            run("sr_input_bad_payload");
            run("sr_input_refusal");
            run("sr_input_bad_participant");
            run("sr_input_refusal");
            Check(occurrences("INPUT_REFUSAL_OK") == 2 && tally(0) == 17 && contains("invalid handle"),
                  "invalid script handles raise native errors after clearing invocation output");

            mode(1);
            run("sr_input_retry");
            mode(2);
            run("sr_input_error");
            Check(contains("INPUT_RETRY_OK") && contains("INPUT_ERROR_OK") && tally(0) == 18,
                  "invocation output distinguishes failure before/after engine entry");

            mode(3);
            run("sr_input_close");
            Check(contains("INPUT_CLOSE_OK") && tally(0) == 19 && tally(3),
                  "callback closes participants and mutates script payload safely");

            mode(4);
            run("sr_input_all");
            Check(occurrences("INPUT_ALL_OK") == 2 && contains("active native operation") && tally(0) == 36,
                  "input callback cannot unload provider");

            mode(5);
            run("sr_input_recursion");
            Check(contains("INPUT_RECURSION_OK") && tally(0) == 44, "compiled script input recursion bounded to eight");
            mode(0);
            run("sr_input_leave");
            epoch();
            run("sr_input_stale");
            ...
            frame();
            run("sr_input_all");
            Check(occurrences("INPUT_SCRIPT_READY") == 2 && occurrences("INPUT_ALL_OK") == 3,
                  "script reload rebinds typed input natives");
            ...
            run("sr plugins load hello");
            run("sr_input_all");
            Check(occurrences("INPUT_SCRIPT_READY") == 3 && occurrences("INPUT_ALL_OK") == 4 && tally(3),
                  "provider reload reacquires input service");

            run("sr_input_leave");
            run("sr plugins unload hello");
            frame();
            Check(stop(), "input host shutdown");
            Check(network_stop(), "input fixture teardown");
        ...
        if (argc == 12 && std::string(argv[10]) == "sdktools") {
            ...
            write_state(0, KEEL_RESULT_OK, false);
            run("sr_sdk_write_unavailable");
            Check(contains("SDKTOOLS_WRITE_UNAVAILABLE_OK") && write_count() == 9,"missing capability prevents write");
            write_state(1, KEEL_RESULT_ENGINE_FAILURE, false);
            run("sr_sdk_write_error");
            Check(contains("SDKTOOLS_WRITE_ERROR_OK") && write_count() == 9,"write failure reaches script");
            write_state(1, KEEL_RESULT_OK, true);
            run("sr_sdk_write_callback");
            Check(contains("SDKTOOLS_WRITE_CALLBACK_OK") && write_count() == 11,
                  "nested callback closes active Entity/Field without invalid access");
            ...
            restore_entities();
            tool_state(0, KEEL_RESULT_OK, KEEL_RESULT_OK, 0);
            run("sr_sdk_tool_unavailable");
            Check(contains("SDKTOOLS_TOOL_UNAVAILABLE_OK") && tool_count() == 4,"unsupported operations have no game calls");
            tool_state(7, KEEL_RESULT_ENGINE_FAILURE, KEEL_RESULT_OK, 0);
            run("sr_sdk_tool_cap_error");
            Check(contains("SDKTOOLS_TOOL_CAP_ERROR_OK") && tool_count() == 4,"capability errors zero script output");
            tool_state(7, KEEL_RESULT_OK, KEEL_RESULT_ENGINE_FAILURE, 0);
            run("sr_sdk_tool_error");
            ...
            restore_entities();
            tool_state(7, KEEL_RESULT_OK, KEEL_RESULT_ENGINE_FAILURE, 1);
            run("sr_sdk_tool_callback_error");
            Check(occurrences("SDKTOOLS_TOOL_CALLBACK_OK") == 4 && tool_count() == 9,
                  "engine error after callback closes the active resource");

            tool_state(7, KEEL_RESULT_OK, KEEL_RESULT_OK, 3);
            run("sr_sdk_tool_cap_close");
            Check(occurrences("SDKTOOLS_TOOL_CALLBACK_OK") == 5 && tool_count() == 9,
                  "close during capability lookup prevents game call");

            tool_state(7, KEEL_RESULT_OK, KEEL_RESULT_OK, 2);
            run("sr_sdk_tool_recursion");
            ...
            Check(occurrences("SDKTOOLS_SCRIPT_OK") == 2 && !contains("SDKTOOLS_FAILED"),
                  "script reload succeeds with new generation and releases old resources");
            ...
            Check(occurrences("SDKTOOLS_SCRIPT_OK") == 3 && !contains("SDKTOOLS_FAILED"),
                  "SDKTools provider reload rebinds script natives");
        ...
        if (argc == 12 && std::string(argv[10]) == "regex") {
            ...
            run("sr_regex_stale");
            Check(contains("stale, foreign or wrong-type handle") && !contains("REGEX_FAILED"),
                  "stale regex handle raises native error");
            ...
            run("sr plugins load hello");
            run("sr_regex_check");
            Check(occurrences("REGEX_SCRIPT_OK") == 3 && !contains("REGEX_FAILED"),
                  "regex extension reload rebinds native functions");
        ...
        if (argc == 12 && std::string(argv[10]) == "geoip") {
            ...
            await([&] {
                frame();
                return contains("GEO_SCRIPT_OK") && contains("GEO_MISSING_OK");
            });
            run("keel plugins unload 2");
        ...
        if (argc == 12 && (std::string(argv[10]) == "clientprefs" || std::string(argv[10]) == "clientprefs_mysql")) {
            ...
            await([&] {
                frame();
                return occurrences("PREFS_SCRIPT_OK") == 1;
            });

            if (std::string(argv[10]) == "clientprefs_mysql")
                await([&] {
                    frame();
                    return occurrences("PREFS_NETWORK_OK") >= 1;
                });
            ...
            await([&] {
                frame();
                return occurrences("PREFS_OFFLINE_SAVED") == 1;
            });
            ...
            await([&] {
                frame();
                return occurrences("PREFS_SCRIPT_OK") == 2;
            });
            run("keel plugins unload 2");
            ...
            for (int type = 0; type < 4; ++type)
                for (int enabled = 1; enabled >= 0; --enabled) {
                    ...
                    Check(root_menu.find("Music setting") != std::string::npos &&
                              root_menu.find("[read only]") != std::string::npos &&
                              root_menu.find("internal") == std::string::npos &&
                              root_menu.find("script-private") == std::string::npos,
                    ...
                    Check(std::string(menu_text()).find(type < 2 ? "Yes" : "On") != std::string::npos &&
                              std::string(menu_text()).find("Back to settings") != std::string::npos,
                          "prefab opens correct choice submenu");

                    if (!enabled)
                        press(KEELS2_BUTTON_BACK);

                    press(KEELS2_BUTTON_USE);
                    Check(std::string(menu_text()).find("Player preferences") != std::string::npos,
                          "selection returns to shared menu");
                    ...
                    await([&] {
                        frame();
                        return occurrences("PREFS_MENU_SAVED") == menu_saved;
                    });

                    if (std::string(argv[10]) == "clientprefs_mysql")
                        await([&] {
                            frame();
                            return occurrences("PREFS_NETWORK_OK") > network_before;
                        });

                    const auto expected = type == 0   ? (enabled ? "yes" : "no")
                                          : type == 2 ? (enabled ? "on" : "off")
                                                      : (enabled ? "1" : "0");

                    Check(contains(("PREFS_MENU_VALUE_" + std::to_string(type) + ":" + expected).c_str()),
                          "selected prefab value is committed and read by compiled script");
                }
            ...
            press(KEELS2_BUTTON_USE);
            Check(std::string(menu_text()).find("Player preferences") != std::string::npos,
                  "explicit submenu Back returns without changing value");
            ...
            await([&] {
                frame();
                return occurrences("PREFS_SCRIPT_OK") == 3;
            });
            run("sr_prefs_pending");
            run("sr plugins reload hello");
            await([&] {
                frame();
                return occurrences("PREFS_SCRIPT_OK") == 4;
            });

            if (std::string(argv[10]) == "clientprefs_mysql")
                await([&] {
                    frame();
                    return occurrences("PREFS_NETWORK_OK") >= 9;
                });
            ...
            Check(!contains("PREFS_FAILED") && !contains("PREFS_UNEXPECTED_CALLBACK"),
                  "preferences ownership, cache and save callbacks isolated by script generation");
            ...
            Check(values.at("music").text == "on-unload",
                  "accepted preference write committed after script unload before host stop");
        ...
        if (argc == 12 && (std::string(argv[10]) == "database_async" || std::string(argv[10]) == "database_configured")) {
            ...
            await([&] {
                frame();
                return occurrences("SQL_ASYNC_OK") == 1 && occurrences("SQL_ASYNC_ERROR_OK") == 1;
            });
            run("keel plugins unload 2");
            ...
            await([&] {
                frame();
                return occurrences("SQL_ASYNC_OK") == 2 && occurrences("SQL_ASYNC_ERROR_OK") == 2;
            });
        ...
        run("sr");
        run("sr help plugins");
        Check(contains("Source2Root Menu:") && contains("Usage: sr plugins <command>"),
              "approved management menus are real commands");

        run("sr plugins list");
        run("sr plugins cmds hello");
        Check(contains("sr_hello | hello | running | permission: demo.hello"),
              "script command inventory has owner and required permission");
        ...
        Check(extension_list.find("sr_example" + extension) != std::string::npos &&
                  extension_list.find("source2root" + extension) == std::string::npos,
              "extension inventory excludes unrelated native framework plugins");

        run("sr config");
        Check(cvars() == 4 && contains("sr_show_activity = \"5\""),
              "core settings read specified cfg path and register real KeelS2 ConVars");
        ...
        Check(set_cvar("sr_show_activity", "9") && set_cvar("sr_chat_public_trigger", "/") &&
                  set_cvar("sr_chat_silent_trigger", "!"),
        ...
        run("keel plugins pause 1");
        Check(set_cvar("sr_show_activity", "1") && set_cvar("sr_chat_public_trigger", "??") &&
                  set_cvar("sr_chat_silent_trigger", "??"),
        ...
        run("sr plugins load hello");
        run("sr version");
        Check(contains("Source2Root Version 1.0.0") && contains("Revision:") && contains("Script API: 2"),
              "version reports product, build and API metadata");
        ...
        for (int i = 0; i < 140; ++i) {
            ...
            run("sr_who");
            Check(std::string(messages()).substr(start).find("#" + std::to_string(71 + i) + " Module fixture player") !=
                      std::string::npos,
        ...
        run("sr_slay [U:1:123]");
        Check(actions(&last) == 2 && last.kind == KEELS2_PLAYER_ACTION_KILL,
              "SourcePawn slay reaches actual Keel kill service");
        ...
        Check(kicks() == 1 && std::string(kick_reason()) == "Native reason",
              "public kick reaches typed engine disconnect with full reason");

        kick_state(true, false, true);
        run("sr_kick [U:1:123]");
        Check(kicks() == 1 && contains("Player is no longer available."),
              "connection changed while acquiring engine interface cannot be kicked");

        kick_state(false, false, false);
        run("sr_kick [U:1:123]");
        Check(kicks() == 1 && contains("Player disconnect is unavailable"),
              "missing engine interface is a useful failed action");
        ...
        Check(kicks() == 2 && std::string(kick_reason()) == "Kicked Module fixture player" &&
                  std::string(chat()).size() == self_chat,
        ...
        Check(contains("Banned 76561197960266727 for 1 minute") && kicks() == 2,
              "offline account ban persists through actual public runtime");
        ...
        Check(contains("typed writes and bounds passed") && contains("type mismatch rejected"),
              "native bridge preserves integer, float and string types");

        run("sr plugins pause variables");
        Check(set_cvar("sr_test_text", "native operator edit") && set_cvar("sr_test_fraction", "0.75"),
              "real engine adapter changes paused script settings");
        ...
        Check(cvars() == 7 &&
                  std::string(messages())
                          ...
                          .find("sr_test_text = \"native operator edit\" | variables | paused") != std::string::npos,
        ...
        Copy(std::filesystem::path(argv[7]).parent_path() / "entity.smx", script / "plugins/entity/entity.smx");
        {
            ...
            metadata
                << R"({"schema":1,"id":"entity","name":"Entity fixture","author":"tests","version":"1.0.0","api":2,"entry":"entity.smx","enabled":false,"dependencies":[]})";
        }

        run("sr plugins load entity");
        Check(command("sr_health", 3) && entity_reads() == 1 &&
                  std::string(chat()).find("entity health 73") != std::string::npos,
        ...
        run("sr_greeting");
        Check(contains("Welcome to the server!") && set_cvar("sr_greeting_message", "Welcome back!"),
              "public greeting example uses real ConVars");
        ...
        run("sr_map de_missing");
        frame();
        Check(map_changes() == 0, "actual typed engine rejects an unavailable map");
        kick_state(false, false, false);
        auto begin_server = std::string(messages()).size();
        run("sr_map de_dust2");
        frame();
        Check(map_changes() == 0 &&
                  std::string(messages()).substr(begin_server).find("unavailable on this game host") !=
                      std::string::npos,
        ...
        frame();
        Check(map_changes() == 1 && std::string(changed_map()) == "de_dust2",
              "typed ChangeLevel receives checked map and null landmark");

        restart_variable(0, false);
        begin_server = std::string(messages()).size();
        run("sr_restart");
        ...
        restart_variable(4, false);
        begin_server = std::string(messages()).size();
        run("sr_restart 5");
        Check(std::string(messages()).substr(begin_server).find("Round restart") != std::string::npos &&
                  cvar_equals("mp_restartgame", "0"),
        ...
        run("sr_restart 60");
        Check(cvar_equals("mp_restartgame", "60"), "restart accepts upper bound");
        run("sr_restart 0");
        run("sr_restart 61");
        Check(cvar_equals("mp_restartgame", "60"), "invalid delays never reach engine");
        restart_variable(2, true);
        begin_server = std::string(messages()).size();
        run("sr_restart 5");
        Check(cvar_equals("mp_restartgame", "60") &&
                  std::string(messages()).substr(begin_server).find("request failed") != std::string::npos,
        ...
        admin_chat = player_chat(3);
        ordinary_chat = player_chat(4);
        ...
        auto occurrences = [](const std::string& text, const std::string& part) {
            ...
            for (std::size_t pos = 0; (pos = text.find(part, pos)) != std::string::npos; pos += part.size())
                ++count;

            return count;
        };
        auto answers = [&] {
            return occurrences(chat(), "SourcePawn and the C++ extension returned 42.");
        };

        if (argc >= 12) {
            input_state(KEELS2_BUTTON_USE, 1, KEEL_RESULT_OK);
            Check(command("sr_hello", 3), "client opens actual module menu");
            Check(std::string(menu_text()).find("Forward/Back: move") != std::string::npos,
                  "module renders action hints through native serialization without an advertisement");

            const auto selected = [&] {
                return occurrences(chat(), "The menu selection ran in SourcePawn.");
            };
            ...
            input_state(KEELS2_BUTTON_USE, 1, KEEL_RESULT_OK);
            frame();
            Check(selected() == before_selection + 1 && !*menu_text(),
                  "native module frame reads Keel input and invokes actual script menu once");

            frame();
            Check(selected() == before_selection + 1, "native module does not repeat held selection");
            input_state(0, 1, KEEL_RESULT_OK);
            Check(command("sr_hello", 3), "reopen native module menu");
            input_state(KEELS2_BUTTON_USE, 2, KEEL_RESULT_OK);
            frame();
            frame();
            Check(selected() == before_selection + 1 && *menu_text(), "native module resets context baseline");
            const auto input_errors = [&] {
                return occurrences(messages(), "menu input unavailable for slot 3 (KeelResult ");
            };
            const auto before_errors = input_errors();
            input_state(0, 2, KEEL_RESULT_NOT_READY);
            frame();
            frame();
            frame();
            ...
            frame();
            input_state(KEELS2_BUTTON_RELOAD, 2, KEEL_RESULT_OK);
            frame();
        ...
        run("sr_hello");
        Check(std::string(messages()).substr(paused_start).find("SourcePawn and the C++ extension returned 42.") ==
                  std::string::npos,
        ...
        Check(!say("/hello \"unterminated", 3) && answers() == chat_answers + 2,
              "malformed known silent command is suppressed without execution");

        std::ofstream(script / "configs/admins.cfg") << R"("Admins" {})";
        run("sr_reloadadmins");
        Check(!say("/hello", 3) && answers() == chat_answers + 2 &&
                  occurrences(chat(), "You do not have access to this command.") == 1,
        ...
        run("sr config sr_chat_public_trigger ?");
        Check(!say("??hello", 3) && answers() == custom_answers + 3,
              "longer silent prefix takes precedence over public prefix");
        ...
        for (const auto& file : std::filesystem::recursive_directory_iterator(plugins / ".runtime"))
            if (file.path().filename() == "sr_example" + extension)
                loaded_test = file.path();
        ...
        Check(during_ok && contains("callback active; retained for unload retry"),
              "active real native call cannot destroy its script VM");
        ...
        if (argc >= 15) {
            ...
            for (const auto* invalid : {"0", "1000001", "x", "2 3"})
                run(std::string("sr_roll ") + invalid);

            const auto usage = std::string(messages()).substr(before_usage);
            Check(usage.find("Usage: sr_roll [sides: 1-1000000]") != std::string::npos &&
                      usage.find("Rolled ") == std::string::npos,
            ...
            Check(retained_random.find("plugin unload is blocked by dependent Source2Root: Source2Root Random") !=
                          std::string::npos &&
        ...
        if (argc == 16) {
            ...
            for (const auto& entry : std::filesystem::directory_iterator(packaged_extensions)) {
                if (!entry.is_regular_file() || entry.path().extension() != extension ||
                    entry.path().filename() == "source2root_random" + extension)
                    continue;
        ...
        if (argc >= 12)
            Check(network_stop(), "native menu message allocations released");
