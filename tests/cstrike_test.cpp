...
KeelResult Validate(KeelPluginHandle plugin, const KeelPlayerConnection* conn, KeelPlayerInfo* info) {
    if (Thread(plugin) != KEEL_RESULT_OK) return KEEL_RESULT_WRONG_THREAD;
    ++validates;
    if (conn->slot != 2 || conn->generation != 19 || (mode == 1 && validates == 2) ||
        (mode == 12 && validates == 3)) return KEEL_RESULT_NOT_FOUND;
...
KeelResult Describe(KeelPluginHandle plugin, KeelEntityHandle entity, KeelEntityInfo* info) {
    ...
    *info = {sizeof(*info), 3, source, 0, 99};
    ...
    if (mode == 13 && describes == 3) ++info->epoch;
    if (mode == 14 && describes == 3) ++info->source2_handle;
    return KEEL_RESULT_OK;
}
...
KeelResult StatisticsCapabilities(KeelPluginHandle plugin, unsigned* read, unsigned* write) {
    if (Thread(plugin) != KEEL_RESULT_OK) return KEEL_RESULT_WRONG_THREAD;
    *read = readable; *write = writable; return stat_cap_result;
}
KeelResult ReadStatistic(KeelPluginHandle plugin, KeelEntityHandle entity, unsigned key, std::int32_t* value) {
    if (plugin != owner || !entities.contains(entity)) return KEEL_RESULT_NOT_FOUND;
    ++reads; *value = statistics[StatisticIndex(key)];
    if (reenter) reenter();
    return read_result;
}
KeelResult WriteStatistic(KeelPluginHandle plugin, KeelEntityHandle entity, unsigned key, std::int32_t value) {
    if (plugin != owner || !entities.contains(entity)) return KEEL_RESULT_NOT_FOUND;
    ++writes; statistics[StatisticIndex(key)] = value;
    if (reenter) reenter();
    return write_result;
}
void Reset() {
    caps = 7; actions = describes = validates = mode = reads = writes = 0;
    readable = writable = 15; statistics = {800,7,3,2};
    cap_result = action_result = stat_cap_result = read_result = write_result = KEEL_RESULT_OK; reenter = {};
}
}
int main() {
    try {
        ...
        const KeelPlayerStatisticsApi stats_api{sizeof(KeelPlayerStatisticsApi),1,&StatisticsCapabilities,&ReadStatistic,&WriteStatistic};
        Players component_players(owner,entity_api,player_api,runtime,management,&stats_api);
        Reset();
        Fails([&] { players.StatisticsCapabilities(); });
        Fails([&] { players.GetStatistic(connection,1); });
        Fails([&] { players.SetStatistic(connection,1,0); });
        ...
        Check(component_players.StatisticsCapabilities() == std::pair<unsigned,unsigned>{15,15}, "unknown statistic capability bits are hidden");
        for (const unsigned key : {1u,2u,4u,8u}) {
            ...
            for (const auto value : {0,std::numeric_limits<std::int32_t>::max()}) {
                auto expected = statistics; expected[index] = value;
        ...
        for (const unsigned key : {0u,3u,16u,UINT32_MAX}) {
            Fails([&] { component_players.GetStatistic(connection,key); });
            Fails([&] { component_players.SetStatistic(connection,key,0); });
        }
        Fails([&] { component_players.SetStatistic(connection,1,-1); });
        Check(!validates && !reads && !writes, "invalid component arguments rejected before lookup");
        for (unsigned fault = 1; fault <= 14; ++fault) {
            Reset(); mode = fault;
            Fails([&] { component_players.GetStatistic(connection,1); });
            Check(reads == (fault >= 12 ? 1u : 0u) && entities.empty(), "component read validates identity before and after host read");
            if (fault <= 11) {
                Reset(); mode = fault;
                Fails([&] { component_players.SetStatistic(connection,1,900); });
        ...
        Fails([&] { component_players.SetStatistic(connection,1,0); }); Check(!writes, "read-only component blocks writes");
        Reset(); readable = 0;
        Fails([&] { component_players.GetStatistic(connection,1); }); Check(!validates && !reads, "missing read capability precedes lookup");
        Reset(); stat_cap_result = KEEL_RESULT_ENGINE_FAILURE;
        Fails([&] { component_players.StatisticsCapabilities(); });
        Fails([&] { component_players.GetStatistic(connection,1); });
        Fails([&] { component_players.SetStatistic(connection,1,0); });
        Check(!validates && !reads && !writes, "component capability failure precedes lookup");
        Reset(); read_result = KEEL_RESULT_ENGINE_FAILURE;
        Fails([&] { component_players.GetStatistic(connection,1); }); Check(reads == 1 && entities.empty(), "component read failure cleanup");
        Reset(); write_result = KEEL_RESULT_ENGINE_FAILURE;
        Fails([&] { component_players.SetStatistic(connection,1,900); });
        Check(writes == 1 && statistics[0] == 900 && entities.empty(), "component notification failure does not promise rollback");
        Reset(); mode = 12;
        component_players.SetStatistic(connection,1,900);
        Check(writes == 1 && validates == 2 && entities.empty(), "dispatched component write does not revalidate after callback");
        Reset(); reenter = [&] { component_players.SetStatistic(connection,1,900); };
        Fails([&] { component_players.SetStatistic(connection,1,900); });
        Check(writes == 8 && entities.empty(), "component write recursion is bounded");
        reenter = {}; component_players.SetStatistic(connection,1,901);
        Check(writes == 9 && entities.empty(), "component recursion count recovers");
        Reset(); reenter = [&] { component_players.GetStatistic(connection,1); };
        Fails([&] { component_players.GetStatistic(connection,1); });
        Check(reads == 8 && entities.empty(), "component read recursion is bounded");
        Reset(); worker_error = {};
        std::thread stats_worker([&] { try {
            Fails([&] { component_players.StatisticsCapabilities(); });
            Fails([&] { component_players.GetStatistic(connection,1); });
            Fails([&] { component_players.SetStatistic(connection,1,0); });
        } catch (...) { worker_error = std::current_exception(); } });
        stats_worker.join(); if (worker_error) std::rethrow_exception(worker_error);
        ...
        for (unsigned fault = 0; fault < 5; ++fault) {
            ...
            if (fault == 0) invalid_stats.size = 0;
            if (fault == 1) ++invalid_stats.api_version;
            if (fault == 2) invalid_stats.capabilities = nullptr;
            if (fault == 3) invalid_stats.read = nullptr;
            if (fault == 4) invalid_stats.write = nullptr;
            Fails([&] { Players invalid(owner,entity_api,player_api,runtime,management,&invalid_stats); });
        }
        RunStatisticsChecks();
        RunRoundChecks();
