...
{
    if (!Entity_ConstructionAvailable()) return false;
...
bool Prepare()
{
    ...
    if (Current == NoEntity || !Entity_IsValid(Current) || !Entity_IsPending(Current)
        ...
        || Entity_FromHandle(Reference) != NoEntity) return false;
    float origin[3] = {1.0,2.0,3.0}, angles[3] = {4.0,5.0,6.0};
    int color[4] = {10,20,30,255};
    if (!Entity_SetKeyString(Current,"model","models/test.vmdl")
        || !Entity_SetKeyBool(Current,"solid",true) || !Entity_SetKeyInt(Current,"flags",7)
        || !Entity_SetKeyFloat(Current,"scale",1.5) || !Entity_SetKeyVector(Current,"origin",origin)
        || !Entity_SetKeyAngles(Current,"angles",angles) || !Entity_SetKeyColor(Current,"rendercolor",color)) return false;
    color[1] = -1;
    if (Entity_SetKeyColor(Current,"rendercolor",color) || Entity_SetKeyBool(Current,"solid",view_as<bool>(2))
        ...
        || Entity_SetKeyString(Current,"classname","other")) return false;
    float position[3] = {10.0,20.0,30.0};
    if (!Entity_Teleport(Current,position,angles,origin,ENTITY_TELEPORT_POSITION)) return false;
    ...
    Pre = Post = 0; Bad = false;
    SpawnHook = SDKHook_Add(Reference,"spawn",SDKHook_Both,OnSpawn);
    return SpawnHook != NoSDKHook;
}
void Clean()
{
    if (SpawnHook != NoSDKHook) { SDKHook_Close(SpawnHook); SpawnHook = NoSDKHook; }
    if (Current != NoEntity) { Entity_Close(Current); Current = NoEntity; }
}
public SDKHookAction OnSpawn(SDKHookFrame frame, SDKHookPhase phase, int data)
{
    if (SDKHook_Entity(frame) != Reference || SDKHook_Kind(frame) != SDKHook_Spawn || data != 0) Bad = true;
    if (phase == SDKHook_Pre)
    {
        ++Pre;
        if (!Entity_IsPending(Current) || Entity_SetKeyInt(Current,"flags",99)) Bad = true;
        if (Mode == 1) return SDKHook_Block;
        if (Mode == 2) { if (!Entity_Close(Current)) Bad = true; Current = NoEntity; }
        if (Mode == 4) { SDKHook_Close(SpawnHook); SpawnHook = NoSDKHook; }
    }
    else
    {
        ++Post;
        if (Mode == 1)
        {
            if ((SDKHook_Flags(frame) & SDKHOOK_ORIGINAL_CALLED) || !Entity_IsPending(Current)) Bad = true;
        }
        else if (!(SDKHook_Flags(frame) & SDKHOOK_ORIGINAL_CALLED)) Bad = true;
    }
...
{
    Mode = mode;
    if (!Prepare()) { LogMessage("CONSTRUCTION_FAILED prepare"); Clean(); return; }
    bool invoked;
    ...
    else valid = valid && !success;
    Clean();
    LogMessage(valid ? "CONSTRUCTION_CASE_OK" : "CONSTRUCTION_FAILED spawn");
}
public void Normal(Player caller, const char[] arguments) { Run(0); }
public void Block(Player caller, const char[] arguments) { Run(1); }
public void CloseOwner(Player caller, const char[] arguments) { Run(2); }
public void CloseHook(Player caller, const char[] arguments) { Run(4); }
public void EngineError(Player caller, const char[] arguments) { Run(3); }
public void Retry(Player caller, const char[] arguments)
{
    Mode = 0;
    if (!Prepare()) { LogMessage("CONSTRUCTION_FAILED retry setup"); return; }
    bool invoked = true;
...
{
    bool invoked;
    bool valid = Entity_DispatchSpawn(Current,invoked) && invoked && Pre == 1 && Post == 1 && !Bad && !Entity_IsPending(Current);
    Clean(); LogMessage(valid ? "CONSTRUCTION_FINISH_OK" : "CONSTRUCTION_FAILED retry finish");
}
...
public void Stale(Player caller, const char[] arguments)
{
    ...
    Clean(); LogMessage(valid ? "CONSTRUCTION_STALE_OK" : "CONSTRUCTION_FAILED stale");
}
public void Cancel(Player caller, const char[] arguments)
{
    Current = Entity_Create("prop_dynamic");
    bool valid = Current != NoEntity && Entity_Close(Current); Current = NoEntity;
    LogMessage(valid ? "CONSTRUCTION_CANCEL_OK" : "CONSTRUCTION_FAILED cancel");
}
public void Grow(Player caller, const char[] arguments)
{
    Entity temporary[96]; bool valid = true;
    for (int i = 0; i < sizeof(temporary); i++)
    {
        temporary[i] = Entity_Find(4);
        if (temporary[i] == NoEntity) valid = false;
    }
    for (int i = 0; i < sizeof(temporary); i++) if (temporary[i] != NoEntity) Entity_Close(temporary[i]);
