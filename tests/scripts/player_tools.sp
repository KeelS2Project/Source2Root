...
public void FindMany(Player caller, const char[] arguments) { Find(caller, arguments, true); }
public void FindOne(Player caller, const char[] arguments) { Find(caller, arguments, false); }
public void Remember(Player caller, const char[] arguments) { remembered = caller; }
public void Current(Player caller, const char[] arguments) { ReplyToCommand(caller, IsPlayerConnected(remembered) ? "connected" : "disconnected"); }
public void BadFormat(Player caller, const char[] arguments) { Format(protectedText, sizeof(protectedText), "%n", 42); }
...
public void Info(Player caller, const char[] arguments)
{
    ...
    if (GetPlayerSteamID(caller, text, sizeof(text), SteamID2)) ReplyToCommand(caller, text);
    if (GetPlayerSteamID(caller, text, sizeof(text), SteamID3)) ReplyToCommand(caller, text);
}
...
public void Tools(Player caller, const char[] arguments)
{
    ...
    Check(Format(output, sizeof(output), "%s %d %i %u %x %X %.2f %c %%", "string", -2147483647 - 1, 42, -1, 255, 255, 1.25, 65) > 0
        && Equal(output, "string -2147483648 42 4294967295 ff FF 1.25 A %"), "FAILED: varargs formatting");
    ...
    Check(GetArgument("one \"two three\"", 1, output, sizeof(output)) && Equal(output, "two three"), "FAILED: quoted argument");
    ...
    Check(GetRemainingArguments("target \"a reason\" with spaces", 1, output, sizeof(output)) && Equal(output, "a reason with spaces"), "FAILED: remaining arguments");
