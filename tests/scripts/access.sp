...
public void Access(Player caller, const char[] arguments)
{
    ...
    else if (arguments[0] == 'g')
    {
        ...
        if (IsAdmin(caller) && GetAdminGroup(caller, group, sizeof(group)) && GetAdminImmunity(caller, immunity))
        {
            ...
            if (immunity == 100)
                ReplyToCommand(caller, "immunity 100");
        }
        else
            ReplyToCommand(caller, "no administrator");
    }
    else if (arguments[0] == 'm')
    {
        Menu menu = CreateMenu("Check target", "admin.kick", Selected);

        if (menu != NoMenu && (!AddMenuItem(menu, "Check target") || !ShowMenu(menu, caller)))
            CloseMenu(menu);
    }
    ...
    else if (arguments[0] == 'i')
    {
        ReplyToCommand(caller,
                       CanTargetIdentity(caller, "invalid", "admin.ban") ? "invalid identity accepted"
    ...
    else if (arguments[0] == 'o')
    {
        ...
        ReplyToCommand(
            caller, CanTarget(caller, target, arguments[0] == 'b' ? "admin.ban" : "admin.kick") ? "allowed" : "denied");
}

public void Selected(Player caller, int item)
{
    if (item == 0)
        ReplyToCommand(caller, CanTarget(caller, target, "admin.kick") ? "allowed" : "denied");
}
