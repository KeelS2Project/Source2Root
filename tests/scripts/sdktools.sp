...
public bool OnPluginStart()
{
    ...
    if (Health == NoSchemaField || Retained == NoEntity || Entity_Index(Retained) != 4 || !Entity_IsValid(Retained))
        return false;

    int value, source;

    if (!Entity_ReadInt(Retained, Health, value) || value != 73 || !Entity_SourceHandle(Retained, source) ||
        source != 0x23004)
        return false;

    Entity same = Entity_FromHandle(source);

    if (same == NoEntity || !Entity_Same(Retained, same) || !Entity_Close(same))
        return false;
    ...
    if (pawn == NoEntity || controller == NoEntity || !Entity_Same(pawn, Retained) || Entity_Index(controller) != 3)
        return false;

    Entity_Close(pawn);
    Entity_Close(controller);
    ...
    if (Schema_FieldType(Health) != Schema_Int32 || Schema_FieldSize(Health) != 4 ||
        ...
        Schema_FieldInfo(Health, small, sizeof(small), name, sizeof(name), profile, sizeof(profile)))
        return false;
    ...
    if (wide == NoSchemaField || Entity_ReadInt(Retained, wide, value) || value != 0 ||
        !Entity_ReadIntegerText(Retained, wide, name, sizeof(name)) || name[0] != '1' || name[19] != '5' ||
        name[20] != 0 || Entity_ReadIntegerText(Retained, wide, small, sizeof(small)))
        return false;
    ...
    if (vector == NoSchemaField || !Entity_ReadVector(Retained, vector, position) || position[0] != 1.0 ||
        position[2] != 3.0 || Entity_ReadInt(Retained, vector, value))
        return false;
    ...
    if (number == NoSchemaField || !Entity_ReadFloat(Retained, number, fraction) || fraction != 1.25)
        return false;

    Schema_Close(number);
    SchemaField flag = Schema_Find("CTestEntity", "m_bFlag", Schema_Bool);

    if (flag == NoSchemaField || !Entity_ReadInt(Retained, flag, value) || value != 1)
        return false;

    Schema_Close(flag);
    SchemaField link = Schema_Find("CTestEntity", "m_hOther", Schema_EntityHandle);

    if (link == NoSchemaField || !Entity_ReadSourceHandle(Retained, link, source) || source != 0x45005)
        return false;

    Entity other = Entity_ReadEntity(Retained, link);

    if (other == NoEntity || Entity_Index(other) != 5 || Entity_Same(Retained, other))
        return false;
    ...
    if (Schema_Find("CBaseEntity", "m_iHealth", Schema_Float32) != NoSchemaField || Entity_Find(-1) != NoEntity ||
        Entity_FromHandle(-1) != NoEntity || Schema_Find("CBaseEntity", "missing", Schema_Int32) != NoSchemaField)
        return false;
...
public void Epoch(Player caller, const char[] arguments)
{
    ...
    if (Entity_IsValid(Retained) || Entity_Index(Retained) != -1 || Entity_ReadInt(Retained, Health, value) ||
        value != 0 || Entity_WriteInt(Retained, Health, 90) || Entity_Remove(Retained) || !Entity_Close(Retained))
    {
        LogMessage("SDKTOOLS_FAILED");
        return;
    }
...
public void WriteFields(Player caller, const char[] arguments)
{
    ...
    {
        LogMessage("SDKTOOLS_FAILED integer write");
        return;
    }
    ...
    if (wide == NoSchemaField || !Entity_WriteIntegerText(Retained, wide, "18446744073709551615") ||
        ...
        Entity_WriteInt(Retained, wide, -1))
    {
        LogMessage("SDKTOOLS_FAILED wide write");
        return;
    }
    ...
    if (number == NoSchemaField || !Entity_WriteFloat(Retained, number, 2.5) ||
        !Entity_ReadFloat(Retained, number, fraction) || fraction != 2.5 || !Entity_WriteFloat(Retained, number, 1.25))
    {
        LogMessage("SDKTOOLS_FAILED float write");
        return;
    }
    ...
    if (vector == NoSchemaField || !Entity_WriteVector(Retained, vector, input) ||
        !Entity_ReadVector(Retained, vector, output) || output[0] != 4.0 || output[2] != 6.0)
    {
        LogMessage("SDKTOOLS_FAILED vector write");
        return;
    }
    ...
    input[1] = 2.0;
    input[2] = 3.0;
    Entity_WriteVector(Retained, vector, input);
    Schema_Close(vector);
    SchemaField flag = Schema_Find("CTestEntity", "m_bFlag", Schema_Bool);

    if (flag == NoSchemaField || !Entity_WriteInt(Retained, flag, 0) || !Entity_ReadInt(Retained, flag, value) ||
        value != 0 || Entity_WriteInt(Retained, flag, 2) || !Entity_WriteInt(Retained, flag, 1))
    {
        LogMessage("SDKTOOLS_FAILED bool write");
        return;
    }
...
{
    ClosingEntity = Entity_Find(4);
    ClosingField = Schema_Find("CBaseEntity", "m_iHealth", Schema_Int32);

    if (ClosingEntity == NoEntity || ClosingField == NoSchemaField ||
        !Entity_WriteInt(ClosingEntity, ClosingField, 81) || ClosingEntity != NoEntity || ClosingField != NoSchemaField)
    {
        LogMessage("SDKTOOLS_FAILED close during write");
        return;
    }
    ...
    {
        LogMessage("SDKTOOLS_FAILED write callback state");
        return;
    }
...
{
    Entity_Close(ClosingEntity);
    Schema_Close(ClosingField);
    ClosingEntity = NoEntity;
    ClosingField = NoSchemaField;
}
...
public void Tools(Player caller, const char[] arguments)
{
    ...
    {
        LogMessage("SDKTOOLS_FAILED teleport");
        return;
    }

    p[0] = view_as<float>(0x7fc00000);

    if (Entity_Teleport(Retained, p, a, v, ENTITY_TELEPORT_POSITION) ||
        !Entity_Teleport(Retained, p, a, v, ENTITY_TELEPORT_VELOCITY) || Entity_SetModel(Retained, "") ||
        Entity_SetModel(Retained, "bad\nmodel") || !Entity_SetModel(Retained, "models/test.vmdl"))
    {
        LogMessage("SDKTOOLS_FAILED model/vectors");
        return;
    }
    ...
    {
        LogMessage("SDKTOOLS_FAILED remove");
        return;
    }
...
public void ToolUnavailable(Player caller, const char[] arguments)
{
    ...
    LogMessage(Entity_GetToolCapabilities(caps) && caps == 0 && !Entity_Teleport(Retained, zero, zero, zero) &&
                       !Entity_SetModel(Retained, "model") && !Entity_Remove(Retained)
                   ? "SDKTOOLS_TOOL_UNAVAILABLE_OK"
                   : "SDKTOOLS_FAILED unavailable tools");
}
...
void CallbackTool(int kind, bool expectFailure)
{
    ...
    if (kind == 1)
        result = Entity_Teleport(ToolEntity, ToolPosition, zero, zero, ENTITY_TELEPORT_POSITION);
    else if (kind == 2)
        result = Entity_SetModel(ToolEntity, ToolModel);
    else
        result = Entity_Remove(ToolEntity);
...
public void ToolCallback(Player caller, const char[] arguments)
{
    ...
    if (!ParseInt(arguments, kind, 1, 4) || kind == 3)
    {
        LogMessage("SDKTOOLS_FAILED callback kind");
        return;
    }
...
public
void ToolCallbackError(Player caller, const char[] arguments)
{
    CallbackTool(2, true);
}

public
void ToolCapClose(Player caller, const char[] arguments)
{
    CallbackTool(2, true);
}
...
{
    if (!Entity_Close(ToolEntity))
    {
        LogMessage("SDKTOOLS_FAILED closing tool entity");
        return;
    }
...
{
    ToolEntity = Entity_Find(4);
    ToolFailures = 0;
    bool result = Entity_SetModel(ToolEntity, "models/outer.vmdl");
    Entity_Close(ToolEntity);
    ToolEntity = NoEntity;
...
{
    if (!Entity_SetModel(ToolEntity, "models/nested.vmdl"))
        ++ToolFailures;
}
