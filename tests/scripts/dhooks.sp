...
{
    if (!DHook_Enable(hook, false))
        LogMessage("DHOOKS_FAILED disable");
}

public void Enable(Player player, const char[] arguments)
{
    if (!DHook_Enable(hook, true))
        LogMessage("DHOOKS_FAILED enable");
}
...
{
    if (!DHook_IsActive(hook))
        LogMessage("DHOOKS_FAULT_RETIRED_OK");
    else
        LogMessage("DHOOKS_FAILED fault did not retire hook");
}
...
public DHookAction Intercept(DHookFrame frame, DHookPhase phase, int data)
{
    ...
    if (phase == DHook_Post)
    {
        if (mode == 0)
        {
            if (!DHook_SetInt(frame, 0, 90))
                LogMessage("DHOOKS_FAILED post return");
    ...
    if (mode == 3)
        return view_as<DHookAction>(42 / (mode - 3));
