...
{
    int direct, queued;
    if (!Entity_GetInputCapabilities(direct,queued) || direct != 511 || queued != 383) return false;
...
void Clean()
{
    if (Current != NoEntity) Entity_Close(Current);
    if (Participant != NoEntity) Entity_Close(Participant);
    Current = Participant = NoEntity;
}
bool Prepare()
{
    Clean(); Current = Entity_Find(4); Participant = Entity_Find(3);
    return Current != NoEntity && Participant != NoEntity;
}
void Report(bool valid, const char[] success)
{
    if (valid) LogMessage(success);
    else { char error[256]; GetLastError(error,sizeof(error)); LogMessage("INPUT_FAILED"); LogMessage(error); }
}
public void All(Player player, const char[] arguments)
{
    bool valid = Prepare(), invoked;
    float vector[3] = {1.0,2.0,3.0}, angles[3] = {4.0,5.0,6.0}; int color[4] = {10,20,30,255};
    for (int i = 0; i < 2; ++i)
    {
        bool queue = i != 0; float delay = queue ? 1.25 : 0.0;
    ...
    Clean(); Report(valid,"INPUT_ALL_OK");
}
...
public void CloseCallback(Player player, const char[] arguments) { Text[0] = 'X'; Clean(); }
public void Recursion(Player player, const char[] arguments)
{
    bool valid = Prepare(), invoked; Depth = Visits = 0; Bad = false;
    valid = Entity_InputVoid(Current,"Enable",invoked) && invoked && valid;
    Clean(); Report(valid && !Bad && Visits == 8,"INPUT_RECURSION_OK");
}
public void Recurse(Player player, const char[] arguments)
{
    ++Depth; ++Visits; bool invoked;
    bool result = Entity_InputVoid(Current,"Enable",invoked);
    if (Depth == 8 ? (result || invoked) : (!result || !invoked)) Bad = true;
    --Depth;
}
public void Leave(Player player, const char[] arguments) { Report(Prepare(),"INPUT_LEAVE_OK"); }
public void Stale(Player player, const char[] arguments)
{
    ...
    Clean(); Report(valid,"INPUT_STALE_OK");
}

public void BadPayload(Player player, const char[] arguments)
{
    Prepare(); LastInvoked = true;
...
{
    Prepare(); LastInvoked = true;
    Entity old = Participant; Entity_Close(Participant); Participant = NoEntity;
...
{
    Report(!LastInvoked,"INPUT_REFUSAL_OK"); Clean();
}
