...
{
    if (SlapPlayer(NoPlayer) || SlayPlayer(NoPlayer) || OpenCommandMenu(NoPlayer, "sr_slap"))
        return false;

    if (SetCommandMenu("sr_slap", Recursive))
        return false;
...
public void Run(Player caller, const char[] arguments)
{
    ...
    else if (arguments[0] == 's')
    {
        ...
        if (GetCommandInfo(caller, "sr_slap", usage, sizeof(usage), hasMenu) || usage[0] || hasMenu)
            LogMessage("FAILED: short usage");
        else
            Failure(caller);
    }
    else if (arguments[0] == 'd')
    {
        if (SlapPlayer(caller, -1) || SlapPlayer(caller, 1001))
            LogMessage("FAILED: invalid damage");
        else
            Failure(caller);
    }
    else if (arguments[0] == 'b')
    {
        ...
        if (SlayPlayer(view_as<Player>(menu)))
            LogMessage("FAILED: wrong handle");
        else
            Failure(caller);

        CloseMenu(menu);
    }
    else if (!OpenCommandMenu(caller, "sr_slap"))
        Failure(caller);
}
...
public void Recursive(Player caller, const char[] arguments)
{
    ...
    if (!OpenCommandMenu(caller, "sr_routes"))
        Failure(caller);
}
...
{
    if (SlayPlayer(NoPlayer) || SetCommandMenu("sr_routes", Recursive))
        LogMessage("FAILED: retiring mutation");
}
