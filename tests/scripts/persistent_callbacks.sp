...
{
    if (PersistentControl(0) != 43)
        LogMessage("PERSISTENT_FAILED nested native dispatch");
}
    ...
    float &real, int[] integers, float[] reals, const int[] input_values)
{
    ...
    if (PersistentControl(1) != 1)
        return -101;

    if (mode == 1)
        return PersistentControl(3) + 1;

    if (mode == 2 && PersistentControl(2) != 1)
        return -102;

    if (mode == 3)
        return 42 / PersistentControl(4);

    if (mode == 4 && PersistentControl(5) != 1)
        return -103;
