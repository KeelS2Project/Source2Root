...
public void Reconnected(Player caller, const char[] arguments)
{
    ...
    if (CS_GetPlayerMoney(Saved, score) || score != 0 || CS_SetPlayerMoney(Saved, 1)) { LogMessage("CSTRIKE_FAILED stale component player"); return; }
    Player players[4];
...
public void Components(Player caller, const char[] arguments)
{
    ...
    if (!CS_GetPlayerStatisticsCapabilities(readable, writable) || readable != 15 || writable != 15
        ...
        || !CS_SetPlayerMoney(Saved, 16000) || !CS_SetPlayerKills(Saved, 19)
        || !CS_SetPlayerDeaths(Saved, 11) || !CS_SetPlayerAssists(Saved, 5)
        || !CS_GetPlayerMoney(Saved, money) || money != 16000 || !CS_GetPlayerKills(Saved, kills) || kills != 19
        || !CS_GetPlayerDeaths(Saved, deaths) || deaths != 11 || !CS_GetPlayerAssists(Saved, assists) || assists != 5
        || !CS_SetPlayerMoney(Saved, 800) || !CS_SetPlayerKills(Saved, 7)
        || !CS_SetPlayerDeaths(Saved, 3) || !CS_SetPlayerAssists(Saved, 2)) LogMessage("CSTRIKE_FAILED component mapping or write");
    else LogMessage("CSTRIKE_COMPONENTS_OK");
}
public void ComponentsReadOnly(Player caller, const char[] arguments)
{
    ...
    else LogMessage("CSTRIKE_COMPONENTS_READONLY_OK");
}
public void ComponentsCapsError(Player caller, const char[] arguments)
{
    ...
    if (CS_GetPlayerStatisticsCapabilities(readable, writable) || readable != 0 || writable != 0)
        { LogMessage("CSTRIKE_FAILED component capability result"); return; }
    GetLastError(error, sizeof(error));
...
public void ComponentsReadError(Player caller, const char[] arguments)
{
    int money = 99;
    char error[256];
    if (CS_GetPlayerMoney(Saved, money) || money != 0) { LogMessage("CSTRIKE_FAILED component read error"); return; }
    GetLastError(error, sizeof(error));
...
public void ComponentsWriteError(Player caller, const char[] arguments)
{
    int money;
    char error[256];
    if (CS_SetPlayerMoney(Saved, 900)) { LogMessage("CSTRIKE_FAILED component notification result"); return; }
    GetLastError(error, sizeof(error));
    if (!error[0] || !CS_GetPlayerMoney(Saved, money) || money != 900) LogMessage("CSTRIKE_FAILED component notification error");
    else LogMessage("CSTRIKE_COMPONENTS_WRITE_ERROR_OK");
}
public void ComponentsRestore(Player caller, const char[] arguments)
{
    if (!CS_SetPlayerMoney(Saved, 800)) LogMessage("CSTRIKE_FAILED component restore");
    else LogMessage("CSTRIKE_COMPONENTS_RESTORE_OK");
}
