...
public bool OnPluginStart()
{
    ...
    if (!tools || !TopMenu_AddCategory(tools, "players", "Player actions", "demo.hello")) return false;
    ...
    if (!localItem || !TopMenu_AddItem(tools, "", "denied", "Denied action", Handle, "not.granted") ||
        ...
        !TopMenu_AddItem(tools, "", "disabled", "Disabled action", Disabled, "", 100)) return false;
...
{
    if (display != NoTopMenuDisplay) TopMenu_CloseDisplay(display);
    display = TopMenu_Show(tools, Target());
    if (display == NoTopMenuDisplay || !TopMenu_IsOpen(display)) LogMessage("TOP_FAILED_SHOW");
}
public void Close(Player caller, const char[] arguments)
{
    if (display != NoTopMenuDisplay) { TopMenu_CloseDisplay(display); display = NoTopMenuDisplay; }
}
public void Closed(Player caller, const char[] arguments)
{
    if (display != NoTopMenuDisplay && !TopMenu_IsOpen(display)) LogMessage("TOP_DISPLAY_CLOSED");
    else LogMessage("TOP_FAILED_CLOSED");
}
public void Permissions(Player caller, const char[] arguments)
{
    if (!ReloadAdmins(caller)) LogMessage("TOP_FAILED_PERMISSIONS");
}
...
{
    if (!TopMenu_SetTitle(tools, "Changed title")) LogMessage("TOP_FAILED_TITLE");
}
public void Mutate(Player caller, const char[] arguments)
{
    ...
    if (display != NoTopMenuDisplay) TopMenu_CloseDisplay(display);
    display = TopMenu_Show(tools, Target());
    if (display != NoTopMenuDisplay) LogMessage("TOP_FAILED_MUTATION");
    else LogMessage("TOP_MUTATION_REJECTED");
}
...
{
    if (!IsPlayerConnected(player) || data != 11 || entry != localItem) LogMessage("TOP_FAILED_CALLBACK_IDENTITY");
    if (action == TopMenu_CheckAccess) {
        if (mutateAccess) { mutateAccess = false; TopMenu_SetLabel(entry, "Mutated action"); }
