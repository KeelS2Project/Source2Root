...
public TopMenuAccess Handle(TopMenuObject entry, Player player, TopMenuAction action, int data)
{
    ...
    if (entry != item || data != 22 || !GetPlayerName(player, name, sizeof(name)))
        LogMessage("TOP_FAILED_CONTRIBUTOR_IDENTITY");

    if (fault)
        TopMenu_SetLabel(view_as<TopMenuObject>(-1), "Invalid resource");

    if (action == TopMenu_Select)
        LogMessage("TOP_OTHER_SELECTED");
