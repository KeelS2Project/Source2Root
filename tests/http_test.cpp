...
int main() {
    try {
        ...
        const auto send = [&](Request request) {
            return client.Perform(std::move(request), [] {
                return false;
            });
        };
        const auto request = [&](const char* path) {
            Request input;
            input.url = root + path;
            return input;
        };
        ...
        for (const auto& [name, value] : result.headers)
            if (name == "x-repeated") {
                Check(value == (++repeated == 1 ? "first" : "second"), "repeated header order");
            }
        ...
        input.parts = {{"text", "hello", "", "", {}},
                       {"binary", std::string("x\0y", 3), "data.bin", "application/octet-stream", {}},
        ...
        Check(result.body.find("name=\"text\"") != result.body.npos && result.body.find("hello") != result.body.npos &&
                  result.body.find(std::string("x\0y", 3)) != result.body.npos &&
                  result.body.find(std::string("file\0\xffpayload", 13)) != result.body.npos,
              "multipart text/binary/file fields preserved");

        input = request("/redirect-307");
        input.method = Method::Post;
        input.body = "payload";
        ...
        Reject(
            ...
            "cross-origin upload redirect refused");

        input = request("/redirect-same");
        input.headers = {{"Authorization", "Bearer secret"}, {"X-Secret", "private"}, {"Cookie", "session=value"}};
        Check(send(input).body.find("private") != std::string::npos, "same-origin headers retained");
        input.url = root + "/redirect-cross";
        result = send(input);
        Check(result.body.find("authorization") == result.body.npos &&
                  result.body.find("x-secret") == result.body.npos && result.body.find("cookie") == result.body.npos,
        ...
        Reject(
            [&] {
                send(request("/redirect-loop"));
            },
            "redirect loop bounded");

        Reject(
            [&] {
                send(request("/redirect-protocol"));
            },
            "non-HTTP redirect refused");
        ...
        for (const auto* path : {"/large", "/gzip", "/too-many-headers", "/long-header", "/short"})
            Reject(
                ...
                "oversized/decompressed/header/partial response rejected");

        input = request("/hello");
        input.response_limit = 8;
        Check(send(input).body.size() == 8, "exact response cap accepted");
        input.response_limit = 7;
        Reject(
            [&] {
                send(input);
            },
            "one-byte body overflow rejected");

        input = request("/hello");
        input.url = secure + "/hello";
        input.ca_file = Env("SR_HTTP_CA");
        Check(send(input).status == 200, "valid private TLS CA accepted");
        input.ca_file = Env("SR_HTTP_BAD_CA");
        Reject(
            [&] {
                send(input);
            },
        ...
        input.ca_file = Env("SR_HTTP_CA");
        input.url = Env("SR_HTTP_WRONG_NAME_URL") + "/hello";
        Reject(
            [&] {
                send(input);
            },
        ...
        std::atomic_bool was_canceled{false};
        std::thread worker([&] {
            try {
                client.Perform(request("/delay"), [&] {
        ...
        Reject(
            [&] {
                client.Perform(request("/hello"), [] {
                    return true;
                });
            },
            "pre-canceled request performs no work");

        for (const auto* url : {"file:///tmp/data",
                                "ftp://localhost/a",
        ...
        for (const auto* header : {"Host", "Content-Length", "Transfer-Encoding", "Proxy-Authorization", "Connection"})
            Reject(
                ...
                "framing/proxy header refused");

        Reject(
            [&] {
                ValidateHeader("X-Value", "bad\r\nheader");
            },
            "header injection refused");

        input = request("/echo");
        input.method = Method::Post;
        input.body.assign(BodyLimit + 1, 'x');
        Reject(
            [&] {
                input.Validate();
            },
        ...
        input = request("/echo");
        input.method = Method::Post;
        input.parts = {{"file", "", "missing.bin", "", files / "missing.bin"}};
        Reject(
            [&] {
                send(input);
            },
            "missing upload file refused");
        ...
        for (int i = 0; i < 4; ++i)
            parallel.emplace_back([&] {
                if (send(request("/hello")).status == 200)
                    ++completed;
            });

        for (auto& thread : parallel)
            thread.join();
