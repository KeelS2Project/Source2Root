...
std::array<void*, 128> message_table{}, messages_table{}, definition_table{}, events_table{}, manager_table{},
    event_table{};
...
void* AsProto(const Packet* packet) {
    return mode == 3 ? nullptr : mode == 11 ? wrong_proto : packet->proto;
}

INetworkMessageInternal* FindName(void*, const char*) {
    return nullptr;
}
...
void Release(void*, INetworkMessageInternal* type, CNetMessage* packet) {
    ...
    if (mode == 10)
        throw std::runtime_error("injected packet release failure");
}
...
void Post(void*, CSplitScreenSlot slot, bool local, int count, const uint64* mask,
    ...
    if (mode == 4)
        throw std::runtime_error("injected engine send failure");
...
IGameEvent* Create(void*, const char* name, bool force, int* cookie) {
    ...
    if (mode == 12)
        throw std::runtime_error("injected event creation failure");
...
void EventText(void*, const GameEventKeySymbol_t& key, const char* value) {
    ...
    if (mode == 6)
        throw std::runtime_error("injected setter failure");
...
void Free(void*, IGameEvent* event) {
    ...
    if (mode == 9)
        throw std::runtime_error("injected event release failure");
}
...
bool Serialize(void*, IGameEvent* event, CNetMessagePB<CMsgSource1LegacyGameEvent>* packet) {
    ...
    Int(proto, "eventid", event_id);
    Text(proto, "event_name", "show_survival_respawn_status");
    auto* user = Add(proto, "keys");
    Int(*user, "type", 3);
    Int(*user, "val_long", player);
    auto* text = Add(proto, "keys");
    Int(*text, "type", 1);
    Text(*text, "val_string", token.c_str());
    auto* expires = Add(proto, "keys");
    Int(*expires, "type", 3);
    Int(*expires, "val_long", duration);
    auto* pawn = Add(proto, "keys");
    Int(*pawn, "type", 3);
    Int(*pawn, "val_long", 10000 + player);
    return true;
}
...
int main(int argc, char** argv) {
    try {
        ...
        for (const auto& file : descriptors.file())
            Check(pool.BuildFile(file), "build upstream schema");
        ...
        sr::Cs2MenuBackend backend(reinterpret_cast<INetworkMessages*>(&messages),
                                   reinterpret_cast<IGameEventSystem*>(&events));
        ...
        Check(backend.Render(native, 3, html, 15000) == KEEL_RESULT_OK && decoded->ParseFromString(wire),
              "render and decode wire payload");
        ...
        Check(Int(*decoded, "eventid") == 413 && Text(*decoded, "event_name") == "show_survival_respawn_status" &&
                  ...
                  Int(Key(*decoded, 0), "val_long") == 3 && Int(Key(*decoded, 3), "val_long") == 10003,
              "wire uses engine key order, types and pawn identity without an advertisement");

        Check(recipients[0] == 8, "private menu targets only slot 3");

        for (std::size_t i = 1; i < recipients.size(); ++i)
            Check(recipients[i] == 0, "no high-slot broadcast");

        Check(backend.Render(native, 3, "", 0) == KEEL_RESULT_OK && decoded->ParseFromString(wire) &&
                  Int(Key(*decoded, 2), "val_long") == 0 && Text(Key(*decoded, 1), "val_string") == " ",
              "clear preserves a nonempty token so the client hide path runs");
        ...
        for (const auto milliseconds : {-1, 0, 120001, INT32_MAX})
            Check(backend.Render(native, 3, html, milliseconds) == KEEL_RESULT_INVALID_ARGUMENT,
                  "invalid lifetime rejected before engine access");
        ...
        Check(backend.Render(native, last, html, 15000) == KEEL_RESULT_OK &&
                  recipients[static_cast<unsigned>(last) / 64] == (uint64{1} << (last % 64)),
              "recipient mask reaches last slot");
        ...
        event_id = 614;
        Check(backend.Render(native, 3, html, 15000) == KEEL_RESULT_OK && decoded->ParseFromString(wire) &&
                  Int(*decoded, "eventid") == 614 && backend.Error().empty(),
              "engine descriptor changes and recovery need no advertisement");
