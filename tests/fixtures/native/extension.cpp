...
public:
    ...
    bool Load() override {
        if (services_.Connect(HostContext()) != KEEL_RESULT_OK) return false;
        const void* value = nullptr;
        if (HostContext().QueryService(SR_EXTENSION_SERVICE, SR_EXTENSION_API_VERSION, &value) != KEEL_RESULT_OK) return false;
        api_ = static_cast<const SrExtensionApi*>(value);
        if (!api_ || api_->size != sizeof(*api_) || api_->api_version != SR_EXTENSION_API_VERSION ||
            !api_->register_native || !api_->unregister_native || !api_->open_menu || !api_->close_menu) return false;
        if (services_.Publish("source2root.example", 1, &marker_, publication_) != KEEL_RESULT_OK) return false;
        const SrNativeSpec spec{sizeof(spec), SR_EXTENSION_API_VERSION, "SR_ExampleAdd", 2, 0,
                               "source2root.example", 1, &Add, this};
        if (!CreateCommand("sr_native_menu", "Open the native menu example", &ExampleExtension::OpenMenu,
                           FCVAR_GAMEDLL | FCVAR_CLIENT_CAN_EXECUTE)) return false;
    ...
    void Unload() override { api_ = nullptr; }
private:
    ...
    void OpenMenu(const CCommandContext& context, const CCommand&) {
        PlayerInfo player;
        if (!GetPlayer(context.GetPlayerSlot(), player)) return;
        ...
        if (result != KEEL_RESULT_OK) PrintToConsole(player.slot, "Source2Root menu unavailable or permission denied.");
    }
    ...
    static KeelResult Add(void*, const int32_t* arguments, uint32_t count, int32_t* result, char*, uint32_t) noexcept {
        if (!arguments || count != 2 || !result) return KEEL_RESULT_INVALID_ARGUMENT;
