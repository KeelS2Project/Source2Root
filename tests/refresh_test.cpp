...
int main(int argc, char** argv) {
    ...
    auto manifest_path = [&](const std::string& id) {
        return root / "plugins" / id / "plugin.json";
    };
    auto bytecode_path = [&](const std::string& id) {
        return root / "plugins" / id / (id + ".smx");
    };
    auto make = [&](const std::string& id, const std::filesystem::path& code, bool enabled = true) {
        ...
        nlohmann::json metadata{{"schema", 1},
                                {"api", 2},
                                {"id", id},
                                {"name", id},
                                {"author", "tests"},
    ...
    std::filesystem::last_write_time(manifest_path("timer"),
                                     std::filesystem::file_time_type::clock::now() + std::chrono::hours(1));

    std::filesystem::last_write_time(bytecode_path("timer"),
                                     std::filesystem::file_time_type::clock::now() + std::chrono::hours(1));
    ...
    Require(!app.Refresh() && host.commands.size() == 3,
            "refresh reports pending lifecycle cleanup without destroying ownership");
    ...
    Require(app.Retry("broken") && status("broken").state == sr::PluginState::Running,
            "corrected failed manifest can be retried");
    ...
    std::cout
        << "real bytecode refresh, unchanged state, paused recovery, disabled authority and discovery dependencies passed\n";
}
