...
public:
    ...
    Host() {
        for (int i=0;i<3;++i) players[i] = {i,static_cast<std::uint64_t>(10+i),76561197960265851ULL+i,true,false,"Player",70+i,2,true};
    }
    ...
    KeelResult NextPlayer(int after, sr::Player& player) override {
        auto it=players.upper_bound(after); if (it==players.end()) return KEEL_RESULT_NOT_FOUND;
        player=it->second; return KEEL_RESULT_OK;
    }
    KeelResult GetListening(const sr::Player& receiver, const sr::Player& sender, bool& value) override {
        if (unavailable) return KEEL_RESULT_UNSUPPORTED;
        if (auto callback=std::exchange(on_read,{})) callback();
        value=Read(receiver.slot,sender.slot); return KEEL_RESULT_OK;
    }
    KeelResult SetListening(const sr::Player& receiver, const sr::Player& sender, bool value) override {
        ...
        if (throw_write) throw std::runtime_error("fixture write");
        if (receiver.slot==fail_write) return KEEL_RESULT_ENGINE_FAILURE;
...
int main() {
    Host h; std::string error; auto sender=h.players.at(0);
    h.listening[{1,0}]=false;
    Require(h.voice.Set(1,sender,true,error),"mute all receivers");
    for (int i=0;i<3;++i) Require(!h.Read(i,0),"all receivers muted");
    h.External(2,0,false); h.External(1,0,true);
    ...
    h.fail_write=1;
    Require(!h.voice.Set(1,sender,true,error) && h.voice.Muted(sender) && error.starts_with("Mute recorded;"),"partial mute remains recorded with explicit failure");
    h.fail_write=-1; Require(h.voice.Refresh(error) && !h.Read(1,0),"restriction retry succeeds");
    h.fail_write=1; Require(!h.voice.Release(1,error) && error.find("retry required")!=std::string::npos,"release refuses unfinished restoration");
    h.External(1,0,false); h.fail_write=-1;
    ...
    h.fail_lookup=1; Require(!h.voice.Release(1,error),"transient receiver lookup retains restore obligation");
    h.fail_lookup=-1; h.writes.clear(); Require(h.voice.Release(1,error) && h.writes.size()==1,"only pending receiver retried");
    Require(h.voice.Set(1,sender,true,error),"mute before replaced receiver");
    ++h.players.at(1).connection; h.writes.clear();
    Require(h.voice.Release(1,error),"replaced receiver can be retired");
    for (const auto& [receiver,from]:h.writes) Require(receiver!=1,"no restore against replacement receiver");
    h.on_read=[&]{++h.players.at(0).connection;};
    Require(!h.voice.Set(1,sender,true,error) && !h.voice.Muted(sender),"sender replacement during initial read prevents ownership");
    sender=h.players.at(0); Require(h.voice.Set(1,sender,true,error),"mute current sender");
    h.voice.Disconnected(0,sender.connection-1); Require(h.voice.Muted(sender),"old disconnect cannot clear new owner");
    h.throw_write=true; Require(!h.voice.Release(1,error),"throwing restore retained");
    h.throw_write=false; h.External(2,0,true);
    ...
    h.fail_write=1; Require(!h.voice.Suspend(error) && h.voice.Muted(sender),"refused pause retains mute intent");
    h.fail_write=-1; Require(h.voice.Refresh(error) && !h.Read(1,0),"refused pause enforcement recovers");
    Require(h.voice.Suspend(error),"native pause retry");
    ++h.players.at(0).connection;
    h.writes.clear(); Require(h.voice.Resume(error) && h.writes.empty(),"resume does not follow a replacement sender");
    sender=h.players.at(0);
    h.unavailable=true; Require(!h.voice.Set(1,sender,true,error) && !h.voice.Muted(sender),"unsupported engine does not record mute");
    h.unavailable=false; Require(h.voice.Set(1,sender,true,error),"mute before map cleanup");
    h.fail_write=2; Require(!h.voice.Release(0,error),"all-owner cleanup retains failure");
    h.fail_write=-1; Require(h.voice.Release(0,error),"all-owner cleanup retry");
    Require(h.voice.Set(1,sender,true,error),"mute before sender disconnect");
    h.players.erase(0); h.writes.clear(); Require(h.voice.Refresh(error) && h.writes.empty(),"disconnected sender retired without writes");
