...
const Definition Music{"music", "Music preference", Access::Public},
    Rank{"rank", "Server-managed rank", Access::Protected};
...
static void LocalChecks(const std::filesystem::path& root) {
    ...
    Reject(
        ...
        "target cannot change under live service");
    ...
    auto invalid = Local("valid");
    invalid["connections"]["clientprefs"]["allow_plugins"] = {"another_plugin"};
    Write(path, invalid);
    Reject(
        [&] {
            fresh();
        },
        "service identity must be permitted");
    ...
    Reject(
        ...
        "unsupported preferences driver rejected before connecting or pinning a target");
    ...
    Reject(
        ...
        "deleted configuration cannot switch configured SQLite to default");
    ...
    Reject(
        [&] {
            ConfiguredStorage(root / "broken-link", path)();
        },
        "broken config link is an error, not implicit default");
...
static void NetworkChecks(const std::filesystem::path& root, const std::filesystem::path& source) {
    ...
    auto run = [&](const std::string& sql) {
        return mysql::Query(settings, {sql, {}}, canceled);
    };
    auto reset = [&] {
        run("DROP TABLE IF EXISTS sr_prefs_values");
        run("DROP TABLE IF EXISTS sr_prefs_cookies");
        run("DROP TABLE IF EXISTS sr_prefs_schema");
    };
    reset();
    auto storage = ConfiguredStorage(data, path);
    ...
    Reject(
        [&] {
            storage()->Register({Music.name, Music.description, Access::Private});
        },
        "metadata conflict refused");
    ...
    Check(persisted.at("music").text == text && persisted.at("music").updated == 4000000000ll &&
              persisted.at("rank").text.empty() && storage()->Load(First + 1).empty(),
          "bound strings, empty values, wide timestamps and account isolation");

    Reject(
        [&] {
            storage()->Save(First, {{"music", {"must-rollback", 9}}, {"zz_missing", {"invalid", 9}}});
        },
        "unknown cookie aborts batch after earlier write");
    ...
    Reject(
        [&] {
            storage()->Save(First, {{"music", {"refused", 10}}});
        },
        "server rejection fails write");
    ...
    const Identity player{0, 1, First};
    {
        ...
        b.Refresh(player);
        Until(b, [&] {
            return b.Status(player) == State::Ready;
        });
        ...
        auto broken = config;
        broken["connections"]["clientprefs"]["password"] = "intentionally-invalid-fixture-password";
        Write(path, broken);
        ...
        Reject(
            ...
            "missing remote config cannot switch to SQLite");
        ...
        Check(storage()->Load(First).at("music").text == "survives-offline-failure",
              "repaired credentials are reread and dirty offline writes commit");
        ...
        Until(a, [&] {
            return a.IdentityPersisted(First + 1);
        });
        Check(storage()->Load(First + 1).at("music").text == "shared-offline-latest",
              "offline account updates commit through shared SQL");
    }
    ...
    // Fill all but one catalog slot, then race two independent registrations.
    {
        ...
        for (unsigned i = 3; i < MaxCookies - 1; ++i) {
            ...
            query.Bind(1, "limit" + std::to_string(i));
    ...
    auto register_last = [&](const char* name) {
        ...
        try {
            storage()->Register({name, "", Access::Public});
            ++accepted;
        } catch (const Error&) {
            ++rejected;
        }
    ...
    one.join();
    two.join();
    Check(accepted == 1 && rejected == 1 && storage()->Catalog().size() == MaxCookies,
          "cross-server registration lock enforces global catalog limit");

    Values batch;

    for (const auto& cookie : storage()->Catalog())
        batch.emplace(cookie.name, Value{std::string(255, 'x'), 4000000001ll});
    ...
    Reject(
        ...
        "nontransactional existing schema refused");
...
int main(int argc, char** argv) {
    try {
        ...
        if (argc == 2)
            ...
            NetworkChecks(root, argv[2]);
#else
        else
            throw std::runtime_error("MySQL driver unavailable");
#endif
