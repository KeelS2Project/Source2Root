...
public void RunBuffers(Player player, const char[] arguments)
{
    ...
    if (target == NoDHookTarget)
    {
        ReplyToCommand(player, "The buffer target is unavailable.");
        return;
    }
    ...
    if (buffers == NoSDKCall)
    {
        ReplyToCommand(player, "The buffer call could not be prepared.");
        return;
    }
    ...
    if (!completed)
    {
        ReplyToCommand(player, "The configured buffer call failed.");
        return;
    }
...
public void Run(Player player, const char[] arguments)
{
    ...
    {
        ReplyToCommand(player, "The configured call failed.");
        return;
    }
...
public void RunPlayer(Player player, const char[] arguments)
{
    ...
    if (target == NoDHookTarget)
    {
        ReplyToCommand(player, "The player target is unavailable.");
        return;
    }
    ...
    if (call == NoSDKCall)
    {
        ReplyToCommand(player, "The player call could not be prepared.");
        return;
    }
    ...
    if (!completed)
    {
        ReplyToCommand(player, "The configured player call failed.");
        return;
    }
