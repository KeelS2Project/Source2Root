...
{
    if (caller == NoPlayer || !IsPlayerAlive(caller))
    {
        ReplyToCommand(caller, "Use this command as a living player.");
        return;
    }
...
Entity Target(const char[] arguments)
{
    ...
    if (!GetArgument(arguments, 0, number, sizeof(number)) || !ParseInt(number, index, 1))
        return NoEntity;
...
public void Model(Player caller, const char[] arguments)
{
    ...
    {
        ReplyToCommand(caller, "Usage: sr_entity_model <index> <model asset>");
        return;
    }
...
public void Remove(Player caller, const char[] arguments)
{
    ...
    {
        ReplyToCommand(caller, "Usage: sr_entity_remove <index>");
        return;
    }
...
public void Spawn(Player caller, const char[] arguments)
{
    ...
    {
        ReplyToCommand(caller, "Usage: sr_entity_spawn <model asset> \"x y z\"");
        return;
    }
    ...
    if (success)
        success = Entity_DispatchSpawn(entity, invoked);

    Result(caller,success,"Prop spawned.");

    if (!success && invoked)
        ReplyToCommand(caller, "Spawn was invoked; this construction cannot be retried.");
...
public void Input(Player caller, const char[] arguments)
{
    ...
    {
        ReplyToCommand(caller, "Usage: sr_entity_input <index> <input name>");
        return;
    }
    ...
    if (!success && invoked)
        ReplyToCommand(caller, "The engine was entered before failure; effects may already have occurred.");
