...
public void ObserveSelf(Player player, const char[] arguments)
{
    ...
    if (pawn == NoEntity) return;
    int reference;
    bool found = Entity_SourceHandle(pawn, reference);
    Entity_Close(pawn);
    if (!found) return;
    if (observer != NoSDKHook) SDKHook_Close(observer);
    observer = SDKHook_Add(reference, "example_damage", SDKHook_Post, Observe);
    if (observer != NoSDKHook) ReplyToCommand(player, "Damage observation attached to your current pawn.");
    else { char error[256]; GetLastError(error, sizeof(error)); ReplyToCommand(player, error); }
}
