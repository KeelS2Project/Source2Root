...
public void Health(Player caller, const char[] arguments)
{
    ...
    if (health == NoSchemaField)
    {
        ReplyToCommand(caller, "This game does not provide the expected health field.");
        return;
    }
    ...
    if (pawn != NoEntity)
    {
        ...
        {
            char text[96];
            Format(text, sizeof(text), "Current pawn health: %d", value);
            ReplyToCommand(caller, text);
        }
        else
            ReplyToCommand(caller, "The health field is unavailable for this pawn.");

        Entity_Close(pawn);
    }
    else
        ReplyToCommand(caller, "Your current pawn is unavailable.");
...
public void Health100(Player caller, const char[] arguments)
{
    ...
    if (health != NoSchemaField && pawn != NoEntity && Entity_WriteInt(pawn, health, 100))
        ...
        GetLastError(message, sizeof(message));
