...
public void RespawnSelf(Player caller, const char[] arguments)
{
    if (caller == NoPlayer)
    {
        ReplyToCommand(caller, "Use this command as a connected T/CT player.");
        return;
    }

    if (CS_RespawnPlayer(caller))
        ReplyToCommand(caller, "Respawn dispatched.");
    else
    {
        char error[256];
...
public void Statistics(Player caller, const char[] arguments)
{
    if (caller == NoPlayer)
    {
        ReplyToCommand(caller, "Use this command as a connected player.");
        return;
    }
    ...
    if (!CS_GetPlayerScore(caller, score) || !CS_GetPlayerMVPs(caller, mvps))
        GetLastError(text, sizeof(text));
    else
        Format(text, sizeof(text), "CS2 score=%d, MVPs=%d", score, mvps);
...
public void RoundCapabilities(Player caller, const char[] arguments)
{
    ...
    if (!CS_GetRoundCapabilities(mask))
        GetLastError(text, sizeof(text));
    else
        Format(text, sizeof(text), "CS2 round termination supported=%d", (mask & CS_ROUND_CAP_TERMINATE) != 0);
...
{
    if (CS_TerminateRound(5.0, CSRound_Draw))
        ReplyToCommand(caller, "Round draw dispatched.");
    else
    {
        char error[256];
        GetLastError(error, sizeof(error));
...
public void MatchStatistics(Player caller, const char[] arguments)
{
    ...
    if (!CS_GetPlayerMoney(caller, money) || !CS_GetPlayerKills(caller, kills)
        ...
        Format(text, sizeof(text), "Money=%d, match kills=%d, deaths=%d, assists=%d", money, kills, deaths, assists);
