...
char titles[][] = {"Mute a player", "Unmute a player", "Gag a player", "Ungag a player", "Silence a player", "Unsilence a player"};
...
int MenuSlot(Player caller)
{
    ...
    for (int i = 0; i < sizeof(menuOwners); i++) {
        ...
        if (free < 0 && !IsPlayerConnected(menuOwners[i])) free = i;
    }
    if (free >= 0) menuOwners[free] = caller;
    return free;
}
...
bool Change(Player player, int action, char[] error, int capacity)
{
    ...
    if (action < 2 || action >= 4) {
        if (!SetPlayerMuted(player, enable)) { GetLastError(error, capacity); return false; }
    }
    if (action >= 2) {
        if (!SetPlayerGagged(player, enable)) {
            GetLastError(error, capacity);
            if (action >= 4) Format(error, capacity, "Voice state changed; chat restriction failed: %s", error);
            return false;
        }
    }
    if (!enable) {
...
void Apply(Player caller, const Player[] targets, int count, int operation)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        bool success = false;
        if (!CanTarget(caller, targets[i], permissions[operation]) || !GetPlayerName(targets[i], name, sizeof(name))) GetLastError(error, sizeof(error));
        else success = Change(targets[i], operation, error, sizeof(error));
        AuditTarget(actor, commands[operation], target, success, 0, false, error);
        if (success) { successes++; SafeName(name); Format(lastName, sizeof(lastName), "%s", name); }
    ...
    if (!successes) {
        if (count == 1) ReplyToCommand(caller, firstError);
            ...
            Format(confirmation, sizeof(confirmation), "No players were %s. %d %s failed: %s", verbs[operation], failed, failed == 1 ? "target" : "targets", firstError);
    ...
    if (failed) {
        Format(confirmation, sizeof(confirmation), "%s %s; %d %s failed: %s", verbs[operation], subject, failed, failed == 1 ? "target" : "targets", firstError);
        if (confirmation[0] >= 'a' && confirmation[0] <= 'z') confirmation[0] -= 'a' - 'A';
    }
...
void Command(Player caller, const char[] arguments, int operation)
{
    if (GetArgumentCount(arguments) != 1) {
        char usage[64]; Format(usage, sizeof(usage), "Usage: %s <target>", commands[operation]); ReplyToCommand(caller, usage); return;
    }
    char selector[257]; Player targets[128];
...
public void Mute(Player caller, const char[] arguments) { Command(caller, arguments, 0); }
public void Unmute(Player caller, const char[] arguments) { Command(caller, arguments, 1); }
public void Gag(Player caller, const char[] arguments) { Command(caller, arguments, 2); }
public void Ungag(Player caller, const char[] arguments) { Command(caller, arguments, 3); }
public void Silence(Player caller, const char[] arguments) { Command(caller, arguments, 4); }
public void Unsilence(Player caller, const char[] arguments) { Command(caller, arguments, 5); }

void TargetsMenu(Player caller, const char[] arguments, int operation)
{
    char usage[64]; Format(usage, sizeof(usage), "Usage: %s <target>", commands[operation]);
    if (!Ready(caller, arguments, permissions[operation], usage)) return;
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (!AddMenuItem(menu, label, CanTarget(caller, players[i], permissions[operation]), view_as<int>(players[i]))) { Failure(caller); CloseMenu(menu); return; }
        added++;
    }
...
{
    int slot = MenuSlot(caller); if (slot < 0) return;
    Player targets[1]; targets[0] = view_as<Player>(value);
    Apply(caller, targets, 1, menuActions[slot]);
}
public void MuteMenu(Player caller, const char[] arguments) { TargetsMenu(caller, arguments, 0); }
public void UnmuteMenu(Player caller, const char[] arguments) { TargetsMenu(caller, arguments, 1); }
public void GagMenu(Player caller, const char[] arguments) { TargetsMenu(caller, arguments, 2); }
public void UngagMenu(Player caller, const char[] arguments) { TargetsMenu(caller, arguments, 3); }
public void SilenceMenu(Player caller, const char[] arguments) { TargetsMenu(caller, arguments, 4); }
public void UnsilenceMenu(Player caller, const char[] arguments) { TargetsMenu(caller, arguments, 5); }

public void Say(Player caller, const char[] arguments)
{
    if (GetArgumentCount(arguments) < 1) { ReplyToCommand(caller, "Usage: sr_say <message>"); return; }
    if (caller != NoPlayer && IsPlayerGagged(caller)) { ReplyToCommand(caller, "You cannot send an announcement while gagged."); return; }
    ...
    if (!GetRemainingArguments(arguments, 0, message, sizeof(message)) || !ActorField(caller, actor, sizeof(actor))
        || (caller != NoPlayer && !GetPlayerName(caller, name, sizeof(name)))) { Failure(caller); return; }
    if (!message[0]) { ReplyToCommand(caller, "Announcement cannot be empty."); return; }
    SafeName(name); Format(output, sizeof(output), "%s: %s", name, message);
    Player players[128]; int count = GetPlayers(players, sizeof(players)), sent, failed;
    if (count < 0) { Failure(caller); return; }
    for (int i = 0; i < count; i++) {
        if (players[i] == caller || IsPlayerBot(players[i])) continue;
        if (ReplyToCommand(players[i], output)) sent++;
        else { failed++; if (!error[0]) GetLastError(error, sizeof(error)); }
    }
    char target[96]; Format(target, sizeof(target), "recipients=%d failed=%d", sent, failed);
    AuditTarget(actor, "sr_say", target, failed == 0, 0, false, failed ? error : message);
    if (failed) Format(output, sizeof(output), "Announcement reached %d players; %d %s failed: %s", sent, failed, failed == 1 ? "delivery" : "deliveries", error);
    else Format(output, sizeof(output), "Announcement sent: %s", message);
