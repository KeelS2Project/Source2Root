...
void KickResult(char[] output, int capacity, const char[] subject, int successes, int failed, const char[] error)
{
    if (!successes)
        Format(output,
               capacity,
               "No players were kicked. %d %s failed: %s",
               failed,
        ...
        Format(output,
               capacity,
               "Kicked %s; %d %s failed: %s",
               subject,
               failed,
               failed == 1 ? "target" : "targets",
               error);
    else
        Format(output, capacity, "Kicked %s", subject);
}
...
void KickTargets(Player caller, Player[] targets, int count, const char[] reason)
{
    ...
    {
        Player last = targets[count - 1];
        targets[count - 1] = caller;
        targets[i] = last;
        break;
    }
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (!CanTarget(caller, targets[i], "admin.kick") || !GetPlayerName(targets[i], name, sizeof(name)))
            GetLastError(error, sizeof(error));
        else
        {
            SafeName(name);
            ...
            if (targets[i] == caller)
            {
                ...
                if (count == 1)
                    Format(subject, sizeof(subject), "%s", name);
                else
                    Format(subject, sizeof(subject), "%d %s", successes + 1, successes ? "players" : "player");
    ...
    if (!successes)
    {
        ReplyToCommand(caller, count == 1 ? firstError : confirmation);
        return;
    }
...
public void Kick(Player caller, const char[] arguments)
{
    ...
    if (count < 1)
    {
        ReplyToCommand(caller, "Usage: sr_kick <target> [reason]");
        return;
    }
    ...
    if (!GetArgument(arguments, 0, selector, sizeof(selector)) ||
        (count > 1 && !GetRemainingArguments(arguments, 1, reason, sizeof(reason))))
    {
        Failure(caller);
        return;
    }
    ...
    if (!reason[0])
    {
        ReplyToCommand(caller, "Kick reason cannot be empty.");
        return;
    }
...
void BanAccount(Player caller, const char[] identity, int minutes, const char[] reason)
{
    ...
    if (count < 0)
        ...
        for (int pass = 0; pass < 2; pass++)
            for (int i = 0; i < count; i++)
            {
                if ((players[i] == caller) != (pass == 1))
                    continue;

                char current[24], disconnect[301], kickTarget[384], kickError[512];

                if (!GetPlayerSteamID(players[i], current, sizeof(current)) || !SameIdentity(current, identity))
                    continue;
                ...
                if (players[i] == caller)
                {
                    if (error[0])
                        Format(disconnect,
                               sizeof(disconnect),
                               "Ban saved for %s; disconnect pending: %s",
                               identity,
                               error);
                    else
                        Format(disconnect, sizeof(disconnect), "%s", confirmation);
                }
                ...
                if (!disconnected)
                {
                    ...
                    RememberEnforced(players[i]);

                AuditTarget(
                    actor, "ban_disconnect", kickTarget, disconnected, 0, false, disconnected ? reason : kickError);
            }

    if (error[0])
        Format(confirmation, sizeof(confirmation), "Ban saved for %s; disconnect pending: %s", identity, error);
...
public void Ban(Player caller, const char[] arguments)
{
    ...
    if (count < 2)
    {
        ReplyToCommand(caller, "Usage: sr_ban <target-or-SteamID> <minutes> [reason]");
        return;
    }
    ...
    if (!GetArgument(arguments, 0, selector, sizeof(selector)) || !GetArgument(arguments, 1, amount, sizeof(amount)) ||
        ...
        (count > 2 && !GetRemainingArguments(arguments, 2, reason, sizeof(reason))))
    {
        Failure(caller);
        return;
    }
    ...
    if (!reason[0])
    {
        ReplyToCommand(caller, "Ban reason cannot be empty.");
        return;
    }
    ...
    if (!NormalizeSteamID(selector, identity, sizeof(identity)))
    {
        ...
        if (FindTargets(caller, selector, targets, sizeof(targets)) != 1 ||
            !GetPlayerSteamID(targets[0], identity, sizeof(identity)))
        {
            Failure(caller);
            return;
        }
...
{
    char actor[384], target[64], error[512], action[128];
    bool removed;
    ...
    AuditTarget(
        actor, "sr_unban", target, success, 0, false, success ? (removed ? "ban removed" : "no stored ban") : error);
    ...
    if (!removed)
    {
        Format(action, sizeof(action), "No stored ban for %s", identity);
        ReplyToCommand(caller, action);
        return;
    }

    Format(action, sizeof(action), "removed the ban for %s", identity);
    Announce(caller, action);
}
...
public void Unban(Player caller, const char[] arguments)
{
    ...
    {
        ReplyToCommand(caller, "Usage: sr_unban <SteamID>");
        return;
    }

    char value[257], identity[24];

    if (!GetArgument(arguments, 0, value, sizeof(value)) || !NormalizeSteamID(value, identity, sizeof(identity)))
    {
        Failure(caller);
        return;
    }
...
int EnforcementSlot(Player player)
{
    ...
    for (int i = 0; i < sizeof(checked); i++) {
        ...
        if (free < 0 && !IsPlayerConnected(checked[i]))
            free = i;
    }
...
{
    if (CreateTimer(1000, EnforceBans, NoPlayer, true) == NoTimer)
    {
        LogMessage("Could not schedule ban enforcement.");
        return;
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (!GetPlayerSteamID(players[i], identity, sizeof(identity)))
            continue;
        ...
        if (slot < 0)
        {
            LogMessage("Too many active ban enforcement connections.");
            return;
        }

        if (!GetBan(identity, reason, sizeof(reason)))
        {
            enforced[slot] = false;
            retry[slot] = 0;
            continue;
...
void PlayerMenu(Player caller, bool ban)
{
    ...
    char permission[16];
    Format(permission, sizeof(permission), ban ? "admin.ban" : "admin.kick");
    Menu menu = CreateMenu(ban ? "Ban a player" : "Kick a player",
                           permission,
                           ban ? BanSelected : KickSelected,
                           ban ? BackToBanDuration : BackToAdministration);
    ...
    for (int i = 0; i < count; i++) {
        char name[129], label[96], identity[24];
        int userid;
        ...
        if (ban)
            allowed = allowed && GetPlayerSteamID(players[i], identity, sizeof(identity));

        if (!AddMenuItem(menu, label, allowed, view_as<int>(players[i])))
        {
            Failure(caller);
            CloseMenu(menu);
            return;
...
{
    if (Ready(caller, arguments, "admin.kick", "Usage: sr_kick <target> [reason]"))
        PlayerMenu(caller, false);
}
...
{
    if (!Ready(caller, arguments, "admin.ban", "Usage: sr_ban <target-or-SteamID> <minutes> [reason]"))
        return;
    ...
    for (int i = 0; i < sizeof(values); i++) {
        ...
        if (values[i])
            Format(label, sizeof(label), "%d minutes", values[i]);
        else
            Format(label, sizeof(label), "Permanent");

        if (!AddMenuItem(menu, label, true, values[i]))
        {
            Failure(caller);
            CloseMenu(menu);
            return;
...
public void DurationSelected(Player caller, int minutes)
{
    ...
    if (!HasPermission(caller, "admin.ban") || minutes < 0 || minutes > 5256000)
        return;

    durations[slot] = minutes;
    PlayerMenu(caller, true);
}
...
public void BanSelected(Player caller, int value)
{
    ...
    Player target = view_as<Player>(value);
    char identity[24];

    if (!CanTarget(caller, target, "admin.ban") || !GetPlayerSteamID(target, identity, sizeof(identity)))
    {
        Failure(caller);
        return;
    }
...
void StoredBansMenu(Player caller)
{
    ...
    if (!count)
    {
        ReplyToCommand(caller, "No bans are stored.");
        return;
    }

    if (pages[slot] >= count)
        pages[slot] = ((count - 1) / 8) * 8;
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (pages[slot] + i >= count)
            continue;

        char label[64], reason[257];

        if (!GetBanIdentity(pages[slot] + i, accounts[slot][i], sizeof(accounts[][])))
        {
            Failure(caller);
            CloseMenu(menu);
            return;
        ...
        Format(label,
               ...
               "%s%s",
               accounts[slot][i],
               GetBan(accounts[slot][i], reason, sizeof(reason)) ? "" : " (expired)");

        if (!AddMenuItem(menu, label, CanTargetIdentity(caller, accounts[slot][i], "admin.unban"), i))
        {
            Failure(caller);
            CloseMenu(menu);
            return;
    ...
    if (pages[slot] > 0 && !AddMenuItem(menu, "Previous bans", true, 1000))
    {
        Failure(caller);
        CloseMenu(menu);
        return;
    ...
    if (pages[slot] + 8 < count && !AddMenuItem(menu, "More bans", true, 1001))
    {
        Failure(caller);
        CloseMenu(menu);
        return;
...
{
    if (!Ready(caller, arguments, "admin.unban", "Usage: sr_unban <SteamID>"))
        return;
    ...
    pages[slot] = 0;
    StoredBansMenu(caller);
}

public void UnbanSelected(Player caller, int value)
{
    if (!HasPermission(caller, "admin.unban"))
    {
    ...
    else if (value >= 0 && value < 8 && accounts[slot][value][0])
        UnbanAccount(caller, accounts[slot][value]);
}
