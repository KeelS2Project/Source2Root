...
int ReadMaps(const char[] filename)
{
    ...
    while ((status = ReadConfigLine(file, line, sizeof(line))) > 0)
    {
        ...
        if (!line[first] || (line[first] == '/' && line[first + 1] == '/')) continue;
        last = first;
        while (line[last]) last++;
        while (last > first && (line[last - 1] == ' ' || line[last - 1] == '\t')) last--;
        if (last - first > 64 || count == sizeof(maps)) { CloseConfigFile(file); return -2; }
        for (int i = first; i < last; i++)
            if (!((line[i] >= 'a' && line[i] <= 'z') || (line[i] >= '0' && line[i] <= '9') || line[i] == '_'))
            { CloseConfigFile(file); return -2; }
        line[last] = 0;
...
{
    if (result == -2) ReplyToCommand(caller, "Map lists accept at most 1024 names, using 1..64 lowercase letters, digits or underscores.");
    else Failure(caller);
}
...
public bool OnPluginStart()
{
    ...
    if (count < 0)
    {
        if (count == -2) LogMessage("Invalid allowed_maps.txt: expected at most 1024 lowercase map names.");
        else { char error[512]; GetLastError(error, sizeof(error)); LogMessage(error); }
        return false;
    }
...
{
    if (!HasPermission(caller, "admin.changemap")) { ReplyToCommand(caller, "You do not have access to this command."); return; }
    int count = ReadMaps("allowed_maps.txt");
    if (count < 0) { ListFailure(caller, count); return; }
    bool allowed = count == 0;
    for (int i = 0; i < count; i++) if (SameText(name, maps[i])) { allowed = true; break; }
    if (!allowed) { ReplyToCommand(caller, "Map is not allowed by allowed_maps.txt."); return; }
    if (!IsMapInstalled(name)) { Failure(caller); return; }
    ...
    if (!ShowActivity(caller, action)) { GetLastError(error, sizeof(error)); LogMessage(error); }
}

public void Map(Player caller, const char[] arguments)
{
    if (GetArgumentCount(arguments) != 1) { ReplyToCommand(caller, "Usage: sr_map <map>"); return; }
    char name[65];
    if (!GetArgument(arguments, 0, name, sizeof(name))) { Failure(caller); return; }
    Change(caller, name);
}

void RequestRestart(Player caller, int seconds)
{
    if (!HasPermission(caller, "admin.restart")) { ReplyToCommand(caller, "You do not have access to this command."); return; }
...
public void Restart(Player caller, const char[] arguments)
{
    ...
    if (count < 0 || count > 1) { ReplyToCommand(caller, "Usage: sr_restart [seconds]"); return; }
    char value[65];
    if (count && (!GetArgument(arguments, 0, value, sizeof(value)) || !ParseInt(value, seconds, 1, 60)))
    { ReplyToCommand(caller, "Restart delay must be from 1 to 60 seconds."); return; }
...
int UserSlot(Player caller)
{
    ...
    if (free >= 0) { owners[free] = caller; pages[free] = 0; }
    return free;
}
...
void MapsMenu(Player caller)
{
    ...
    int count = ReadMaps("allowed_maps.txt");
    if (count == 0) count = ReadMaps("map_menu.txt");
    if (count < 0) { ListFailure(caller, count); return; }
    int installed;
    for (int i = 0; i < count; i++) if (IsMapInstalled(maps[i]))
    {
        if (installed != i) Format(maps[installed], sizeof(maps[]), "%s", maps[i]);
        installed++;
    }
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        if (pages[slot] + i >= installed) continue;
        Format(selections[slot][i], sizeof(selections[][]), "%s", maps[pages[slot] + i]);
        if (!AddMenuItem(menu, selections[slot][i], true, i)) { Failure(caller); CloseMenu(menu); return; }
    }
    if ((pages[slot] > 0 && !AddMenuItem(menu, "Previous maps", true, 1000))
        || (pages[slot] + 8 < installed && !AddMenuItem(menu, "More maps", true, 1001))
        || !ShowMenu(menu, caller)) { Failure(caller); CloseMenu(menu); }
}

public void MapMenu(Player caller, const char[] arguments)
{
    if (!Ready(caller, arguments, "admin.changemap", "Usage: sr_map <map>")) return;
    if (caller == NoPlayer) { ReplyToCommand(caller, "Use sr_map <map> in the server console."); return; }
    int slot = UserSlot(caller);
    if (slot < 0) { ReplyToCommand(caller, "Too many active menu users."); return; }
    pages[slot] = 0; MapsMenu(caller);
}

public void MapSelected(Player caller, int value)
{
    int slot = UserSlot(caller); if (slot < 0) return;
    if (value == 1000) { pages[slot] = pages[slot] >= 8 ? pages[slot] - 8 : 0; MapsMenu(caller); }
    else if (value == 1001) { pages[slot] += 8; MapsMenu(caller); }
    else if (value >= 0 && value < 8 && selections[slot][value][0])
    {
        char name[65]; Format(name, sizeof(name), "%s", selections[slot][value]);
        Change(caller, name);
    }
}
...
public void RestartMenu(Player caller, const char[] arguments)
{
    ...
    {
        char label[64]; Format(label, sizeof(label), "Restart in %d %s", delays[i], delays[i] == 1 ? "second" : "seconds");
        if (!AddMenuItem(menu, label, true, delays[i])) { Failure(caller); CloseMenu(menu); return; }
    }
    if (!ShowMenu(menu, caller)) { Failure(caller); CloseMenu(menu); }
}
public void RestartSelected(Player caller, int seconds) { RequestRestart(caller, seconds); }
