...
char routes[][] = {"sr_help", "sr_who", "sr_reloadadmins", "sr_slap", "sr_slay", "sr_kick", "sr_ban", "sr_unban", "sr_mute", "sr_unmute", "sr_gag", "sr_ungag", "sr_silence", "sr_unsilence", "sr_map", "sr_restart"};
char labels[][] = {"Command help", "Connected players", "Reload administrators", "Slap a player", "Slay a player", "Kick a player", "Ban a player", "Remove a ban", "Mute a player", "Unmute a player", "Gag a player", "Ungag a player", "Silence a player", "Unsilence a player", "Change map", "Restart round"};
...
{
    if (!Ready(caller, arguments, "admin.help", "Usage: sr_help")) return;
    ReplyToCommand(caller, "Source2Root administration:");
    char names[][] = {"sr_help", "sr_admin", "sr_who", "sr_reloadadmins", "sr_slap", "sr_slay", "sr_kick", "sr_ban", "sr_unban", "sr_mute", "sr_unmute", "sr_gag", "sr_ungag", "sr_silence", "sr_unsilence", "sr_say", "sr_map", "sr_restart"};
    char descriptions[][] = {"Show command help", "Open the administration menu", "List connected players",
        "Reload administrators and groups", "Slap players", "Slay players", "Kick players", "Ban an account", "Remove a stored ban", "Mute voice", "Unmute voice", "Block chat", "Allow chat", "Block voice and chat", "Allow voice and chat", "Send a public announcement", "Change the map", "Restart the round"};
    ...
    ReplyToCommand(caller, "Quote names containing spaces. Group selectors require a command that accepts multiple targets.");
    Audit(caller, "sr_help");
}

public void Who(Player caller, const char[] arguments)
{
    if (!Ready(caller, arguments, "admin.who", "Usage: sr_who")) return;
    Player players[128];
    int count = GetPlayers(players, sizeof(players));
    if (count < 0) { Failure(caller); return; }
    if (!count) ReplyToCommand(caller, "No players are connected.");
    for (int i = 0; i < count; i++)
    {
        ...
        if (!GetPlayerName(players[i], name, sizeof(name)) || !GetPlayerUserID(players[i], userid)
            || !GetPlayerTeam(players[i], team)) continue;
        SafeName(name);
        if (IsPlayerBot(players[i])) Format(identity, sizeof(identity), "bot");
        ...
        else Format(identity, sizeof(identity), "Steam authentication pending");
...
{
    if (!Ready(caller, arguments, "admin.reloadadmins", "Usage: sr_reloadadmins")) return;
    if (!ReloadAdmins(caller)) { Failure(caller); return; }
...
{
    if (!Ready(caller, arguments, "admin.menu", "Usage: sr_admin")) return;
    if (caller == NoPlayer) { ReplyToCommand(caller, "Use sr_help in the server console."); return; }
    ...
    for (int i = 0; i < sizeof(routes); i++)
    {
        ...
        if (!AddMenuItem(menu, labels[i], available, i)) { Failure(caller); CloseMenu(menu); return; }
    }
    if (!ShowMenu(menu, caller)) { Failure(caller); CloseMenu(menu); return; }
    Audit(caller, "sr_admin");
}

public void Selected(Player caller, int item)
{
    if (item >= 0 && item < sizeof(routes) && !OpenCommandMenu(caller, routes[item])) Failure(caller);
}
