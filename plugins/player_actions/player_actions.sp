...
int MenuSlot(Player caller)
{
    ...
    for (int i = 0; i < sizeof(menuOwners); i++)
    {
        ...
        if (free == -1 && !IsPlayerConnected(menuOwners[i]))
            free = i;
    }
...
void Apply(Player caller, const Player[] targets, int count, bool slap, int damage)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (!CanTarget(caller, targets[i], permission))
            GetLastError(error, sizeof(error));
        else if (!GetPlayerName(targets[i], name, sizeof(name)))
            GetLastError(error, sizeof(error));
        else if (!IsPlayerAlive(targets[i]))
            Format(error, sizeof(error), "Target is not alive.");
        else
        {
    ...
    if (slap)
        Format(action, sizeof(action), "slapped %s (%d damage)", subject, damage);
    else
        Format(action, sizeof(action), "killed %s", subject);

    if (failed)
    {
        Format(confirmation,
               sizeof(confirmation),
               "Killed %s; %d %s failed: %s",
               subject,
               failed,
        ...
        if (slap)
            Format(confirmation,
                   sizeof(confirmation),
                   "Slapped %s (%d damage); %d %s failed: %s",
                   subject,
                   damage,
                   failed,
                   failed == 1 ? "target" : "targets",
...
void Command(Player caller, const char[] arguments, bool slap)
{
    ...
    if (slap && count == 2)
    {
        if (!GetArgument(arguments, 1, amount, sizeof(amount)) || !ParseInt(amount, damage, 0, 1000))
        {
            Failure(caller);
            return;
        }
...
public
void Slap(Player caller, const char[] arguments)
{
    Command(caller, arguments, true);
}

public
void Slay(Player caller, const char[] arguments)
{
    Command(caller, arguments, false);
}
...
void TargetsMenu(Player caller, bool slap, int damage)
{
    ...
    if (slap)
        Format(title, sizeof(title), "Slap a player (%d damage)", damage);
    else
        Format(title, sizeof(title), "Slay a player");

    Format(permission, sizeof(permission), slap ? "admin.slap" : "admin.slay");
    Menu menu = CreateMenu(
        title, permission, slap ? SlapSelected : SlaySelected, slap ? BackToSlapDamage : BackToAdministration);
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (!AddMenuItem(menu,
                         label,
                         IsPlayerAlive(players[i]) && CanTarget(caller, players[i], permission),
                         view_as<int>(players[i])))
        {
            Failure(caller);
            CloseMenu(menu);
            return;
...
{
    if (!Ready(caller, arguments, "admin.slap", "Usage: sr_slap <target> [damage]"))
        return;
    ...
    for (int i = 0; i < sizeof(amounts); i++)
    {
        ...
        if (!AddMenuItem(menu, label, true, amounts[i]))
        {
            Failure(caller);
            CloseMenu(menu);
            return;
...
{
    if (Ready(caller, arguments, "admin.slay", "Usage: sr_slay <target>"))
        TargetsMenu(caller, false, 0);
}

public void DamageSelected(Player caller, int damage)
{
    if (HasPermission(caller, "admin.slap") && damage >= 0 && damage <= 1000)
        TargetsMenu(caller, true, damage);
}
